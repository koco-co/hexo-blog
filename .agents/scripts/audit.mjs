...
const SENSITIVE_KEY_PATTERN =
  /\b(api[_-]?key|apikey|access[_-]?token|secret|password|client[_-]?id|token|key)\b\s*[:=]/gi;
...
const LEARN_TOPIC_ENTRY_HEADINGS = [
  "课程目标",
  "前置条件",
  "学习路径",
  "文章安排",
...
const LEARN_TOPIC_PLACEHOLDER_HEADINGS = new Set([
  "文章职责",
  "内容边界",
  "正文编排",
  "视觉与复习",
...
const LEARN_TOPIC_LEGACY_PLACEHOLDER_HEADINGS = new Set([
  "学习目标",
  "章节计划",
  "验证方式",
]);
...
const LEARN_TOPIC_PUBLIC_COPY_RULES = [
  {
    code: "LEARN_TOPIC_NAVIGATION_COPY_FORBIDDEN",
    pattern:
      /前置文章|前置篇|文章\([一二三四五六七八九十]+\)|上一(?:篇|章|文章)|下一(?:篇|章|文章)|从第[一二三四五六七八九十]+篇|课程(?:阅读)?进度|文章进度/,
  },
  {
    code: "LEARN_TOPIC_LEDGER_COPY_FORBIDDEN",
    pattern:
      /本文分配(?:的)?能力|能力(?:账本|编号|ID)|课程候选门禁|公开候选门禁|候选门禁|进阶门禁|本篇新建|稳定复习节点|审计版本|\b(?:L2|IP|TR|HTTP|MOD|DIAG|ADV|CAP)-\d{3}\b|\bCN-(?:L2|IP|TR|HTTP|MOD|DIAG|ADV|CAP)-\d{3}\b/,
  },
  {
    code: "LEARN_TOPIC_INTERNAL_COPY_FORBIDDEN",
    pattern: /学习目标|章节计划|验证方式|课程占位标记|占位合同|课程扩展/,
  },
];
const ARTICLE_HEADING_MAX_CHARS = 15;
const ARTICLE_HEADING_SECTION_EXEMPTIONS = new Set(["常见问题", "参考资料"]);
const ARTICLE_HEADING_CHAT_PATTERN =
  /^(?:Q\d+\s*[:：]\s*)?(?:为什么|如何|怎么|是否|是不是|能否|可以(?:否)?|有没有|什么是)/;
...
const REFERENCE_IMAGE_PATH_HINT =
  /(?:favicon|favicons|icon|icons|logo|brand|symbol|mark)(?:[._/-]|$)/i;
const REFERENCE_GENERIC_IMAGE_PATH =
  /(?:^|[/_.-])(avatar|default|placeholder|cover|course-cover)(?:[/_.-]|$)/i;
...
const LEARN_TOPIC_NAVIGATION_CARD_PATTERN =
  /从哪一篇|从第[一二三四五六七八九十]+篇|课程.{0,12}(?:开始|顺序)|(?:是否|会不会).{0,8}阻塞.{0,8}(?:实战|主线|路线)|如何使用(?:这套)?课程|哪一篇.{0,8}(?:开始|选学)/;
...
const CODE_ROOTS = [
  "scripts",
  "source/js",
  ".agents/skills/hexo-learn-topic/scripts",
];
...
const CI_PAGES_REPOSITORY = "koco-co/koco-co.github.io";
const CI_PAGES_BRANCH = "main";
const OLD_IMAGE_HOST_URL_PATTERN =
  /https?:\/\/(?:cdn\.jsdelivr\.net\/gh\/koco-co\/picgo-images(?:@[^/\s)'"<>}]+)?|raw\.githubusercontent\.com\/koco-co\/picgo-images\/[^/\s)'"<>}]+|github\.com\/koco-co\/picgo-images\/raw\/[^/\s)'"<>}]+)\/[^\s)'"<>}]+/g;
...
function readJson(target) {
  try {
    ...
    throw new Error(`JSON parse failed: ${target}`, { cause: error });
  }
}
...
function isStructuralConnection(line) {
  ...
  return (
    ...
    /^(?:接下来|随后|然后|最后|综上|换句话说|在这个例子中|下面进入|下面转向|因此可以看到)[^。！？!?]{0,32}[。！？!?]?$/.test(
      normalized,
    )
  );
}
...
function structuralLeadLineNumbers(text) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      /^\s*(?:[-*+]\s+|\d+[.)]\s+|\||`{3,}|~{3,}|\{%\s*(?:mermaid|timeline|tabs|chartjs)\b)/.test(
        next,
      )
    ) {
      leads.add(index + 1);
...
function conceptStoryRegions(text) {
  ...
  for (const [index, raw] of markdownBodyLines(text).entries()) {
    ...
    const marker =
      !inComment && raw.match(/^<!-- concept-story:(start|end) -->\s*$/);
    if (marker) {
      if (marker[1] === "start") {
        if (story) {
    ...
    if (!inComment && /<!--\s*concept-story\b/.test(visible)) {
      reject(
        "CONCEPT_STORY_MARKER_INVALID",
        line,
        "故事标记必须独占一行且不缩进，使用 <!-- concept-story:start --> 或 <!-- concept-story:end -->。",
      );
    }
    ...
    if (
      story &&
      !inDiagram &&
      /<\/?[A-Za-z][A-Za-z0-9-]*(?:\s[^>]*|\s*\/?)>/.test(visible)
    ) {
      reject(
        "CONCEPT_STORY_HTML",
        line,
        "故事片段不使用原生 HTML 容器；改用直接可见的 Markdown、图表或图片。",
      );
    }
    for (const html of (inDiagram ? "" : visible).matchAll(
      /<(\/?)(details)\b[^>]*>/gi,
    )) {
    ...
    if (
      story &&
      (/^\s{0,3}#{1,6}\s/.test(visible) ||
        /^\s{0,3}(?:=+|-+)\s*$/.test(visible))
    ) {
      reject(
        "CONCEPT_STORY_HEADING",
        line,
        "故事片段不能跨章节或包含标题；在同一概念内结束叙事，再解释机制。",
      );
    }
    for (const token of visible.matchAll(
      /\{%\s*([A-Za-z][A-Za-z0-9_-]*)\b[^%]*%\}/g,
    )) {
      const name = token[1];
      if (story && hiddenTags.has(name))
        reject(
          "CONCEPT_STORY_HIDDEN",
          line,
          "故事主线不能使用折叠、隐藏、轮播、页签或闪卡标签。",
        );
    ...
    if (
      story &&
      !stack.length &&
      !isSingleTagLine(visible) &&
      !/^\s*(?:!\[|<)/.test(visible) &&
      /[\p{L}\p{N}]/u.test(visible)
    )
      story.hasProse = true;
  }
  if (story)
    reject(
      "CONCEPT_STORY_UNCLOSED",
      story.start,
      "故事片段缺少结束标记；不会豁免后续正文。",
    );
...
function learnTopicPlainBodyBlocks(text, storyRanges = []) {
  ...
  for (const [index, rawLine] of rows.entries()) {
    ...
    if (
      storyRanges.some(
        (range) => line.line >= range.start && line.line <= range.end,
      )
    ) {
      flush();
      inList = false;
    ...
    if (
      /^\s*!\[[^\]]*\]\([^)]*\)\s*$/.test(rawLine) ||
      isStructuralConnection(rawLine)
    ) {
      flush();
      continue;
    }
...
function hasCourseDraftBody(h2Headings) {
  const publicHeadings = h2Headings.filter(
    (heading) => !LEARN_TOPIC_PLACEHOLDER_HEADINGS.has(heading),
  );
...
function headingStyleIssue(heading, currentH2) {
  if (
    ...
    ARTICLE_HEADING_SECTION_EXEMPTIONS.has(currentH2)
  )
    return null;
  if (
    LEARN_TOPIC_ENTRY_HEADINGS.includes(heading.text) ||
    headingLooksLikeCode(heading.text)
  )
    return null;
...
function frontMatterScalarValue(text, field) {
  ...
  const match = rawFrontMatter(text).match(
  ...
  for (let index = 0; index < match[1].length; index += 1) {
    ...
    if (quote) {
      ...
      escapedCharacter =
        quote === '"' && character === "\\" && !escapedCharacter;
...
function topLevelFrontMatterKeys(text) {
  ...
  if (!match) return [];
  return [...match[1].matchAll(/^([a-zA-Z][a-zA-Z0-9_-]*):/gm)].map(
    (item) => item[1],
  );
}
...
function resolveThemeRoot(root, themeName = "butterfly") {
  if (!isNonEmptyString(themeName)) return null;
  const projectRoot = path.resolve(root);
  const localRoot = path.join(projectRoot, "themes", themeName);
  if (existsSync(localRoot)) {
    return {
      source: "local",
      packageName: null,
      root: localRoot,
      relative: path.posix.join("themes", themeName),
    };
  }

  const packageName = `hexo-theme-${themeName}`;
  ...
  return {
    ...
    packageName,
    root: packageRoot,
    relative: path.posix.join("node_modules", packageName),
  };
}

function configuredTheme(root) {
  const target = path.join(root, "_config.yml");
  if (!existsSync(target)) return null;
  try {
    return resolveThemeRoot(root, parseYaml(readText(target))?.theme);
  } catch {
    return null;
  }
}

function ciWorkflowFacts(root) {
  ...
  const facts = {
    ...
    publishBranch: null,
    publishDir: null,
    deployKeySecret: null,
    hasLint: false,
    hasAuditTests: false,
    hasBuild: false,
    hasReleaseAudit: false,
  };
  if (!facts.present) return facts;

  const text = readText(target);
  facts.externalRepository =
    text.match(/^\s*external_repository:\s*['"]?([^\s#'"]+)/m)?.[1] ?? null;
  facts.publishBranch =
    text.match(/^\s*publish_branch:\s*['"]?([^\s#'"]+)/m)?.[1] ?? null;
  facts.publishDir =
    text.match(/^\s*publish_dir:\s*['"]?([^\s#'"]+)/m)?.[1] ?? null;
  facts.deployKeySecret =
    text.match(
      /^\s*deploy_key:\s*\$\{\{\s*secrets\.([A-Z0-9_]+)\s*\}\}/im,
    )?.[1] ?? null;
  facts.hasLint = /audit\.mjs\s+lint\b/.test(text);
  facts.hasAuditTests =
    /node\s+--test\s+\.agents\/scripts\/audit\.test\.mjs\b/.test(text);
  facts.hasBuild = /npm\s+run\s+build\b/.test(text);
  facts.hasReleaseAudit = /audit\.mjs\s+release\b[^\n]*--route\s+ci\b/.test(
    text,
  );
  return facts;
}
...
function inspectGit(target) {
  ...
  if (!existsSync(path.join(target, ".git")))
    return { present: false, dirty: null, entries: 0 };

  const result = spawnSync(
    "git",
    ["-C", target, "status", "--porcelain", "--untracked-files=normal"],
    {
      encoding: "utf8",
    },
  );
...
function sensitiveKeyLocations(root, relativeFiles) {
  ...
  for (const relativeFile of relativeFiles) {
    ...
    const pattern = new RegExp(
      SENSITIVE_KEY_PATTERN.source,
      SENSITIVE_KEY_PATTERN.flags,
    );
    let match;
...
function localInjectedAssets(config) {
  ...
  const values = [
    ...(Array.isArray(inject.head) ? inject.head : []),
    ...(Array.isArray(inject.bottom) ? inject.bottom : []),
  ];
...
function localCdnAssets(config) {
  const option = config?.CDN?.option;
...
function isRenderedImageReference(line, index, frontMatter) {
  ...
  return (
    ...
    /\{%\s*(?:galleryGroup|inlineImg|image|inlineimage|link|site|cell)\b/.test(
      line,
    )
  );
}
...
function markdownImageReferences(root) {
  ...
  for (const target of files) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      const startsImageTag =
        /\{%\s*(?:galleryGroup|inlineImg|image|inlineimage|link|site|cell)\b/.test(
          line,
        );
      const renderedTagLine = imageTagContinuation || startsImageTag;
      const oldHostPattern = new RegExp(
        OLD_IMAGE_HOST_URL_PATTERN.source,
        OLD_IMAGE_HOST_URL_PATTERN.flags,
      );
      for (const match of line.matchAll(oldHostPattern)) {
        ...
        if (
          !fence &&
          (renderedTagLine ||
            isRenderedImageReference(line, match.index, frontMatter))
        )
          references.liveOldHost.push(reference);
        else references.tutorialOldHost.push(reference);
      }

      if (fence) continue;
      const localPattern = new RegExp(
        LOCAL_IMAGE_URL_PATTERN.source,
        LOCAL_IMAGE_URL_PATTERN.flags,
      );
...
function resolveLocalImage(root, imageUrl) {
  ...
  const theme = configuredTheme(root);
  const roots = [path.resolve(root, "source")];
  if (theme) roots.push(path.join(theme.root, "source"));
  const candidates = roots.map((sourceRoot) => ({
    sourceRoot,
    target: path.resolve(sourceRoot, relativePath),
  }));
  if (
    candidates.some(
      ({ sourceRoot, target }) =>
        target !== sourceRoot && !target.startsWith(`${sourceRoot}${path.sep}`),
    )
  )
    return null;
  return (
    candidates.find(({ target }) => existsSync(target))?.target ??
    candidates[0].target
  );
}
...
export function auditAssets({ root = process.cwd() } = {}) {
  ...
  if (!existsSync(manifestTarget)) {
    ...
    try {
      ...
      report.facts.sourceRepository = isNonEmptyString(
        manifest.sourceRepository,
      )
        ? sanitizeUrl(manifest.sourceRepository)
        : null;
      report.facts.sourceCommit = isNonEmptyString(manifest.sourceCommit)
        ? manifest.sourceCommit
        : null;
      report.facts.expectedAssetCount = Number(manifest.assetCount ?? 0);
      report.facts.expectedOccurrenceCount = Number(
      ...
      if (
        assets.length === 0 ||
        assets.length !== report.facts.expectedAssetCount
      ) {
        report.errors.push(
          finding(
            "IMAGE_MANIFEST_COUNT_INVALID",
            IMAGE_MIGRATION_MANIFEST,
            "assetCount 与迁移条目数量不一致。",
          ),
        );
      }
      ...
      for (const asset of assets) {
        const targetPath =
          typeof asset?.targetPath === "string" ? asset.targetPath : "";
        if (
          !targetPath.startsWith("source/img/picgo-images/") ||
          targets.has(targetPath)
        ) {
          report.errors.push(
            finding(
              "IMAGE_MANIFEST_TARGET_INVALID",
              IMAGE_MIGRATION_MANIFEST,
              "迁移目标必须唯一且位于 source/img/picgo-images/。",
            ),
          );
          continue;
        }
        ...
        if (
          !target.startsWith(
            `${path.resolve(projectRoot, "source/img/picgo-images")}${path.sep}`,
          ) ||
          !existsSync(target)
        ) {
          report.errors.push(
            finding(
              "MIGRATED_IMAGE_MISSING",
              targetPath,
              "迁移清单中的本地图片不存在。",
            ),
          );
        ...
        if (
          ...
          report.errors.push(
            finding(
              "MIGRATED_IMAGE_HASH_MISMATCH",
              targetPath,
              "本地图片 SHA-256 与迁移清单不一致。",
            ),
          );
          continue;
        }
        ...
        if (
          ...
          !imageSignatureMatches(target)
        ) {
          report.errors.push(
            finding(
              "MIGRATED_IMAGE_MEDIA_TYPE_MISMATCH",
              targetPath,
              "迁移清单 mediaType、目标扩展名与文件签名不一致。",
            ),
          );
          continue;
        }
      ...
      if (actualTotalBytes !== report.facts.expectedTotalBytes) {
        report.errors.push(
          finding(
            "IMAGE_MANIFEST_BYTES_INVALID",
            IMAGE_MIGRATION_MANIFEST,
            "totalBytes 与迁移图片实际大小总和不一致。",
          ),
        );
      }
    } catch {
  ...
  for (const reference of references.liveOldHost) {
    report.errors.push(
      finding(
        "OLD_IMAGE_HOST_LIVE_REFERENCE",
        reference.file,
        `第 ${reference.line} 行仍在真实渲染内容中引用旧图床。`,
      ),
    );
  }
  ...
  const renderedAssetSources = repositoryEntries(projectRoot).filter((item) => {
    ...
    return (
      /^_config(?:\.[^.]+)?\.ya?ml$/.test(item.relative) ||
  ...
  for (const item of renderedAssetSources) {
    ...
    for (const match of text.matchAll(OLD_IMAGE_HOST_URL_PATTERN)) {
      report.errors.push(
        finding(
          "OLD_IMAGE_HOST_RENDERED_SOURCE",
          item.relative,
          `第 ${lineForOffset(text, match.index)} 行仍在站点配置或前端源码中引用旧图床。`,
        ),
      );
    }
      ...
      if (!target || !existsSync(target))
        report.errors.push(
          finding(
            "LOCAL_IMAGE_MISSING",
            item.relative,
            `第 ${lineForOffset(text, match.index)} 行引用的本地图片不存在。`,
          ),
        );
    }
  }
  ...
  const imageFiles = repositoryEntries(projectRoot).filter((item) => {
    if (
      !item.entry.isFile() ||
      !imageExtensions.has(path.extname(item.relative).toLowerCase())
    )
      return false;
    return (
      item.relative.startsWith("source/img/") ||
      item.relative.includes("/assets/")
    );
  });
  report.facts.checkedImageFileCount = imageFiles.length;
  for (const item of imageFiles) {
    if (!imageSignatureMatches(item.target))
      report.errors.push(
        finding(
          "IMAGE_FILE_INVALID",
          item.relative,
          "图片为空、扩展名与文件签名不一致，或不是支持的图片格式。",
        ),
      );
  }
...
function registeredContainerTagNames(source) {
  ...
  for (const match of source.matchAll(
    ...
    if (
      /\{\s*ends\s*:\s*true\s*\}/.test(registration) ||
      /,\s*true\s*\)/.test(registration)
    )
      names.add(match[1]);
  }
  return [...names].sort();
...
function sourceFilesUnder(root, extensions, skippedDirectories = new Set()) {
  ...
  for (const entry of readdirSync(root, { withFileTypes: true })) {
    ...
    if (entry.isDirectory() && !skippedDirectories.has(entry.name))
      files.push(...sourceFilesUnder(target, extensions, skippedDirectories));
    else if (
      entry.isFile() &&
...
function packageEntrySources(projectRoot, packageName) {
  ...
  while (pending.length > 0) {
    ...
    for (const specification of specifications)
      pending.push(path.resolve(path.dirname(resolved), specification));
  }
  return [...visited];
}
...
function installedHexoTagSources(root) {
  const projectRoot = path.resolve(root);
  const theme = configuredTheme(projectRoot);
  const targets = [
    ...sourceFilesUnder(path.join(projectRoot, "scripts"), [
      ".js",
      ".mjs",
      ".cjs",
    ]),
    ...(theme
      ? sourceFilesUnder(path.join(theme.root, "scripts/tag"), [
          ".js",
          ".mjs",
          ".cjs",
        ])
      : []),
    ...sourceFilesUnder(
      path.join(projectRoot, "node_modules/hexo/dist/plugins/tag"),
      [".js", ".mjs", ".cjs"],
    ),
  ];
  ...
  if (existsSync(packageTarget)) {
    try {
      ...
      for (const packageName of Object.keys(dependencies).filter((name) =>
        name.startsWith("hexo-"),
      )) {
...
function installedTagRegistry(root) {
  ...
  for (const target of installedHexoTagSources(root)) {
    ...
    for (const name of registeredContainerTagNames(source))
      containers.add(name);
  }
...
function validateFlashcards(report, projectRoot, markdownFiles, tagCounts) {
  ...
  const parserTarget = path.join(
    projectRoot,
    "node_modules",
    FLASHCARD_PLUGIN_PACKAGE,
    "lib/parser.js",
  );
  if (!existsSync(parserTarget)) {
    report.errors.push(
      finding(
        "FLASHCARD_VALIDATOR_MISSING",
        parserTarget,
        "文章使用了闪卡标签，但已安装插件缺少 lib/parser.js，无法执行字段、唯一 ID 与引用校验。",
      ),
    );
    return;
  }
  ...
  try {
    ...
    if (typeof collectFlashcards !== "function")
      throw new Error("collectFlashcards is unavailable");
    const documents = markdownFiles.map((target) => {
      ...
      return {
        ...
        defaultDeck: isNonEmptyString(data.flashcard_deck)
          ? data.flashcard_deck
          : "",
      };
    });
    ...
    report.errors.push(
      finding(
        "FLASHCARD_CONTENT_INVALID",
        source,
        `闪卡校验失败${field}：${error?.message || "插件解析器拒绝了当前内容。"}`,
      ),
    );
  }
}
...
function checkTagContainers(
  report,
  relativeFile,
  tokens,
  registeredContainers = KNOWN_CONTAINER_TAGS,
) {
  ...
  for (const token of tokens) {
    ...
    if (stack.length === 0) {
      report.errors.push(
        finding(
          "TAG_CONTAINER_UNEXPECTED_END",
          relativeFile,
          `第 ${token.line} 行的 ${token.name} 没有对应的开始标签。`,
        ),
      );
      continue;
    }
  ...
  for (const token of stack) {
    report.errors.push(
      finding(
        "TAG_CONTAINER_UNCLOSED",
        relativeFile,
        `第 ${token.line} 行的 ${token.name} 缺少 end${token.name}。`,
      ),
    );
  }
}
...
function contentFieldError(report, relativeFile, field, message) {
  report.errors.push(
    finding("FRONT_MATTER_FIELD_INVALID", relativeFile, `${field}: ${message}`),
  );
}
...
function validateCapabilityLedger(report, contractPath, payload, articles) {
  const fail = (message) =>
    report.errors.push(
      finding("LEARN_TOPIC_LEDGER_INVALID", contractPath, message),
    );
  ...
  if (payload.capabilities && payload.ledger?.items) {
    if (!isObject(payload.source_manifest))
      fail("能力账本缺少对象形式的 source_manifest。");
    ...
    const dispositionIndex =
      payload.ledger.fields?.indexOf("disposition") ?? -1;
    const articleIndex =
      payload.ledger.fields?.indexOf("article") >= 0
        ? payload.ledger.fields.indexOf("article")
        : (payload.ledger.fields?.indexOf("target") ?? -1);
    dispositions = Object.values(payload.ledger.items).map((item) =>
      dispositionIndex >= 0 ? item?.[dispositionIndex] : null,
    );
    ...
    Array.isArray(payload.official_capabilities) &&
    Array.isArray(payload.coverage_ledger)
  ) {
    if (!isObject(payload.source_manifest))
      fail("能力账本缺少对象形式的 source_manifest。");
    ...
    articleAssignments = payload.coverage_ledger.map((item) => ({
      disposition: item?.disposition,
      reference: item?.article,
    }));
    zeroChecks = [
      "official_only",
      "ledger_only",
      "duplicate",
      "undisposed",
    ].map((key) => [key, payload.collection_summary?.[key]]);
  } else if (
    Array.isArray(payload.current_universe) &&
    Array.isArray(payload.ledger)
  ) {
    if (!isObject(payload.manifest) || !isObject(payload.sources))
      fail("Playwright 压缩账本缺少对象形式的 manifest 或 sources。");
    ...
    hasInvalidArticleIndex = payload.ledger.some((item) => {
      ...
      return excluded
        ...
        : !isNonEmptyString(code) ||
            !Object.hasOwn(payload.articles ?? {}, code);
    });
    ...
    zeroChecks = [
      ...
      "duplicate",
      "undisposed",
    ].map((key) => [key, payload.set_summary?.[key]]);
    ...
    dispositions = payload.current.map(
      (item) => dispositionDictionary[item?.[dispositionIndex]],
    );
    hasInvalidArticleIndex =
      articleIndex < 0 ||
      payload.current.some((item) => {
        const value = item?.[articleIndex];
        return (
          !Number.isInteger(value) ||
          value < 0 ||
          !Object.hasOwn(articleDictionary, value)
        );
      });
    ...
    if (
      Array.isArray(payload.manifest) &&
      payload.manifest.length > 0 &&
      payload.manifest.every(
        (item) => Number.isInteger(item?.count) && item.count >= 0,
      )
    ) {
      manifestUniverse = payload.manifest.reduce(
    ...
    zeroChecks = ["officialOnly", "ledgerOnly", "duplicates", "unassigned"].map(
      (key) => [key, payload.summary?.[key]],
    );
  } else {
  ...
  if (
    !universeIds.every(isNonEmptyString) ||
    !ledgerIds.every(isNonEmptyString)
  ) {
  ...
  if (
    reportedUniverse !== universeIds.length ||
    reportedLedger !== ledgerIds.length
  ) {
  ...
  if (
    dispositions.length !== ledgerIds.length ||
    !dispositions.every(isNonEmptyString)
  ) {
    fail("处置账本存在未分类条目。");
  }
  ...
  for (const assignment of articleAssignments) {
    ...
    if (!excluded && !isNonEmptyString(assignment.reference)) {
      fail(
        `处置为 ${assignment.disposition || "<空>"} 的能力必须指定唯一主文章。`,
      );
    }
  }
  ...
  for (const reference of new Set(
    capabilityLedgerReferences(payload, articles),
  )) {
    if (!actualTitles.has(reference))
      fail(`能力账本引用了课程中不存在的文章：${reference}`);
  }
}

function validateCourseContract(
  report,
  projectRoot,
  courseKey,
  articles,
  ...
  const relativeContract = path.posix.join(
  ...
  try {
    ...
    report.errors.push(
      finding(
        "LEARN_TOPIC_CONTRACT_PARSE_FAILED",
        relativeContract,
        "课程契约不是有效 JSON。",
      ),
    );
  ...
  const fail = (message) =>
    report.errors.push(
      finding("LEARN_TOPIC_CONTRACT_INVALID", relativeContract, message),
    );
  if (
    contract?.schema_version !== 2 ||
    !contract.course ||
    typeof contract.course !== "object"
  ) {
  ...
  if (
    course.public_article_contract !== undefined &&
    course.public_article_contract !== "v1"
  ) {
    fail("course.public_article_contract 只能使用当前公开正文合同版本 v1。");
  }
  if (
    course.forbid_local_absolute_paths !== undefined &&
    typeof course.forbid_local_absolute_paths !== "boolean"
  ) {
    fail("course.forbid_local_absolute_paths 必须是布尔值。");
  }
  if (course.slug !== courseKey) fail("course.slug 必须与课程目录名一致。");
  if (!isNonEmptyString(course.series))
    fail("course.series 必须是非空字符串。");
  if (seriesNames.size === 1 && !seriesNames.has(course.series))
    fail("course.series 与文章 Front Matter 的 series 不一致。");
  if (
  ...
  if (
    topics.some(
      (topic) =>
        topic === LEARN_TOPIC_ENTRY_ARTICLE ||
        topic === LEARN_TOPIC_ADVANCED_ARTICLE ||
        LEARN_TOPIC_FINAL_ARTICLES.has(topic),
    )
  ) {
    fail("course.topics 只能包含第一轮学习主题，不得混入入门、进阶或收束篇。");
  }
  if (
    !Array.isArray(optionalArticles) ||
    optionalArticles.length > 2 ||
    !optionalArticles.every(isNonEmptyString)
  ) {
    fail("course.optional_articles 必须是 0～2 个可选篇名称。");
    return;
  }
  const advancedCount = optionalArticles.filter(
    (name) => name === LEARN_TOPIC_ADVANCED_ARTICLE,
  ).length;
  const finalCount = optionalArticles.filter((name) =>
    LEARN_TOPIC_FINAL_ARTICLES.has(name),
  ).length;
  if (
    advancedCount > 1 ||
    finalCount > 1 ||
    optionalArticles.some(
      (name) =>
        name !== LEARN_TOPIC_ADVANCED_ARTICLE &&
        !LEARN_TOPIC_FINAL_ARTICLES.has(name),
    )
  ) {
  ...
  if (
    advancedCount === 1 &&
    finalCount === 1 &&
    optionalArticles[0] !== LEARN_TOPIC_ADVANCED_ARTICLE
  ) {
  ...
  if (
    suffixes.length < topics.length + 1 ||
    suffixes.length > topics.length + 3
  ) {
  ...
  const expected = suffixes.map((suffix, index) => {
    ...
    const kind =
      index === 0
        ...
        : index <= topics.length
          ? "topic"
          : suffix === LEARN_TOPIC_ADVANCED_ARTICLE
            ? "advanced"
            : "final";
    return { order: index + 1, title, file: `${title}.md`, kind };
  });
  if (
    !Array.isArray(declaredArticles) ||
    declaredArticles.length !== expected.length
  ) {
    ...
    for (const [index, expectedArticle] of expected.entries()) {
      ...
      if (
        !declared ||
        declared.order !== expectedArticle.order ||
        declared.title !== expectedArticle.title ||
        declared.file !== expectedArticle.file ||
        declared.kind !== expectedArticle.kind
      ) {
        fail(
          `course.articles 第 ${index + 1} 项与 topics/optional_articles 推导结果不一致。`,
        );
      }
    }
  }
  ...
  for (const [index, expectedArticle] of expected.entries()) {
    ...
    if (
      !actualArticle ||
      actualArticle.order !== expectedArticle.order ||
      actualArticle.title !== expectedArticle.title ||
      path.basename(actualArticle.file) !== expectedArticle.file
    ) {
...
export function auditContent({ root = process.cwd(), release = false } = {}) {
  ...
  if (!existsSync(postsRoot)) {
    report.errors.push(
      finding("POSTS_DIR_MISSING", "source/_posts", "文章目录不存在。"),
    );
    return finalize(report);
  }
  ...
  report.facts.checkedFiles = files.map((target) =>
    safeRelative(projectRoot, target),
  );
  if (files.length === 0)
    report.warnings.push(
      finding("NO_POSTS", "source/_posts", "没有发现 Markdown 文章。"),
    );
  ...
  for (const target of files) {
    ...
    if (parsed.error) {
      report.errors.push(
        finding(
          ...
          relativeFile,
          `Front Matter 无法解析，约在第 ${parsed.error.line} 行。`,
        ),
      );
      continue;
    }
    ...
    if (!markdownBodyLines(text).join("\n").trim())
      report.errors.push(
        finding("ARTICLE_BODY_EMPTY", relativeFile, "文章正文不得为空。"),
      );
    ...
    if (openFence)
      report.errors.push(
        finding(
          ...
          relativeFile,
          `第 ${openFence.line} 行开始的代码围栏未闭合。`,
        ),
      );
    for (const field of CONTENT_REQUIRED_FIELDS) {
      if (!(field in data))
        contentFieldError(report, relativeFile, field, "缺少必需字段");
    }
    if ("title" in data && !isNonEmptyString(data.title))
      contentFieldError(report, relativeFile, "title", "必须是非空字符串");
    ...
    if ("tags" in data && !isNonEmptyStringArray(data.tags))
      contentFieldError(report, relativeFile, "tags", "必须是非空字符串数组");
    if ("categories" in data && !isNonEmptyStringArray(data.categories))
      contentFieldError(
        report,
        relativeFile,
        "categories",
    ...
    if (
      "date" in data &&
      !isValidDateValue(data.date, frontMatterScalarValue(text, "date"))
    )
      contentFieldError(report, relativeFile, "date", "必须是有效的非空日期值");
    ...
    ].some((value) => typeof value === "string" && value.includes("待补充"));
    if (hasScaffoldPlaceholder)
      report.errors.push(
        finding(
          "ARTICLE_SCAFFOLD_PLACEHOLDER",
          relativeFile,
          "文章仍包含脚手架待补充占位值。",
        ),
      );
    ...
    for (const heading of headings.filter((item) => item.level === 1)) {
      report.errors.push(
        finding(
          "ARTICLE_BODY_H1_FORBIDDEN",
          relativeFile,
          `第 ${heading.line} 行不得使用 H1；文章标题由 Front Matter title 提供。`,
        ),
      );
    }
    ...
    for (const heading of headings) {
      if (heading.level > previousHeadingLevel + 1) {
        report.errors.push(
          finding(
            "ARTICLE_HEADING_LEVEL_SKIPPED",
            relativeFile,
            `第 ${heading.line} 行标题从 H${previousHeadingLevel} 跳到 H${heading.level}。`,
          ),
        );
      }
      ...
      if (headingIssue)
        report.errors.push(
          finding("ARTICLE_HEADING_STYLE_INVALID", relativeFile, headingIssue),
        );
    ...
    if (relativeFile.startsWith(LEGACY_LEARN_ROOT)) {
      report.errors.push(
        finding(
          "LEARN_TOPIC_LEGACY_PATH",
          relativeFile,
          "系统课程必须迁移到 source/_posts/learn-topic/<主题路径段>/。",
        ),
      );
    }
    ...
    if (isLearnTopic) {
      ...
      for (const field of Object.keys(data).filter((field) =>
        field.startsWith(LEARN_TOPIC_INTERNAL_FRONT_MATTER_PREFIX),
      )) {
        report.errors.push(
          finding(
            field === LEARN_TOPIC_LEGACY_LEDGER_FIELD
              ? "LEARN_TOPIC_LEDGER_FRONT_MATTER_FORBIDDEN"
              : "LEARN_TOPIC_INTERNAL_FRONT_MATTER_FORBIDDEN",
            relativeFile,
      ...
      const courseContractTarget = path.join(
        projectRoot,
        LEARN_TOPIC_CONTRACT_ROOT,
        `${courseKey}.json`,
      );
      if (existsSync(courseContractTarget)) {
        try {
          ...
          enforcesPublishedArticleContract =
            courseContract?.course?.public_article_contract === "v1";
          forbidsLocalAbsolutePaths =
            courseContract?.course?.forbid_local_absolute_paths === true;
        } catch {
      ...
      if (pathParts.length !== 5 || !courseKey) {
        report.errors.push(
          finding(
            "LEARN_TOPIC_PATH_INVALID",
            relativeFile,
            "课程文章必须直接位于 source/_posts/learn-topic/<单个主题路径段>/，不得增加嵌套目录。",
          ),
        );
      }
      const fileName = path.basename(relativeFile);
      const rawFileMatch = fileName.match(
        /^(.+)\(([一二三四五六七八九十]+)\)([^\s].*)\.md$/,
      );
      const rawTitleMatch = String(data.title ?? "").match(
        /^(.+)\(([一二三四五六七八九十]+)\)([^\s].*)$/,
      );
      const fileMatch =
        rawFileMatch && !rawFileMatch[1].endsWith("文档") ? rawFileMatch : null;
      const titleMatch =
        rawTitleMatch && !rawTitleMatch[1].endsWith("文档")
          ? rawTitleMatch
          : null;
      if (!fileMatch)
        report.errors.push(
          finding(
            "LEARN_TOPIC_FILENAME_INVALID",
            relativeFile,
            "文件名必须使用 主题(中文序号)简短主题.md，不保留“文档”前缀。",
          ),
        );
      if (!titleMatch)
        report.errors.push(
          finding(
            "LEARN_TOPIC_TITLE_INVALID",
            relativeFile,
            "title 必须使用 主题(中文序号)简短主题，不在右括号后留空格。",
          ),
        );
      if (
        fileMatch &&
        titleMatch &&
        (fileMatch[1] !== titleMatch[1] ||
          fileMatch[2] !== titleMatch[2] ||
          fileMatch[3] !== titleMatch[3])
      ) {
        report.errors.push(
          finding(
            "LEARN_TOPIC_SEQUENCE_MISMATCH",
            relativeFile,
            "文件名与 title 的系列名、中文序号或简短主题不一致。",
          ),
        );
      }
      if (fileMatch && courseKey) {
        ...
        if (expectedOrder === 1 && fileMatch[3] !== "入门路线") {
          report.errors.push(
            finding(
              "LEARN_TOPIC_ENTRY_ROUTE_INVALID",
              relativeFile,
              "系列第一篇必须命名为 主题(一)入门路线。",
            ),
          );
        }
        if (fileMatch[3] === "进阶内容") {
          report.errors.push(
            finding(
              "LEARN_TOPIC_ADVANCED_ROUTE_INVALID",
              relativeFile,
              "可选进阶篇必须使用“进阶路线”，不得使用“进阶内容”。",
            ),
          );
        }
        ...
        if (!Number.isInteger(data.series_order) || data.series_order <= 0) {
          report.errors.push(
            finding(
              "LEARN_TOPIC_SERIES_ORDER_INVALID",
              relativeFile,
              "series_order 必须是正整数。",
            ),
          );
          ...
          report.errors.push(
            finding(
              "LEARN_TOPIC_SERIES_ORDER_MISMATCH",
              relativeFile,
              "series_order 必须与文件名中的中文序号一致。",
            ),
          );
        }
      }
      ...
      if (
        data.published === true &&
        enforcesPublishedArticleContract &&
        !courseTags.some((token) =>
          LEARN_TOPIC_SEMANTIC_BLOCK_TAGS.has(token.name),
        )
      ) {
        report.errors.push(
          finding(
            "LEARN_TOPIC_VISUAL_COMPOSITION_MISSING",
            relativeFile,
            "公开课程正文必须至少包含一个承担真实信息结构的块级标签；仅有 course_series、资料链接、行内标签或纯 Markdown 正文不满足可读性合同。",
          ),
        );
      }
      const courseNavigationCount = courseTags.filter(
        (token) => token.name === "course_series",
      ).length;
      if (courseNavigationCount !== 1) {
        report.errors.push(
          finding(
            "LEARN_TOPIC_COURSE_SERIES_MISSING",
            relativeFile,
            "课程文章正文必须且只能使用一次 {% course_series %}。",
          ),
        );
      }
      if (courseTags.some((token) => token.name === "series")) {
        report.errors.push(
          finding(
            "LEARN_TOPIC_BUILTIN_SERIES_USED",
            relativeFile,
            "系统课程不得使用按标题或日期排序的 Butterfly {% series %}。",
          ),
        );
      }
      for (const fence of markdownFenceBlocks(text).filter(
        (item) => item.info.toLowerCase() === "mermaid",
      )) {
        report.errors.push(
          finding(
            "LEARN_TOPIC_MERMAID_FENCE_FORBIDDEN",
            relativeFile,
            `第 ${fence.line} 行 Mermaid 必须使用 {% mermaid %} 与 {% endmermaid %}。`,
          ),
        );
      }

      if (
        frontMatterFieldSpansPhysicalLines(rawFrontMatter(text), "description")
      ) {
        report.errors.push(
          finding(
            "LEARN_TOPIC_DESCRIPTION_BLOCK",
            relativeFile,
            "description 必须是单行普通 YAML 字符串。",
          ),
        );
      }
      if (!Object.hasOwn(data, "published")) {
        report.errors.push(
          finding(
            "LEARN_TOPIC_PUBLISHED_MISSING",
            relativeFile,
            "课程文章必须显式声明 published 布尔值。",
          ),
        );
      }
      const hasPlaceholderMarker = text.includes(
        LEARN_TOPIC_PLACEHOLDER_MARKER,
      );
      if (data.published === false && !hasPlaceholderMarker) {
        report.errors.push(
          finding(
            "LEARN_TOPIC_FALSE_WITHOUT_PLACEHOLDER",
            relativeFile,
            "只有带占位标记的课程占位文章可以使用 published: false。",
          ),
        );
      }
      if (data.published === true && hasPlaceholderMarker) {
        report.errors.push(
          finding(
            "LEARN_TOPIC_PUBLISHED_PLACEHOLDER",
            relativeFile,
            "公开正文不得保留课程占位标记。",
          ),
        );
      ...
      if (data.published === false && hasPlaceholderMarker) {
        ...
        const hasPlaceholderContract = [
          ...LEARN_TOPIC_PLACEHOLDER_HEADINGS,
        ].every((heading) => placeholderHeadings.includes(heading));
        const hasDraftContract = hasCourseDraftBody(placeholderHeadings);
        if (!hasPlaceholderContract && !hasDraftContract) {
          report.errors.push(
            finding(
              "LEARN_TOPIC_PLACEHOLDER_CONTRACT_MISSING",
              relativeFile,
              "课程占位文章必须包含“文章职责”“内容边界”“正文编排”“视觉与复习”和“验收证据”合同。",
            ),
          );
        } else {
        ...
        for (const issue of learnTopicPublicCopyIssues(text)) {
          report.errors.push(
            finding(
              issue.code,
              relativeFile,
              `第 ${issue.line} 行包含不应出现在公开正文中的课程导航、进度、能力账本或内部合同文案：${issue.text}`,
            ),
          );
        }
      ...
      if (
        h2Headings.includes("来源") ||
        h2Headings.includes("来源与核验范围")
      ) {
        report.errors.push(
          finding(
            "LEARN_TOPIC_SOURCE_HEADING",
            relativeFile,
            "公开课程统一使用“参考资料”，不得使用“来源”或“来源与核验范围”。",
          ),
        );
      }
      ...
      for (const heading of markdownHeadings(text).filter(
        (item) =>
          [2, 3].includes(item.level) && publicInternalHeadings.has(item.text),
      )) {
        if (
          (data.published === true && enforcesPublishedArticleContract) ||
          heading.text === "统一封面与文章收尾"
        ) {
          report.errors.push(
            finding(
              "LEARN_TOPIC_META_HEADING_FORBIDDEN",
              relativeFile,
              `第 ${heading.line} 行的 H${heading.level}“${heading.text}”属于课程生产元信息或冗余路线文案；请改为主题相关的简洁标题。`,
            ),
          );
        }
      }
      if (/核验于\s*\d{4}-\d{2}-\d{2}/.test(text)) {
        report.errors.push(
          finding(
            "LEARN_TOPIC_VERIFICATION_COPY",
            relativeFile,
            "公开课程不得包含“核验于 YYYY-MM-DD”文案。",
          ),
        );
      }
      if (data.published === true && courseArticleKind === "entry") {
        if (
          JSON.stringify(h2Headings) !==
          JSON.stringify(LEARN_TOPIC_ENTRY_HEADINGS)
        ) {
          report.errors.push(
            finding(
              "LEARN_TOPIC_ENTRY_HEADINGS_INVALID",
              relativeFile,
              `入门路线 H2 必须按顺序使用：${LEARN_TOPIC_ENTRY_HEADINGS.join("、")}。`,
            ),
          );
        }
        const entryCards = renderedTagTokens(text).filter(
          (token) =>
            token.name === "flashcard" || token.name === "flashcard_ref",
        );
        if (entryCards.length > 0) {
          report.errors.push(
            finding(
              "LEARN_TOPIC_ENTRY_FLASHCARD_FORBIDDEN",
              relativeFile,
              `入门路线不得包含 flashcard 或 flashcard_ref（第 ${entryCards[0].line} 行）。`,
            ),
          );
        }
        if (
          !renderedTagTokens(text).some((token) => token.name === "mermaid")
        ) {
          report.errors.push(
            finding(
              "LEARN_TOPIC_ENTRY_MERMAID_REQUIRED",
              relativeFile,
              "入门路线必须用 Mermaid 展示学习路径，不得只用文字描述。",
            ),
          );
        }
        if (!renderedTagTokens(text).some((token) => token.name === "note")) {
          report.errors.push(
            finding(
              "LEARN_TOPIC_ENTRY_NOTE_REQUIRED",
              relativeFile,
              "入门路线必须用 note 提示主题范围或学习边界。",
            ),
          );
        }
        ...
        if (h2Headings.includes("常见问题")) {
          ...
          if (
            ...
            report.errors.push(
              finding(
                "LEARN_TOPIC_FAQ_FLASHCARD_REQUIRED",
                relativeFile,
                "存在“常见问题”时必须使用 flashcard 或 flashcard_ref；没有真实复习题时请删除该章节。",
              ),
            );
          }
        }
        if (h2Headings.at(-1) !== "参考资料") {
          report.errors.push(
            finding(
              "LEARN_TOPIC_REFERENCE_HEADING_REQUIRED",
              relativeFile,
              "公开主题文章和实战文章最后一个 H2 必须是“参考资料”。",
            ),
          );
        }
      }
      if (data.published === true) {
        for (const card of text.matchAll(
          /\{%\s*flashcard\b[\s\S]*?\{%\s*endflashcard\s*%\}/g,
        )) {
          if (LEARN_TOPIC_NAVIGATION_CARD_PATTERN.test(card[0])) {
            report.errors.push(
              finding(
                "LEARN_TOPIC_NAVIGATION_CARD_FORBIDDEN",
                relativeFile,
                `第 ${lineForOffset(text, card.index)} 行闪卡是课程导航问题，不属于主题复习内容。`,
              ),
            );
          }
        }
        ...
        if (
          !referenceTags.some((token) => token.name === "linkgroup") ||
          !referenceTags.some((token) => token.name === "link")
        ) {
          report.errors.push(
            finding(
              "LEARN_TOPIC_REFERENCE_TAG_REQUIRED",
              relativeFile,
              "公开课程的“参考资料”必须使用 {% linkgroup %} 包裹至少一个 {% link %} 资料卡片。",
            ),
          );
        }
        if (referenceLinks.length === 0) {
          report.errors.push(
            finding(
              "LEARN_TOPIC_REFERENCE_LINK_REQUIRED",
              relativeFile,
              "公开课程的“参考资料”必须包含至少一个 HTTP(S) 资料链接。",
            ),
          );
        }
        for (const reference of referenceLinks) {
          if (!isValidHttpUrl(reference.target)) {
            report.errors.push(
              finding(
                "LEARN_TOPIC_REFERENCE_LINK_INVALID",
                relativeFile,
                `第 ${reference.line} 行“参考资料”中的 ${reference.kind} 链接不是有效的 HTTP(S) 地址。`,
              ),
            );
          }
          if (
            reference.kind === "tag" &&
            (!reference.imageValid || reference.image === reference.target)
          ) {
            report.errors.push(
              finding(
    ...
    if (
      ...
      const item = finding(
        ...
        relativeFile,
        release
          ? "发布前必须由 hexo-abbrlink 生成 abbrlink。"
          : "当前可在构建前缺省；构建后需要重新校验。",
      );
  ...
  for (const [courseKey, sequence] of courseSequences) {
    ...
    if (numbers.some((number, index) => number !== expected[index])) {
      report.errors.push(
        finding(
          "LEARN_TOPIC_SEQUENCE_INVALID",
          `${LEARN_TOPIC_ROOT}${courseKey}`,
          "课程中文序号必须从一开始连续且不重复。",
        ),
      );
    }
  ...
  for (const [courseKey, articles] of courseArticles) {
    validateCourseContract(
      report,
      projectRoot,
      courseKey,
      articles,
      courseSeriesNames.get(courseKey) ?? new Set(),
    );
  }
...
export function auditTags({ root = process.cwd() } = {}) {
  ...
  if (!existsSync(packageTarget)) {
    ...
    try {
      ...
      report.facts.plugin.declaredVersion =
        dependencies[TAG_PLUGIN_PACKAGE] ?? null;
      report.facts.flashcardPlugin.declaredVersion =
        dependencies[FLASHCARD_PLUGIN_PACKAGE] ?? null;
      if (dependencies["hexo-renderer-marked"])
        report.facts.plugin.rendererPackage = "hexo-renderer-marked";
      else if (dependencies["hexo-renderer-kramed"])
        report.facts.plugin.rendererPackage = "hexo-renderer-kramed";
      report.facts.plugin.rendererDeclared = Boolean(
        report.facts.plugin.rendererPackage,
      );
      if (!report.facts.plugin.declaredVersion) {
        report.errors.push(
          finding(
            "TAG_PLUGIN_DEPENDENCY_UNDECLARED",
            "package.json",
            `${TAG_PLUGIN_PACKAGE} 未声明为项目依赖。`,
          ),
        );
      }
      if (!report.facts.plugin.rendererDeclared) {
        report.warnings.push(
          finding(
            "TAG_RENDERER_COMPATIBILITY_RISK",
            "package.json",
            "未声明受支持的 Markdown 渲染器，复杂外挂标签可能无法正确渲染。",
          ),
        );
      }
    } catch {
  ...
  if (existsSync(flashcardPackageTarget)) {
    try {
      report.facts.flashcardPlugin.installedVersion =
        readJson(flashcardPackageTarget)?.version ?? null;
    } catch {
      report.errors.push(
        finding(
          "FLASHCARD_PLUGIN_PACKAGE_INVALID",
          path.posix.join(
            "node_modules",
            FLASHCARD_PLUGIN_PACKAGE,
            "package.json",
    ...
    report.errors.push(
      finding(
        "FLASHCARD_PLUGIN_NOT_INSTALLED",
        path.posix.join("node_modules", FLASHCARD_PLUGIN_PACKAGE),
        "已声明闪卡插件但当前未安装。",
      ),
    );
  }

  const pluginPackageTarget = path.join(
    projectRoot,
    "node_modules",
    TAG_PLUGIN_PACKAGE,
    "package.json",
  );
  const pluginSourceTarget = path.join(
    projectRoot,
    "node_modules",
    TAG_PLUGIN_PACKAGE,
    "index.js",
  ...
  if (!existsSync(pluginPackageTarget) || !existsSync(pluginSourceTarget)) {
    report.errors.push(
      finding(
        "TAG_PLUGIN_NOT_INSTALLED",
        path.posix.join("node_modules", TAG_PLUGIN_PACKAGE),
        "未发现可检查的已安装外挂标签插件。",
      ),
    );
  } else {
    try {
      report.facts.plugin.installedVersion =
        readJson(pluginPackageTarget).version ?? null;
    } catch {
      report.errors.push(
        finding(
          "JSON_PARSE_FAILED",
    ...
    const missingTags = TAG_PLUGIN_EXPECTED_TAGS.filter(
      (name) => !registeredTags.includes(name),
    );
  ...
  if (
    report.facts.plugin.installedVersion &&
    report.facts.plugin.installedVersion !== TAG_PLUGIN_BASELINE_VERSION
  ) {
    report.warnings.push(
      finding(
  ...
  const butterflyConfig = parseProjectYaml(
    report,
    projectRoot,
    "_config.butterfly.yml",
    "BUTTERFLY_CONFIG_MISSING",
  );
  ...
  if (!tagPluginConfig) {
    ...
    if (!report.facts.plugin.enabled) {
      report.errors.push(
        finding(
          "TAG_PLUGIN_DISABLED",
          report.facts.plugin.configurationSource,
          "tag_plugins.enable 未开启。",
        ),
      );
    }
    if (!isObject(tagPluginConfig.CDN)) {
      report.errors.push(
        finding(
          "TAG_PLUGIN_CDN_CONFIG_MISSING",
          report.facts.plugin.configurationSource,
          "tag_plugins.CDN 必须是映射，插件生成阶段会读取该配置。",
        ),
      );
    }
    if (
      !isObject(tagPluginConfig.link) ||
      !isNonEmptyString(tagPluginConfig.link.placeholder)
    ) {
      report.errors.push(
        finding(
          "TAG_PLUGIN_LINK_CONFIG_MISSING",
          report.facts.plugin.configurationSource,
          "tag_plugins.link.placeholder 未配置。",
        ),
      );
    }
  }
  ...
  if (isObject(flashcardConfig)) {
    ...
    report.errors.push(
      finding(
        "FLASHCARD_PLUGIN_CONFIG_MISSING",
        "_config.yml",
        "已声明闪卡插件但缺少 flashcard 配置。",
      ),
    );
  }
  ...
  for (const target of markdownFiles) {
    ...
    for (const token of tokens) {
      ...
      if (!token.name.startsWith("end") && !registry.tags.has(token.name)) {
        report.errors.push(
          finding(
            "TAG_UNREGISTERED",
            relativeFile,
            `第 ${token.line} 行使用了未在当前 Hexo 项目注册的标签 ${token.name}。`,
          ),
        );
      }
      ...
      if (
        token.name === "issues" &&
        report.facts.plugin.issuesEnabled === false
      ) {
        report.errors.push(
          finding(
            "TAG_CAPABILITY_DISABLED",
            relativeFile,
            `第 ${token.line} 行使用了 issues，但 tag_plugins.issues 当前关闭。`,
          ),
        );
      }
    }
  ...
  if (tagCounts.has("mermaid") && butterflyConfig?.mermaid?.enable !== true) {
    report.errors.push(
      finding(
        "TAG_CAPABILITY_DISABLED",
        "_config.butterfly.yml",
        "文章使用了 mermaid，但 mermaid.enable 未开启。",
      ),
    );
  }
  if (tagCounts.has("chartjs") && butterflyConfig?.chartjs?.enable !== true) {
    report.errors.push(
      finding(
        "TAG_CAPABILITY_DISABLED",
        "_config.butterfly.yml",
        "文章使用了 chartjs，但 chartjs.enable 未开启。",
      ),
    );
  }

  report.facts.usage.usedPluginTags = [...usedPluginTags].sort();
  report.facts.usage.usedFlashcardTags = ["flashcard", "flashcard_ref"].filter(
    (name) => tagCounts.has(name),
  );
  report.facts.usage.tagCounts = Object.fromEntries(
...
function parseProjectYaml(report, root, relativeFile, missingCode) {
  ...
  if (!existsSync(target)) {
    report.errors.push(
      finding(missingCode, relativeFile, "必需配置文件不存在。"),
    );
    return null;
  }
  try {
    ...
    report.errors.push(
      finding(
        "YAML_PARSE_FAILED",
        relativeFile,
        `YAML 无法解析，约在第 ${Number(error?.mark?.line ?? 0) + 1} 行。`,
      ),
    );
    return null;
  }
...
export function auditProject({ root = process.cwd() } = {}) {
  ...
  const report = reportFor("project", {
    projectRootType: existsSync(path.join(projectRoot, ".git"))
      ? "git"
      : "directory",
    runtime: {
    ...
    ci: ciWorkflowFacts(projectRoot),
    activeFeatures: {},
    localInjectedAssets: [],
    localCdnAssets: [],
  ...
  const requiredPaths = [
    ...
    "_config.butterfly.yml",
    "scaffolds/post.md",
    "scripts/course-series.js",
    "source/_posts",
  ];
  for (const relativePath of requiredPaths) {
    if (!existsSync(path.join(projectRoot, relativePath))) {
      report.errors.push(
        finding("REQUIRED_PATH_MISSING", relativePath, "项目必需路径不存在。"),
      );
    }
  }
  ...
  if (existsSync(packageTarget)) {
    try {
      const projectPackage = readJson(packageTarget);
      report.facts.packages.declaredHexo =
        projectPackage?.dependencies?.hexo ?? null;
      report.facts.packages.declaredButterfly =
        projectPackage?.dependencies?.["hexo-theme-butterfly"] ?? null;
    } catch {
      report.errors.push(
        finding("JSON_PARSE_FAILED", "package.json", "package.json 无法解析。"),
      );
  ...
  const installedHexoTarget = path.join(
    projectRoot,
    "node_modules/hexo/package.json",
  );
  if (existsSync(installedHexoTarget)) {
    try {
      ...
      report.facts.runtime.requiredNode = minimumVersion(
        installedHexo?.engines?.node,
      );
      if (report.facts.runtime.requiredNode) {
        report.facts.runtime.compatible =
          compareVersions(
            process.versions.node,
            report.facts.runtime.requiredNode,
          ) >= 0;
  ...
  if (!activeTheme) {
    ...
    if (
      ...
      report.errors.push(
        finding(
          "THEME_DEPENDENCY_UNDECLARED",
          "package.json",
          "npm Butterfly 主题必须声明为项目依赖。",
        ),
      );
    }
    report.facts.site.themeSource = activeTheme.source;
    report.facts.site.themeRoot = activeTheme.relative;
    const themePackageTarget = path.join(activeTheme.root, "package.json");
    if (existsSync(themePackageTarget)) {
      try {
        report.facts.packages.butterfly =
          readJson(themePackageTarget).version ?? null;
      } catch {
        report.warnings.push(
          finding(
            "JSON_PARSE_FAILED",
            path.posix.join(activeTheme.relative, "package.json"),
            "主题包元数据无法解析。",
          ),
        );
      }
  ...
  if (hexoConfig) {
    ...
    report.facts.site.theme =
      typeof hexoConfig.theme === "string" ? hexoConfig.theme : null;
    report.facts.deploy.type =
      typeof hexoConfig?.deploy?.type === "string"
        ? hexoConfig.deploy.type
        : null;
    report.facts.deploy.repo = sanitizeUrl(hexoConfig?.deploy?.repo);
    report.facts.deploy.branch =
      typeof hexoConfig?.deploy?.branch === "string"
  ...
  if (butterflyConfig) {
    ...
    for (const [feature, dottedPath] of Object.entries(featurePaths)) {
      ...
      if (typeof value === "boolean" || typeof value === "string")
        report.facts.activeFeatures[feature] = value;
    }
    ...
    for (const asset of report.facts.localInjectedAssets) {
      ...
      if (!existsSync(target)) {
        report.warnings.push(
          finding(
            "INJECTED_ASSET_MISSING",
            path.posix.join("source", asset),
            "Butterfly 注入配置引用的本地资源不存在。",
          ),
        );
      }
    }

    report.facts.localCdnAssets = localCdnAssets(butterflyConfig);
    for (const asset of report.facts.localCdnAssets) {
      const target = path.join(projectRoot, "source", asset.replace(/^\//, ""));
      if (!existsSync(target)) {
        report.errors.push(
          finding(
            "CDN_ASSET_MISSING",
            path.posix.join("source", asset),
            "Butterfly CDN.option 引用的本地资源不存在。",
          ),
        );
      }
    }
  ...
  if (existsSync(scaffoldTarget)) {
    const scaffoldText = readText(scaffoldTarget);
    const keys = new Set(topLevelFrontMatterKeys(scaffoldText));
    ...
    if (
      ...
      requiredPlaceholders.some(
        (placeholder) => !scaffoldText.includes(placeholder),
      )
    ) {
      report.warnings.push(
        finding(
          "SCAFFOLD_SCHEMA_DRIFT",
          "scaffolds/post.md",
          "文章脚手架缺少必需字段、数组结构或明确的待替换占位值。",
        ),
      );
    }
  }
  ...
  if (existsSync(workflowTarget) && report.facts.runtime.requiredNode) {
    ...
    if (match) {
      ...
      if (compareVersions(match[1], report.facts.runtime.requiredNode) < 0) {
        report.warnings.push(
          finding(
            "CI_NODE_INCOMPATIBLE",
            ".github/workflows/deploy.yml",
            "GitHub Actions 的 Node.js 版本低于已安装 Hexo 的最低要求。",
          ),
        );
      }
    }
  ...
  const themeGit =
    activeTheme?.source === "local" ? inspectGit(activeTheme.root) : null;
  const legacyThemeGit = inspectGit(
    path.join(projectRoot, "themes/butterfly-legacy"),
  );
  const deployGit = inspectGit(path.join(projectRoot, ".deploy_git"));
  report.facts.git = {
    root: rootGit,
    theme: themeGit,
    legacyTheme: legacyThemeGit,
    deploy: deployGit,
  };
  ...
  if (!rootGit.present)
    report.warnings.push(
      finding(
        "ROOT_NOT_GIT",
        ".",
        "项目根目录没有 Git 元数据，无法提供源码级回滚证据。",
      ),
    );
  if (activeTheme?.source === "local") {
    if (!themeGit?.present)
      report.warnings.push(
        finding(
          "THEME_NOT_GIT",
          activeTheme.relative,
          "当前运行主题目录不是可检查的独立 Git 工作树。",
        ),
      );
    else if (themeGit.dirty === null)
      report.warnings.push(
        finding(
    ...
    else if (themeGit.dirty)
      report.warnings.push(
        finding(
          "THEME_DIRTY",
          activeTheme.relative,
          `当前运行主题工作树存在 ${themeGit.entries} 个未提交状态项。`,
        ),
      );
  }
  ...
  else if (deployGit.dirty)
    report.warnings.push(
      finding(
        "DEPLOY_WORKTREE_DIRTY",
        ".deploy_git",
        `部署工作树存在 ${deployGit.entries} 个未提交状态项。`,
      ),
    );
  ...
  if (existsSync(javascriptFiles)) {
    sensitiveFiles.push(
      ...readdirSync(javascriptFiles, { withFileTypes: true })
        .filter((entry) => entry.isFile() && entry.name.endsWith(".js"))
        .map((entry) => path.posix.join("source/js", entry.name)),
    );
  }
  report.facts.sensitiveKeyLocations = sensitiveKeyLocations(
    projectRoot,
    sensitiveFiles,
  );
...
function repositoryEntries(root, current = root) {
  ...
  for (const entry of readdirSync(current, { withFileTypes: true })) {
    ...
    if (current === root && REPOSITORY_IGNORED_ROOT_FILES.has(entry.name))
      continue;
    const target = path.join(current, entry.name);
    const relative = safeRelative(root, target);
    if (
      relative === "themes/butterfly" ||
      relative === "themes/butterfly-legacy"
    )
      continue;
...
function cssSyntaxError(text) {
  ...
  const statementError = (frame, raw, offset) => {
    ...
    if (frame.kind === "style") {
      ...
      if (!/^(?:--[\w-]+|[-*_]?[a-zA-Z_][\w-]*)$/.test(property))
        return { line, reason: `CSS 属性名无效：${property || "(空)"}` };
      if (!value) return { line, reason: `CSS 属性 ${property} 缺少值` };
      return null;
    }
    if (frame.kind === "root" || frame.kind === "group") {
      if (!/^@[a-z-]+\b/i.test(statement))
        return {
          line,
          reason: "样式表或分组规则中出现了未包裹在选择器内的声明",
        };
      return null;
    }
  ...
  for (let index = 0; index < text.length; index += 1) {
    ...
    if (character === ";") {
      const error = statementError(
        frame,
        text.slice(frame.statementStart, index),
        frame.statementStart,
      );
    ...
    if (character === "{") {
      ...
      if (!prelude)
        return {
          line: lineForOffset(text, index),
          reason: "CSS 规则缺少选择器或 at-rule 前导",
        };
      let kind = "style";
      const atRule = prelude.match(/^@([a-z-]+)/i)?.[1]?.toLowerCase();
      if (atRule === "keyframes" || atRule?.endsWith("keyframes"))
        kind = "keyframes";
      else if (
        atRule &&
        !["font-face", "page", "property", "counter-style"].includes(atRule)
      )
        kind = "group";
    ...
    if (character === "}") {
      if (frames.length === 1)
        return {
          line: lineForOffset(text, index),
          reason: "出现了没有开始块的右花括号",
        };
      const closingFrame = frames[frames.length - 1];
      const error = statementError(
        closingFrame,
        text.slice(closingFrame.statementStart, index),
        closingFrame.statementStart,
      );
      if (error) return error;
      frames.pop();
  ...
  if (pairs.length > 0)
    return {
      line: lineForOffset(text, pairs.at(-1).index),
      reason: `CSS ${pairs.at(-1).character} 未闭合`,
    };
  if (frames.length !== 1)
    return { line: text.split(/\r?\n/).length, reason: "CSS 规则花括号未闭合" };
  const trailingError = statementError(
    frames[0],
    text.slice(frames[0].statementStart),
    frames[0].statementStart,
  );
...
function imageSignatureMatches(target) {
  ...
  if (extension === ".svg")
    return /^\s*(?:<\?xml[^>]*>\s*)?<svg\b/i.test(buffer.toString("utf8"));
  return true;
}
...
function courseReferenceLinks(text, projectRoot = process.cwd()) {
  ...
  for (const match of markdown.matchAll(/\{%\s*link\s+([^%]*)%\}/g)) {
    ...
    links.push({
      ...
      line: section.startLine - 1 + lineForOffset(markdown, match.index),
      imageValid: isValidReferencePreviewImage(
        projectRoot,
        image,
        argumentsList[1] ?? "",
      ),
...
function isRelatedReferenceImage(target, image) {
  ...
  try {
    ...
    if (
      targetHost === imageHost ||
      targetDomain === imageDomain ||
      imageHost.endsWith(`.${targetHost}`) ||
      targetHost.endsWith(`.${imageHost}`)
    )
      return true;
    return REFERENCE_OFFICIAL_CDN_ALIASES.some(
      (alias) =>
        alias.targetHost.test(targetHost) &&
...
function isValidReferencePreviewImage(
  projectRoot,
  value,
  referenceTarget = "",
) {
  if (!isNonEmptyString(value) || REFERENCE_GENERIC_IMAGE_PATH.test(value))
    return false;
  if (value.startsWith("/img/")) {
    const target = resolveLocalImage(projectRoot, value);
    return Boolean(
      target && existsSync(target) && imageSignatureMatches(target),
    );
  }
  ...
  try {
    const url = new URL(value);
    return (
      (REFERENCE_IMAGE_EXTENSIONS.test(url.pathname) ||
        REFERENCE_IMAGE_PATH_HINT.test(url.pathname)) &&
...
function resolvedMarkdownLink(projectRoot, sourceFile, linkTarget) {
  if (
    /^(?:https?:|mailto:|tel:|data:|app:|#|\/)/i.test(linkTarget) ||
    /\{\{.*\}\}/.test(linkTarget)
  )
    return null;
  let decoded;
...
export function auditStructure({ root = process.cwd() } = {}) {
  ...
  for (const relativePath of requiredPaths) {
    if (!existsSync(path.join(projectRoot, relativePath))) {
      report.errors.push(
        finding(
          "REPOSITORY_REQUIRED_PATH_MISSING",
          relativePath,
          "全仓库 lint 所需路径不存在。",
        ),
      );
  ...
  for (const legacyScript of [
    ...
    if (existsSync(path.join(projectRoot, legacyScript))) {
      report.errors.push(
        finding(
          "AUDIT_SCRIPT_LEGACY_LOCATION",
          legacyScript,
          "全仓库审计脚本必须只保存在项目根目录 .agents/scripts/。",
        ),
      );
    }
  }
  ...
  for (const { target, entry, relative } of ownedEntries) {
    ...
    if (!repositoryNamesByParent.has(parent))
      repositoryNamesByParent.set(parent, new Map());
    const siblings = repositoryNamesByParent.get(parent);
    if (
      siblings.has(normalizedName) &&
      siblings.get(normalizedName) !== entry.name
    ) {
      report.errors.push(
        finding(
          "REPOSITORY_PATH_COLLISION",
          relative,
          `与同目录下的 ${siblings.get(normalizedName)} 发生大小写或 Unicode 规范化冲突。`,
        ),
      );
    ...
    if (
      FORBIDDEN_SOURCE_ARTIFACT_PATTERNS.some((pattern) =>
        pattern.test(entry.name),
      )
    ) {
      report.errors.push(
        finding(
          "REPOSITORY_ARTIFACT_FORBIDDEN",
          relative,
          "仓库维护范围内不得保留系统元数据、备份或临时文件。",
        ),
      );
    }
    ...
    if (
      ...
      report.errors.push(
        finding(
          "HEXO_SCRIPT_FILE_INVALID",
          relative,
          "Hexo 自动加载的 scripts/ 只允许 JavaScript 扩展。",
        ),
      );
    if (
      entry.isFile() &&
      relative.startsWith("source/js/") &&
      !relative.endsWith(".js")
    )
      report.errors.push(
        finding(
          "SOURCE_JAVASCRIPT_FILE_INVALID",
          relative,
          "source/js/ 只允许 JavaScript 文件。",
        ),
      );
    if (
      entry.isFile() &&
      relative.startsWith("source/css/") &&
      !relative.endsWith(".css")
    )
      report.errors.push(
        finding(
          "SOURCE_STYLESHEET_FILE_INVALID",
          relative,
          "source/css/ 只允许 CSS 文件。",
        ),
      );
    if (
      entry.isFile() &&
      relative.startsWith("source/_data/") &&
      !/\.(?:json|ya?ml)$/i.test(relative)
    )
      report.errors.push(
        finding(
          "SOURCE_DATA_FILE_INVALID",
          relative,
          "source/_data/ 只允许 JSON 或 YAML 数据文件。",
        ),
      );
  }
  ...
  for (const { target, entry } of sourceEntries) {
    ...
    if (
      entry.isFile() &&
      relativePath.startsWith("source/_posts/") &&
      !entry.name.endsWith(".md")
    ) {
      report.errors.push(
        finding(
          "POST_FILE_TYPE_INVALID",
          relativePath,
          "source/_posts 只允许 Markdown 文章和课程目录。",
        ),
      );
    }
  }

  const pages = markdownFilesUnder(sourceRoot).filter((target) => {
    const relative = safeRelative(projectRoot, target);
    return (
      !relative.startsWith("source/_posts/") &&
  ...
  for (const target of pages) {
    ...
    if (parsed.error) {
      report.errors.push(
        finding(
          "PAGE_FRONT_MATTER_PARSE_FAILED",
          relativeFile,
          `页面 Front Matter 无法解析，约在第 ${parsed.error.line} 行。`,
        ),
      );
      continue;
    }
    ...
    if (
      !isValidDateValue(parsed.data.date, frontMatterScalarValue(text, "date"))
    )
      report.errors.push(
        finding(
          "PAGE_DATE_INVALID",
  ...
  if (existsSync(learnRoot)) {
    for (const entry of readdirSync(learnRoot, { withFileTypes: true })) {
      ...
      if (
        ...
        report.errors.push(
          finding(
            "LEARN_TOPIC_DIRECTORY_INVALID",
            relativePath,
            "课程路径段必须是唯一一层 kebab-case 小写目录。",
          ),
        );
      }
    }
  ...
  if (existsSync(contractRoot)) {
    ...
    report.facts.contractCount = contractEntries.filter(
      (entry) => entry.isFile() && entry.name.endsWith(".json"),
    ).length;
    const courseKeys = new Set(
      existsSync(learnRoot)
    ...
    for (const entry of contractEntries) {
      ...
      if (
        ...
        report.errors.push(
          finding(
            "LEARN_TOPIC_CONTRACT_FILE_INVALID",
            relativePath,
            "课程数据目录只允许与课程 slug 同名的 JSON 契约。",
          ),
        );
        continue;
      }
      if (!courseKeys.has(entry.name.replace(/\.json$/, ""))) {
        report.errors.push(
          finding(
            "LEARN_TOPIC_CONTRACT_ORPHANED",
            relativePath,
            "契约没有对应的 source/_posts/learn-topic 课程目录。",
          ),
        );
      }
    }
...
export function auditConfig({ root = process.cwd() } = {}) {
  ...
  const entries = repositoryEntries(projectRoot).filter((item) =>
    item.entry.isFile(),
  );
  ...
  const yamlFiles = entries.filter((item) => {
    ...
    return (
      item.relative.startsWith("_config") ||
      CONFIG_YAML_ROOTS.some(
        (rootPath) =>
          item.relative === rootPath ||
          item.relative.startsWith(`${rootPath}/`),
      )
    );
  });
  ...
  for (const item of jsonFiles) {
    try {
      ...
      report.errors.push(
        finding("JSON_PARSE_FAILED", item.relative, "JSON 文件无法解析。"),
      );
    }
  }
  ...
  if (packageData) {
    ...
    if (packageData.private !== true)
      report.errors.push(
        finding(
          "PACKAGE_PRIVATE_REQUIRED",
          "package.json",
          "博客站点包必须声明 private: true，避免误发布到 npm。",
        ),
      );
    ...
    if (!sameStringRecord(dependencies, lockedDependencies)) {
      report.errors.push(
        finding(
          "PACKAGE_LOCK_DEPENDENCY_DRIFT",
          "package-lock.json",
          "根依赖声明与 package.json 不一致。",
        ),
      );
    }
    for (const dependency of Object.keys(dependencies)) {
      if (
        !existsSync(
          path.join(projectRoot, "node_modules", dependency, "package.json"),
        )
      ) {
        report.errors.push(
          finding(
            ...
            "node_modules",
            `package.json 声明的依赖 ${dependency} 未安装。`,
          ),
        );
      }
    }
    if (existsSync(path.join(projectRoot, "node_modules"))) {
      const npmResult = spawnSync("npm", ["ls", "--depth=0", "--json"], {
        cwd: projectRoot,
        encoding: "utf8",
      });
      if (npmResult.status !== 0)
        report.errors.push(
          finding(
            ...
            "node_modules",
            "npm 顶层依赖树与 package.json/package-lock.json 不一致。",
          ),
        );
    }
  }
  ...
  if (existsSync(configTarget)) {
    try {
      ...
      if (!isNonEmptyString(config.url) || !/^https?:\/\//.test(config.url))
        report.errors.push(
          finding(
            "SITE_URL_INVALID",
            "_config.yml",
            "url 必须是有效的 HTTP(S) 站点地址。",
          ),
        );
      if (
        !isNonEmptyString(config.theme) ||
        !resolveThemeRoot(projectRoot, config.theme)
      )
        report.errors.push(
          finding(
            "THEME_PATH_INVALID",
            "_config.yml",
            "theme 必须指向可用的本地主题或已安装 npm 主题。",
          ),
        );
      if (!isNonEmptyString(config.timezone))
        report.errors.push(
          finding("TIMEZONE_MISSING", "_config.yml", "timezone 必须显式声明。"),
        );
    } catch {
      // YAML 解析错误已在上面报告。
    }
...
export function auditCode({ root = process.cwd() } = {}) {
  const projectRoot = path.resolve(root);
  const report = reportFor("code", {
    javascriptFileCount: 0,
    cssFileCount: 0,
    shellFileCount: 0,
  });
  const files = repositoryEntries(projectRoot).filter((item) =>
    item.entry.isFile(),
  );
  const javascriptFiles = files.filter(
    (item) =>
      /\.(?:js|mjs|cjs)$/.test(item.relative) &&
      CODE_ROOTS.some((rootPath) => item.relative.startsWith(`${rootPath}/`)),
  );
  const cssFiles = files.filter(
    (item) =>
      item.relative.startsWith("source/css/") && item.relative.endsWith(".css"),
  );
  ...
  for (const item of javascriptFiles) {
    const result = spawnSync(process.execPath, ["--check", item.target], {
      encoding: "utf8",
    });
    if (result.status !== 0) {
      ...
      report.errors.push(
        finding(
          ...
          item.relative,
          `JavaScript 无法通过 Node.js 语法检查${line ? `，约在第 ${line} 行` : ""}。`,
        ),
      );
    }
  }
    ...
    if (result.status !== 0) {
      ...
      report.errors.push(
        finding(
          ...
          item.relative,
          `Shell 脚本无法通过 bash -n${line ? `，约在第 ${line} 行` : ""}。`,
        ),
      );
    }
  }
...
export function auditSkills({ root = process.cwd() } = {}) {
  ...
  for (const skillName of skillDirectories) {
    ...
    if (!existsSync(skillFile)) {
      report.errors.push(
        finding(
          "SKILL_ENTRY_MISSING",
          path.posix.join(relativeSkill, "SKILL.md"),
          "Skill 目录缺少 SKILL.md。",
        ),
      );
      continue;
    ...
    if (!existsSync(mirror) || symlinkTarget(mirror) !== expectedTarget) {
      report.errors.push(
        finding(
          "SKILL_MIRROR_INVALID",
          path.posix.join(".claude/skills", skillName),
          `必须是指向 ${expectedTarget} 的相对符号链接。`,
        ),
      );
    } else report.facts.checkedSymlinkCount += 1;
    ...
    for (const entry of readdirSync(skillDirectory, { withFileTypes: true })) {
      if (entry.isDirectory() && !allowedDirectories.has(entry.name))
        report.errors.push(
          finding(
            ...
            path.posix.join(relativeSkill, entry.name),
            "Skill 根目录包含职责未定义的目录。",
          ),
        );
      if (entry.isFile() && entry.name !== "SKILL.md")
        report.errors.push(
          finding(
            "SKILL_ROOT_FILE_INVALID",
            path.posix.join(relativeSkill, entry.name),
            "Skill 根目录只允许 SKILL.md 和职责目录。",
          ),
        );
    }
    for (const item of entries) {
      const relativeFile = safeRelative(projectRoot, item.target);
      if (item.entry.isFile() && statSync(item.target).size === 0)
        report.errors.push(
          finding("SKILL_EMPTY_FILE", relativeFile, "Skill 不得包含空文件。"),
        );
      if (item.entry.isDirectory() && readdirSync(item.target).length === 0)
        report.errors.push(
          finding(
            "SKILL_EMPTY_DIRECTORY",
            relativeFile,
            "Skill 不得包含空目录。",
          ),
        );
      if (
        item.entry.isFile() &&
        relativeFile.includes("/prompts/") &&
        !relativeFile.endsWith(".agent.md")
      )
        report.errors.push(
          finding(
            "SKILL_PROMPT_NAME_INVALID",
            relativeFile,
            "Prompt 文件必须使用 .agent.md 后缀。",
          ),
        );
      if (
        item.entry.isFile() &&
        relativeFile.includes("/workflows/") &&
        !/^§\d{2}-.+\.md$/.test(item.entry.name)
      )
        report.errors.push(
          finding(
            "SKILL_WORKFLOW_NAME_INVALID",
            relativeFile,
            "工作流文件必须使用 §NN-name.md。",
          ),
        );
      if (
        item.entry.isFile() &&
        relativeFile.includes("/templates/") &&
        !/\.template\.[^.]+$/.test(item.entry.name)
      )
        report.errors.push(
          finding(
            "SKILL_TEMPLATE_NAME_INVALID",
            relativeFile,
            "模板文件必须使用 .template.<扩展名> 后缀。",
          ),
        );
      if (
        item.entry.isFile() &&
        relativeFile.includes("/examples/") &&
        !/\.example\.[^.]+$/.test(item.entry.name)
      )
        report.errors.push(
          finding(
            "SKILL_EXAMPLE_NAME_INVALID",
            relativeFile,
            "示例文件必须使用 .example.<扩展名> 后缀。",
          ),
        );
      if (
        item.entry.isFile() &&
        ["/checklists/", "/references/", "/rules/"].some((segment) =>
          relativeFile.includes(segment),
        ) &&
        !relativeFile.endsWith(".md")
      )
        report.errors.push(
          finding(
            "SKILL_DOCUMENT_FILE_INVALID",
            relativeFile,
            "Skill 清单、参考资料和规则文件必须使用 Markdown。",
          ),
        );
      if (
        item.entry.isFile() &&
        relativeFile.includes("/data/") &&
        !relativeFile.endsWith(".json")
      )
        report.errors.push(
          finding(
            "SKILL_DATA_FILE_INVALID",
            relativeFile,
            "Skill data/ 只允许 JSON 数据文件。",
          ),
        );
    }
    ...
    for (const markdownTarget of new Set(markdownResources)) {
      ...
      for (const match of markdownText.matchAll(
        /`((?:workflows|rules|templates|examples|checklists|prompts|references|assets|scripts|data)\/[^`\s]+)`/g,
      )) {
        report.facts.checkedReferenceCount += 1;
        const reference = match[1];
        const dynamicReference = /[<>{}]/.test(reference);
        const skillResourceExists = existsSync(
          path.join(skillDirectory, reference),
        );
        const projectResourceExists = existsSync(
  ...
  if (existsSync(mirrorRoot)) {
    for (const entry of readdirSync(mirrorRoot, { withFileTypes: true })) {
      if (!skillDirectories.includes(entry.name))
        report.errors.push(
          finding(
            "SKILL_MIRROR_ORPHANED",
            path.posix.join(".claude/skills", entry.name),
            "Claude Skill 镜像没有对应的 .agents/skills 维护源。",
          ),
        );
    }
  }
...
export function auditDocs({ root = process.cwd() } = {}) {
  ...
  for (const item of markdownFiles) {
    ...
    for (const link of markdownLocalLinks(text)) {
      const resolved = resolvedMarkdownLink(
        projectRoot,
        item.target,
        link.target,
      );
      ...
      if (!resolved)
        report.errors.push(
          finding(
            ...
            item.relative,
            `第 ${link.line} 行的本地链接无法解析：${link.target}`,
          ),
        );
    }
  }
...
export function auditLint({ root = process.cwd() } = {}) {
  ...
  const report = reportFor("lint", {
    checks: Object.fromEntries(
      checks.map((check) => [check.mode, check.status]),
    ),
  });
  for (const check of checks) {
    for (const item of check.errors)
      report.errors.push({
        ...item,
        code: `${check.mode.toUpperCase()}_${item.code}`,
      });
    for (const item of check.blockers)
      report.blockers.push({
        ...item,
        code: `${check.mode.toUpperCase()}_${item.code}`,
      });
...
export function auditRelease({ root = process.cwd(), route = "local" } = {}) {
  ...
  if (!["local", "ci"].includes(route)) {
    report.blockers.push(
      finding("ROUTE_INVALID", "route", "发布路由必须是 local 或 ci。"),
    );
  }
  ...
  for (const item of project.warnings) {
    const routeSpecificBlocker =
      route === "ci" && item.code === "CI_NODE_INCOMPATIBLE";
    const localSpecificBlocker =
      route === "local" &&
      [
        "DEPLOY_WORKTREE_MISSING",
        "DEPLOY_WORKTREE_DIRTY",
    ...
    if (
      COMMON_RELEASE_BLOCKERS.has(item.code) ||
      routeSpecificBlocker ||
      localSpecificBlocker
    )
  ...
  if (
    !project.facts.deploy.type ||
    !project.facts.deploy.repo ||
    !project.facts.deploy.branch
  ) {
    report.blockers.push(
      finding(
  ...
  if (route === "ci") {
    const ci = ciWorkflowFacts(root);
    const configuredRepository = normalizeGitRepository(
      project.facts.deploy.repo,
    );
    if (!ci.present)
      report.blockers.push(
        finding("CI_WORKFLOW_MISSING", ci.path, "缺少跨仓库发布工作流。"),
      );
    if (
      ci.externalRepository !== CI_PAGES_REPOSITORY ||
      ci.externalRepository !== configuredRepository
    ) {
      report.blockers.push(
        finding(
          "CI_TARGET_MISMATCH",
          ci.path,
          "CI 成品仓库必须与确认的 Pages 仓库及 _config.yml 保持一致。",
        ),
      );
    }
    if (
      ci.publishBranch !== CI_PAGES_BRANCH ||
      ci.publishBranch !== project.facts.deploy.branch
    ) {
      report.blockers.push(
        finding(
          "CI_BRANCH_MISMATCH",
          ci.path,
          "CI 发布分支必须与确认的 Pages 分支及 _config.yml 保持一致。",
        ),
      );
    }
    if (ci.publishDir !== "./public")
      report.blockers.push(
        finding(
          "CI_PUBLISH_DIR_INVALID",
          ci.path,
          "CI 必须发布 ./public 生成目录。",
        ),
      );
    if (ci.deployKeySecret !== "PAGES_DEPLOY_KEY")
      report.blockers.push(
        finding(
          "CI_DEPLOY_KEY_MISSING",
          ci.path,
          "CI 必须引用受限的 PAGES_DEPLOY_KEY Secret。",
        ),
      );
    if (
      !ci.hasLint ||
      ...
      report.blockers.push(
        finding(
          "CI_QUALITY_GATE_MISSING",
          ci.path,
          "CI 必须执行 lint、审计测试、构建和 CI 发布预检。",
        ),
      );
    }
  }
...
function runCli() {
  ...
  if (options.mode === "assets") report = auditAssets({ root: options.root });
    ...
    report = auditConfig({ root: options.root });
  else if (options.mode === "content")
    report = auditContent({ root: options.root, release: options.release });
    ...
    report = auditProject({ root: options.root });
  else if (options.mode === "release")
    report = auditRelease({ root: options.root, route: options.route });
  else if (options.mode === "skills")
    report = auditSkills({ root: options.root });
  else if (options.mode === "structure")
    report = auditStructure({ root: options.root });
  else if (options.mode === "tags") report = auditTags({ root: options.root });
  else {
    process.stderr.write(
      "未知审计模式。请使用 assets、code、config、content、docs、lint、project、release、skills、structure 或 tags。\n",
    );
    return 1;
  }
