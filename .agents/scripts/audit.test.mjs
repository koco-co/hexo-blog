...
function validPlaywrightCoursePost({
  number = "一",
  topic = "入门路线",
  published = true,
  placeholder = false,
...
function writeCourseContract(
  ...
  write(
    root,
    `.agents/skills/hexo-learn-topic/data/${slug}.json`,
    JSON.stringify({
      schema_version: 2,
      course: {
        slug,
...
function createProjectFixture(root) {
  ...
  write(
    ...
    "hexo.extend.tag.register('course_series', () => '')\n",
  );
...
function createTagPluginFixture(
  root,
  {
    enable = true,
    issues = false,
    ...
    JSON.stringify({
      ...
      dependencies: {
        ...
        "hexo-flashcard-plugin":
          "https://example.com/hexo-flashcard-plugin.tgz",
  ...
  write(
    root,
    "node_modules/hexo-butterfly-tag-plugins-plus/package.json",
    JSON.stringify({
  ...
  write(
    root,
    "node_modules/hexo-flashcard-plugin/package.json",
    JSON.stringify({
  ...
  write(
    root,
    "node_modules/hexo-flashcard-plugin/lib/parser.js",
    readFileSync(
      path.join(
        projectRoot,
        "node_modules/hexo-flashcard-plugin/lib/parser.js",
  ...
  write(
    root,
    "node_modules/hexo-flashcard-plugin/lib/errors.js",
    readFileSync(
      path.join(
        projectRoot,
        "node_modules/hexo-flashcard-plugin/lib/errors.js",
...
function createAssetFixture(
  root,
  {
    markdown = "![示例](/img/picgo-images/example.png)\n",
    image = Buffer.from([
  ...
  write(
    root,
    "tools/hexo-blog/image-migration-map.json",
    JSON.stringify({
...
test("course contract migration preserves the ledger and removes only the legacy field", () => {
  ...
  const ledger = {
    ...
    ledger: {
      fields: ["disposition", "target"],
      items: { cap: ["core", "Playwright(二)快速开始 / 核心功能"] },
    },
  ...
  const migratedEntry = readFileSync(
    path.join(
      root,
      "source/_posts/learn-topic/playwright/Playwright(一)入门路线.md",
    ),
  ...
  const contract = JSON.parse(
    readFileSync(
      path.join(root, ".agents/skills/hexo-learn-topic/data/playwright.json"),
      "utf8",
    ),
  );
...
test("content audit allows a pre-build missing abbrlink but blocks release mode", () => {
  ...
  assert.ok(
    draftReport.warnings.some((item) => item.code === "ABBRLINK_PENDING"),
  );

  const releaseReport = auditContent({ root, release: true });
  assert.equal(releaseReport.status, "blocked");
  assert.ok(
    releaseReport.blockers.some((item) => item.code === "ABBRLINK_MISSING"),
  );
});
...
test("CLI returns a non-zero exit code for warning as well as blocked reports", () => {
  ...
  const result = spawnSync(
    process.execPath,
    [
      path.join(projectRoot, ".agents/scripts/audit.mjs"),
      "content",
      "--root",
      root,
      "--json",
...
test("content and page audits reject impossible or unparseable dates", () => {
  ...
  write(
    ...
    validPost({ abbrlink: "bad-date" }).replace(
      "2026-08-10 12:00:00",
      "2026-02-30 12:00:00 # invalid calendar date",
    ),
  );
  write(
    root,
  ...
  assert.ok(
    content.errors.some(
      (item) =>
        item.code === "FRONT_MATTER_FIELD_INVALID" &&
        item.path === "source/_posts/example.md",
    ),
  );
  assert.ok(
    structure.errors.some(
      (item) =>
        item.code === "PAGE_DATE_INVALID" &&
        item.path === "source/about/index.md",
    ),
  );
});
...
test("content audit rejects duplicate abbrlinks without exposing front matter values", () => {
  ...
  write(
    ...
    validPost({ abbrlink: "duplicate", title: "第一篇" }),
  );
  write(
    root,
    "source/_posts/two.md",
    validPost({ abbrlink: "duplicate", title: "第二篇" }),
  );
...
test("content audit recursively checks nested learning posts", () => {
  ...
  write(
    ...
    validPost({ abbrlink: "root123", title: "根文章" }),
  );
  write(
    root,
    "source/_posts/archive/Playwright/01-browser.md",
    validPost({ abbrlink: "nested123", title: "嵌套文章" }),
  );
...
test("content audit blocks empty article bodies and unclosed Markdown fences", () => {
  ...
  write(
    root,
    "source/_posts/empty.md",
    validPost({ abbrlink: "empty" }).replace("\n正文。\n", "\n"),
  );
  write(
    root,
    "source/_posts/fence.md",
    `${validPost({ abbrlink: "fence", title: "围栏文章" })}\n\`\`\`js\nconst value = 1\n`,
  );
  const report = auditContent({ root, release: true });
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "ARTICLE_BODY_EMPTY" &&
        item.path === "source/_posts/empty.md",
    ),
  );
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "MARKDOWN_FENCE_UNCLOSED" &&
        item.path === "source/_posts/fence.md",
    ),
  );
});
...
test("content audit enforces concise non-FAQ headings and ignores code or FAQ headings", () => {
  ...
  const issues = report.errors.filter(
    (item) => item.code === "ARTICLE_HEADING_STYLE_INVALID",
  );
  assert.equal(issues.length, 2);
  assert.ok(
    issues.some((item) => item.message.includes("传输层：端点、首部和语义")),
  );
...
test("published course chapters reject internal chapter plans instead of synchronizing them", () => {
  ...
  const topic = validPlaywrightCoursePost({
    number: "二",
    topic: "快速开始",
  }).replace(
    "## 主题内容",
  ...
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "LEARN_TOPIC_META_HEADING_FORBIDDEN" &&
        item.message.includes("章节计划"),
    ),
  );
});
...
test("release audit rejects published false outside a valid course placeholder", () => {
  ...
  assert.ok(
    placeholderReport.errors.some(
      (item) => item.code === "PUBLISHED_FALSE_NOT_COURSE_PLACEHOLDER",
    ),
  );
  assert.ok(
    placeholderReport.blockers.some((item) => item.code === "ABBRLINK_MISSING"),
  );
...
test("content audit rejects a non-boolean published value", () => {
  ...
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "FRONT_MATTER_FIELD_INVALID" &&
        item.message.startsWith("published:"),
    ),
  );
});
...
test("learn-topic audit accepts a complete unpublished draft while retaining the placeholder marker", () => {
  const root = makeRoot();
  const entry = validPlaywrightCoursePost();
  const draft = validPlaywrightCoursePost({
    number: "二",
    topic: "快速开始",
    published: false,
  })
...
test("learn-topic audit requires a v2 course contract and rejects the legacy ledger field", () => {
  ...
  assert.ok(
    missingReport.errors.some(
      (item) => item.code === "LEARN_TOPIC_CONTRACT_MISSING",
    ),
  );
  assert.ok(
    missingReport.errors.some(
      (item) => item.code === "LEARN_TOPIC_LEDGER_FRONT_MATTER_FORBIDDEN",
    ),
  );
  ...
  assert.ok(
    mismatchReport.errors.some(
      (item) => item.code === "LEARN_TOPIC_CONTRACT_INVALID",
    ),
  );
});
...
test("learn-topic audit enforces Mermaid containers and flashcards only for a real FAQ", () => {
  ...
  const topic = validPlaywrightCoursePost({ number: "二", topic: "快速开始" })
    .replace("正文。", "```mermaid\nflowchart LR\nA --> B\n```")
    .replace(
      /\{% flashcard basic[\s\S]*?\{% endflashcard %\}/,
      "普通文本问答。",
    );
  write(
    root,
  ...
  assert.ok(
    report.errors.some(
      (item) => item.code === "LEARN_TOPIC_MERMAID_FENCE_FORBIDDEN",
    ),
  );
  assert.ok(
    report.errors.some(
      (item) => item.code === "LEARN_TOPIC_FAQ_FLASHCARD_REQUIRED",
    ),
  );
});
...
test("learn-topic audit requires a visual tag composition for published course正文", () => {
  ...
  assert.ok(
    report.errors.some(
      (item) => item.code === "LEARN_TOPIC_VISUAL_COMPOSITION_MISSING",
    ),
  );
});
...
test("learn-topic audit rejects naked explanation blocks and repeated course navigation copy", () => {
  const root = makeRoot();
  const entry = validPlaywrightCoursePost();
  const naked = validPlaywrightCoursePost({
    number: "二",
    topic: "快速开始",
  }).replace(
    "{% note info flat %}\n正文。\n{% endnote %}",
  ...
  assert.ok(
    nakedReport.errors.some(
      (item) => item.code === "LEARN_TOPIC_PLAIN_BODY_BLOCK",
    ),
  );

  const structuralConnector = validPlaywrightCoursePost({
    number: "二",
    topic: "快速开始",
  }).replace(
    "{% note info flat %}\n正文。\n{% endnote %}",
  ...
  assert.ok(
    !connectorReport.errors.some(
      (item) => item.code === "LEARN_TOPIC_PLAIN_BODY_BLOCK",
    ),
  );

  const disguisedExplanation = validPlaywrightCoursePost({
    number: "二",
    topic: "快速开始",
  }).replace(
    "{% note info flat %}\n正文。\n{% endnote %}",
    'Playwright 会持续重新查询 Locator，直到条件成立或超时。这不是一次读取结果的普通断言，失败时必须区分定位、等待与业务状态，而且不能用扩大超时掩盖真实原因：\n\n```python\nprint("ok")\n```',
  );
  write(
    root,
    "source/_posts/learn-topic/playwright/Playwright(二)快速开始.md",
    disguisedExplanation,
  );
  const disguisedReport = auditContent({ root, release: true });
  assert.ok(
    disguisedReport.errors.some(
      (item) => item.code === "LEARN_TOPIC_PLAIN_BODY_BLOCK",
    ),
  );

  const navigation = validPlaywrightCoursePost({
    number: "二",
    topic: "快速开始",
  }).replace(
    "说明主题机制、示例和边界。",
    "前置文章是上一篇，本文分配能力为 FIXTURE-001。",
  );
  write(
    root,
    "source/_posts/learn-topic/playwright/Playwright(二)快速开始.md",
  ...
  assert.ok(
    navigationReport.errors.some(
      (item) => item.code === "LEARN_TOPIC_NAVIGATION_COPY_FORBIDDEN",
    ),
  );
  assert.ok(
    navigationReport.errors.some(
      (item) => item.code === "LEARN_TOPIC_LEDGER_COPY_FORBIDDEN",
    ),
  );

  const internalCopy = validPlaywrightCoursePost({
    number: "二",
    topic: "快速开始",
  }).replace(
    "说明主题机制、示例和边界。",
    "文章(七)的实验使用 MOD-001 记录，正文不应暴露这些内部编号。",
  );
  write(
    root,
    "source/_posts/learn-topic/playwright/Playwright(二)快速开始.md",
  ...
  const tagOnly = validPlaywrightCoursePost({
    number: "二",
    topic: "快速开始",
...
test("concept stories exempt only bounded prose and preserve checks outside and inside the story", () => {
  ...
  write(
    root,
    target,
    withStory.replace("## 常见问题", "标记外仍然需要语义承载。\n\n## 常见问题"),
  );
  const outside = auditContent({ root, release: true }).errors.find(
    (item) => item.code === "LEARN_TOPIC_PLAIN_BODY_BLOCK",
  );
  ...
  write(
    root,
    target,
    withStory.replace("守门人按姓名找人", "前置文章是上一篇，守门人按姓名找人"),
  );
  assert.ok(
    auditContent({ root, release: true }).errors.some(
      (item) => item.code === "LEARN_TOPIC_NAVIGATION_COPY_FORBIDDEN",
    ),
  );
  write(
    ...
    withStory.replace(
      "守门人按姓名找人",
      "/tmp/story-result 是示例路径，守门人按姓名找人",
    ),
  );
  ...
  assert.ok(
    auditContent({ root, release: true }).errors.some(
      (item) => item.code === "LEARN_TOPIC_LOCAL_ABSOLUTE_PATH_FORBIDDEN",
    ),
  );
});
...
test("concept story boundaries fail closed for malformed, nested, hidden and unclosed regions", () => {
  ...
  for (const [fragment, code] of cases) {
    write(
      root,
      target,
      validPlaywrightCoursePost({ number: "二", topic: "快速开始" }).replace(
        "## 主题内容",
        `## 主题内容\n\n${fragment}\n\n仍需检查的普通解释。`,
      ),
    );
    ...
    assert.ok(
      ...
      `invalid region must not exempt prose: ${code}`,
    );
  }
});
...
test("story markers in fenced code or inline code do not activate an exception", () => {
  ...
  for (const fragment of [
    ...
    const report = auditContent({ root, release: true });
    assert.ok(
      !report.errors.some((item) => item.code.startsWith("CONCEPT_STORY_")),
...
test("unpublished course drafts also validate story boundaries", () => {
  ...
  assert.ok(
    auditContent({ root }).errors.some(
      (item) => item.code === "CONCEPT_STORY_UNCLOSED",
    ),
  );
});
...
test("learn-topic audit rejects local filesystem absolute paths when the course contract enables the gate", () => {
  const root = makeRoot();
  const entry = validPlaywrightCoursePost();
  const baseTopic = validPlaywrightCoursePost({
    number: "二",
    topic: "快速开始",
  });
...
test("learn-topic audit requires published reference card groups and valid HTTP links", () => {
  ...
  const missingCardsReport = auditContent({
    root: missingCardsRoot,
    release: true,
  });
  assert.ok(
    missingCardsReport.errors.some(
      (item) => item.code === "LEARN_TOPIC_REFERENCE_TAG_REQUIRED",
    ),
  );
  assert.ok(
    !missingCardsReport.errors.some(
      (item) => item.code === "LEARN_TOPIC_REFERENCE_LINK_REQUIRED",
    ),
  );
  ...
  const invalidLinkReport = auditContent({
    root: invalidLinkRoot,
    release: true,
  });
  assert.ok(
    invalidLinkReport.errors.some(
      (item) => item.code === "LEARN_TOPIC_REFERENCE_LINK_INVALID",
    ),
  );
  ...
  assert.equal(
    validTagReport.status,
    "pass",
    JSON.stringify(validTagReport.errors),
  );
});
...
test("learn-topic audit rejects default, page and unrelated reference preview images", () => {
  const cases = [
    ...
    [
      "page",
      "{% link Playwright, https://playwright.dev/python/, https://playwright.dev/python/ %}",
    ],
    [
      "default",
      "{% link Playwright, https://playwright.dev/python/, https://example.com/img/avatar.png %}",
    ],
    [
      "unrelated",
      "{% link Playwright, https://playwright.dev/python/, https://cdn.example.net/logo.svg %}",
    ],
  ];
  for (const [label, replacement] of cases) {
    ...
    assert.ok(
      report.errors.some(
        (item) => item.code === "LEARN_TOPIC_REFERENCE_PREVIEW_INVALID",
      ),
...
test("repository content audit applies reference preview lint outside learn-topic", () => {
  ...
  const report = auditContent({ root, release: true });
  assert.ok(
    report.errors.some((item) => item.code === "REFERENCE_PREVIEW_INVALID"),
  );
});
...
test("learn-topic audit rejects internal planning fields and navigation cards", () => {
  ...
  assert.ok(
    fieldReport.errors.some(
      (item) => item.code === "LEARN_TOPIC_INTERNAL_FRONT_MATTER_FORBIDDEN",
    ),
  );
  ...
  const topic = validPlaywrightCoursePost({
    number: "二",
    topic: "快速开始",
  }).replace("问题。", "课程应该从哪一篇开始，进阶路线会不会阻塞项目实战？");
  write(
    root,
    "source/_posts/learn-topic/playwright/Playwright(二)快速开始.md",
    topic,
  ...
  assert.ok(
    cardReport.errors.some(
      (item) => item.code === "LEARN_TOPIC_NAVIGATION_CARD_FORBIDDEN",
    ),
  );
});
...
test("learn-topic audit blocks a capability ledger that is not closed", () => {
  ...
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "LEARN_TOPIC_LEDGER_INVALID" &&
        item.path === contractPath,
    ),
  );
});
...
test("learn-topic ledger requires article assignments except for excluded capabilities", () => {
  ...
  assert.ok(
    auditContent({ root, release: true }).errors.some(
      (item) => item.code === "LEARN_TOPIC_LEDGER_INVALID",
    ),
  );

  contract.capability_ledger.ledger.items["fixture-capability"][0] = "excluded";
  contract.capability_ledger.ledger.items["fixture-capability"][1] =
    "Playwright(二)快速开始 / 核心功能";
  write(root, contractPath, JSON.stringify(contract));
  assert.ok(
    auditContent({ root, release: true }).errors.some(
      (item) => item.code === "LEARN_TOPIC_LEDGER_INVALID",
...
test("compressed ledger blocks invalid article indexes and source manifest count drift", () => {
  ...
  write(root, contractPath, JSON.stringify(contract));
  const messages = auditContent({ root, release: true })
    .errors.filter((item) => item.code === "LEARN_TOPIC_LEDGER_INVALID")
...
test("course contract template renders into a contract accepted by course lint", () => {
  ...
  let template = readFileSync(
    path.join(
      projectRoot,
      ".agents/skills/hexo-learn-topic/templates/course-contract.template.json",
    ),
    "utf8",
  );
  ...
  write(
    root,
    "source/_posts/learn-topic/playwright/Playwright(三)页面定位.md",
    validPlaywrightCoursePost({ number: "三", topic: "页面定位" }),
  );
  assert.equal(auditContent({ root, release: true }).status, "pass");
});
...
test("learn-topic course audit rejects the legacy document prefix and route names", () => {
  const root = makeRoot();
  const legacyEntry = validPlaywrightCoursePost().replace(
    "title: Playwright(一)入门路线",
    "title: Playwright文档(一) 学习路线",
  );
  write(
    root,
    "source/_posts/learn-topic/playwright/Playwright文档(一)学习路线.md",
    legacyEntry,
  );
  write(
    root,
    "source/_posts/learn-topic/playwright/Playwright(二)进阶内容.md",
    validPlaywrightCoursePost({
      number: "二",
      topic: "进阶内容",
    }),
  ...
  write(
    root,
    "source/_posts/learn-topic/fastapi/FastAPI(一)快速开始.md",
    validPlaywrightCoursePost({
      topic: "快速开始",
  ...
  assert.equal(report.status, "blocked");
  assert.ok(
    report.errors.some((item) => item.code === "LEARN_TOPIC_FILENAME_INVALID"),
  );
  assert.ok(
    report.errors.some((item) => item.code === "LEARN_TOPIC_TITLE_INVALID"),
  );
  assert.ok(
    report.errors.some(
      (item) => item.code === "LEARN_TOPIC_ADVANCED_ROUTE_INVALID",
    ),
  );
  assert.ok(
    report.errors.some(
      (item) => item.code === "LEARN_TOPIC_ENTRY_ROUTE_INVALID",
    ),
  );
});
...
test("learn-topic course audit enforces stable series order and course navigation", () => {
  ...
  assert.ok(
    report.errors.some(
      (item) => item.code === "LEARN_TOPIC_SERIES_ORDER_MISMATCH",
    ),
  );
  assert.ok(
    report.errors.some(
      (item) => item.code === "LEARN_TOPIC_COURSE_SERIES_MISSING",
    ),
  );
  assert.ok(
    report.errors.some(
...
test("learn-topic course audit rejects legacy path and public-copy violations", () => {
  ...
  write(
    root,
    "source/_posts/learn/playwright-python/00-route.md",
    validPost({ abbrlink: "legacy" }),
  );
  ...
  assert.equal(report.status, "blocked");
  assert.ok(
    report.errors.some((item) => item.code === "LEARN_TOPIC_LEGACY_PATH"),
  );
  assert.ok(
    report.errors.some((item) => item.code === "LEARN_TOPIC_DESCRIPTION_BLOCK"),
  );
  assert.ok(
    report.errors.some((item) => item.code === "LEARN_TOPIC_SOURCE_HEADING"),
  );
  assert.ok(
    report.errors.some((item) => item.code === "LEARN_TOPIC_VERIFICATION_COPY"),
  );
  assert.ok(
    report.errors.some(
      (item) => item.code === "LEARN_TOPIC_ENTRY_HEADINGS_INVALID",
    ),
  );
});
...
test("learn-topic audit rejects quoted and plain descriptions that span physical lines", () => {
  ...
  assert.equal(
    report.errors.filter(
      (item) => item.code === "LEARN_TOPIC_DESCRIPTION_BLOCK",
    ).length,
    2,
  );
});
...
test("learn-topic audit applies the same contract to non-Playwright courses", () => {
  ...
  write(
    root,
    "source/_posts/learn-topic/fastapi/FastAPI(一)入门路线.md",
    markdown,
  );
  const topic = validPlaywrightCoursePost({ number: "二", topic: "快速开始" })
    .replaceAll("Playwright", "FastAPI")
  ...
  write(
    root,
    "source/_posts/learn-topic/fastapi/FastAPI(二)快速开始.md",
    topic,
  );
...
test("learn-topic audit rejects nested paths and validates each course sequence independently", () => {
  ...
  write(
    root,
    "source/_posts/learn-topic/playwright/nested/Playwright(一)入门路线.md",
    validPlaywrightCoursePost(),
  );
  ...
  assert.equal(report.status, "blocked");
  assert.ok(
    report.errors.some((item) => item.code === "LEARN_TOPIC_PATH_INVALID"),
  );
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "LEARN_TOPIC_SEQUENCE_INVALID" &&
        item.path.endsWith("/fastapi"),
    ),
  );
});
...
test("asset audit blocks a live old-host image without treating plain tutorial text as rendered", () => {
  ...
  assert.equal(report.facts.tutorialOldHostExampleCount, 1);
  assert.ok(
    report.errors.some((item) => item.code === "OLD_IMAGE_HOST_LIVE_REFERENCE"),
  );
});
...
test("asset audit blocks missing local images and migrated-byte drift", () => {
  ...
  assert.equal(report.status, "blocked");
  assert.ok(
    report.errors.some((item) => item.code === "MIGRATED_IMAGE_SIZE_MISMATCH"),
  );
  assert.ok(report.errors.some((item) => item.code === "LOCAL_IMAGE_MISSING"));
});
...
test("project audit reports schema, CI and injected-asset drift while redacting secrets", () => {
  ...
  assert.ok(
    report.warnings.some((item) => item.code === "SCAFFOLD_SCHEMA_DRIFT"),
  );
  assert.ok(
    report.warnings.some((item) => item.code === "CI_NODE_INCOMPATIBLE"),
  );
  assert.ok(
    report.warnings.some((item) => item.code === "INJECTED_ASSET_MISSING"),
  );
  assert.ok(
    report.facts.sensitiveKeyLocations.some(
      (item) => item.identifier === "apikey",
    ),
  );
...
test("project audit rejects missing local Butterfly CDN assets", () => {
  const root = makeRoot();
  createProjectFixture(root);
  write(
    root,
    "_config.butterfly.yml",
    "CDN:\n  option:\n    mermaid: /vendor/butterfly/mermaid/mermaid.min.js\n",
  );
  ...
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "CDN_ASSET_MISSING" &&
        item.path === "source/vendor/butterfly/mermaid/mermaid.min.js",
    ),
  );
});

test("project and config audits accept a locked npm Butterfly theme without an active local theme", () => {
  const root = makeRoot();
  createProjectFixture(root);
  rmSync(path.join(root, "themes/butterfly"), { recursive: true, force: true });
  write(
    root,
    "package.json",
    JSON.stringify({
      ...
      dependencies: { hexo: "^8.0.0", "hexo-theme-butterfly": "^5.7.0" },
    }),
  );
  write(
    root,
  ...
  write(
    root,
    "node_modules/hexo-theme-butterfly/package.json",
    JSON.stringify({ version: "5.7.0" }),
  );
  write(
    root,
    "_config.yml",
  ...
  const projectReport = auditProject({ root });
  const configReport = auditConfig({ root });
  assert.equal(projectReport.facts.site.themeSource, "npm");
  assert.equal(projectReport.facts.packages.butterfly, "5.7.0");
  assert.ok(
    !projectReport.warnings.some((item) => item.code.startsWith("THEME_")),
  );
  assert.ok(
    !configReport.errors.some((item) => item.code === "THEME_PATH_INVALID"),
  );
});

test("CI release validates the external repository, branch, deploy key and quality gates", () => {
  const root = makeRoot();
  createProjectFixture(root);
  rmSync(path.join(root, "themes/butterfly"), { recursive: true, force: true });
  write(
  ...
  write(
    root,
    "_config.yml",
    "url: https://example.com\ntheme: butterfly\ndeploy:\n  type: git\n  repo: https://github.com/koco-co/koco-co.github.io.git\n  branch: main\n",
  );
  write(
    root,
    ...
    [
      "node-version: '24'",
      "run: node .agents/scripts/audit.mjs lint --json",
      "run: node --test .agents/scripts/audit.test.mjs",
      "run: npm run build",
      "run: node .agents/scripts/audit.mjs release --route ci --json",
      "external_repository: koco-co/koco-co.github.io",
      "publish_branch: main",
      "publish_dir: ./public",
  ...
  const valid = auditRelease({ root, route: "ci" });
  assert.ok(
    !valid.blockers.some((item) => item.code.startsWith("CI_")),
    formatReport(valid),
  );
  ...
  write(
    root,
    ".github/workflows/deploy.yml",
    "external_repository: source/repository\npublish_branch: gh-pages\n",
  );
  const invalid = auditRelease({ root, route: "ci" });
  assert.ok(
    invalid.blockers.some((item) => item.code === "CI_TARGET_MISMATCH"),
  );
  assert.ok(
    invalid.blockers.some((item) => item.code === "CI_BRANCH_MISMATCH"),
  );
  assert.ok(
    invalid.blockers.some((item) => item.code === "CI_DEPLOY_KEY_MISSING"),
  );
  assert.ok(
    invalid.blockers.some((item) => item.code === "CI_QUALITY_GATE_MISSING"),
  );
});

test("content audit rejects unreplaced scaffold placeholders", () => {
  const root = makeRoot();
  write(
    root,
    "source/_posts/draft.md",
    validPost({
      title: "待补充标题",
      description: "待补充：文章摘要",
    }),
  );
  const report = auditContent({ root });
  assert.ok(
    report.errors.some((item) => item.code === "ARTICLE_SCAFFOLD_PLACEHOLDER"),
  );
});
...
test("release audit blocks a dirty deployment worktree", () => {
  ...
  assert.equal(report.status, "blocked");
  assert.ok(
    report.blockers.some((item) => item.code === "DEPLOY_WORKTREE_DIRTY"),
  );
});
...
test("CI release keeps local deployment worktree drift as a warning, not a blocker", () => {
  ...
  assert.ok(
    report.warnings.some((item) => item.code === "DEPLOY_WORKTREE_DIRTY"),
  );
  assert.ok(
    !report.blockers.some((item) => item.code === "DEPLOY_WORKTREE_DIRTY"),
  );
...
test("tag plugin audit blocks disabled issues capability when content uses it", () => {
  ...
  assert.equal(report.status, "blocked");
  assert.ok(
    report.errors.some((item) => item.code === "TAG_CAPABILITY_DISABLED"),
  );
});
...
test("tag plugin audit rejects tags that are not registered in the current project", () => {
  ...
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "TAG_UNREGISTERED" &&
        item.path === "source/_posts/tags.md",
    ),
  );
});
...
test("tag registry ignores registrations in package files unreachable from the loaded entry", () => {
  ...
  write(
    root,
    "node_modules/hexo-butterfly-tag-plugins-plus/test/dead.js",
    "hexo.extend.tag.register('dead_widget', () => {})\n",
  );
  const report = auditTags({ root });
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "TAG_UNREGISTERED" &&
        item.message.includes("dead_widget"),
    ),
  );
});
...
test("tag lint invokes the installed flashcard parser for fields, IDs and references", () => {
  ...
  createTagPluginFixture(root, {
    markdown:
      '{% flashcard basic id:bad-card deck:"Fixture" priority:9 %}\n--- question\n问题\n--- answer\n回答\n--- explanation\n解析\n{% endflashcard %}\n',
  });
  const report = auditTags({ root });
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "FLASHCARD_CONTENT_INVALID" &&
        item.path === "source/_posts/tags.md",
    ),
  );
});
...
test("tag lint blocks duplicate flashcard IDs and references to missing cards", () => {
  const card =
    '{% flashcard basic id:shared-card deck:"Fixture" priority:2 %}\n--- question\n问题\n--- answer\n回答\n--- explanation\n解析\n{% endflashcard %}\n';
  ...
  assert.ok(
    duplicateReport.errors.some(
      (item) =>
        item.code === "FLASHCARD_CONTENT_INVALID" &&
        item.message.includes("duplicates"),
    ),
  );

  const referenceRoot = makeRoot();
  createTagPluginFixture(referenceRoot, {
    markdown: "{% flashcard_ref id:missing-card %}\n",
  });
  const referenceReport = auditTags({ root: referenceRoot });
  assert.ok(
    referenceReport.errors.some(
      (item) =>
        item.code === "FLASHCARD_CONTENT_INVALID" &&
        item.message.includes("does not exist"),
    ),
  );
});
...
test("tag plugin audit rejects an unclosed container with a source line", () => {
  ...
  const error = report.errors.find(
    (item) => item.code === "TAG_CONTAINER_UNCLOSED",
  );
...
test("tag plugin audit detects registry drift from the documented baseline", () => {
  ...
  assert.ok(
    report.errors.some(
      (item) => item.code === "TAG_PLUGIN_REGISTRATION_MISSING",
    ),
  );
});
...
test("tag plugin reference names every registered baseline capability", () => {
  const reference = readFileSync(
    path.join(
      projectRoot,
      ".agents/skills/hexo-blog-maintenance/references/butterfly-tag-plugins-plus.md",
    ),
    "utf8",
  );
...
test("repository structure audit rejects temporary source artifacts with an exact path", () => {
  ...
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "REPOSITORY_ARTIFACT_FORBIDDEN" &&
        item.path === "source/_posts/.DS_Store",
    ),
  );
});
...
test("repository structure audit covers forbidden artifacts outside source", () => {
  ...
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "REPOSITORY_ARTIFACT_FORBIDDEN" &&
        item.path === ".agents/skills/example/.DS_Store",
    ),
  );
});

test("repository structure audit does not treat vendored Markdown licenses as Hexo pages", () => {
  const root = makeRoot();
  write(root, "source/vendor/library/LICENSE.md", "MIT License\n");
  const report = auditStructure({ root });
  assert.ok(
    !report.errors.some(
      (item) =>
        item.path === "source/vendor/library/LICENSE.md" &&
        item.code.startsWith("PAGE_"),
    ),
  );
});
...
test("config audit blocks JSON, YAML, package-lock and installed-dependency drift", () => {
  ...
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "JSON_PARSE_FAILED" &&
        item.path === ".agents/invalid.json",
    ),
  );
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "YAML_PARSE_FAILED" &&
        item.path === "_config.butterfly.yml",
    ),
  );
  assert.ok(
    report.errors.some((item) => item.code === "PACKAGE_LOCK_DEPENDENCY_DRIFT"),
  );
  assert.ok(
    report.errors.some((item) => item.code === "DEPENDENCY_NOT_INSTALLED"),
  );
});
...
test("config audit compares dependency maps without depending on key order", () => {
  ...
  write(
    ...
    JSON.stringify({
      packages: {
        "": { dependencies: { "js-yaml": "^4.1.0", hexo: "^8.0.0" } },
      },
    }),
  );
  write(
    root,
    "node_modules/hexo/package.json",
    JSON.stringify({ name: "hexo" }),
  );
  write(
    root,
    "node_modules/js-yaml/package.json",
    JSON.stringify({ name: "js-yaml" }),
  );
  write(
    root,
    "_config.yml",
  ...
  assert.ok(
    !report.errors.some(
      (item) => item.code === "PACKAGE_LOCK_DEPENDENCY_DRIFT",
    ),
  );
});
...
test("config audit excludes the generated root db.json file", () => {
  ...
  assert.ok(
    !report.errors.some(
      (item) => item.code === "JSON_PARSE_FAILED" && item.path === "db.json",
    ),
  );
...
test("code audit checks JavaScript, CSS and Shell syntax with exact paths", () => {
  ...
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "JAVASCRIPT_SYNTAX_INVALID" &&
        item.path === "source/js/bad.js",
    ),
  );
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "CSS_SYNTAX_INVALID" &&
        item.path === "source/css/bad.css",
    ),
  );
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "CSS_SYNTAX_INVALID" &&
        item.path === "source/css/bad-declaration.css",
    ),
  );
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "CSS_SYNTAX_INVALID" &&
        item.path === "source/css/bad-function.css",
    ),
  );
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "SHELL_SYNTAX_INVALID" && item.path === "broken.sh",
    ),
  );
});
...
test("skill audit checks project instruction links, mirrors, front matter and resource references", () => {
  ...
  write(
    root,
    ".agents/skills/example/rules/guide.md",
    "继续读取 `data/missing.json`。\n",
  );
  ...
  assert.ok(
    report.errors.some((item) => item.code === "SKILL_DESCRIPTION_MISSING"),
  );
  assert.ok(
    report.errors.some((item) => item.code === "SKILL_REFERENCE_MISSING"),
  );
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "SKILL_REFERENCE_MISSING" &&
        item.path === ".agents/skills/example/rules/guide.md",
    ),
  );
...
test("documentation audit blocks broken repository-local Markdown links", () => {
  ...
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "MARKDOWN_LOCAL_LINK_MISSING" &&
        item.path === "README.md",
    ),
  );
});
...
test("asset audit rejects image extensions that disagree with file signatures", () => {
  ...
  write(
    root,
    "source/img/picgo-images/fake.png",
  ...
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "IMAGE_FILE_INVALID" &&
        item.path === "source/img/picgo-images/fake.png",
    ),
  );
});
...
test("asset audit cross-checks manifest mediaType against extension and signature", () => {
  ...
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "MIGRATED_IMAGE_MEDIA_TYPE_MISMATCH" &&
        item.path === "source/img/picgo-images/example.png",
    ),
  );
});
...
test("asset audit checks rendered image references in site configuration", () => {
  ...
  write(
    root,
    "_config.butterfly.yml",
    "cover: https://cdn.jsdelivr.net/gh/koco-co/picgo-images/picgo-images/example.png\nfallback: /img/missing.png\n",
  );
  const report = auditAssets({ root });
  assert.ok(
    report.errors.some(
      (item) =>
        item.code === "OLD_IMAGE_HOST_RENDERED_SOURCE" &&
        item.path === "_config.butterfly.yml",
    ),
  );
  assert.ok(
