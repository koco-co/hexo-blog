...
2. 按 `rules/technical-writing-style.md` 从读者正在观察的问题、对象或操作进入主题，用因果关系组织解释；先判断当前内容属于概念解释、技术问答、故障与方案分析、项目经历还是长篇章节，再选用与当前任务相符的信息要素，不能把内部组织方法写成公开模型、步骤标签或固定栏目。难点按该规则先规划概念开头的故事或替代推演，随后给出机制、映射、边界和验证。
...
9. 容器标签必须按正确顺序成对闭合；参数分隔符必须保持实现要求：有的使用半角逗号，有的要求逗号后保留一个空格，另有 ` | ` 或 `||`，不能机械互换。
...
13. 面试题按 `rules/technical-writing-style.md` 选择信息顺序：技术题尽快给出判断，并用机制、例证和边界支撑；故障或选型题先交代可观察现状和约束，再闭合建议、取舍与验证；项目和行为题突出本人行动、结果与复盘，且不得补写未经用户或资料证明的经历。解析涉及缩写、命令、日志或系统报告时，再按“术语—用途—证据—边界”逐卡检查：术语要有中文含义和作用，诊断工具要说明能证明什么与不能证明什么，抽象题要有最小场景、对照或判断目标；不以固定字数代替理解检查。
14. 编辑已有文章时，复查该篇全部原始 `flashcard` 定义，不能只修用户指出的一张；用户明确要求“所有/全站面试题解析”时，将范围扩展到所有课程的原始 `flashcard` 定义，记录已检查、已修改、已保留和范围外 `flashcard_ref` 的统计；未进入当前范围的历史卡片作为待迁移内容报告，不阻断无关文章。
