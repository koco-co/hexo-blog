...
5. 完成课程文章
   ...
   - 每篇文章完成公开门禁后，由维护 Skill 完成真实构建、全仓库 lint 和目标路由浏览器验收；不自动启动长期后台服务，也不进入 CI、本地部署或 `npm run deploy`。
...
- 不默认 clone 到持久目录、修改外部仓库、执行 Patch、commit、push、创建 Issue/PR、部署、安装依赖、清理生成物、启动长期后台服务或修改 Butterfly 主题。
- 构建或浏览器验收失败时先读取具体错误，自行处理任务范围内可修复问题并重试；不得以错误为由进入部署流程。外部不可控阻塞只能记录已完成的处理和阻塞原因。
