...
3. 设计并复核文章地图
   - 为每篇文章确定真实名称、唯一职责、前置、详细大纲、示例、视觉机会、FAQ、自测和来源，同时冻结全系列共用的一张封面合同。
...
5. 完成课程文章
   ...
   - 每篇先更新资料，再调用 `hexo-blog-maintenance` 完成视觉预案、正文、标签、图片和未发布文章的草稿验证；初稿必须交给干净上下文 Article Reviewer 查漏。
