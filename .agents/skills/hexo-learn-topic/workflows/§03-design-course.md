...
5. 使用覆盖处置账本为官方能力全集的每个稳定标识选择 `核心详解`、`正文简述`、`进阶内容`、`弃用迁移` 或 `明确不纳入`。前四类指定唯一主文章与目标章节；明确不纳入项不得指定文章或章节。交叉内容通过链接说明，不复制成多篇主线。
...
完成信号：文章依赖无环，职责不重叠，命名能独立表达内容，能力全集没有未归属或无依据排除项。

## Phase 3：启动干净上下文 Reviewer

1. 冻结用户需求摘要、已确认路线、文章地图、官方能力全集、覆盖处置账本、旧接口补充集、旧接口迁移映射和来源清单。
