...
4. 按 `templates/article-map.template.md` 先记录完整主题映射与数量核算，再为每篇列出前置、详细 H2/H3 大纲、完整示例、视觉计划、真实复习候选与来源，并按 `rules/learning-article-quality.md` 规划参考资料卡片及必要分组。入门路线只规划主题介绍、学习路径和开始学习，不安排 FAQ/闪卡。同步冻结系列封面的简短文字、准确主题标识、4～5 个来自已确认文章地图的课程关键词和本地文件名；关键词必须能代表系列主线，使用适合缩略图的短词，不临时发明未纳入课程的工具或概念。
...
完成信号：学习主题与主题文章一一对应，系列总数为 `N+1～3`，文章依赖无环，职责不重叠，能力全集没有未归属或无依据排除项。

## Phase 3：启动干净上下文 Reviewer

1. 冻结用户需求摘要、已确认学习大纲及其学习主题表、文章地图、官方能力全集、覆盖处置账本、旧接口补充集、旧接口迁移映射和来源清单。
