...
- `.agents/scripts/`：全仓库 Hexo lint 与审计测试的唯一位置。
- `.agents/skills/hexo-learn-topic/scripts/`：课程契约迁移脚本的位置。
...
6. 交付用户验收前必须运行 `node .agents/scripts/audit.mjs lint --json`；只接受 `status: pass`。出现任一具体文件错误时先修复并重跑，不把未通过的文章交给用户验收。
...
- 每门系统课程必须在 `.agents/skills/hexo-learn-topic/data/<主题路径段>.json` 持久化 schema v2 课程契约，包含第一轮学习主题、可选篇、完整文章清单和能力账本；文章 Front Matter 禁止任何 `learn_topic_*` 内部字段。
...
- 项目状态：`node .agents/scripts/audit.mjs project --json`
- 图片检查：`node .agents/scripts/audit.mjs assets --json`
- 内容检查：`node .agents/scripts/audit.mjs content --json`
- 发布级内容检查：`node .agents/scripts/audit.mjs content --release --json`
- 标签检查：`node .agents/scripts/audit.mjs tags --json`
- 目录检查：`node .agents/scripts/audit.mjs structure --json`
- 配置与依赖检查：`node .agents/scripts/audit.mjs config --json`
- JavaScript、CSS 与 Shell 语法检查：`node .agents/scripts/audit.mjs code --json`
- Skill、项目指令与软链接检查：`node .agents/scripts/audit.mjs skills --json`
- 仓库文档本地链接检查：`node .agents/scripts/audit.mjs docs --json`
- 全仓库 Hexo lint：`node .agents/scripts/audit.mjs lint --json`
- 审计工具测试：`node --test .agents/scripts/audit.test.mjs`
- 生成站点：`npm run build`
- 本地预览：`npm run server`
...
- 修改 `.agents/scripts/audit.mjs` 或其契约时运行完整 Node 测试。
