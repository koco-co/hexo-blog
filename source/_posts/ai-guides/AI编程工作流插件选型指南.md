...
纳入比较的 8 个项目均超过 10,000 Star。这里的筛选口径是 {% label 完整开发主线 green %}: 包括原生插件、具有完整开发主线的 Skills 套件, 以及向 Agent 分发指令的规格工具。通用工具合集、资源导航和独立 Agent 客户端不在讨论范围内。
...
{% timeline 阅读顺序, blue %}
<!-- timeline 需求澄清 -->
先确定目标、范围、约束和验收口径, 再让 Agent 开始提出方案。
<!-- endtimeline -->
<!-- timeline 设计与规格 -->
把决定过的行为、接口和边界写成后续步骤可以引用的依据。
<!-- endtimeline -->
<!-- timeline 任务拆分 -->
将方案切成可执行、可验证并且能够交接的小任务。
<!-- endtimeline -->
<!-- timeline 实现 -->
修改代码并补上与行为对应的测试, 保持每一步都能回看。
<!-- endtimeline -->
<!-- timeline 验证与审查 -->
分别检查需求符合性、代码质量和真实运行结果, 发现偏差就回到任务或规格。
<!-- endtimeline -->
<!-- timeline 交付与沉淀 -->
完成提交、合入和发布, 再把可复用的结论留给下一轮开发。
<!-- endtimeline -->
{% endtimeline %}

这条主线不是每个项目都完整覆盖: Spec Kit、OpenSpec 更突出规格的组织和演进; Superpowers 更突出实施与验证纪律; BMAD 展开产品和技术决策; Compound 则把已解决问题重新送入后续开发。
...
{% tabs 对照场景, 1 %}
<!-- tab Spec Kit / OpenSpec @fas fa-code-branch -->
如果需要先确定导出权限、异步接口和任务契约, 并追溯这些要求如何进入实施, Spec Kit 的 `spec`、`plan`、`tasks` 关系更直接。如果已有稳定接口, 本次只是调整导出行为, OpenSpec 可以把修改放进独立变更, 评审规格增量, 完成后再同步主规格。关键在于需要建立需求基线, 还是维护基线之上的变更, 不必严格按新旧项目划分。
<!-- endtab -->
<!-- tab Superpowers / Matt Pocock @fas fa-tasks -->
当产品尚未决定导出哪些字段、谁可以下载结果, Matt 的追问和决策记录应先介入。当这些规则已确定, 工作重点变成队列任务、权限检查和回归测试, Superpowers 的逐任务实施与审查更有针对性。两者都包含 TDD, 但解决返工的起点不同。
<!-- endtab -->
<!-- tab BMAD / gstack @fas fa-layer-group -->
如果导出功能牵涉产品套餐、角色权限、交互与后台架构, BMAD 适合展开这些相互影响的决定。如果方案已定, 主要风险是进度展示、失败重试和下载路径是否真的可用, gstack 的浏览器 QA 与交付流程更贴近问题。
<!-- endtab -->
<!-- tab Addy / Compound @fas fa-shield-alt -->
当导出功能需要补上大数据量性能、可观测性和失败恢复, Addy 的专项 Skills 能把这些工作纳入当前计划。如果仓库过去已经出现过队列重复执行或权限泄漏, Compound 更关注把这些经过验证的历史结论带入本次开发, 避免重复探索。
<!-- endtab -->
{% endtabs %}
