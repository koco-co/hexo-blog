---
title: Agent 架构与 Harness(七)Context组装与状态
tags:
  - Agent 架构与 Harness
  - Context组装与状态
categories:
  - Learn Topic
  - Agent 架构与 Harness
description: 系统指令、历史、工具和检索内容优先级明确
...
{% note primary flat %}
本节围绕“Context组装与状态”建立可复核的工作模型。学习成果：系统指令、历史、工具和检索内容优先级明确。实验：实现可记录来源的上下文组装器，验证优先级、截断和注入隔离。视觉辅助：Context assembly Mermaid；预算表。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
上下文组装器把网页摘录排在系统约束之前，模型便把网页句子当作规则。来源、优先级和预算快照让冲突重新可见。

本篇把“Context组装与状态”落到一次可回放的本地任务：实现可记录来源的上下文组装器，验证优先级、截断和注入隔离。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：系统指令、历史、工具和检索内容优先级明确。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“Context 的来源和优先级；预算、摘要、筛选和注入；运行状态与持久状态”拆成输入、状态、输出和证据。Context 不是聊天记录堆积，而是带来源、优先级、预算和版本的运行快照；状态则记录过程当前处于哪一阶段。
{% endnote %}
...
{% note primary flat %}
实验步骤：实现可记录来源的上下文组装器，验证优先级、截断和注入隔离。

验收产物：带来源和预算的 Context 快照

失败注入：不可信内容覆盖约束、预算超限、状态不可追踪
{% endnote %}

## 上下文来源

{% note info flat %}
本节任务：理解Context 的来源和优先级并建立可复核判断。核心内容：Context 的来源和优先级。

机制：Context 的来源和优先级。先核对输入、状态、输出、失败和复测证据，再用“不可信内容覆盖约束、预算超限、状态不可追踪”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“Context来源”聚焦“Context 的来源和优先级”。围绕“Context 的来源和优先级”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“Context 的来源和优先级”变成一个可观察字段，再以“实现可记录来源的上下文组装器，验证优先级、截断和注入隔离。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“不可信内容覆盖约束、预算超限、状态不可追踪”，恢复或放行动作必须可重放。
{% endnote %}

### Context预算

{% note info flat %}
“Context预算”聚焦“预算、摘要、筛选和注入”。把数据内容与控制指令分离，并沿存储、Context、日志、缓存和外发路径检查；测试通过要同时说明覆盖范围和未覆盖面。具体取舍是：保留“Context预算”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“不可信内容覆盖约束、预算超限、状态不可追踪”，先停止并保留原始事件。本节的失败样例是“不可信内容覆盖约束、预算超限、状态不可追踪”，恢复或放行动作必须可重放。
{% endnote %}

## 预算与优先级

{% note info flat %}
本节任务：理解预算、摘要、筛选和注入并建立可复核判断。核心内容：预算、摘要、筛选和注入。

机制：预算、摘要、筛选和注入。先核对输入、状态、输出、失败和复测证据，再用“不可信内容覆盖约束、预算超限、状态不可追踪”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“状态分层”聚焦“预算、摘要、筛选和注入”。把数据内容与控制指令分离，并沿存储、Context、日志、缓存和外发路径检查；测试通过要同时说明覆盖范围和未覆盖面。具体取舍是：保留“状态分层”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“不可信内容覆盖约束、预算超限、状态不可追踪”，先停止并保留原始事件。本节的失败样例是“不可信内容覆盖约束、预算超限、状态不可追踪”，恢复或放行动作必须可重放。
{% endnote %}

### 冲突追踪

{% note info flat %}
“冲突追踪”聚焦“运行状态与持久状态”。围绕“运行状态与持久状态”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：带来源和预算的 Context 快照应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“不可信内容覆盖约束、预算超限、状态不可追踪”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：实现可记录来源的上下文组装器，验证优先级、截断和注入隔离。

机制：实现可记录来源的上下文组装器，验证优先级、截断和注入隔离。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“不可信内容覆盖约束、预算超限、状态不可追踪”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“运行状态与持久状态”。围绕“运行状态与持久状态”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：带来源和预算的 Context 快照应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“不可信内容覆盖约束、预算超限、状态不可追踪”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Agent 架构与 Harness","article":"Agent 架构与 Harness(七)Context组装与状态","order":7,"topic":"Context组装与状态","experiment":"实现可记录来源的上下文组装器，验证优先级、截断和注入隔离。","focus":["Context 的来源和优先级","预算、摘要、筛选和注入","运行状态与持久状态","上下文冲突和可追踪性"],"artifacts":["带来源和预算的 Context 快照"],"failures":["不可信内容覆盖约束、预算超限、状态不可追踪"],"mode":"harness"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    snapshot = read_json(root / "context-snapshot.json")
    bad_layers = list(snapshot["layers"])
    bad_layers[0] = "tool"
    try:
        if bad_layers[0] != "system":
            raise PermissionError("untrusted tool content outranked system policy")
    ...
    snapshot["layers"] = ["system", "project", "input", "tool"]
    write_json(root / "context-snapshot.json", snapshot)
    assert read_json(root / "context-snapshot.json")["layers"][0] == "system"
    record_event(events_file, "recovery_verified", action="restore-context-precedence")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"harness","online_evidence":"not_run","scenario":"context-state","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "harness", "online_evidence": "not_run", "scenario": "context-state", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“上下文冲突和可追踪性”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。先把“上下文冲突和可追踪性”变成一个可观察字段，再以“实现可记录来源的上下文组装器，验证优先级、截断和注入隔离。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“不可信内容覆盖约束、预算超限、状态不可追踪”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“Context 的来源和优先级”。围绕“Context 的来源和优先级”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“不可信内容覆盖约束、预算超限、状态不可追踪”，先停止并保留原始事件。本节的失败样例是“不可信内容覆盖约束、预算超限、状态不可追踪”，恢复或放行动作必须可重放。
{% endnote %}

## 状态追踪

{% note info flat %}
本节任务：理解上下文冲突和可追踪性并建立可复核判断。核心内容：上下文冲突和可追踪性。

机制：上下文冲突和可追踪性。先核对输入、状态、输出、失败和复测证据，再用“不可信内容覆盖约束、预算超限、状态不可追踪”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“冲突追踪”聚焦“上下文冲突和可追踪性”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。先把“上下文冲突和可追踪性”变成一个可观察字段，再以“实现可记录来源的上下文组装器，验证优先级、截断和注入隔离。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“不可信内容覆盖约束、预算超限、状态不可追踪”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：带来源和预算的 Context 快照。预期：系统指令、历史、工具和检索内容优先级明确。失败复验：不可信内容覆盖约束、预算超限、状态不可追踪。

机制：验收证据：带来源和预算的 Context 快照。预期：系统指令、历史、工具和检索内容优先级明确。失败复验：不可信内容覆盖约束、预算超限、状态不可追踪。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“不可信内容覆盖约束、预算超限、状态不可追踪”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“Context 的来源和优先级”。围绕“Context 的来源和优先级”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“不可信内容覆盖约束、预算超限、状态不可追踪”，先停止并保留原始事件。本节的失败样例是“不可信内容覆盖约束、预算超限、状态不可追踪”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 带来源和预算的 Context 快照 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 不可信内容覆盖约束、预算超限、状态不可追踪 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“预算、摘要、筛选和注入”。把数据内容与控制指令分离，并沿存储、Context、日志、缓存和外发路径检查；测试通过要同时说明覆盖范围和未覆盖面。复核时把动作和产物对应起来：带来源和预算的 Context 快照应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“不可信内容覆盖约束、预算超限、状态不可追踪”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-agent-harness-7-1 deck:"Agent 架构与 Harness" priority:1 tags:"边界验证" %}
--- question
Context 组装为什么是 Harness 的核心职责
--- answer
Harness 负责把系统规则、项目约定、任务输入、历史和工具结果按优先级与预算组装，并留下快照。
--- explanation
Context 不是聊天记录堆积，而是带来源、优先级、预算和版本的运行快照；状态则记录过程当前处于哪一阶段。问题“Context 组装为什么是 Harness 的核心职责”的关键不在背诵名词，而在于：Harness 负责把系统规则、项目约定、任务输入、历史和工具结果按优先级与预算组装，并留下快照。 先判断题目中的硬门槛和适用范围，再看“不可信内容覆盖约束、预算超限、状态不可追踪”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-agent-harness-7-2 deck:"Agent 架构与 Harness" priority:1 tags:"证据链" %}
--- question
动态上下文如何保留来源
--- answer
给每段动态上下文保存来源、时间、版本和用途；截断或冲突时仍能追溯它为何进入最终 Context。
--- explanation
Context 不是聊天记录堆积，而是带来源、优先级、预算和版本的运行快照；状态则记录过程当前处于哪一阶段。问题“动态上下文如何保留来源”的关键不在背诵名词，而在于：给每段动态上下文保存来源、时间、版本和用途；截断或冲突时仍能追溯它为何进入最终 Context。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
