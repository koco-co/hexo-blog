---
title: Agent 架构与 Harness(三)Agent Loop与ReAct
tags:
  - Agent 架构与 Harness
  - Agent Loop与ReAct
categories:
  - Learn Topic
  - Agent 架构与 Harness
description: 每轮状态、工具结果、预算和停止原因可重放
...
{% note primary flat %}
本节围绕“Agent Loop与ReAct”建立可复核的工作模型。学习成果：每轮状态、工具结果、预算和停止原因可重放。实验：使用确定性本地工具实现可停止的最小 Agent Loop，不调用外部模型。视觉辅助：Agent Loop Mermaid；状态机。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
一个 Loop 拿到工具结果后继续重试，却没有写下 Stop 事件。操作者只看到答案，无法判断是预算耗尽、工具失败还是任务完成。

本篇把“Agent Loop与ReAct”落到一次可回放的本地任务：使用确定性本地工具实现可停止的最小 Agent Loop，不调用外部模型。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：每轮状态、工具结果、预算和停止原因可重放。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“一次 turn 与一次任务的区别；Observation、Action、Tool Result 和 Stop；ReAct 的收益与暴露推理边界”拆成输入、状态、输出和证据。Loop 的可观测单位是 turn：Action 只能基于当前 Observation，Tool Result 必须回到下一轮，Stop 是状态而不是一句话。
{% endnote %}
...
{% note primary flat %}
实验步骤：使用确定性本地工具实现可停止的最小 Agent Loop，不调用外部模型。

验收产物：本地 Loop 状态、工具调用和停止事件

失败注入：无终止条件、工具结果未回传、错误无限重试
{% endnote %}

## 循环状态

{% note info flat %}
本节任务：理解一次 turn 与一次任务的区别并建立可复核判断。核心内容：一次 turn 与一次任务的区别。

机制：一次 turn 与一次任务的区别。先核对输入、状态、输出、失败和复测证据，再用“无终止条件、工具结果未回传、错误无限重试”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“任务与轮次”聚焦“一次 turn 与一次任务的区别”。围绕“一次 turn 与一次任务的区别”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“一次 turn 与一次任务的区别”变成一个可观察字段，再以“使用确定性本地工具实现可停止的最小 Agent Loop，不调用外部模型。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“无终止条件、工具结果未回传、错误无限重试”，恢复或放行动作必须可重放。
{% endnote %}

### 循环事件

{% note info flat %}
“循环事件”聚焦“Observation、Action、Tool Result 和 Stop”。围绕“Observation、Action、Tool Result 和 Stop”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“循环事件”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“无终止条件、工具结果未回传、错误无限重试”，先停止并保留原始事件。本节的失败样例是“无终止条件、工具结果未回传、错误无限重试”，恢复或放行动作必须可重放。
{% endnote %}

## 工具反馈

{% note info flat %}
本节任务：理解Observation、Action、Tool Result 和 Stop并建立可复核判断。核心内容：Observation、Action、Tool Result 和 Stop。

机制：Observation、Action、Tool Result 和 Stop。先核对输入、状态、输出、失败和复测证据，再用“无终止条件、工具结果未回传、错误无限重试”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“ReAct边界”聚焦“Observation、Action、Tool Result 和 Stop”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“ReAct边界”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“无终止条件、工具结果未回传、错误无限重试”，先停止并保留原始事件。本节的失败样例是“无终止条件、工具结果未回传、错误无限重试”，恢复或放行动作必须可重放。
{% endnote %}

### 终止与错误

{% note info flat %}
“终止与错误”聚焦“ReAct 的收益与暴露推理边界”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：本地 Loop 状态、工具调用和停止事件应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“无终止条件、工具结果未回传、错误无限重试”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：使用确定性本地工具实现可停止的最小 Agent Loop，不调用外部模型。

机制：使用确定性本地工具实现可停止的最小 Agent Loop，不调用外部模型。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“无终止条件、工具结果未回传、错误无限重试”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“ReAct 的收益与暴露推理边界”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：本地 Loop 状态、工具调用和停止事件应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“无终止条件、工具结果未回传、错误无限重试”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Agent 架构与 Harness","article":"Agent 架构与 Harness(三)Agent Loop与ReAct","order":3,"topic":"Agent Loop与ReAct","experiment":"使用确定性本地工具实现可停止的最小 Agent Loop，不调用外部模型。","focus":["一次 turn 与一次任务的区别","Observation、Action、Tool Result 和 Stop","ReAct 的收益与暴露推理边界","循环预算、终止和错误传播"],"artifacts":["本地 Loop 状态、工具调用和停止事件"],"failures":["无终止条件、工具结果未回传、错误无限重试"],"mode":"harness"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    state = read_json(root / "loop-state.json")
    incomplete = dict(state)
    incomplete["steps"] = [step for step in state["steps"] if step != "stop"]
    incomplete["status"] = "running"
    try:
        if incomplete["status"] == "running" and "stop" not in incomplete["steps"]:
            raise RuntimeError("loop has no terminal event")
    except RuntimeError as error:
        failure_event = {"status": "blocked", "type": str(error)}
        record_event(events_file, "failure_blocked", type=failure_event["type"])
    state["status"] = "stopped"
    if state["steps"][-1] != "stop":
        state["steps"].append("stop")
    write_json(root / "loop-state.json", state)
    assert state["status"] == "stopped" and state["steps"][-1] == "stop"
    record_event(events_file, "recovery_verified", action="append-terminal-stop")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"harness","online_evidence":"not_run","scenario":"loop-events","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "harness", "online_evidence": "not_run", "scenario": "loop-events", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“循环预算、终止和错误传播”。围绕“循环预算、终止和错误传播”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“循环预算、终止和错误传播”变成一个可观察字段，再以“使用确定性本地工具实现可停止的最小 Agent Loop，不调用外部模型。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“无终止条件、工具结果未回传、错误无限重试”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“一次 turn 与一次任务的区别”。围绕“一次 turn 与一次任务的区别”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“无终止条件、工具结果未回传、错误无限重试”，先停止并保留原始事件。本节的失败样例是“无终止条件、工具结果未回传、错误无限重试”，恢复或放行动作必须可重放。
{% endnote %}

## 停止条件

{% note info flat %}
本节任务：理解循环预算、终止和错误传播并建立可复核判断。核心内容：循环预算、终止和错误传播。

机制：循环预算、终止和错误传播。先核对输入、状态、输出、失败和复测证据，再用“无终止条件、工具结果未回传、错误无限重试”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“终止与错误”聚焦“循环预算、终止和错误传播”。围绕“循环预算、终止和错误传播”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“循环预算、终止和错误传播”变成一个可观察字段，再以“使用确定性本地工具实现可停止的最小 Agent Loop，不调用外部模型。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“无终止条件、工具结果未回传、错误无限重试”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：本地 Loop 状态、工具调用和停止事件。预期：每轮状态、工具结果、预算和停止原因可重放。失败复验：无终止条件、工具结果未回传、错误无限重试。

机制：验收证据：本地 Loop 状态、工具调用和停止事件。预期：每轮状态、工具结果、预算和停止原因可重放。失败复验：无终止条件、工具结果未回传、错误无限重试。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“无终止条件、工具结果未回传、错误无限重试”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“一次 turn 与一次任务的区别”。围绕“一次 turn 与一次任务的区别”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“无终止条件、工具结果未回传、错误无限重试”，先停止并保留原始事件。本节的失败样例是“无终止条件、工具结果未回传、错误无限重试”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 本地 Loop 状态、工具调用和停止事件 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 无终止条件、工具结果未回传、错误无限重试 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“Observation、Action、Tool Result 和 Stop”。围绕“Observation、Action、Tool Result 和 Stop”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：本地 Loop 状态、工具调用和停止事件应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“无终止条件、工具结果未回传、错误无限重试”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-agent-harness-3-1 deck:"Agent 架构与 Harness" priority:1 tags:"边界验证" %}
--- question
Agent Loop 的状态最少需要什么
--- answer
最少要记录输入、上下文、动作、观察、预算和停止状态；只有最终文本不足以重放 Loop。
--- explanation
Loop 的可观测单位是 turn：Action 只能基于当前 Observation，Tool Result 必须回到下一轮，Stop 是状态而不是一句话。问题“Agent Loop 的状态最少需要什么”的关键不在背诵名词，而在于：最少要记录输入、上下文、动作、观察、预算和停止状态；只有最终文本不足以重放 Loop。 先判断题目中的硬门槛和适用范围，再看“无终止条件、工具结果未回传、错误无限重试”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-agent-harness-3-2 deck:"Agent 架构与 Harness" priority:1 tags:"证据链" %}
--- question
没有停止条件会发生什么
--- answer
没有停止条件，Loop 可能无限重试、耗尽预算或重复副作用；应设置完成、失败、取消和预算上限。
--- explanation
Loop 的可观测单位是 turn：Action 只能基于当前 Observation，Tool Result 必须回到下一轮，Stop 是状态而不是一句话。问题“没有停止条件会发生什么”的关键不在背诵名词，而在于：没有停止条件，Loop 可能无限重试、耗尽预算或重复副作用；应设置完成、失败、取消和预算上限。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
