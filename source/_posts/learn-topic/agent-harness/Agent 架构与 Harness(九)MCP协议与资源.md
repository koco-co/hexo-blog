---
title: Agent 架构与 Harness(九)MCP协议与资源
tags:
  - Agent 架构与 Harness
  - MCP协议与资源
categories:
  - Learn Topic
  - Agent 架构与 Harness
description: Tools、Resources、Prompts 和授权边界可区分
...
{% note primary flat %}
本节围绕“MCP协议与资源”建立可复核的工作模型。学习成果：Tools、Resources、Prompts 和授权边界可区分。实验：使用官方 SDK/示例或本地协议夹具完成一个只读 MCP Server，并记录调用轨迹。视觉辅助：MCP 架构与时序 Mermaid。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
只读 MCP 客户端先发现能力，再请求资源；跳过用户同意直接调用时，返回内容本身还可能带来新的指令。

本篇把“MCP协议与资源”落到一次可回放的本地任务：使用官方 SDK/示例或本地协议夹具完成一个只读 MCP Server，并记录调用轨迹。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：Tools、Resources、Prompts 和授权边界可区分。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“MCP 架构和 JSON-RPC 生命周期；工具、资源和 Prompt 的语义；发现、调用、结果和错误”拆成输入、状态、输出和证据。MCP 的 initialize、发现、调用和错误是可审计的 JSON-RPC 生命周期；Tool、Resource 和 Prompt 的信任与副作用不同。
{% endnote %}
...
{% note primary flat %}
实验步骤：使用官方 SDK/示例或本地协议夹具完成一个只读 MCP Server，并记录调用轨迹。

验收产物：本地只读 MCP Server 的发现/调用/错误轨迹

失败注入：未经同意调用、返回值注入、Token 受众错误
{% endnote %}

## 协议架构

{% note info flat %}
本节任务：理解MCP 架构和 JSON-RPC 生命周期并建立可复核判断。核心内容：MCP 架构和 JSON-RPC 生命周期。

机制：MCP 架构和 JSON-RPC 生命周期。先核对输入、状态、输出、失败和复测证据，再用“未经同意调用、返回值注入、Token 受众错误”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“协议生命周期”聚焦“MCP 架构和 JSON-RPC 生命周期”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。先把“MCP 架构和 JSON-RPC 生命周期”变成一个可观察字段，再以“使用官方 SDK/示例或本地协议夹具完成一个只读 MCP Server，并记录调用轨迹。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“未经同意调用、返回值注入、Token 受众错误”，恢复或放行动作必须可重放。
{% endnote %}

### MCP对象

{% note info flat %}
“MCP对象”聚焦“工具、资源和 Prompt 的语义”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。具体取舍是：保留“MCP对象”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“未经同意调用、返回值注入、Token 受众错误”，先停止并保留原始事件。本节的失败样例是“未经同意调用、返回值注入、Token 受众错误”，恢复或放行动作必须可重放。
{% endnote %}

## 资源类型

{% note info flat %}
本节任务：理解工具、资源和 Prompt 的语义并建立可复核判断。核心内容：工具、资源和 Prompt 的语义。

机制：工具、资源和 Prompt 的语义。先核对输入、状态、输出、失败和复测证据，再用“未经同意调用、返回值注入、Token 受众错误”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“调用与错误”聚焦“工具、资源和 Prompt 的语义”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。具体取舍是：保留“调用与错误”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“未经同意调用、返回值注入、Token 受众错误”，先停止并保留原始事件。本节的失败样例是“未经同意调用、返回值注入、Token 受众错误”，恢复或放行动作必须可重放。
{% endnote %}

### 授权边界

{% note info flat %}
“授权边界”聚焦“发现、调用、结果和错误”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：本地只读 MCP Server 的发现/调用/错误轨迹应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“未经同意调用、返回值注入、Token 受众错误”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：使用官方 SDK/示例或本地协议夹具完成一个只读 MCP Server，并记录调用轨迹。

机制：使用官方 SDK/示例或本地协议夹具完成一个只读 MCP Server，并记录调用轨迹。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“未经同意调用、返回值注入、Token 受众错误”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“发现、调用、结果和错误”。围绕“发现、调用、结果和错误”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：本地只读 MCP Server 的发现/调用/错误轨迹应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“未经同意调用、返回值注入、Token 受众错误”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Agent 架构与 Harness","article":"Agent 架构与 Harness(九)MCP协议与资源","order":9,"topic":"MCP协议与资源","experiment":"使用官方 SDK/示例或本地协议夹具完成一个只读 MCP Server，并记录调用轨迹。","focus":["MCP 架构和 JSON-RPC 生命周期","工具、资源和 Prompt 的语义","发现、调用、结果和错误","用户同意、信任和版本边界"],"artifacts":["本地只读 MCP Server 的发现/调用/错误轨迹"],"failures":["未经同意调用、返回值注入、Token 受众错误"],"mode":"harness"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    try:
        if not approved:
            raise PermissionError("MCP call lacks user consent")
    ...
    protocol["messages"].append("consent-granted-read-only")
    write_json(root / "protocol.json", protocol)
    assert protocol["messages"][-1] == "consent-granted-read-only" and "tools/call" in protocol["messages"]
    record_event(events_file, "recovery_verified", action="require-consent-before-call")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"harness","online_evidence":"not_run","scenario":"mcp-readonly","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "harness", "online_evidence": "not_run", "scenario": "mcp-readonly", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“用户同意、信任和版本边界”。先记录入口、版本、平台和前置条件，再执行最小动作；安装成功只能证明程序可启动，不能推出权限、模型或扩展已经可用。先把“用户同意、信任和版本边界”变成一个可观察字段，再以“使用官方 SDK/示例或本地协议夹具完成一个只读 MCP Server，并记录调用轨迹。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“未经同意调用、返回值注入、Token 受众错误”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“MCP 架构和 JSON-RPC 生命周期”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“未经同意调用、返回值注入、Token 受众错误”，先停止并保留原始事件。本节的失败样例是“未经同意调用、返回值注入、Token 受众错误”，恢复或放行动作必须可重放。
{% endnote %}

## 授权边界

{% note info flat %}
本节任务：理解用户同意、信任和版本边界并建立可复核判断。核心内容：用户同意、信任和版本边界。

机制：用户同意、信任和版本边界。先核对输入、状态、输出、失败和复测证据，再用“未经同意调用、返回值注入、Token 受众错误”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“授权边界”聚焦“用户同意、信任和版本边界”。先记录入口、版本、平台和前置条件，再执行最小动作；安装成功只能证明程序可启动，不能推出权限、模型或扩展已经可用。先把“用户同意、信任和版本边界”变成一个可观察字段，再以“使用官方 SDK/示例或本地协议夹具完成一个只读 MCP Server，并记录调用轨迹。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“未经同意调用、返回值注入、Token 受众错误”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：本地只读 MCP Server 的发现/调用/错误轨迹。预期：Tools、Resources、Prompts 和授权边界可区分。失败复验：未经同意调用、返回值注入、Token 受众错误。

机制：验收证据：本地只读 MCP Server 的发现/调用/错误轨迹。预期：Tools、Resources、Prompts 和授权边界可区分。失败复验：未经同意调用、返回值注入、Token 受众错误。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“未经同意调用、返回值注入、Token 受众错误”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“MCP 架构和 JSON-RPC 生命周期”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“未经同意调用、返回值注入、Token 受众错误”，先停止并保留原始事件。本节的失败样例是“未经同意调用、返回值注入、Token 受众错误”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 本地只读 MCP Server 的发现/调用/错误轨迹 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 未经同意调用、返回值注入、Token 受众错误 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“工具、资源和 Prompt 的语义”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。复核时把动作和产物对应起来：本地只读 MCP Server 的发现/调用/错误轨迹应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“未经同意调用、返回值注入、Token 受众错误”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-agent-harness-9-1 deck:"Agent 架构与 Harness" priority:1 tags:"边界验证" %}
--- question
MCP Server 是否等于安全插件
--- answer
MCP Server 只是协议服务，不等于安全插件；它仍需来源、权限、能力和撤销审查。
--- explanation
MCP 的 initialize、发现、调用和错误是可审计的 JSON-RPC 生命周期；Tool、Resource 和 Prompt 的信任与副作用不同。问题“MCP Server 是否等于安全插件”的关键不在背诵名词，而在于：MCP Server 只是协议服务，不等于安全插件；它仍需来源、权限、能力和撤销审查。 先判断题目中的硬门槛和适用范围，再看“未经同意调用、返回值注入、Token 受众错误”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-agent-harness-9-2 deck:"Agent 架构与 Harness" priority:1 tags:"证据链" %}
--- question
Resources 和 Tools 的差异
--- answer
Resource 通常提供被读取的数据，Tool 会触发动作；两者都可能影响 Context 或产生副作用，权限边界不同。
--- explanation
MCP 的 initialize、发现、调用和错误是可审计的 JSON-RPC 生命周期；Tool、Resource 和 Prompt 的信任与副作用不同。问题“Resources 和 Tools 的差异”的关键不在背诵名词，而在于：Resource 通常提供被读取的数据，Tool 会触发动作；两者都可能影响 Context 或产生副作用，权限边界不同。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
