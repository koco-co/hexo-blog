---
title: Agent 架构与 Harness(二)Agent与Workflow
tags:
  - Agent 架构与 Harness
  - Agent与Workflow
categories:
  - Learn Topic
  - Agent 架构与 Harness
description: 每个任务都有固定工作流、Agent 或混合方案及停止条件
...
{% note primary flat %}
本节围绕“Agent与Workflow”建立可复核的工作模型。学习成果：每个任务都有固定工作流、Agent 或混合方案及停止条件。实验：将三个任务分别建模为固定工作流、单 Agent 和混合工作流，记录选择理由。视觉辅助：Workflow/Agent 决策树；状态流图。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
值班路由器遇到日报、未知故障和生产发布三个请求。固定流程、开放决策和高风险审批不能混成同一种执行方式，否则成本和停止条件都会失真。

本篇把“Agent与Workflow”落到一次可回放的本地任务：将三个任务分别建模为固定工作流、单 Agent 和混合工作流，记录选择理由。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：每个任务都有固定工作流、Agent 或混合方案及停止条件。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“Agent、Workflow、Assistant 和 Automation 的边界；固定流程与动态决策；开放任务的状态与停止条件”拆成输入、状态、输出和证据。Workflow 通过固定转移和显式停止条件降低不确定性；Agent 只在需要选择工具或补全未知步骤时承担决策。
{% endnote %}
...
{% note primary flat %}
实验步骤：将三个任务分别建模为固定工作流、单 Agent 和混合工作流，记录选择理由。

验收产物：三个任务的架构决策表

失败注入：开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算
{% endnote %}

## 任务边界

{% note info flat %}
本节任务：理解Agent、Workflow、Assistant 和 Automation 的边界并建立可复核判断。核心内容：Agent、Workflow、Assistant 和 Automation 的边界。

机制：Agent、Workflow、Assistant 和 Automation 的边界。先核对输入、状态、输出、失败和复测证据，再用“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“Agent边界”聚焦“Agent、Workflow、Assistant 和 Automation 的边界”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。先把“Agent、Workflow、Assistant 和 Automation 的边界”变成一个可观察字段，再以“将三个任务分别建模为固定工作流、单 Agent 和混合工作流，记录选择理由。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”，恢复或放行动作必须可重放。
{% endnote %}

### 流程与决策

{% note info flat %}
“流程与决策”聚焦“固定流程与动态决策”。围绕“固定流程与动态决策”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“流程与决策”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”，先停止并保留原始事件。本节的失败样例是“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”，恢复或放行动作必须可重放。
{% endnote %}

## 架构选择

{% note info flat %}
本节任务：理解固定流程与动态决策并建立可复核判断。核心内容：固定流程与动态决策。

机制：固定流程与动态决策。先核对输入、状态、输出、失败和复测证据，再用“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“状态与停止”聚焦“固定流程与动态决策”。围绕“固定流程与动态决策”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“状态与停止”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”，先停止并保留原始事件。本节的失败样例是“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”，恢复或放行动作必须可重放。
{% endnote %}

### 混合架构

{% note info flat %}
“混合架构”聚焦“开放任务的状态与停止条件”。围绕“开放任务的状态与停止条件”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：三个任务的架构决策表应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：将三个任务分别建模为固定工作流、单 Agent 和混合工作流，记录选择理由。

机制：将三个任务分别建模为固定工作流、单 Agent 和混合工作流，记录选择理由。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“开放任务的状态与停止条件”。围绕“开放任务的状态与停止条件”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：三个任务的架构决策表应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Agent 架构与 Harness","article":"Agent 架构与 Harness(二)Agent与Workflow","order":2,"topic":"Agent与Workflow","experiment":"将三个任务分别建模为固定工作流、单 Agent 和混合工作流，记录选择理由。","focus":["Agent、Workflow、Assistant 和 Automation 的边界","固定流程与动态决策","开放任务的状态与停止条件","混合架构的选择"],"artifacts":["三个任务的架构决策表"],"failures":["开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算"],"mode":"harness"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    bad_release = dict(next(task for task in read_json(root / "architecture-decisions.json")["tasks"] if task["name"] == "release"))
    bad_release["kind"] = "agent"
    try:
        if bad_release["kind"] != "hybrid" or bad_release["stop"] != "approval":
            raise ValueError("high-risk release lost its approval boundary")
    except ValueError as error:
        failure_event = {"status": "blocked", "type": str(error)}
        record_event(events_file, "failure_blocked", type=failure_event["type"])
    recovered = read_json(root / "architecture-decisions.json")
    release = next(task for task in recovered["tasks"] if task["name"] == "release")
    assert release["kind"] == "hybrid" and release["stop"] == "approval"
    record_event(events_file, "recovery_verified", action="restore-release-approval")
...
assert result == {"artifact_count":1,"checks":3,"failure_count":1,"mode":"harness","online_evidence":"not_run","scenario":"architecture-choice","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 3, "failure_count": 1, "mode": "harness", "online_evidence": "not_run", "scenario": "architecture-choice", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“混合架构的选择”。围绕“混合架构的选择”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“混合架构的选择”变成一个可观察字段，再以“将三个任务分别建模为固定工作流、单 Agent 和混合工作流，记录选择理由。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“Agent、Workflow、Assistant 和 Automation 的边界”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”，先停止并保留原始事件。本节的失败样例是“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”，恢复或放行动作必须可重放。
{% endnote %}

## 选择验证

{% note warning flat %}
本节任务：判断当前方案何时不能照搬。核心内容：开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算。

机制：开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算。选择依据：当前文章的核心选择或范围需要直接可见，不强行套 warning。先核对限制、风险、恢复或替代动作，再用“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”作为反例；去掉颜色后仍保留完整文字。
{% endnote %}
...
{% note info flat %}
“混合架构”聚焦“混合架构的选择”。围绕“混合架构的选择”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“混合架构的选择”变成一个可观察字段，再以“将三个任务分别建模为固定工作流、单 Agent 和混合工作流，记录选择理由。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：三个任务的架构决策表。预期：每个任务都有固定工作流、Agent 或混合方案及停止条件。失败复验：开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算。

机制：验收证据：三个任务的架构决策表。预期：每个任务都有固定工作流、Agent 或混合方案及停止条件。失败复验：开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“Agent、Workflow、Assistant 和 Automation 的边界”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”，先停止并保留原始事件。本节的失败样例是“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 三个任务的架构决策表 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“固定流程与动态决策”。围绕“固定流程与动态决策”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：三个任务的架构决策表应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-agent-harness-2-1 deck:"Agent 架构与 Harness" priority:1 tags:"边界验证" %}
--- question
什么情况下不应该使用 Agent
--- answer
只有任务需要开放式推理、动态工具选择或不确定恢复时才使用 Agent；固定、可预测的步骤优先使用 Workflow。
--- explanation
Workflow 通过固定转移和显式停止条件降低不确定性；Agent 只在需要选择工具或补全未知步骤时承担决策。问题“什么情况下不应该使用 Agent”的关键不在背诵名词，而在于：只有任务需要开放式推理、动态工具选择或不确定恢复时才使用 Agent；固定、可预测的步骤优先使用 Workflow。 先判断题目中的硬门槛和适用范围，再看“开放任务被硬编码、固定流程被无谓交给 Agent、成本超预算”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-agent-harness-2-2 deck:"Agent 架构与 Harness" priority:1 tags:"证据链" %}
--- question
Workflow 和 Agent 可以如何组合
--- answer
Workflow 负责确定性步骤，Agent 负责开放决策；可以在审批、工具调用和结果验收处组合两者。
--- explanation
Workflow 通过固定转移和显式停止条件降低不确定性；Agent 只在需要选择工具或补全未知步骤时承担决策。问题“Workflow 和 Agent 可以如何组合”的关键不在背诵名词，而在于：Workflow 负责确定性步骤，Agent 负责开放决策；可以在审批、工具调用和结果验收处组合两者。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
...
{% link LangGraph 官方文档/仓库, https://github.com/langchain-ai/langgraph/tree/11ee185999b86bfea2d8c0e69cef9a5e37acf686, https://github.com/favicon.ico %}
{% endlinkgroup %}
