---
title: Agent 架构与 Harness(五)Tool契约与执行
tags:
  - Agent 架构与 Harness
  - Tool契约与执行
categories:
...
{% note primary flat %}
本节围绕“Tool契约与执行”建立可复核的工作模型。学习成果：非法参数和越权调用在执行前被拒绝。实验：实现本地文件查询和安全写入工具，验证参数、权限、超时、错误和重复调用。视觉辅助：Tool call 生命周期图；契约表。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
模型把路径参数拼成工作区外的文件名。执行器在写入前校验 Schema、权限和取消信号，拒绝请求并保留审计记录。

本篇把“Tool契约与执行”落到一次可回放的本地任务：实现本地文件查询和安全写入工具，验证参数、权限、超时、错误和重复调用。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：非法参数和越权调用在执行前被拒绝。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“工具 Schema 和输入校验；执行器、结果类型和错误分类；权限、幂等、截止时间和取消”拆成输入、状态、输出和证据。工具契约把模型想做什么和执行器允许做什么分开，用 Schema、结果类型、错误分类和幂等键阻止自然语言越权。
{% endnote %}
...
{% note primary flat %}
实验步骤：实现本地文件查询和安全写入工具，验证参数、权限、超时、错误和重复调用。

验收产物：工具 Schema、权限矩阵、错误和取消日志

失败注入：工具自行扩大权限、重复副作用、取消不传播
{% endnote %}

## 工具Schema

{% note info flat %}
本节任务：理解工具 Schema 和输入校验并建立可复核判断。核心内容：工具 Schema 和输入校验。

机制：工具 Schema 和输入校验。先核对输入、状态、输出、失败和复测证据，再用“工具自行扩大权限、重复副作用、取消不传播”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“工具Schema”聚焦“工具 Schema 和输入校验”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。先把“工具 Schema 和输入校验”变成一个可观察字段，再以“实现本地文件查询和安全写入工具，验证参数、权限、超时、错误和重复调用。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“工具自行扩大权限、重复副作用、取消不传播”，恢复或放行动作必须可重放。
{% endnote %}

### 执行与错误

{% note info flat %}
“执行与错误”聚焦“执行器、结果类型和错误分类”。围绕“执行器、结果类型和错误分类”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“执行与错误”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“工具自行扩大权限、重复副作用、取消不传播”，先停止并保留原始事件。本节的失败样例是“工具自行扩大权限、重复副作用、取消不传播”，恢复或放行动作必须可重放。
{% endnote %}

## 执行状态

{% note info flat %}
本节任务：理解执行器、结果类型和错误分类并建立可复核判断。核心内容：执行器、结果类型和错误分类。

机制：执行器、结果类型和错误分类。先核对输入、状态、输出、失败和复测证据，再用“工具自行扩大权限、重复副作用、取消不传播”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“执行边界”聚焦“执行器、结果类型和错误分类”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“执行边界”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“工具自行扩大权限、重复副作用、取消不传播”，先停止并保留原始事件。本节的失败样例是“工具自行扩大权限、重复副作用、取消不传播”，恢复或放行动作必须可重放。
{% endnote %}

### 工具审计

{% note info flat %}
“工具审计”聚焦“权限、幂等、截止时间和取消”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。复核时把动作和产物对应起来：工具 Schema、权限矩阵、错误和取消日志应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“工具自行扩大权限、重复副作用、取消不传播”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：实现本地文件查询和安全写入工具，验证参数、权限、超时、错误和重复调用。

机制：实现本地文件查询和安全写入工具，验证参数、权限、超时、错误和重复调用。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“工具自行扩大权限、重复副作用、取消不传播”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“权限、幂等、截止时间和取消”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：工具 Schema、权限矩阵、错误和取消日志应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“工具自行扩大权限、重复副作用、取消不传播”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Agent 架构与 Harness","article":"Agent 架构与 Harness(五)Tool契约与执行","order":5,"topic":"Tool契约与执行","experiment":"实现本地文件查询和安全写入工具，验证参数、权限、超时、错误和重复调用。","focus":["工具 Schema 和输入校验","执行器、结果类型和错误分类","权限、幂等、截止时间和取消","工具调用轨迹与审计"],"artifacts":["工具 Schema、权限矩阵、错误和取消日志"],"failures":["工具自行扩大权限、重复副作用、取消不传播"],"mode":"harness"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    request = {"name": "write_file", "path": "../outside.txt", "content": "blocked"}
    try:
        target = (root / request["path"]).resolve()
        target.relative_to(root.resolve())
        raise RuntimeError("path unexpectedly escaped validation")
    ...
    safe_request = {"name": "read_file", "path": "README.md"}
    assert (root / safe_request["path"]).is_file() and not (root.parent / "outside.txt").exists()
    record_event(events_file, "recovery_verified", action="reject-outside-write")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"harness","online_evidence":"not_run","scenario":"tool-contract","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "harness", "online_evidence": "not_run", "scenario": "tool-contract", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“工具调用轨迹与审计”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。先把“工具调用轨迹与审计”变成一个可观察字段，再以“实现本地文件查询和安全写入工具，验证参数、权限、超时、错误和重复调用。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“工具自行扩大权限、重复副作用、取消不传播”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“工具 Schema 和输入校验”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“工具自行扩大权限、重复副作用、取消不传播”，先停止并保留原始事件。本节的失败样例是“工具自行扩大权限、重复副作用、取消不传播”，恢复或放行动作必须可重放。
{% endnote %}

## 权限与取消

{% note info flat %}
本节任务：理解工具调用轨迹与审计并建立可复核判断。核心内容：工具调用轨迹与审计。

机制：工具调用轨迹与审计。先核对输入、状态、输出、失败和复测证据，再用“工具自行扩大权限、重复副作用、取消不传播”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“工具审计”聚焦“工具调用轨迹与审计”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。先把“工具调用轨迹与审计”变成一个可观察字段，再以“实现本地文件查询和安全写入工具，验证参数、权限、超时、错误和重复调用。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“工具自行扩大权限、重复副作用、取消不传播”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：工具 Schema、权限矩阵、错误和取消日志。预期：非法参数和越权调用在执行前被拒绝。失败复验：工具自行扩大权限、重复副作用、取消不传播。

机制：验收证据：工具 Schema、权限矩阵、错误和取消日志。预期：非法参数和越权调用在执行前被拒绝。失败复验：工具自行扩大权限、重复副作用、取消不传播。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“工具自行扩大权限、重复副作用、取消不传播”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“工具 Schema 和输入校验”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“工具自行扩大权限、重复副作用、取消不传播”，先停止并保留原始事件。本节的失败样例是“工具自行扩大权限、重复副作用、取消不传播”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 工具 Schema、权限矩阵、错误和取消日志 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 工具自行扩大权限、重复副作用、取消不传播 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“执行器、结果类型和错误分类”。围绕“执行器、结果类型和错误分类”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：工具 Schema、权限矩阵、错误和取消日志应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“工具自行扩大权限、重复副作用、取消不传播”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-agent-harness-5-1 deck:"Agent 架构与 Harness" priority:1 tags:"边界验证" %}
--- question
工具结果为什么必须带状态
--- answer
工具结果至少要带 success/error、状态、输出和可重试性；调用方才能区分完成、失败和待处理。
--- explanation
工具契约把模型想做什么和执行器允许做什么分开，用 Schema、结果类型、错误分类和幂等键阻止自然语言越权。问题“工具结果为什么必须带状态”的关键不在背诵名词，而在于：工具结果至少要带 success/error、状态、输出和可重试性；调用方才能区分完成、失败和待处理。 先判断题目中的硬门槛和适用范围，再看“工具自行扩大权限、重复副作用、取消不传播”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-agent-harness-5-2 deck:"Agent 架构与 Harness" priority:1 tags:"证据链" %}
--- question
工具参数校验应由谁负责
--- answer
由 Harness 在工具边界做 Schema、权限和策略校验，工具实现还要做业务参数校验；不能只依赖模型。
--- explanation
工具契约把模型想做什么和执行器允许做什么分开，用 Schema、结果类型、错误分类和幂等键阻止自然语言越权。问题“工具参数校验应由谁负责”的关键不在背诵名词，而在于：由 Harness 在工具边界做 Schema、权限和策略校验，工具实现还要做业务参数校验；不能只依赖模型。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
