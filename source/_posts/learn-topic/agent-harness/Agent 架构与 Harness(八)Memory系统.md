---
title: Agent 架构与 Harness(八)Memory系统
tags:
  - Agent 架构与 Harness
  - Memory系统
categories:
  - Learn Topic
  - Agent 架构与 Harness
description: 项目/全局记忆隔离，错误记忆可修正和删除
...
{% note primary flat %}
本节围绕“Memory系统”建立可复核的工作模型。学习成果：项目/全局记忆隔离，错误记忆可修正和删除。实验：用本地 JSON 存储实现带作用域、版本和删除操作的记忆组件。视觉辅助：Memory 生命周期图；作用域表。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
项目记忆把旧接口名带进新项目，助手连续给出错误建议。带作用域、版本、过期和删除证据的记忆才适合重新进入 Context。

本篇把“Memory系统”落到一次可回放的本地任务：用本地 JSON 存储实现带作用域、版本和删除操作的记忆组件。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：项目/全局记忆隔离，错误记忆可修正和删除。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“会话历史与短期记忆；长期记忆、项目记忆和全局记忆；写入触发、检索、冲突和过期”拆成输入、状态、输出和证据。Memory 的价值取决于作用域、时效和可删除性；项目事实不能无条件进入全局，冲突事实也不能只靠最新写入解决。
{% endnote %}
...
{% note primary flat %}
实验步骤：用本地 JSON 存储实现带作用域、版本和删除操作的记忆组件。

验收产物：作用域、版本、读取、过期和删除记录

失败注入：跨项目泄露、旧记忆覆盖新事实、删除无证据
{% endnote %}

## 记忆作用域

{% note info flat %}
本节任务：理解会话历史与短期记忆并建立可复核判断。核心内容：会话历史与短期记忆。

机制：会话历史与短期记忆。先核对输入、状态、输出、失败和复测证据，再用“跨项目泄露、旧记忆覆盖新事实、删除无证据”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“短期记忆”聚焦“会话历史与短期记忆”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。先把“会话历史与短期记忆”变成一个可观察字段，再以“用本地 JSON 存储实现带作用域、版本和删除操作的记忆组件。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“跨项目泄露、旧记忆覆盖新事实、删除无证据”，恢复或放行动作必须可重放。
{% endnote %}

### 长期记忆

{% note info flat %}
“长期记忆”聚焦“长期记忆、项目记忆和全局记忆”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。具体取舍是：保留“长期记忆”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“跨项目泄露、旧记忆覆盖新事实、删除无证据”，先停止并保留原始事件。本节的失败样例是“跨项目泄露、旧记忆覆盖新事实、删除无证据”，恢复或放行动作必须可重放。
{% endnote %}

## 写入与读取

{% note info flat %}
本节任务：理解长期记忆、项目记忆和全局记忆并建立可复核判断。核心内容：长期记忆、项目记忆和全局记忆。

机制：长期记忆、项目记忆和全局记忆。先核对输入、状态、输出、失败和复测证据，再用“跨项目泄露、旧记忆覆盖新事实、删除无证据”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“记忆检索”聚焦“长期记忆、项目记忆和全局记忆”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。具体取舍是：保留“记忆检索”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“跨项目泄露、旧记忆覆盖新事实、删除无证据”，先停止并保留原始事件。本节的失败样例是“跨项目泄露、旧记忆覆盖新事实、删除无证据”，恢复或放行动作必须可重放。
{% endnote %}

### 记忆修复

{% note info flat %}
“记忆修复”聚焦“写入触发、检索、冲突和过期”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。复核时把动作和产物对应起来：作用域、版本、读取、过期和删除记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“跨项目泄露、旧记忆覆盖新事实、删除无证据”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：用本地 JSON 存储实现带作用域、版本和删除操作的记忆组件。

机制：用本地 JSON 存储实现带作用域、版本和删除操作的记忆组件。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“跨项目泄露、旧记忆覆盖新事实、删除无证据”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“写入触发、检索、冲突和过期”。围绕“写入触发、检索、冲突和过期”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：作用域、版本、读取、过期和删除记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“跨项目泄露、旧记忆覆盖新事实、删除无证据”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Agent 架构与 Harness","article":"Agent 架构与 Harness(八)Memory系统","order":8,"topic":"Memory系统","experiment":"用本地 JSON 存储实现带作用域、版本和删除操作的记忆组件。","focus":["会话历史与短期记忆","长期记忆、项目记忆和全局记忆","写入触发、检索、冲突和过期","隐私删除与错误记忆修正"],"artifacts":["作用域、版本、读取、过期和删除记录"],"failures":["跨项目泄露、旧记忆覆盖新事实、删除无证据"],"mode":"harness"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    stale = {"version": 1, "value": "old"}
    try:
        if stale["version"] < memory["project"]["version"]:
            raise ValueError("stale project memory would overwrite the current fact")
    ...
    del memory["project"]
    write_json(root / "memory.json", memory)
    restored = read_json(root / "memory.json")
    assert "project" not in restored and "global" in restored
    record_event(events_file, "recovery_verified", action="delete-stale-project-memory")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"harness","online_evidence":"not_run","scenario":"memory-scope","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "harness", "online_evidence": "not_run", "scenario": "memory-scope", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“隐私删除与错误记忆修正”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。先把“隐私删除与错误记忆修正”变成一个可观察字段，再以“用本地 JSON 存储实现带作用域、版本和删除操作的记忆组件。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“跨项目泄露、旧记忆覆盖新事实、删除无证据”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“会话历史与短期记忆”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“跨项目泄露、旧记忆覆盖新事实、删除无证据”，先停止并保留原始事件。本节的失败样例是“跨项目泄露、旧记忆覆盖新事实、删除无证据”，恢复或放行动作必须可重放。
{% endnote %}

## 过期与删除

{% note info flat %}
本节任务：理解隐私删除与错误记忆修正并建立可复核判断。核心内容：隐私删除与错误记忆修正。

机制：隐私删除与错误记忆修正。先核对输入、状态、输出、失败和复测证据，再用“跨项目泄露、旧记忆覆盖新事实、删除无证据”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“记忆修复”聚焦“隐私删除与错误记忆修正”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。先把“隐私删除与错误记忆修正”变成一个可观察字段，再以“用本地 JSON 存储实现带作用域、版本和删除操作的记忆组件。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“跨项目泄露、旧记忆覆盖新事实、删除无证据”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：作用域、版本、读取、过期和删除记录。预期：项目/全局记忆隔离，错误记忆可修正和删除。失败复验：跨项目泄露、旧记忆覆盖新事实、删除无证据。

机制：验收证据：作用域、版本、读取、过期和删除记录。预期：项目/全局记忆隔离，错误记忆可修正和删除。失败复验：跨项目泄露、旧记忆覆盖新事实、删除无证据。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“跨项目泄露、旧记忆覆盖新事实、删除无证据”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“会话历史与短期记忆”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“跨项目泄露、旧记忆覆盖新事实、删除无证据”，先停止并保留原始事件。本节的失败样例是“跨项目泄露、旧记忆覆盖新事实、删除无证据”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 作用域、版本、读取、过期和删除记录 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 跨项目泄露、旧记忆覆盖新事实、删除无证据 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“长期记忆、项目记忆和全局记忆”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。复核时把动作和产物对应起来：作用域、版本、读取、过期和删除记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“跨项目泄露、旧记忆覆盖新事实、删除无证据”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-agent-harness-8-1 deck:"Agent 架构与 Harness" priority:1 tags:"边界验证" %}
--- question
记忆为什么可能降低质量
--- answer
过期、冲突、错误或越权记忆会污染 Context，使 Agent 复用错误事实；写入成功不等于记忆有益。
--- explanation
Memory 的价值取决于作用域、时效和可删除性；项目事实不能无条件进入全局，冲突事实也不能只靠最新写入解决。问题“记忆为什么可能降低质量”的关键不在背诵名词，而在于：过期、冲突、错误或越权记忆会污染 Context，使 Agent 复用错误事实；写入成功不等于记忆有益。 先判断题目中的硬门槛和适用范围，再看“跨项目泄露、旧记忆覆盖新事实、删除无证据”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-agent-harness-8-2 deck:"Agent 架构与 Harness" priority:1 tags:"证据链" %}
--- question
项目记忆和全局记忆如何隔离
--- answer
项目记忆以项目标识隔离，全局记忆只能存放明确允许共享的内容；读取和删除都要检查作用域。
--- explanation
Memory 的价值取决于作用域、时效和可删除性；项目事实不能无条件进入全局，冲突事实也不能只靠最新写入解决。问题“项目记忆和全局记忆如何隔离”的关键不在背诵名词，而在于：项目记忆以项目标识隔离，全局记忆只能存放明确允许共享的内容；读取和删除都要检查作用域。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
