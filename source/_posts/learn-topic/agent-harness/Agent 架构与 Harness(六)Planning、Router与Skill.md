---
title: Agent 架构与 Harness(六)Planning、Router与Skill
tags:
  - Agent 架构与 Harness
  - Planning、Router与Skill
categories:
  - Learn Topic
  - Agent 架构与 Harness
description: 选择理由、步骤、失败重规划和 Skill 加载可解释
...
{% note primary flat %}
本节围绕“Planning、Router与Skill”建立可复核的工作模型。学习成果：选择理由、步骤、失败重规划和 Skill 加载可解释。实验：为三个任务实现直接、Router 和 Skill 三种入口，比较步骤数、错误和可解释性。视觉辅助：路由与 Skill 发现图；策略表。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
同一个请求有时适合直接执行，有时需要 Router 选择 Skill。关键不是每次都规划，而是让选择理由和重规划条件可解释。

本篇把“Planning、Router与Skill”落到一次可回放的本地任务：为三个任务实现直接、Router 和 Skill 三种入口，比较步骤数、错误和可解释性。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：选择理由、步骤、失败重规划和 Skill 加载可解释。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“直接执行、规划后执行和分层规划；Router 的意图、能力和置信度；Skill 的发现、加载和边界”拆成输入、状态、输出和证据。规划是成本换取可见中间状态；Router 选择能力，Skill 封装任务方法，三者都需要置信度、范围和重新规划条件。
{% endnote %}
...
{% note primary flat %}
实验步骤：为三个任务实现直接、Router 和 Skill 三种入口，比较步骤数、错误和可解释性。

验收产物：三个任务的路由/规划/Skill 选择表

失败注入：所有任务强行规划、路由误判、Skill 全量注入上下文
{% endnote %}

## 规划策略

{% note info flat %}
本节任务：理解直接执行、规划后执行和分层规划并建立可复核判断。核心内容：直接执行、规划后执行和分层规划。

机制：直接执行、规划后执行和分层规划。先核对输入、状态、输出、失败和复测证据，再用“所有任务强行规划、路由误判、Skill 全量注入上下文”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“规划层级”聚焦“直接执行、规划后执行和分层规划”。围绕“直接执行、规划后执行和分层规划”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“直接执行、规划后执行和分层规划”变成一个可观察字段，再以“为三个任务实现直接、Router 和 Skill 三种入口，比较步骤数、错误和可解释性。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“所有任务强行规划、路由误判、Skill 全量注入上下文”，恢复或放行动作必须可重放。
{% endnote %}

### 路由置信度

{% note info flat %}
“路由置信度”聚焦“Router 的意图、能力和置信度”。围绕“Router 的意图、能力和置信度”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“路由置信度”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“所有任务强行规划、路由误判、Skill 全量注入上下文”，先停止并保留原始事件。本节的失败样例是“所有任务强行规划、路由误判、Skill 全量注入上下文”，恢复或放行动作必须可重放。
{% endnote %}

## 路由与Skill

{% note info flat %}
本节任务：理解Router 的意图、能力和置信度并建立可复核判断。核心内容：Router 的意图、能力和置信度。

机制：Router 的意图、能力和置信度。先核对输入、状态、输出、失败和复测证据，再用“所有任务强行规划、路由误判、Skill 全量注入上下文”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“Skill边界”聚焦“Router 的意图、能力和置信度”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。具体取舍是：保留“Skill边界”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“所有任务强行规划、路由误判、Skill 全量注入上下文”，先停止并保留原始事件。本节的失败样例是“所有任务强行规划、路由误判、Skill 全量注入上下文”，恢复或放行动作必须可重放。
{% endnote %}

### 重新规划

{% note info flat %}
“重新规划”聚焦“Skill 的发现、加载和边界”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。复核时把动作和产物对应起来：三个任务的路由/规划/Skill 选择表应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“所有任务强行规划、路由误判、Skill 全量注入上下文”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：为三个任务实现直接、Router 和 Skill 三种入口，比较步骤数、错误和可解释性。

机制：为三个任务实现直接、Router 和 Skill 三种入口，比较步骤数、错误和可解释性。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“所有任务强行规划、路由误判、Skill 全量注入上下文”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“Skill 的发现、加载和边界”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。复核时把动作和产物对应起来：三个任务的路由/规划/Skill 选择表应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“所有任务强行规划、路由误判、Skill 全量注入上下文”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Agent 架构与 Harness","article":"Agent 架构与 Harness(六)Planning、Router与Skill","order":6,"topic":"Planning、Router与Skill","experiment":"为三个任务实现直接、Router 和 Skill 三种入口，比较步骤数、错误和可解释性。","focus":["直接执行、规划后执行和分层规划","Router 的意图、能力和置信度","Skill 的发现、加载和边界","规划失败与重新规划"],"artifacts":["三个任务的路由/规划/Skill 选择表"],"failures":["所有任务强行规划、路由误判、Skill 全量注入上下文"],"mode":"harness"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    routes = read_json(root / "routing.json")["routes"]
    bad_route = "unknown-skill"
    try:
        if bad_route not in {"direct", "router", "hybrid"}:
            raise LookupError("router selected an unavailable skill")
    ...
    routes[-1] = "hybrid"
    write_json(root / "routing.json", {"routes": routes})
    assert read_json(root / "routing.json")["routes"] == ["direct", "router", "hybrid"]
    record_event(events_file, "recovery_verified", action="route-to-known-capability")
...
assert result == {"artifact_count":1,"checks":3,"failure_count":1,"mode":"harness","online_evidence":"not_run","scenario":"planning-router","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 3, "failure_count": 1, "mode": "harness", "online_evidence": "not_run", "scenario": "planning-router", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“规划失败与重新规划”。围绕“规划失败与重新规划”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“规划失败与重新规划”变成一个可观察字段，再以“为三个任务实现直接、Router 和 Skill 三种入口，比较步骤数、错误和可解释性。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“所有任务强行规划、路由误判、Skill 全量注入上下文”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“直接执行、规划后执行和分层规划”。围绕“直接执行、规划后执行和分层规划”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“所有任务强行规划、路由误判、Skill 全量注入上下文”，先停止并保留原始事件。本节的失败样例是“所有任务强行规划、路由误判、Skill 全量注入上下文”，恢复或放行动作必须可重放。
{% endnote %}

## 失败重规划

{% note info flat %}
本节任务：理解规划失败与重新规划并建立可复核判断。核心内容：规划失败与重新规划。

机制：规划失败与重新规划。先核对输入、状态、输出、失败和复测证据，再用“所有任务强行规划、路由误判、Skill 全量注入上下文”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“重新规划”聚焦“规划失败与重新规划”。围绕“规划失败与重新规划”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“规划失败与重新规划”变成一个可观察字段，再以“为三个任务实现直接、Router 和 Skill 三种入口，比较步骤数、错误和可解释性。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“所有任务强行规划、路由误判、Skill 全量注入上下文”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：三个任务的路由/规划/Skill 选择表。预期：选择理由、步骤、失败重规划和 Skill 加载可解释。失败复验：所有任务强行规划、路由误判、Skill 全量注入上下文。

机制：验收证据：三个任务的路由/规划/Skill 选择表。预期：选择理由、步骤、失败重规划和 Skill 加载可解释。失败复验：所有任务强行规划、路由误判、Skill 全量注入上下文。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“所有任务强行规划、路由误判、Skill 全量注入上下文”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“直接执行、规划后执行和分层规划”。围绕“直接执行、规划后执行和分层规划”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“所有任务强行规划、路由误判、Skill 全量注入上下文”，先停止并保留原始事件。本节的失败样例是“所有任务强行规划、路由误判、Skill 全量注入上下文”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 三个任务的路由/规划/Skill 选择表 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 所有任务强行规划、路由误判、Skill 全量注入上下文 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“Router 的意图、能力和置信度”。围绕“Router 的意图、能力和置信度”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：三个任务的路由/规划/Skill 选择表应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“所有任务强行规划、路由误判、Skill 全量注入上下文”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-agent-harness-6-1 deck:"Agent 架构与 Harness" priority:1 tags:"边界验证" %}
--- question
规划是否总能提升 Agent 质量
--- answer
规划只有在任务不确定、步骤较多或需要检查点时才可能提升质量；简单、低风险任务的规划成本可能不值得。
--- explanation
规划是成本换取可见中间状态；Router 选择能力，Skill 封装任务方法，三者都需要置信度、范围和重新规划条件。问题“规划是否总能提升 Agent 质量”的关键不在背诵名词，而在于：规划只有在任务不确定、步骤较多或需要检查点时才可能提升质量；简单、低风险任务的规划成本可能不值得。 先判断题目中的硬门槛和适用范围，再看“所有任务强行规划、路由误判、Skill 全量注入上下文”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-agent-harness-6-2 deck:"Agent 架构与 Harness" priority:1 tags:"证据链" %}
--- question
Skill 与 Tool 的职责差异
--- answer
Tool 是一次受权限约束的能力调用，Skill 是可复用的任务方法与约束；Skill 可以编排多个 Tool，但不应伪装成单个工具。
--- explanation
规划是成本换取可见中间状态；Router 选择能力，Skill 封装任务方法，三者都需要置信度、范围和重新规划条件。问题“Skill 与 Tool 的职责差异”的关键不在背诵名词，而在于：Tool 是一次受权限约束的能力调用，Skill 是可复用的任务方法与约束；Skill 可以编排多个 Tool，但不应伪装成单个工具。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
