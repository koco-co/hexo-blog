---
title: Agent 架构与 Harness(十)人工审批与工作流
tags:
  - Agent 架构与 Harness
  - 人工审批与工作流
categories:
  - Learn Topic
  - Agent 架构与 Harness
description: 批准、拒绝、修改、暂停和恢复均可审计
...
{% note primary flat %}
本节围绕“人工审批与工作流”建立可复核的工作模型。学习成果：批准、拒绝、修改、暂停和恢复均可审计。实验：为删除、外发和部署三个动作设计审批流，验证拒绝和恢复。视觉辅助：审批状态机；风险矩阵。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
自动化助手准备删除旧数据，审批人拒绝后它仍沿原队列执行。暂停状态机把拒绝、修改、恢复和不可逆动作分开。

本篇把“人工审批与工作流”落到一次可回放的本地任务：为删除、外发和部署三个动作设计审批流，验证拒绝和恢复。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：批准、拒绝、修改、暂停和恢复均可审计。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“风险分级与审批点；暂停/恢复状态机；拒绝、修改参数和人工接管”拆成输入、状态、输出和证据。审批把高风险动作从模型建议变成待决请求，状态机必须区分 pending、rejected 和 executed，避免拒绝后沿旧队列运行。
{% endnote %}
...
{% note primary flat %}
实验步骤：为删除、外发和部署三个动作设计审批流，验证拒绝和恢复。

验收产物：删除/外发/部署三种审批状态机记录

失败注入：审批绕过、拒绝后仍执行、恢复重复副作用
{% endnote %}

## 风险分级

{% note info flat %}
本节任务：理解风险分级与审批点并建立可复核判断。核心内容：风险分级与审批点。

机制：风险分级与审批点。先核对输入、状态、输出、失败和复测证据，再用“审批绕过、拒绝后仍执行、恢复重复副作用”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“风险分级”聚焦“风险分级与审批点”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。先把“风险分级与审批点”变成一个可观察字段，再以“为删除、外发和部署三个动作设计审批流，验证拒绝和恢复。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“审批绕过、拒绝后仍执行、恢复重复副作用”，恢复或放行动作必须可重放。
{% endnote %}

### 暂停与恢复

{% note info flat %}
“暂停与恢复”聚焦“暂停/恢复状态机”。围绕“暂停/恢复状态机”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“暂停与恢复”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“审批绕过、拒绝后仍执行、恢复重复副作用”，先停止并保留原始事件。本节的失败样例是“审批绕过、拒绝后仍执行、恢复重复副作用”，恢复或放行动作必须可重放。
{% endnote %}

## 暂停恢复

{% note info flat %}
本节任务：理解暂停/恢复状态机并建立可复核判断。核心内容：暂停/恢复状态机。

机制：暂停/恢复状态机。先核对输入、状态、输出、失败和复测证据，再用“审批绕过、拒绝后仍执行、恢复重复副作用”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“人工接管”聚焦“暂停/恢复状态机”。围绕“暂停/恢复状态机”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“人工接管”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“审批绕过、拒绝后仍执行、恢复重复副作用”，先停止并保留原始事件。本节的失败样例是“审批绕过、拒绝后仍执行、恢复重复副作用”，恢复或放行动作必须可重放。
{% endnote %}

### 审批审计

{% note info flat %}
“审批审计”聚焦“拒绝、修改参数和人工接管”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：删除/外发/部署三种审批状态机记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“审批绕过、拒绝后仍执行、恢复重复副作用”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：为删除、外发和部署三个动作设计审批流，验证拒绝和恢复。

机制：为删除、外发和部署三个动作设计审批流，验证拒绝和恢复。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“审批绕过、拒绝后仍执行、恢复重复副作用”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“拒绝、修改参数和人工接管”。围绕“拒绝、修改参数和人工接管”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：删除/外发/部署三种审批状态机记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“审批绕过、拒绝后仍执行、恢复重复副作用”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Agent 架构与 Harness","article":"Agent 架构与 Harness(十)人工审批与工作流","order":10,"topic":"人工审批与工作流","experiment":"为删除、外发和部署三个动作设计审批流，验证拒绝和恢复。","focus":["风险分级与审批点","暂停/恢复状态机","拒绝、修改参数和人工接管","审批记录与可追责性"],"artifacts":["删除/外发/部署三种审批状态机记录"],"failures":["审批绕过、拒绝后仍执行、恢复重复副作用"],"mode":"harness"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    try:
        if approval["decisions"]["delete"] != "approved":
            raise PermissionError("rejected delete remained executable")
    ...
    approval["executed"] = [name for name in approval["executed"] if name != "delete"]
    write_json(root / "approval.json", approval)
    assert "delete" not in read_json(root / "approval.json")["executed"]
    record_event(events_file, "recovery_verified", action="clear-rejected-action")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"harness","online_evidence":"not_run","scenario":"approval-flow","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "harness", "online_evidence": "not_run", "scenario": "approval-flow", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“审批记录与可追责性”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。先把“审批记录与可追责性”变成一个可观察字段，再以“为删除、外发和部署三个动作设计审批流，验证拒绝和恢复。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“审批绕过、拒绝后仍执行、恢复重复副作用”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“风险分级与审批点”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“审批绕过、拒绝后仍执行、恢复重复副作用”，先停止并保留原始事件。本节的失败样例是“审批绕过、拒绝后仍执行、恢复重复副作用”，恢复或放行动作必须可重放。
{% endnote %}

## 审批审计

{% note info flat %}
本节任务：理解审批记录与可追责性并建立可复核判断。核心内容：审批记录与可追责性。

机制：审批记录与可追责性。先核对输入、状态、输出、失败和复测证据，再用“审批绕过、拒绝后仍执行、恢复重复副作用”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“审批审计”聚焦“审批记录与可追责性”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。先把“审批记录与可追责性”变成一个可观察字段，再以“为删除、外发和部署三个动作设计审批流，验证拒绝和恢复。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“审批绕过、拒绝后仍执行、恢复重复副作用”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：删除/外发/部署三种审批状态机记录。预期：批准、拒绝、修改、暂停和恢复均可审计。失败复验：审批绕过、拒绝后仍执行、恢复重复副作用。

机制：验收证据：删除/外发/部署三种审批状态机记录。预期：批准、拒绝、修改、暂停和恢复均可审计。失败复验：审批绕过、拒绝后仍执行、恢复重复副作用。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“审批绕过、拒绝后仍执行、恢复重复副作用”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“风险分级与审批点”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“审批绕过、拒绝后仍执行、恢复重复副作用”，先停止并保留原始事件。本节的失败样例是“审批绕过、拒绝后仍执行、恢复重复副作用”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 删除/外发/部署三种审批状态机记录 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 审批绕过、拒绝后仍执行、恢复重复副作用 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“暂停/恢复状态机”。围绕“暂停/恢复状态机”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：删除/外发/部署三种审批状态机记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“审批绕过、拒绝后仍执行、恢复重复副作用”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-agent-harness-10-1 deck:"Agent 架构与 Harness" priority:1 tags:"边界验证" %}
--- question
审批应放在模型决定前还是工具执行前
--- answer
审批应位于高风险工具真正执行之前，并绑定具体参数、受众、有效期和幂等键。
--- explanation
审批把高风险动作从模型建议变成待决请求，状态机必须区分 pending、rejected 和 executed，避免拒绝后沿旧队列运行。问题“审批应放在模型决定前还是工具执行前”的关键不在背诵名词，而在于：审批应位于高风险工具真正执行之前，并绑定具体参数、受众、有效期和幂等键。 先判断题目中的硬门槛和适用范围，再看“审批绕过、拒绝后仍执行、恢复重复副作用”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-agent-harness-10-2 deck:"Agent 架构与 Harness" priority:1 tags:"证据链" %}
--- question
拒绝后的上下文如何处理
--- answer
拒绝后把请求标记为 rejected，清除或隔离待执行动作并记录原因；不能让 Agent 沿旧队列继续执行。
--- explanation
审批把高风险动作从模型建议变成待决请求，状态机必须区分 pending、rejected 和 executed，避免拒绝后沿旧队列运行。问题“拒绝后的上下文如何处理”的关键不在背诵名词，而在于：拒绝后把请求标记为 rejected，清除或隔离待执行动作并记录原因；不能让 Agent 沿旧队列继续执行。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
