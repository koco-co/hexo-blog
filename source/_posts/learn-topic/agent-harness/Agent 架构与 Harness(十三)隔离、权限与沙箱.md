---
title: Agent 架构与 Harness(十三)隔离、权限与沙箱
tags:
  - Agent 架构与 Harness
  - 隔离、权限与沙箱
categories:
...
{% note primary flat %}
本节围绕“隔离、权限与沙箱”建立可复核的工作模型。学习成果：最小权限矩阵在本地沙箱中生效。实验：为一个代码 Agent 写最小权限矩阵，使用本地目录和无网络夹具验证拒绝。视觉辅助：权限边界图；威胁面表。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
代码 Agent 只被允许读取临时目录，却尝试访问秘密文件和网络地址。最小权限矩阵让拒绝结果成为可检查证据。

本篇把“隔离、权限与沙箱”落到一次可回放的本地任务：为一个代码 Agent 写最小权限矩阵，使用本地目录和无网络夹具验证拒绝。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：最小权限矩阵在本地沙箱中生效。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“能力、身份和权限；工作区、容器、虚拟机和网络隔离；读写、执行、外发和秘密边界”拆成输入、状态、输出和证据。安全边界由身份、能力、资源和动作共同组成；沙箱限制运行面，但不能替代秘密不进入 Context、外发控制和审计。
{% endnote %}
...
{% note primary flat %}
实验步骤：为一个代码 Agent 写最小权限矩阵，使用本地目录和无网络夹具验证拒绝。

验收产物：读/写/执行/网络/秘密的拒绝测试

失败注入：沙箱逃逸、读取秘密、外发和破坏性命令未阻断
{% endnote %}

## 权限模型

{% note info flat %}
本节任务：理解能力、身份和权限并建立可复核判断。核心内容：能力、身份和权限。

机制：能力、身份和权限。先核对输入、状态、输出、失败和复测证据，再用“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“能力权限”聚焦“能力、身份和权限”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。先把“能力、身份和权限”变成一个可观察字段，再以“为一个代码 Agent 写最小权限矩阵，使用本地目录和无网络夹具验证拒绝。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”，恢复或放行动作必须可重放。
{% endnote %}

### 运行隔离

{% note info flat %}
“运行隔离”聚焦“工作区、容器、虚拟机和网络隔离”。围绕“工作区、容器、虚拟机和网络隔离”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“运行隔离”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”，先停止并保留原始事件。本节的失败样例是“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”，恢复或放行动作必须可重放。
{% endnote %}

## 运行隔离

{% note info flat %}
本节任务：理解工作区、容器、虚拟机和网络隔离并建立可复核判断。核心内容：工作区、容器、虚拟机和网络隔离。

机制：工作区、容器、虚拟机和网络隔离。先核对输入、状态、输出、失败和复测证据，再用“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“外发边界”聚焦“工作区、容器、虚拟机和网络隔离”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“外发边界”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”，先停止并保留原始事件。本节的失败样例是“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”，恢复或放行动作必须可重放。
{% endnote %}

### 沙箱降级

{% note info flat %}
“沙箱降级”聚焦“读写、执行、外发和秘密边界”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：读/写/执行/网络/秘密的拒绝测试应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：为一个代码 Agent 写最小权限矩阵，使用本地目录和无网络夹具验证拒绝。

机制：为一个代码 Agent 写最小权限矩阵，使用本地目录和无网络夹具验证拒绝。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“读写、执行、外发和秘密边界”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：读/写/执行/网络/秘密的拒绝测试应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Agent 架构与 Harness","article":"Agent 架构与 Harness(十三)隔离、权限与沙箱","order":13,"topic":"隔离、权限与沙箱","experiment":"为一个代码 Agent 写最小权限矩阵，使用本地目录和无网络夹具验证拒绝。","focus":["能力、身份和权限","工作区、容器、虚拟机和网络隔离","读写、执行、外发和秘密边界","沙箱失败与降级"],"artifacts":["读/写/执行/网络/秘密的拒绝测试"],"failures":["沙箱逃逸、读取秘密、外发和破坏性命令未阻断"],"mode":"harness"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    escaped = (sandbox / "../secret.txt").resolve()
    try:
        escaped.relative_to(sandbox.resolve())
        raise RuntimeError("sandbox path check failed")
    ...
    permissions = read_json(root / "permission-check.json")["permissions"]
    assert permissions["network"] is False and permissions["secret"] is False and not escaped.exists()
    record_event(events_file, "recovery_verified", action="deny-escape-and-secret-read")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"harness","online_evidence":"not_run","scenario":"sandbox-deny","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "harness", "online_evidence": "not_run", "scenario": "sandbox-deny", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“沙箱失败与降级”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。先把“沙箱失败与降级”变成一个可观察字段，再以“为一个代码 Agent 写最小权限矩阵，使用本地目录和无网络夹具验证拒绝。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“能力、身份和权限”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”，先停止并保留原始事件。本节的失败样例是“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”，恢复或放行动作必须可重放。
{% endnote %}

## 拒绝验证

{% note info flat %}
本节任务：理解沙箱失败与降级并建立可复核判断。核心内容：沙箱失败与降级。

机制：沙箱失败与降级。先核对输入、状态、输出、失败和复测证据，再用“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“沙箱降级”聚焦“沙箱失败与降级”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。先把“沙箱失败与降级”变成一个可观察字段，再以“为一个代码 Agent 写最小权限矩阵，使用本地目录和无网络夹具验证拒绝。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：读/写/执行/网络/秘密的拒绝测试。预期：最小权限矩阵在本地沙箱中生效。失败复验：沙箱逃逸、读取秘密、外发和破坏性命令未阻断。

机制：验收证据：读/写/执行/网络/秘密的拒绝测试。预期：最小权限矩阵在本地沙箱中生效。失败复验：沙箱逃逸、读取秘密、外发和破坏性命令未阻断。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“能力、身份和权限”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”，先停止并保留原始事件。本节的失败样例是“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 读/写/执行/网络/秘密的拒绝测试 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 沙箱逃逸、读取秘密、外发和破坏性命令未阻断 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“工作区、容器、虚拟机和网络隔离”。围绕“工作区、容器、虚拟机和网络隔离”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：读/写/执行/网络/秘密的拒绝测试应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-agent-harness-13-1 deck:"Agent 架构与 Harness" priority:1 tags:"边界验证" %}
--- question
沙箱不能解决哪些问题
--- answer
沙箱不能证明模型不会泄露、策略没有配置错误、依赖没有恶意行为，也不能替代审批和审计。
--- explanation
安全边界由身份、能力、资源和动作共同组成；沙箱限制运行面，但不能替代秘密不进入 Context、外发控制和审计。问题“沙箱不能解决哪些问题”的关键不在背诵名词，而在于：沙箱不能证明模型不会泄露、策略没有配置错误、依赖没有恶意行为，也不能替代审批和审计。 先判断题目中的硬门槛和适用范围，再看“沙箱逃逸、读取秘密、外发和破坏性命令未阻断”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-agent-harness-13-2 deck:"Agent 架构与 Harness" priority:1 tags:"证据链" %}
--- question
为什么读取秘密也属于高风险能力
--- answer
读取秘密会把敏感数据带入 Context、日志或外发路径，即使没有写入文件，也已经扩大了影响面。
--- explanation
安全边界由身份、能力、资源和动作共同组成；沙箱限制运行面，但不能替代秘密不进入 Context、外发控制和审计。问题“为什么读取秘密也属于高风险能力”的关键不在背诵名词，而在于：读取秘密会把敏感数据带入 Context、日志或外发路径，即使没有写入文件，也已经扩大了影响面。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
