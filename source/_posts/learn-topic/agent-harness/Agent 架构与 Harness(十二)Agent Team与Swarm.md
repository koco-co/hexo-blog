---
title: Agent 架构与 Harness(十二)Agent Team与Swarm
tags:
  - Agent 架构与 Harness
  - Agent Team与Swarm
categories:
  - Learn Topic
  - Agent 架构与 Harness
description: 角色、通信、冲突、重复和收敛有独立指标
...
{% note primary flat %}
本节围绕“Agent Team与Swarm”建立可复核的工作模型。学习成果：角色、通信、冲突、重复和收敛有独立指标。实验：用本地任务队列模拟集中式和去中心化协作，比较冲突与收敛证据。视觉辅助：Team/Swarm 拓扑图；取舍表。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
三个角色同时更新共享黑板，最后答案完整却丢了冲突记录。集中式队列和去中心化消息都需要先定义收敛条件。

本篇把“Agent Team与Swarm”落到一次可回放的本地任务：用本地任务队列模拟集中式和去中心化协作，比较冲突与收敛证据。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：角色、通信、冲突、重复和收敛有独立指标。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“角色划分与协调器；共享黑板、邮箱和事件总线；冲突、重复和收敛”拆成输入、状态、输出和证据。Team 偏显式角色和协调，Swarm 偏分散协作；两者都必须定义共享状态的写入规则、冲突仲裁和收敛条件。
{% endnote %}
...
{% note primary flat %}
实验步骤：用本地任务队列模拟集中式和去中心化协作，比较冲突与收敛证据。

验收产物：集中式与去中心化协作轨迹

失败注入：循环通信、共享状态竞争、无法收敛
{% endnote %}

## 角色协调

{% note info flat %}
本节任务：理解角色划分与协调器并建立可复核判断。核心内容：角色划分与协调器。

机制：角色划分与协调器。先核对输入、状态、输出、失败和复测证据，再用“循环通信、共享状态竞争、无法收敛”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“角色协调”聚焦“角色划分与协调器”。围绕“角色划分与协调器”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“角色划分与协调器”变成一个可观察字段，再以“用本地任务队列模拟集中式和去中心化协作，比较冲突与收敛证据。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“循环通信、共享状态竞争、无法收敛”，恢复或放行动作必须可重放。
{% endnote %}

### 通信方式

{% note info flat %}
“通信方式”聚焦“共享黑板、邮箱和事件总线”。围绕“共享黑板、邮箱和事件总线”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“通信方式”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“循环通信、共享状态竞争、无法收敛”，先停止并保留原始事件。本节的失败样例是“循环通信、共享状态竞争、无法收敛”，恢复或放行动作必须可重放。
{% endnote %}

## 共享状态

{% note info flat %}
本节任务：理解共享黑板、邮箱和事件总线并建立可复核判断。核心内容：共享黑板、邮箱和事件总线。

机制：共享黑板、邮箱和事件总线。先核对输入、状态、输出、失败和复测证据，再用“循环通信、共享状态竞争、无法收敛”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“冲突收敛”聚焦“共享黑板、邮箱和事件总线”。围绕“共享黑板、邮箱和事件总线”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“冲突收敛”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“循环通信、共享状态竞争、无法收敛”，先停止并保留原始事件。本节的失败样例是“循环通信、共享状态竞争、无法收敛”，恢复或放行动作必须可重放。
{% endnote %}

### 终止边界

{% note info flat %}
“终止边界”聚焦“冲突、重复和收敛”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：集中式与去中心化协作轨迹应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“循环通信、共享状态竞争、无法收敛”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：用本地任务队列模拟集中式和去中心化协作，比较冲突与收敛证据。

机制：用本地任务队列模拟集中式和去中心化协作，比较冲突与收敛证据。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“循环通信、共享状态竞争、无法收敛”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“冲突、重复和收敛”。围绕“冲突、重复和收敛”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：集中式与去中心化协作轨迹应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“循环通信、共享状态竞争、无法收敛”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Agent 架构与 Harness","article":"Agent 架构与 Harness(十二)Agent Team与Swarm","order":12,"topic":"Agent Team与Swarm","experiment":"用本地任务队列模拟集中式和去中心化协作，比较冲突与收敛证据。","focus":["角色划分与协调器","共享黑板、邮箱和事件总线","冲突、重复和收敛","隔离、成本和终止"],"artifacts":["集中式与去中心化协作轨迹"],"failures":["循环通信、共享状态竞争、无法收敛"],"mode":"harness"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    queue = read_json(root / "team-queue.json")["events"]
    conflict = queue + ["merge"]
    try:
        if len(conflict) != len(set(conflict)):
            raise RuntimeError("team result contains a duplicate merge event")
    ...
    write_json(root / "team-queue.json", {"events": queue, "shared_result": "merged"})
    assert read_json(root / "team-queue.json")["events"] == ["claim", "work", "merge", "stop"]
    record_event(events_file, "recovery_verified", action="deduplicate-team-events")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"harness","online_evidence":"not_run","scenario":"team-convergence","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "harness", "online_evidence": "not_run", "scenario": "team-convergence", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“隔离、成本和终止”。先固定分母、基线、版本和观测窗口，再解释指标；平均分或一次成功不能替代难例、长尾、成本和失败归因。先把“隔离、成本和终止”变成一个可观察字段，再以“用本地任务队列模拟集中式和去中心化协作，比较冲突与收敛证据。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“循环通信、共享状态竞争、无法收敛”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“角色划分与协调器”。围绕“角色划分与协调器”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“循环通信、共享状态竞争、无法收敛”，先停止并保留原始事件。本节的失败样例是“循环通信、共享状态竞争、无法收敛”，恢复或放行动作必须可重放。
{% endnote %}

## 收敛与终止

{% note info flat %}
本节任务：理解隔离、成本和终止并建立可复核判断。核心内容：隔离、成本和终止。

机制：隔离、成本和终止。先核对输入、状态、输出、失败和复测证据，再用“循环通信、共享状态竞争、无法收敛”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“终止边界”聚焦“隔离、成本和终止”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。先把“隔离、成本和终止”变成一个可观察字段，再以“用本地任务队列模拟集中式和去中心化协作，比较冲突与收敛证据。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“循环通信、共享状态竞争、无法收敛”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：集中式与去中心化协作轨迹。预期：角色、通信、冲突、重复和收敛有独立指标。失败复验：循环通信、共享状态竞争、无法收敛。

机制：验收证据：集中式与去中心化协作轨迹。预期：角色、通信、冲突、重复和收敛有独立指标。失败复验：循环通信、共享状态竞争、无法收敛。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“循环通信、共享状态竞争、无法收敛”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“角色划分与协调器”。围绕“角色划分与协调器”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“循环通信、共享状态竞争、无法收敛”，先停止并保留原始事件。本节的失败样例是“循环通信、共享状态竞争、无法收敛”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 集中式与去中心化协作轨迹 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 循环通信、共享状态竞争、无法收敛 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“共享黑板、邮箱和事件总线”。围绕“共享黑板、邮箱和事件总线”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：集中式与去中心化协作轨迹应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“循环通信、共享状态竞争、无法收敛”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-agent-harness-12-1 deck:"Agent 架构与 Harness" priority:1 tags:"边界验证" %}
--- question
Team 和 Swarm 的核心差异
--- answer
Team 通常有显式角色和协调者，Swarm 更强调去中心化协作；差异在于协调、共享状态和收敛机制。
--- explanation
Team 偏显式角色和协调，Swarm 偏分散协作；两者都必须定义共享状态的写入规则、冲突仲裁和收敛条件。问题“Team 和 Swarm 的核心差异”的关键不在背诵名词，而在于：Team 通常有显式角色和协调者，Swarm 更强调去中心化协作；差异在于协调、共享状态和收敛机制。 先判断题目中的硬门槛和适用范围，再看“循环通信、共享状态竞争、无法收敛”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-agent-harness-12-2 deck:"Agent 架构与 Harness" priority:1 tags:"证据链" %}
--- question
多 Agent 为什么可能比单 Agent 更不稳定
--- answer
多 Agent 会增加通信、竞争、重复和冲突；没有收敛条件、预算和责任归属时，可能比单 Agent 更不稳定。
--- explanation
Team 偏显式角色和协调，Swarm 偏分散协作；两者都必须定义共享状态的写入规则、冲突仲裁和收敛条件。问题“多 Agent 为什么可能比单 Agent 更不稳定”的关键不在背诵名词，而在于：多 Agent 会增加通信、竞争、重复和冲突；没有收敛条件、预算和责任归属时，可能比单 Agent 更不稳定。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
