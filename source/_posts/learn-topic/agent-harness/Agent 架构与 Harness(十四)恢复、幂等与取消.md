---
title: Agent 架构与 Harness(十四)恢复、幂等与取消
tags:
  - Agent 架构与 Harness
  - 恢复、幂等与取消
categories:
  - Learn Topic
  - Agent 架构与 Harness
description: checkpoint、幂等键和取消传播均可复测
...
{% note primary flat %}
本节围绕“恢复、幂等与取消”建立可复核的工作模型。学习成果：checkpoint、幂等键和取消传播均可复测。实验：在本地 Harness 中注入崩溃、超时和重复事件，验证 checkpoint 和幂等。视觉辅助：恢复状态机；取消传播图。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
工具调用提交后进程崩溃，重启后的 Agent 不知道副作用是否发生。Checkpoint、幂等键和取消传播把恢复从猜测变成状态转移。

本篇把“恢复、幂等与取消”落到一次可回放的本地任务：在本地 Harness 中注入崩溃、超时和重复事件，验证 checkpoint 和幂等。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：checkpoint、幂等键和取消传播均可复测。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“错误分类和恢复策略；检查点、幂等键和去重；取消信号和子任务传播”拆成输入、状态、输出和证据。恢复不是重新调用一次模型，而是从检查点判断已完成副作用、选择幂等重试，并把取消沿执行树传播。
{% endnote %}

{% mermaid %}
flowchart TD
  A[Checkpoint] --> B[恢复上下文]
  B --> C{幂等键已处理?}
  C -->|是| D[跳过副作用]
  C -->|否| E[继续执行]
  E --> F[取消传播]
...
{% note primary flat %}
实验步骤：在本地 Harness 中注入崩溃、超时和重复事件，验证 checkpoint 和幂等。

验收产物：崩溃/超时/重复/取消注入报告

失败注入：重试重复副作用、取消只停 UI、恢复丢失上下文
{% endnote %}

## 错误分类

{% note info flat %}
本节任务：理解错误分类和恢复策略并建立可复核判断。核心内容：错误分类和恢复策略。

机制：错误分类和恢复策略。先核对输入、状态、输出、失败和复测证据，再用“重试重复副作用、取消只停 UI、恢复丢失上下文”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
Claude Code 的入口可用原生安装器或 npm 包；先用 claude --version 固定版本，再区分 CLI 能启动和 Anthropic 认证已通过。本节围绕“恢复策略”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“重试重复副作用、取消只停 UI、恢复丢失上下文”，对应产物为崩溃/超时/重复/取消注入报告。
{% endnote %}

### 检查点与去重

{% note info flat %}
Claude Code 的入口可用原生安装器或 npm 包；先用 claude --version 固定版本，再区分 CLI 能启动和 Anthropic 认证已通过。本节围绕“检查点与去重”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“重试重复副作用、取消只停 UI、恢复丢失上下文”，对应产物为崩溃/超时/重复/取消注入报告。
{% endnote %}

## 检查点与去重

{% note info flat %}
本节任务：理解检查点、幂等键和去重并建立可复核判断。核心内容：检查点、幂等键和去重。

机制：检查点、幂等键和去重。先核对输入、状态、输出、失败和复测证据，再用“重试重复副作用、取消只停 UI、恢复丢失上下文”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
模型与权限是两条链：/login 或 API Key 负责身份，权限设置决定文件、命令和网络动作是否需要确认；不要用一次回答证明两者都可用。本节围绕“取消传播”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“重试重复副作用、取消只停 UI、恢复丢失上下文”，对应产物为崩溃/超时/重复/取消注入报告。
{% endnote %}

### 恢复审计

{% note info flat %}
模型与权限是两条链：/login 或 API Key 负责身份，权限设置决定文件、命令和网络动作是否需要确认；不要用一次回答证明两者都可用。本节围绕“恢复审计”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“重试重复副作用、取消只停 UI、恢复丢失上下文”，对应产物为崩溃/超时/重复/取消注入报告。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：在本地 Harness 中注入崩溃、超时和重复事件，验证 checkpoint 和幂等。

机制：在本地 Harness 中注入崩溃、超时和重复事件，验证 checkpoint 和幂等。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“重试重复副作用、取消只停 UI、恢复丢失上下文”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
CLAUDE.md 会为项目任务提供上下文，但规则文本不等于工具授权；保留加载位置、作用域和与当前任务相关的最小片段。本节围绕“准备输入”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“重试重复副作用、取消只停 UI、恢复丢失上下文”，对应产物为崩溃/超时/重复/取消注入报告。
{% endnote %}
...
DATA = json.loads('{"series":"Agent 架构与 Harness","article":"Agent 架构与 Harness(十四)恢复、幂等与取消","order":14,"topic":"恢复、幂等与取消","experiment":"在本地 Harness 中注入崩溃、超时和重复事件，验证 checkpoint 和幂等。","focus":["错误分类和恢复策略","检查点、幂等键和去重","取消信号和子任务传播","恢复后的上下文与审计"],"artifacts":["崩溃/超时/重复/取消注入报告"],"failures":["重试重复副作用、取消只停 UI、恢复丢失上下文"],"mode":"harness"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    effects = read_json(root / "effects.json")
    retry_count = effects["event-1"] + 1
    try:
        if retry_count != 1:
            raise RuntimeError("retry would duplicate an external side effect")
    ...
    assert read_json(root / "effects.json")["event-1"] == 1
    record_event(events_file, "recovery_verified", action="honor-idempotency-key")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"harness","online_evidence":"not_run","scenario":"recovery-idempotency","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "harness", "online_evidence": "not_run", "scenario": "recovery-idempotency", "status": "pass"}
```

### 执行步骤

{% note info flat %}
CLAUDE.md 会为项目任务提供上下文，但规则文本不等于工具授权；保留加载位置、作用域和与当前任务相关的最小片段。本节围绕“执行步骤”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“重试重复副作用、取消只停 UI、恢复丢失上下文”，对应产物为崩溃/超时/重复/取消注入报告。
{% endnote %}

### 观察输出

{% note info flat %}
CLAUDE.md 会为项目任务提供上下文，但规则文本不等于工具授权；保留加载位置、作用域和与当前任务相关的最小片段。本节围绕“观察输出”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“重试重复副作用、取消只停 UI、恢复丢失上下文”，对应产物为崩溃/超时/重复/取消注入报告。
{% endnote %}

## 取消传播

{% note info flat %}
本节任务：理解恢复后的上下文与审计并建立可复核判断。核心内容：恢复后的上下文与审计。

机制：恢复后的上下文与审计。先核对输入、状态、输出、失败和复测证据，再用“重试重复副作用、取消只停 UI、恢复丢失上下文”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
Hooks 与 Subagents 会改变调用前后行为；审查 hook 命令、子代理获得的 Context 和退出状态，再比较主任务 Diff。本节围绕“恢复审计”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“重试重复副作用、取消只停 UI、恢复丢失上下文”，对应产物为崩溃/超时/重复/取消注入报告。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：崩溃/超时/重复/取消注入报告。预期：checkpoint、幂等键和取消传播均可复测。失败复验：重试重复副作用、取消只停 UI、恢复丢失上下文。

机制：验收证据：崩溃/超时/重复/取消注入报告。预期：checkpoint、幂等键和取消传播均可复测。失败复验：重试重复副作用、取消只停 UI、恢复丢失上下文。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“重试重复副作用、取消只停 UI、恢复丢失上下文”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
工作树与 CI/Agent SDK 是不同执行环境；本地一行 README 修改通过不代表 headless 或 CI 中的凭据、网络和测试条件一致。本节围绕“验收证据”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“重试重复副作用、取消只停 UI、恢复丢失上下文”，对应产物为崩溃/超时/重复/取消注入报告。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 崩溃/超时/重复/取消注入报告 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 重试重复副作用、取消只停 UI、恢复丢失上下文 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
工作树与 CI/Agent SDK 是不同执行环境；本地一行 README 修改通过不代表 headless 或 CI 中的凭据、网络和测试条件一致。本节围绕“复测动作”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“重试重复副作用、取消只停 UI、恢复丢失上下文”，对应产物为崩溃/超时/重复/取消注入报告。
{% endnote %}
...
{% flashcard basic id:ai-agent-harness-14-1 deck:"Agent 架构与 Harness" priority:1 tags:"边界验证" %}
--- question
重试前必须判断什么
--- answer
重试前要判断错误是否可重试、动作是否幂等、预算是否足够，以及副作用是否已经发生。
--- explanation
恢复不是重新调用一次模型，而是从检查点判断已完成副作用、选择幂等重试，并把取消沿执行树传播。问题“重试前必须判断什么”的关键不在背诵名词，而在于：重试前要判断错误是否可重试、动作是否幂等、预算是否足够，以及副作用是否已经发生。 先判断题目中的硬门槛和适用范围，再看“重试重复副作用、取消只停 UI、恢复丢失上下文”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-agent-harness-14-2 deck:"Agent 架构与 Harness" priority:1 tags:"证据链" %}
--- question
取消为什么需要沿工具链传播
--- answer
取消必须从用户请求传播到 Loop、模型、工具、子任务和网络等待；每层都要停止或报告不可取消的部分。
--- explanation
恢复不是重新调用一次模型，而是从检查点判断已完成副作用、选择幂等重试，并把取消沿执行树传播。问题“取消为什么需要沿工具链传播”的关键不在背诵名词，而在于：取消必须从用户请求传播到 Loop、模型、工具、子任务和网络等待；每层都要停止或报告不可取消的部分。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
