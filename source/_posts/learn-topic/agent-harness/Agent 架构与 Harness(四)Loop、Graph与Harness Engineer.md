---
title: Agent 架构与 Harness(四)Loop、Graph与Harness Engineer
tags:
  - Agent 架构与 Harness
  - Loop、Graph与Harness Engineer
categories:
  - Learn Topic
  - Agent 架构与 Harness
description: 能解释状态转移、Harness 分层和恢复差异
...
{% note primary flat %}
本节围绕“Loop、Graph与Harness Engineer”建立可复核的工作模型。学习成果：能解释状态转移、Harness 分层和恢复差异。实验：将同一个审批任务分别实现为 Loop 和 Graph，比较恢复点、可观测性和修改成本。视觉辅助：三种架构对比 Mermaid；职责矩阵。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
审批任务在循环实现里卡住后只能从头再来。带节点状态的 Graph 暴露恢复点，但节点没有幂等边界时也会重复副作用。

本篇把“Loop、Graph与Harness Engineer”落到一次可回放的本地任务：将同一个审批任务分别实现为 Loop 和 Graph，比较恢复点、可观测性和修改成本。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：能解释状态转移、Harness 分层和恢复差异。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“循环编排与图状态转移；Harness 的模型、工具、上下文、权限和观测层；Loop Engineer、Graph Engineer 与 Harness Engineer 的职责边界”拆成输入、状态、输出和证据。Loop 适合迭代策略，Graph 适合把节点、转移和恢复点写成状态；Harness 把模型决策与外部执行的边界固定下来。
{% endnote %}
...
{% note primary flat %}
实验步骤：将同一个审批任务分别实现为 Loop 和 Graph，比较恢复点、可观测性和修改成本。

验收产物：Loop/Graph 对照实现和恢复点表

失败注入：图节点无幂等、循环与状态机边界混乱
{% endnote %}

## 编排模型

{% note info flat %}
本节任务：理解循环编排与图状态转移并建立可复核判断。核心内容：循环编排与图状态转移。

机制：循环编排与图状态转移。先核对输入、状态、输出、失败和复测证据，再用“图节点无幂等、循环与状态机边界混乱”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“循环与图”聚焦“循环编排与图状态转移”。围绕“循环编排与图状态转移”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“循环编排与图状态转移”变成一个可观察字段，再以“将同一个审批任务分别实现为 Loop 和 Graph，比较恢复点、可观测性和修改成本。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“图节点无幂等、循环与状态机边界混乱”，恢复或放行动作必须可重放。
{% endnote %}

### Harness分层

{% note info flat %}
“Harness分层”聚焦“Harness 的模型、工具、上下文、权限和观测层”。把模型/Provider 选择、认证状态和任务输入分成三列；凭据缺失时应停止在本地静态演示，不输出看似真实的服务响应。具体取舍是：保留“Harness分层”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“图节点无幂等、循环与状态机边界混乱”，先停止并保留原始事件。本节的失败样例是“图节点无幂等、循环与状态机边界混乱”，恢复或放行动作必须可重放。
{% endnote %}

## Harness分层

{% note info flat %}
本节任务：理解Harness 的模型、工具、上下文、权限和观测层并建立可复核判断。核心内容：Harness 的模型、工具、上下文、权限和观测层。

机制：Harness 的模型、工具、上下文、权限和观测层。先核对输入、状态、输出、失败和复测证据，再用“图节点无幂等、循环与状态机边界混乱”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“工程职责”聚焦“Harness 的模型、工具、上下文、权限和观测层”。把模型/Provider 选择、认证状态和任务输入分成三列；凭据缺失时应停止在本地静态演示，不输出看似真实的服务响应。具体取舍是：保留“工程职责”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“图节点无幂等、循环与状态机边界混乱”，先停止并保留原始事件。本节的失败样例是“图节点无幂等、循环与状态机边界混乱”，恢复或放行动作必须可重放。
{% endnote %}

### 图与循环

{% note info flat %}
“图与循环”聚焦“Loop Engineer、Graph Engineer 与 Harness Engineer 的职责边界”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：Loop/Graph 对照实现和恢复点表应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“图节点无幂等、循环与状态机边界混乱”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：将同一个审批任务分别实现为 Loop 和 Graph，比较恢复点、可观测性和修改成本。

机制：将同一个审批任务分别实现为 Loop 和 Graph，比较恢复点、可观测性和修改成本。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“图节点无幂等、循环与状态机边界混乱”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“Loop Engineer、Graph Engineer 与 Harness Engineer 的职责边界”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：Loop/Graph 对照实现和恢复点表应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“图节点无幂等、循环与状态机边界混乱”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Agent 架构与 Harness","article":"Agent 架构与 Harness(四)Loop、Graph与Harness Engineer","order":4,"topic":"Loop、Graph与Harness Engineer","experiment":"将同一个审批任务分别实现为 Loop 和 Graph，比较恢复点、可观测性和修改成本。","focus":["循环编排与图状态转移","Harness 的模型、工具、上下文、权限和观测层","Loop Engineer、Graph Engineer 与 Harness Engineer 的职责边界","何时采用图而不是循环"],"artifacts":["Loop/Graph 对照实现和恢复点表"],"failures":["图节点无幂等、循环与状态机边界混乱"],"mode":"harness"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    graph = {"start": "approve", "approve": "finish"}
    checkpoint = {"state": "approve", "trace": ["start", "approve"]}
    try:
        bad_state = "approve"
        if bad_state not in graph or checkpoint["trace"].count(bad_state) > 1:
            raise ValueError("graph node is not restart-safe")
        checkpoint["trace"].append(bad_state)
        if checkpoint["trace"].count(bad_state) > 1:
            raise RuntimeError("non-idempotent recovery point")
    except (ValueError, RuntimeError) as error:
        failure_event = {"status": "blocked", "type": str(error)}
        record_event(events_file, "failure_blocked", type=failure_event["type"])
    checkpoint["trace"] = ["start", "approve"]
    checkpoint["state"] = "approve"
    assert checkpoint["trace"][-1] == checkpoint["state"] and checkpoint["trace"].count("approve") == 1
    record_event(events_file, "recovery_verified", action="restore-idempotent-checkpoint")
...
assert result == {"artifact_count":1,"checks":3,"failure_count":1,"mode":"harness","online_evidence":"not_run","scenario":"loop-graph","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 3, "failure_count": 1, "mode": "harness", "online_evidence": "not_run", "scenario": "loop-graph", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“何时采用图而不是循环”。围绕“何时采用图而不是循环”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“何时采用图而不是循环”变成一个可观察字段，再以“将同一个审批任务分别实现为 Loop 和 Graph，比较恢复点、可观测性和修改成本。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“图节点无幂等、循环与状态机边界混乱”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“循环编排与图状态转移”。围绕“循环编排与图状态转移”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“图节点无幂等、循环与状态机边界混乱”，先停止并保留原始事件。本节的失败样例是“图节点无幂等、循环与状态机边界混乱”，恢复或放行动作必须可重放。
{% endnote %}

## 职责边界

{% note info flat %}
本节任务：理解何时采用图而不是循环并建立可复核判断。核心内容：何时采用图而不是循环。

机制：何时采用图而不是循环。先核对输入、状态、输出、失败和复测证据，再用“图节点无幂等、循环与状态机边界混乱”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“图与循环”聚焦“何时采用图而不是循环”。围绕“何时采用图而不是循环”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“何时采用图而不是循环”变成一个可观察字段，再以“将同一个审批任务分别实现为 Loop 和 Graph，比较恢复点、可观测性和修改成本。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“图节点无幂等、循环与状态机边界混乱”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：Loop/Graph 对照实现和恢复点表。预期：能解释状态转移、Harness 分层和恢复差异。失败复验：图节点无幂等、循环与状态机边界混乱。

机制：验收证据：Loop/Graph 对照实现和恢复点表。预期：能解释状态转移、Harness 分层和恢复差异。失败复验：图节点无幂等、循环与状态机边界混乱。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“图节点无幂等、循环与状态机边界混乱”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“循环编排与图状态转移”。围绕“循环编排与图状态转移”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“图节点无幂等、循环与状态机边界混乱”，先停止并保留原始事件。本节的失败样例是“图节点无幂等、循环与状态机边界混乱”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | Loop/Graph 对照实现和恢复点表 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 图节点无幂等、循环与状态机边界混乱 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“Harness 的模型、工具、上下文、权限和观测层”。把模型/Provider 选择、认证状态和任务输入分成三列；凭据缺失时应停止在本地静态演示，不输出看似真实的服务响应。复核时把动作和产物对应起来：Loop/Graph 对照实现和恢复点表应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“图节点无幂等、循环与状态机边界混乱”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-agent-harness-4-1 deck:"Agent 架构与 Harness" priority:1 tags:"边界验证" %}
--- question
Harness 和 Agent 本体有什么不同
--- answer
Agent 是决策与行动主体，Harness 负责上下文、工具、权限、状态、预算和证据等运行边界。
--- explanation
Loop 适合迭代策略，Graph 适合把节点、转移和恢复点写成状态；Harness 把模型决策与外部执行的边界固定下来。问题“Harness 和 Agent 本体有什么不同”的关键不在背诵名词，而在于：Agent 是决策与行动主体，Harness 负责上下文、工具、权限、状态、预算和证据等运行边界。 先判断题目中的硬门槛和适用范围，再看“图节点无幂等、循环与状态机边界混乱”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-agent-harness-4-2 deck:"Agent 架构与 Harness" priority:1 tags:"证据链" %}
--- question
图编排为什么有利于恢复
--- answer
Graph 把节点状态、转移条件和恢复点显式化；失败后可从最近的幂等节点恢复，而不是重跑全部步骤。
--- explanation
Loop 适合迭代策略，Graph 适合把节点、转移和恢复点写成状态；Harness 把模型决策与外部执行的边界固定下来。问题“图编排为什么有利于恢复”的关键不在背诵名词，而在于：Graph 把节点状态、转移条件和恢复点显式化；失败后可从最近的幂等节点恢复，而不是重跑全部步骤。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
...
{% linkgroup %}
{% link ReAct: Synergizing Reasoning and Acting in Language Models, https://arxiv.org/abs/2210.03629, https://arxiv.org/favicon.ico %}
{% endlinkgroup %}
