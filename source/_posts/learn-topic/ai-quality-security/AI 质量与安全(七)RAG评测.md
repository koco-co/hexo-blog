...
categories:
  - Learn Topic
  - AI 质量与安全
description: 分别证明召回、上下文利用、引用正确性、答案支持和噪声敏感性
...
{% note primary flat %}
本节围绕“RAG评测”建立可复核的工作模型。学习成果：分别证明召回、上下文利用、引用正确性、答案支持和噪声敏感性。实验：使用本地版本化 Markdown 文档构造命中、漏召回、冲突和过期样本，分别计算召回、引用支持和答案通过率。视觉辅助：RAG 评测流水线 Mermaid；召回—引用—生成矩阵。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
RAG 回答引用文档，却没有召回正确版本；有出处不等于证据支持。拆开召回、上下文、引用和生成才能定位失败。

本篇把“RAG评测”落到一次可回放的本地任务：使用本地版本化 Markdown 文档构造命中、漏召回、冲突和过期样本，分别计算召回、引用支持和答案通过率。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：分别证明召回、上下文利用、引用正确性、答案支持和噪声敏感性。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“检索召回、排序和难例；上下文利用、引用定位和答案支持；无结果、冲突、过期和噪声文档”拆成输入、状态、输出和证据。RAG 质量要分开召回、排序、上下文利用、引用定位和答案支持；组件分数不能替代端到端证据。
{% endnote %}
...
{% note primary flat %}
实验步骤：使用本地版本化 Markdown 文档构造命中、漏召回、冲突和过期样本，分别计算召回、引用支持和答案通过率。

验收产物：命中/漏召回/冲突/过期 fixture、检索指标、引用支持和端到端报告

失败注入：漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案
{% endnote %}

## 召回质量

{% note info flat %}
本节任务：理解检索召回、排序和难例并建立可复核判断。核心内容：检索召回、排序和难例。

机制：检索召回、排序和难例。先核对输入、状态、输出、失败和复测证据，再用“漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
先用 query_id 对命中、漏召回、排序位置和文档版本逐条编号，再计算 Recall@k 与 MRR；命中旧版本不能算支持当前问题。把噪声文档另列，避免把生成阶段错误误判成检索成功。验证材料：命中/漏召回/冲突/过期 fixture、检索指标、引用支持和端到端报告。失败边界：漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案；若该边界被触发，停止并保留可重放记录。
{% endnote %}

### 引用支持

{% note info flat %}
答案中的每个声明都要反查到 context 中的文档片段和版本；引用存在但片段不支持声明时，归为 attribution failure，而不是给引用列满分。这个映射也为后续端到端评分提供证据。验证材料：命中/漏召回/冲突/过期 fixture、检索指标、引用支持和端到端报告。失败边界：漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案；若该边界被触发，停止并保留可重放记录。
{% endnote %}

## 答案支持

{% note info flat %}
本节任务：理解上下文利用、引用定位和答案支持并建立可复核判断。核心内容：上下文利用、引用定位和答案支持。

机制：上下文利用、引用定位和答案支持。先核对输入、状态、输出、失败和复测证据，再用“漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“证据异常”聚焦“上下文利用、引用定位和答案支持”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。具体取舍是：保留“证据异常”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案”，先停止并保留原始事件。本节的失败样例是“漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案”，恢复或放行动作必须可重放。
{% endnote %}

### 端到端归因

{% note info flat %}
“端到端归因”聚焦“无结果、冲突、过期和噪声文档”。围绕“无结果、冲突、过期和噪声文档”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：命中/漏召回/冲突/过期 fixture、检索指标、引用支持和端到端报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：使用本地版本化 Markdown 文档构造命中、漏召回、冲突和过期样本，分别计算召回、引用支持和答案通过率。

机制：使用本地版本化 Markdown 文档构造命中、漏召回、冲突和过期样本，分别计算召回、引用支持和答案通过率。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“无结果、冲突、过期和噪声文档”。围绕“无结果、冲突、过期和噪声文档”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：命中/漏召回/冲突/过期 fixture、检索指标、引用支持和端到端报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"AI 质量与安全","article":"AI 质量与安全(七)RAG评测","order":7,"topic":"RAG评测","experiment":"使用本地版本化 Markdown 文档构造命中、漏召回、冲突和过期样本，分别计算召回、引用支持和答案通过率。","focus":["检索召回、排序和难例","上下文利用、引用定位和答案支持","无结果、冲突、过期和噪声文档","组件指标不能替代端到端质量"],"artifacts":["命中/漏召回/冲突/过期 fixture、检索指标、引用支持和端到端报告"],"failures":["漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案"],"mode":"quality"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    report = read_json(root / "rag-report.json")
    report["citation"] = "d3"
    try:
        if report["citation"] not in report["retrieved"]:
            raise ValueError("citation does not support the answer")
    ...
    report["citation"] = "d1"
    write_json(root / "rag-report.json", report)
    assert read_json(root / "rag-report.json")["citation"] == "d1"
    record_event(events_file, "recovery_verified", action="restore-supported-citation")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"quality","online_evidence":"not_run","scenario":"rag-attribution","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "quality", "online_evidence": "not_run", "scenario": "rag-attribution", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“组件指标不能替代端到端质量”。先固定分母、基线、版本和观测窗口，再解释指标；平均分或一次成功不能替代难例、长尾、成本和失败归因。先把“组件指标不能替代端到端质量”变成一个可观察字段，再以“使用本地版本化 Markdown 文档构造命中、漏召回、冲突和过期样本，分别计算召回、引用支持和答案通过率。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“检索召回、排序和难例”。围绕“检索召回、排序和难例”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案”，先停止并保留原始事件。本节的失败样例是“漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案”，恢复或放行动作必须可重放。
{% endnote %}

## 生成归因

{% note warning flat %}
本节任务：判断当前方案何时不能照搬。核心内容：漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案。

机制：漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案。选择依据：当前文章的核心选择或范围需要直接可见，不强行套 warning。先核对限制、风险、恢复或替代动作，再用“漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案”作为反例；去掉颜色后仍保留完整文字。
{% endnote %}
...
{% note info flat %}
“端到端归因”聚焦“组件指标不能替代端到端质量”。先固定分母、基线、版本和观测窗口，再解释指标；平均分或一次成功不能替代难例、长尾、成本和失败归因。先把“组件指标不能替代端到端质量”变成一个可观察字段，再以“使用本地版本化 Markdown 文档构造命中、漏召回、冲突和过期样本，分别计算召回、引用支持和答案通过率。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：命中/漏召回/冲突/过期 fixture、检索指标、引用支持和端到端报告。预期：分别证明召回、上下文利用、引用正确性、答案支持和噪声敏感性。失败复验：漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案。

机制：验收证据：命中/漏召回/冲突/过期 fixture、检索指标、引用支持和端到端报告。预期：分别证明召回、上下文利用、引用正确性、答案支持和噪声敏感性。失败复验：漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“检索召回、排序和难例”。围绕“检索召回、排序和难例”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案”，先停止并保留原始事件。本节的失败样例是“漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 命中/漏召回/冲突/过期 fixture、检索指标、引用支持和端到端报告 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“上下文利用、引用定位和答案支持”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。复核时把动作和产物对应起来：命中/漏召回/冲突/过期 fixture、检索指标、引用支持和端到端报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-ai-quality-security-7-1 deck:"AI 质量与安全" priority:1 tags:"边界验证" %}
--- question
RAG 评测为什么要把检索和生成分开
--- answer
检索决定证据是否进入 Context，生成决定如何使用证据；分开测量才能区分召回错误、引用错误和生成错误。
--- explanation
RAG 质量要分开召回、排序、上下文利用、引用定位和答案支持；组件分数不能替代端到端证据。问题“RAG 评测为什么要把检索和生成分开”的关键不在背诵名词，而在于：检索决定证据是否进入 Context，生成决定如何使用证据；分开测量才能区分召回错误、引用错误和生成错误。 先判断题目中的硬门槛和适用范围，再看“漏召回、冲突证据、过期引用、引用不支持声明或只看最终答案”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-ai-quality-security-7-2 deck:"AI 质量与安全" priority:1 tags:"证据链" %}
--- question
引用正确但答案错误如何归因
--- answer
若引用正确但答案错误，检索与引用定位可判通过，生成/归因判失败；不要把错误归给检索整体。
--- explanation
RAG 质量要分开召回、排序、上下文利用、引用定位和答案支持；组件分数不能替代端到端证据。问题“引用正确但答案错误如何归因”的关键不在背诵名词，而在于：若引用正确但答案错误，检索与引用定位可判通过，生成/归因判失败；不要把错误归给检索整体。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
...
{% linkgroup %}
{% link RAGAS, https://arxiv.org/abs/2309.15217, https://arxiv.org/favicon.ico %}
{% link RAGChecker, https://arxiv.org/abs/2408.08067, https://arxiv.org/favicon.ico %}
{% link RAGE citation evaluation, https://aclanthology.org/2024.konvens-main.6/, https://aclanthology.org/aclicon.ico %}
{% endlinkgroup %}

### 官方资料

{% linkgroup %}
{% link OpenAI Embeddings Guide, https://platform.openai.com/docs/guides/embeddings, https://cdn.openai.com/API/logo-openai.svg %}
{% endlinkgroup %}
