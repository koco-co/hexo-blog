---
title: AI 质量与安全(三)Benchmark与数据集
tags:
  - AI 质量与安全
  - Benchmark与数据集
categories:
...
{% note primary flat %}
本节围绕“Benchmark与数据集”建立可复核的工作模型。学习成果：污染、泄漏和代表性有明确检查。实验：创建一个脱敏小数据集，记录来源、版本、标签、边界样本和变更差异。视觉辅助：数据集生命周期；版本字段表。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
评测分数突然上升，追查发现新数据与训练样本重叠，边界样本也被删掉。来源、授权、分层和版本必须先于指标。

本篇把“Benchmark与数据集”落到一次可回放的本地任务：创建一个脱敏小数据集，记录来源、版本、标签、边界样本和变更差异。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：污染、泄漏和代表性有明确检查。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“任务样本、标签、版本和来源；分层抽样、难例和边界集；污染、泄漏、个人数据和授权”拆成输入、状态、输出和证据。Benchmark 不是一组随手样本；来源、授权、标签、分层、难例、去污染和版本快照决定分数能否复现。
{% endnote %}
...
{% note primary flat %}
实验步骤：创建一个脱敏小数据集，记录来源、版本、标签、边界样本和变更差异。

验收产物：脱敏数据集版本、来源、标签和变更差异

失败注入：数据集不可复现、个人数据进入评测
{% endnote %}

## 数据版本

{% note info flat %}
本节任务：理解任务样本、标签、版本和来源并建立可复核判断。核心内容：任务样本、标签、版本和来源。

机制：任务样本、标签、版本和来源。先核对输入、状态、输出、失败和复测证据，再用“数据集不可复现、个人数据进入评测”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“样本版本”聚焦“任务样本、标签、版本和来源”。先记录入口、版本、平台和前置条件，再执行最小动作；安装成功只能证明程序可启动，不能推出权限、模型或扩展已经可用。先把“任务样本、标签、版本和来源”变成一个可观察字段，再以“创建一个脱敏小数据集，记录来源、版本、标签、边界样本和变更差异。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“数据集不可复现、个人数据进入评测”，恢复或放行动作必须可重放。
{% endnote %}

### 难例抽样

{% note info flat %}
“难例抽样”聚焦“分层抽样、难例和边界集”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“难例抽样”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“数据集不可复现、个人数据进入评测”，先停止并保留原始事件。本节的失败样例是“数据集不可复现、个人数据进入评测”，恢复或放行动作必须可重放。
{% endnote %}

## 代表性

{% note info flat %}
本节任务：理解分层抽样、难例和边界集并建立可复核判断。核心内容：分层抽样、难例和边界集。

机制：分层抽样、难例和边界集。先核对输入、状态、输出、失败和复测证据，再用“数据集不可复现、个人数据进入评测”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“数据风险”聚焦“分层抽样、难例和边界集”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“数据风险”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“数据集不可复现、个人数据进入评测”，先停止并保留原始事件。本节的失败样例是“数据集不可复现、个人数据进入评测”，恢复或放行动作必须可重放。
{% endnote %}

### 数据回归

{% note info flat %}
“数据回归”聚焦“污染、泄漏、个人数据和授权”。把数据内容与控制指令分离，并沿存储、Context、日志、缓存和外发路径检查；测试通过要同时说明覆盖范围和未覆盖面。复核时把动作和产物对应起来：脱敏数据集版本、来源、标签和变更差异应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“数据集不可复现、个人数据进入评测”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：创建一个脱敏小数据集，记录来源、版本、标签、边界样本和变更差异。

机制：创建一个脱敏小数据集，记录来源、版本、标签、边界样本和变更差异。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“数据集不可复现、个人数据进入评测”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“污染、泄漏、个人数据和授权”。把数据内容与控制指令分离，并沿存储、Context、日志、缓存和外发路径检查；测试通过要同时说明覆盖范围和未覆盖面。复核时把动作和产物对应起来：脱敏数据集版本、来源、标签和变更差异应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“数据集不可复现、个人数据进入评测”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"AI 质量与安全","article":"AI 质量与安全(三)Benchmark与数据集","order":3,"topic":"Benchmark与数据集","experiment":"创建一个脱敏小数据集，记录来源、版本、标签、边界样本和变更差异。","focus":["任务样本、标签、版本和来源","分层抽样、难例和边界集","污染、泄漏、个人数据和授权","数据集变更与回归"],"artifacts":["脱敏数据集版本、来源、标签和变更差异"],"failures":["数据集不可复现、个人数据进入评测"],"mode":"quality"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    dataset = read_json(root / "dataset-v1.json")
    duplicate = list(dataset["samples"]) + [dict(dataset["samples"][0])]
    try:
        if len({sample["id"] for sample in duplicate}) != len(duplicate):
            raise ValueError("dataset snapshot contains a duplicate sample")
    ...
    dataset["samples"] = [dict(sample) for sample in dataset["samples"]]
    write_json(root / "dataset-v1.json", dataset)
    assert len(read_json(root / "dataset-v1.json")["samples"]) == 3
    record_event(events_file, "recovery_verified", action="restore-versioned-dataset")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"quality","online_evidence":"not_run","scenario":"dataset-version","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "quality", "online_evidence": "not_run", "scenario": "dataset-version", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“数据集变更与回归”。把数据内容与控制指令分离，并沿存储、Context、日志、缓存和外发路径检查；测试通过要同时说明覆盖范围和未覆盖面。先把“数据集变更与回归”变成一个可观察字段，再以“创建一个脱敏小数据集，记录来源、版本、标签、边界样本和变更差异。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“数据集不可复现、个人数据进入评测”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“任务样本、标签、版本和来源”。先记录入口、版本、平台和前置条件，再执行最小动作；安装成功只能证明程序可启动，不能推出权限、模型或扩展已经可用。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“数据集不可复现、个人数据进入评测”，先停止并保留原始事件。本节的失败样例是“数据集不可复现、个人数据进入评测”，恢复或放行动作必须可重放。
{% endnote %}

## 污染防护

{% note info flat %}
本节任务：理解数据集变更与回归并建立可复核判断。核心内容：数据集变更与回归。

机制：数据集变更与回归。先核对输入、状态、输出、失败和复测证据，再用“数据集不可复现、个人数据进入评测”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“数据回归”聚焦“数据集变更与回归”。把数据内容与控制指令分离，并沿存储、Context、日志、缓存和外发路径检查；测试通过要同时说明覆盖范围和未覆盖面。先把“数据集变更与回归”变成一个可观察字段，再以“创建一个脱敏小数据集，记录来源、版本、标签、边界样本和变更差异。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“数据集不可复现、个人数据进入评测”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：脱敏数据集版本、来源、标签和变更差异。预期：污染、泄漏和代表性有明确检查。失败复验：数据集不可复现、个人数据进入评测。

机制：验收证据：脱敏数据集版本、来源、标签和变更差异。预期：污染、泄漏和代表性有明确检查。失败复验：数据集不可复现、个人数据进入评测。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“数据集不可复现、个人数据进入评测”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“任务样本、标签、版本和来源”。先记录入口、版本、平台和前置条件，再执行最小动作；安装成功只能证明程序可启动，不能推出权限、模型或扩展已经可用。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“数据集不可复现、个人数据进入评测”，先停止并保留原始事件。本节的失败样例是“数据集不可复现、个人数据进入评测”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 脱敏数据集版本、来源、标签和变更差异 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 数据集不可复现、个人数据进入评测 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“分层抽样、难例和边界集”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：脱敏数据集版本、来源、标签和变更差异应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“数据集不可复现、个人数据进入评测”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-ai-quality-security-3-1 deck:"AI 质量与安全" priority:1 tags:"边界验证" %}
--- question
评测数据为什么需要版本控制
--- answer
数据版本控制要固定来源、授权、脱敏、标签、分层、变更和快照，使回归能复现。
--- explanation
Benchmark 不是一组随手样本；来源、授权、标签、分层、难例、去污染和版本快照决定分数能否复现。问题“评测数据为什么需要版本控制”的关键不在背诵名词，而在于：数据版本控制要固定来源、授权、脱敏、标签、分层、变更和快照，使回归能复现。 先判断题目中的硬门槛和适用范围，再看“数据集不可复现、个人数据进入评测”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-ai-quality-security-3-2 deck:"AI 质量与安全" priority:1 tags:"证据链" %}
--- question
数据泄漏如何影响结论
--- answer
训练/调参数据与评测数据重叠会让分数虚高，无法代表泛化；应去重、隔离并报告泄漏检查。
--- explanation
Benchmark 不是一组随手样本；来源、授权、标签、分层、难例、去污染和版本快照决定分数能否复现。问题“数据泄漏如何影响结论”的关键不在背诵名词，而在于：训练/调参数据与评测数据重叠会让分数虚高，无法代表泛化；应去重、隔离并报告泄漏检查。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
