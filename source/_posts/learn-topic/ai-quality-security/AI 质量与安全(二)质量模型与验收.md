...
categories:
  - Learn Topic
  - AI 质量与安全
description: 结果质量、过程质量、安全和人工责任分开验收
...
{% note primary flat %}
本节围绕“质量模型与验收”建立可复核的工作模型。学习成果：结果质量、过程质量、安全和人工责任分开验收。实验：为问答、RAG 和代码修改各写一份质量契约，区分硬门槛和评分项。视觉辅助：质量模型分层图；验收矩阵。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
团队把“回答正确”写成总分，发布后才发现安全拒绝和过程失败被平均掉。质量契约应拆开硬门槛、评分项、责任和停止条件。

本篇把“质量模型与验收”落到一次可回放的本地任务：为问答、RAG 和代码修改各写一份质量契约，区分硬门槛和评分项。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：结果质量、过程质量、安全和人工责任分开验收。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“正确性、相关性、完整性、可解释性和安全性；任务成功与过程质量；验收阈值、人工复核和停止条件”拆成输入、状态、输出和证据。质量模型把结果正确性、过程质量、安全硬门槛和人工责任拆开，冲突时先执行不可绕过的门禁。
{% endnote %}
...
{% note primary flat %}
实验步骤：为问答、RAG 和代码修改各写一份质量契约，区分硬门槛和评分项。

验收产物：问答/RAG/代码三份质量契约
...
{% note info flat %}
本节任务：理解正确性、相关性、完整性、可解释性和安全性并建立可复核判断。核心内容：正确性、相关性、完整性、可解释性和安全性。

机制：正确性、相关性、完整性、可解释性和安全性。先核对输入、状态、输出、失败和复测证据，再用“只看最终答案、指标互相冲突”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
把正确性、相关性、完整性、可解释性和安全性作为不同列：答案可能正确但引用不全，也可能完整却越过权限门槛。先给每列定义可观察标签，再规定安全硬门槛不能被其他分数抵消。验证材料：问答/RAG/代码三份质量契约。失败边界：只看最终答案、指标互相冲突；若该边界被触发，停止并保留可重放记录。
{% endnote %}

### 任务质量

{% note info flat %}
对同一任务保存输入、期望字段、模型输出和人工判定；把“答案对了但过程越权”与“过程合规但字段缺失”分成两个样例。验收因此能解释扣分来自结果还是过程。验证材料：问答/RAG/代码三份质量契约。失败边界：只看最终答案、指标互相冲突；若该边界被触发，停止并保留可重放记录。
{% endnote %}

## 验收门槛

{% note info flat %}
本节任务：理解任务成功与过程质量并建立可复核判断。核心内容：任务成功与过程质量。

机制：任务成功与过程质量。先核对输入、状态、输出、失败和复测证据，再用“只看最终答案、指标互相冲突”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“验收阈值”聚焦“任务成功与过程质量”。围绕“任务成功与过程质量”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“验收阈值”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“只看最终答案、指标互相冲突”，先停止并保留原始事件。本节的失败样例是“只看最终答案、指标互相冲突”，恢复或放行动作必须可重放。
{% endnote %}

### 产品责任

{% note info flat %}
“产品责任”聚焦“验收阈值、人工复核和停止条件”。围绕“验收阈值、人工复核和停止条件”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：问答/RAG/代码三份质量契约应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“只看最终答案、指标互相冲突”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：为问答、RAG 和代码修改各写一份质量契约，区分硬门槛和评分项。

机制：为问答、RAG 和代码修改各写一份质量契约，区分硬门槛和评分项。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“只看最终答案、指标互相冲突”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“验收阈值、人工复核和停止条件”。围绕“验收阈值、人工复核和停止条件”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：问答/RAG/代码三份质量契约应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“只看最终答案、指标互相冲突”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"AI 质量与安全","article":"AI 质量与安全(二)质量模型与验收","order":2,"topic":"质量模型与验收","experiment":"为问答、RAG 和代码修改各写一份质量契约，区分硬门槛和评分项。","focus":["正确性、相关性、完整性、可解释性和安全性","任务成功与过程质量","验收阈值、人工复核和停止条件","质量模型与产品责任"],"artifacts":["问答/RAG/代码三份质量契约"],"failures":["只看最终答案、指标互相冲突"],"mode":"quality"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    contract = read_json(root / "quality-contract.json")
    contract["checks"][1]["pass"] = False
    try:
        if not all(check["pass"] for check in contract["checks"] if check["hard"]):
            raise PermissionError("hard safety gate failed despite answer score")
    ...
    contract["checks"][1]["pass"] = True
    write_json(root / "quality-contract.json", contract)
    assert all(check["pass"] for check in read_json(root / "quality-contract.json")["checks"])
    record_event(events_file, "recovery_verified", action="restore-hard-safety-gate")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"quality","online_evidence":"not_run","scenario":"quality-contract","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "quality", "online_evidence": "not_run", "scenario": "quality-contract", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“质量模型与产品责任”。把模型/Provider 选择、认证状态和任务输入分成三列；凭据缺失时应停止在本地静态演示，不输出看似真实的服务响应。先把“质量模型与产品责任”变成一个可观察字段，再以“为问答、RAG 和代码修改各写一份质量契约，区分硬门槛和评分项。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“只看最终答案、指标互相冲突”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“正确性、相关性、完整性、可解释性和安全性”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“只看最终答案、指标互相冲突”，先停止并保留原始事件。本节的失败样例是“只看最终答案、指标互相冲突”，恢复或放行动作必须可重放。
{% endnote %}

## 责任边界

{% note info flat %}
本节任务：理解质量模型与产品责任并建立可复核判断。核心内容：质量模型与产品责任。

机制：质量模型与产品责任。先核对输入、状态、输出、失败和复测证据，再用“只看最终答案、指标互相冲突”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“产品责任”聚焦“质量模型与产品责任”。把模型/Provider 选择、认证状态和任务输入分成三列；凭据缺失时应停止在本地静态演示，不输出看似真实的服务响应。先把“质量模型与产品责任”变成一个可观察字段，再以“为问答、RAG 和代码修改各写一份质量契约，区分硬门槛和评分项。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“只看最终答案、指标互相冲突”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：问答/RAG/代码三份质量契约。预期：结果质量、过程质量、安全和人工责任分开验收。失败复验：只看最终答案、指标互相冲突。

机制：验收证据：问答/RAG/代码三份质量契约。预期：结果质量、过程质量、安全和人工责任分开验收。失败复验：只看最终答案、指标互相冲突。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“只看最终答案、指标互相冲突”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“正确性、相关性、完整性、可解释性和安全性”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“只看最终答案、指标互相冲突”，先停止并保留原始事件。本节的失败样例是“只看最终答案、指标互相冲突”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 问答/RAG/代码三份质量契约 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 只看最终答案、指标互相冲突 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“任务成功与过程质量”。围绕“任务成功与过程质量”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：问答/RAG/代码三份质量契约应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“只看最终答案、指标互相冲突”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-ai-quality-security-2-1 deck:"AI 质量与安全" priority:1 tags:"边界验证" %}
--- question
质量指标如何避免互相冲突
--- answer
把质量拆成硬门槛与评分项，并规定优先级、冲突处理和停止条件；不能用平均分掩盖安全或过程失败。
--- explanation
质量模型把结果正确性、过程质量、安全硬门槛和人工责任拆开，冲突时先执行不可绕过的门禁。问题“质量指标如何避免互相冲突”的关键不在背诵名词，而在于：把质量拆成硬门槛与评分项，并规定优先级、冲突处理和停止条件；不能用平均分掩盖安全或过程失败。 先判断题目中的硬门槛和适用范围，再看“只看最终答案、指标互相冲突”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-ai-quality-security-2-2 deck:"AI 质量与安全" priority:1 tags:"证据链" %}
--- question
最终答案正确但过程危险是否算成功
--- answer
最终答案正确但过程越权、泄密或产生未批准副作用时，应判定为未通过安全门禁，而不是用结果分抵消。
--- explanation
质量模型把结果正确性、过程质量、安全硬门槛和人工责任拆开，冲突时先执行不可绕过的门禁。问题“最终答案正确但过程危险是否算成功”的关键不在背诵名词，而在于：最终答案正确但过程越权、泄密或产生未批准副作用时，应判定为未通过安全门禁，而不是用结果分抵消。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
