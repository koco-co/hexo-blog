---
title: AI 质量与安全(五)LLM Judge与评分器
tags:
  - AI 质量与安全
  - LLM Judge与评分器
categories:
...
{% note primary flat %}
本节围绕“LLM Judge与评分器”建立可复核的工作模型。学习成果：Judge 分歧和漂移可发现。实验：用固定样本和人工标签比较两个评分器，记录分歧和校准后的阈值。视觉辅助：评分流程 Mermaid；评分器校准表。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
Judge 给出高分却与人工标签相反，团队差点把自评当真值。Rubric、盲测、校准和抽检共同决定评分器是否可信。

本篇把“LLM Judge与评分器”落到一次可回放的本地任务：用固定样本和人工标签比较两个评分器，记录分歧和校准后的阈值。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：Judge 分歧和漂移可发现。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“规则评分、模型评分和人工评分；Rubric、pairwise 和 reference-based 评估；校准、盲测、一致性和偏差”拆成输入、状态、输出和证据。LLM Judge 是带偏差的测量工具；Rubric、人工盲测、校准、一致性和抽检决定它能否用于门禁。
{% endnote %}
...
{% note primary flat %}
实验步骤：用固定样本和人工标签比较两个评分器，记录分歧和校准后的阈值。

验收产物：Rubric、人工标签、盲测和校准报告

失败注入：模型自评当真值、评分器偏置未抽检
{% endnote %}

## 评分标准

{% note info flat %}
本节任务：理解规则评分、模型评分和人工评分并建立可复核判断。核心内容：规则评分、模型评分和人工评分。

机制：规则评分、模型评分和人工评分。先核对输入、状态、输出、失败和复测证据，再用“模型自评当真值、评分器偏置未抽检”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“评分方式”聚焦“规则评分、模型评分和人工评分”。把模型/Provider 选择、认证状态和任务输入分成三列；凭据缺失时应停止在本地静态演示，不输出看似真实的服务响应。先把“规则评分、模型评分和人工评分”变成一个可观察字段，再以“用固定样本和人工标签比较两个评分器，记录分歧和校准后的阈值。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“模型自评当真值、评分器偏置未抽检”，恢复或放行动作必须可重放。
{% endnote %}

### 评分方法

{% note info flat %}
“评分方法”聚焦“Rubric、pairwise 和 reference-based 评估”。围绕“Rubric、pairwise 和 reference-based 评估”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“评分方法”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“模型自评当真值、评分器偏置未抽检”，先停止并保留原始事件。本节的失败样例是“模型自评当真值、评分器偏置未抽检”，恢复或放行动作必须可重放。
{% endnote %}

## 校准过程

{% note info flat %}
本节任务：理解Rubric、pairwise 和 reference-based 评估并建立可复核判断。核心内容：Rubric、pairwise 和 reference-based 评估。

机制：Rubric、pairwise 和 reference-based 评估。先核对输入、状态、输出、失败和复测证据，再用“模型自评当真值、评分器偏置未抽检”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“校准与偏差”聚焦“Rubric、pairwise 和 reference-based 评估”。围绕“Rubric、pairwise 和 reference-based 评估”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“校准与偏差”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“模型自评当真值、评分器偏置未抽检”，先停止并保留原始事件。本节的失败样例是“模型自评当真值、评分器偏置未抽检”，恢复或放行动作必须可重放。
{% endnote %}

### 评分器漂移

{% note info flat %}
“评分器漂移”聚焦“校准、盲测、一致性和偏差”。围绕“校准、盲测、一致性和偏差”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：Rubric、人工标签、盲测和校准报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“模型自评当真值、评分器偏置未抽检”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：用固定样本和人工标签比较两个评分器，记录分歧和校准后的阈值。

机制：用固定样本和人工标签比较两个评分器，记录分歧和校准后的阈值。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“模型自评当真值、评分器偏置未抽检”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“校准、盲测、一致性和偏差”。围绕“校准、盲测、一致性和偏差”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：Rubric、人工标签、盲测和校准报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“模型自评当真值、评分器偏置未抽检”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"AI 质量与安全","article":"AI 质量与安全(五)LLM Judge与评分器","order":5,"topic":"LLM Judge与评分器","experiment":"用固定样本和人工标签比较两个评分器，记录分歧和校准后的阈值。","focus":["规则评分、模型评分和人工评分","Rubric、pairwise 和 reference-based 评估","校准、盲测、一致性和偏差","评分器漂移与人工抽检"],"artifacts":["Rubric、人工标签、盲测和校准报告"],"failures":["模型自评当真值、评分器偏置未抽检"],"mode":"quality"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    rows = read_json(root / "judge-calibration.json")["rows"]
    disagreement = [row for row in rows if not row["match"]]
    try:
        if disagreement:
            raise ValueError("judge disagreement requires human calibration")
    ...
    assert len(disagreement) == 1 and sum(row["match"] for row in rows) == 3
    record_event(events_file, "recovery_verified", action="route-disagreement-to-review")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"quality","online_evidence":"not_run","scenario":"judge-calibration","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "quality", "online_evidence": "not_run", "scenario": "judge-calibration", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“评分器漂移与人工抽检”。围绕“评分器漂移与人工抽检”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“评分器漂移与人工抽检”变成一个可观察字段，再以“用固定样本和人工标签比较两个评分器，记录分歧和校准后的阈值。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“模型自评当真值、评分器偏置未抽检”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“规则评分、模型评分和人工评分”。把模型/Provider 选择、认证状态和任务输入分成三列；凭据缺失时应停止在本地静态演示，不输出看似真实的服务响应。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“模型自评当真值、评分器偏置未抽检”，先停止并保留原始事件。本节的失败样例是“模型自评当真值、评分器偏置未抽检”，恢复或放行动作必须可重放。
{% endnote %}

## 偏差检查

{% note info flat %}
本节任务：理解评分器漂移与人工抽检并建立可复核判断。核心内容：评分器漂移与人工抽检。

机制：评分器漂移与人工抽检。先核对输入、状态、输出、失败和复测证据，再用“模型自评当真值、评分器偏置未抽检”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“评分器漂移”聚焦“评分器漂移与人工抽检”。围绕“评分器漂移与人工抽检”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“评分器漂移与人工抽检”变成一个可观察字段，再以“用固定样本和人工标签比较两个评分器，记录分歧和校准后的阈值。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“模型自评当真值、评分器偏置未抽检”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：Rubric、人工标签、盲测和校准报告。预期：Judge 分歧和漂移可发现。失败复验：模型自评当真值、评分器偏置未抽检。

机制：验收证据：Rubric、人工标签、盲测和校准报告。预期：Judge 分歧和漂移可发现。失败复验：模型自评当真值、评分器偏置未抽检。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“模型自评当真值、评分器偏置未抽检”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“规则评分、模型评分和人工评分”。把模型/Provider 选择、认证状态和任务输入分成三列；凭据缺失时应停止在本地静态演示，不输出看似真实的服务响应。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“模型自评当真值、评分器偏置未抽检”，先停止并保留原始事件。本节的失败样例是“模型自评当真值、评分器偏置未抽检”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | Rubric、人工标签、盲测和校准报告 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 模型自评当真值、评分器偏置未抽检 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“Rubric、pairwise 和 reference-based 评估”。围绕“Rubric、pairwise 和 reference-based 评估”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：Rubric、人工标签、盲测和校准报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“模型自评当真值、评分器偏置未抽检”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-ai-quality-security-5-1 deck:"AI 质量与安全" priority:1 tags:"边界验证" %}
--- question
LLM Judge 为什么不能单独作为真值
--- answer
LLM Judge 是带偏差的评分器，不是自动真值；必须用明确 Rubric、人工盲测、校准和抽检验证。
--- explanation
LLM Judge 是带偏差的测量工具；Rubric、人工盲测、校准、一致性和抽检决定它能否用于门禁。问题“LLM Judge 为什么不能单独作为真值”的关键不在背诵名词，而在于：LLM Judge 是带偏差的评分器，不是自动真值；必须用明确 Rubric、人工盲测、校准和抽检验证。 先判断题目中的硬门槛和适用范围，再看“模型自评当真值、评分器偏置未抽检”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-ai-quality-security-5-2 deck:"AI 质量与安全" priority:1 tags:"证据链" %}
--- question
评分标准如何减少主观漂移
--- answer
减少漂移要把分档、反例、边界和证据要求写进 Rubric，并定期与人工标签比较、复校评分器。
--- explanation
LLM Judge 是带偏差的测量工具；Rubric、人工盲测、校准、一致性和抽检决定它能否用于门禁。问题“评分标准如何减少主观漂移”的关键不在背诵名词，而在于：减少漂移要把分档、反例、边界和证据要求写进 Rubric，并定期与人工标签比较、复校评分器。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
