...
categories:
  - Learn Topic
  - AI 质量与安全
description: 分别证明记忆准确性、时效性、隔离、选择性遗忘和删除后的行为
...
{% note primary flat %}
本节围绕“Memory评测”建立可复核的工作模型。学习成果：分别证明记忆准确性、时效性、隔离、选择性遗忘和删除后的行为。实验：用本地 JSON 记忆组件注入正确、过期、冲突和恶意记忆，验证作用域、读取、淘汰、删除和审计。视觉辅助：Memory 生命周期与评测点图；作用域矩阵。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
删除用户记忆后，摘要缓存仍召回旧事实。作用域、时效、冲突、删除和修复证据必须一起测试。

本篇把“Memory评测”落到一次可回放的本地任务：用本地 JSON 记忆组件注入正确、过期、冲突和恶意记忆，验证作用域、读取、淘汰、删除和审计。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：分别证明记忆准确性、时效性、隔离、选择性遗忘和删除后的行为。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“短期、长期、项目和全局记忆的测试边界；记忆写入触发、召回准确性和时效性；错误记忆、冲突记忆、越权读取和污染”拆成输入、状态、输出和证据。Memory 评测必须覆盖写入触发、作用域、召回、冲突、时效、选择性遗忘和派生副本删除，而不是只问一次回答。
{% endnote %}
...
{% note primary flat %}
实验步骤：用本地 JSON 记忆组件注入正确、过期、冲突和恶意记忆，验证作用域、读取、淘汰、删除和审计。

验收产物：写入/读取/更新/过期/删除/越权的版本化记忆报告

失败注入：旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散
{% endnote %}

## 作用域测试

{% note info flat %}
本节任务：理解短期、长期、项目和全局记忆的测试边界并建立可复核判断。核心内容：短期、长期、项目和全局记忆的测试边界。

机制：短期、长期、项目和全局记忆的测试边界。先核对输入、状态、输出、失败和复测证据，再用“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“记忆作用域”聚焦“短期、长期、项目和全局记忆的测试边界”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。先把“短期、长期、项目和全局记忆的测试边界”变成一个可观察字段，再以“用本地 JSON 记忆组件注入正确、过期、冲突和恶意记忆，验证作用域、读取、淘汰、删除和审计。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”，恢复或放行动作必须可重放。
{% endnote %}

### 写入召回

{% note info flat %}
“写入召回”聚焦“记忆写入触发、召回准确性和时效性”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。具体取舍是：保留“写入召回”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”，先停止并保留原始事件。本节的失败样例是“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”，恢复或放行动作必须可重放。
{% endnote %}

## 时效与冲突

{% note info flat %}
本节任务：理解记忆写入触发、召回准确性和时效性并建立可复核判断。核心内容：记忆写入触发、召回准确性和时效性。

机制：记忆写入触发、召回准确性和时效性。先核对输入、状态、输出、失败和复测证据，再用“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“记忆污染”聚焦“记忆写入触发、召回准确性和时效性”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。具体取舍是：保留“记忆污染”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”，先停止并保留原始事件。本节的失败样例是“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”，恢复或放行动作必须可重放。
{% endnote %}

### 删除证明

{% note info flat %}
“删除证明”聚焦“错误记忆、冲突记忆、越权读取和污染”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。复核时把动作和产物对应起来：写入/读取/更新/过期/删除/越权的版本化记忆报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：用本地 JSON 记忆组件注入正确、过期、冲突和恶意记忆，验证作用域、读取、淘汰、删除和审计。

机制：用本地 JSON 记忆组件注入正确、过期、冲突和恶意记忆，验证作用域、读取、淘汰、删除和审计。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“错误记忆、冲突记忆、越权读取和污染”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。复核时把动作和产物对应起来：写入/读取/更新/过期/删除/越权的版本化记忆报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"AI 质量与安全","article":"AI 质量与安全(八)Memory评测","order":8,"topic":"Memory评测","experiment":"用本地 JSON 记忆组件注入正确、过期、冲突和恶意记忆，验证作用域、读取、淘汰、删除和审计。","focus":["短期、长期、项目和全局记忆的测试边界","记忆写入触发、召回准确性和时效性","错误记忆、冲突记忆、越权读取和污染","删除、过期和修复证据"],"artifacts":["写入/读取/更新/过期/删除/越权的版本化记忆报告"],"failures":["旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散"],"mode":"quality"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    memory["tenant-a"] = {"scope": "project", "version": 1, "value": "old", "expired": True}
    try:
        if memory["tenant-a"]["expired"]:
            raise ValueError("expired memory was returned to retrieval")
    ...
    assert "tenant-a" not in read_json(root / "memory.json") and "tenant-b" in read_json(root / "memory.json")
    record_event(events_file, "recovery_verified", action="purge-expired-memory")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"quality","online_evidence":"not_run","scenario":"memory-eval","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "quality", "online_evidence": "not_run", "scenario": "memory-eval", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“删除、过期和修复证据”。围绕“删除、过期和修复证据”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“删除、过期和修复证据”变成一个可观察字段，再以“用本地 JSON 记忆组件注入正确、过期、冲突和恶意记忆，验证作用域、读取、淘汰、删除和审计。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“短期、长期、项目和全局记忆的测试边界”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”，先停止并保留原始事件。本节的失败样例是“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”，恢复或放行动作必须可重放。
{% endnote %}

## 删除证明

{% note info flat %}
本节任务：理解删除、过期和修复证据并建立可复核判断。核心内容：删除、过期和修复证据。

机制：删除、过期和修复证据。先核对输入、状态、输出、失败和复测证据，再用“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“删除证明”聚焦“删除、过期和修复证据”。围绕“删除、过期和修复证据”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“删除、过期和修复证据”变成一个可观察字段，再以“用本地 JSON 记忆组件注入正确、过期、冲突和恶意记忆，验证作用域、读取、淘汰、删除和审计。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：写入/读取/更新/过期/删除/越权的版本化记忆报告。预期：分别证明记忆准确性、时效性、隔离、选择性遗忘和删除后的行为。失败复验：旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散。

机制：验收证据：写入/读取/更新/过期/删除/越权的版本化记忆报告。预期：分别证明记忆准确性、时效性、隔离、选择性遗忘和删除后的行为。失败复验：旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“短期、长期、项目和全局记忆的测试边界”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”，先停止并保留原始事件。本节的失败样例是“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 写入/读取/更新/过期/删除/越权的版本化记忆报告 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“记忆写入触发、召回准确性和时效性”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。复核时把动作和产物对应起来：写入/读取/更新/过期/删除/越权的版本化记忆报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-ai-quality-security-8-1 deck:"AI 质量与安全" priority:1 tags:"边界验证" %}
--- question
记忆评测和普通对话评测有什么不同
--- answer
记忆评测还要验证跨会话持久性、作用域、时效、冲突、选择性遗忘和删除后的不可召回，而不只是当前回答。
--- explanation
Memory 评测必须覆盖写入触发、作用域、召回、冲突、时效、选择性遗忘和派生副本删除，而不是只问一次回答。问题“记忆评测和普通对话评测有什么不同”的关键不在背诵名词，而在于：记忆评测还要验证跨会话持久性、作用域、时效、冲突、选择性遗忘和删除后的不可召回，而不只是当前回答。 先判断题目中的硬门槛和适用范围，再看“旧事实被召回、跨租户读取、删除后由摘要/向量/缓存复原、污染扩散”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-ai-quality-security-8-2 deck:"AI 质量与安全" priority:1 tags:"证据链" %}
--- question
如何证明一条错误记忆已经被清除
--- answer
写入删除标记后，要从主存储、摘要、向量索引、缓存和日志副本复查，注入查询确认旧事实不再被召回。
--- explanation
Memory 评测必须覆盖写入触发、作用域、召回、冲突、时效、选择性遗忘和派生副本删除，而不是只问一次回答。问题“如何证明一条错误记忆已经被清除”的关键不在背诵名词，而在于：写入删除标记后，要从主存储、摘要、向量索引、缓存和日志副本复查，注入查询确认旧事实不再被召回。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
...
{% linkgroup %}
{% link LongMemEval, https://arxiv.org/abs/2410.10813, https://arxiv.org/favicon.ico %}
{% link AgentMemBench, https://arxiv.org/abs/2606.29788, https://arxiv.org/favicon.ico %}
{% endlinkgroup %}

### 源码框架

{% linkgroup %}
{% link LongMemEval 官方仓库, https://github.com/xiaowu0162/LongMemEval/tree/9e0b455f4ef0e2ab8f2e582289761153549043fc, https://github.com/favicon.ico %}
{% link AgentMemBench 官方仓库, https://github.com/mazaiying/AgentMemBench/tree/186c9a54edd47aae42d8b6990520f8e902b60303, https://github.com/favicon.ico %}
