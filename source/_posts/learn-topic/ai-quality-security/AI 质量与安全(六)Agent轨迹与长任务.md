...
{% note primary flat %}
本节围绕“Agent轨迹与长任务”建立可复核的工作模型。学习成果：能区分计划、工具、恢复和结果失败。实验：对本地 Agent 运行记录逐步评分，区分结果失败、工具失败和计划失败。视觉辅助：Agent Trace 图；过程指标表。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
长任务最终返回成功，中途计划却已失效且工具重复执行。逐步 Trace 与中间验收把归因落到第一处偏差。

本篇把“Agent轨迹与长任务”落到一次可回放的本地任务：对本地 Agent 运行记录逐步评分，区分结果失败、工具失败和计划失败。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：能区分计划、工具、恢复和结果失败。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“最终结果与过程轨迹；工具正确性、步骤数、回滚和恢复；长任务分段验收”拆成输入、状态、输出和证据。Agent 轨迹评测把最终结果拆回计划、工具、恢复和停止事件，才能定位长任务的第一处偏差并保护隐私。
{% endnote %}
...
{% note primary flat %}
实验步骤：对本地 Agent 运行记录逐步评分，区分结果失败、工具失败和计划失败。

验收产物：逐步 Trace 和中间验收记录

失败注入：长任务只看末态、轨迹含未脱敏输入
{% endnote %}

## 轨迹结构

{% note info flat %}
本节任务：理解最终结果与过程轨迹并建立可复核判断。核心内容：最终结果与过程轨迹。

机制：最终结果与过程轨迹。先核对输入、状态、输出、失败和复测证据，再用“长任务只看末态、轨迹含未脱敏输入”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“结果与轨迹”聚焦“最终结果与过程轨迹”。围绕“最终结果与过程轨迹”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“最终结果与过程轨迹”变成一个可观察字段，再以“对本地 Agent 运行记录逐步评分，区分结果失败、工具失败和计划失败。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“长任务只看末态、轨迹含未脱敏输入”，恢复或放行动作必须可重放。
{% endnote %}

### 工具与恢复

{% note info flat %}
“工具与恢复”聚焦“工具正确性、步骤数、回滚和恢复”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。具体取舍是：保留“工具与恢复”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“长任务只看末态、轨迹含未脱敏输入”，先停止并保留原始事件。本节的失败样例是“长任务只看末态、轨迹含未脱敏输入”，恢复或放行动作必须可重放。
{% endnote %}

## 过程指标

{% note info flat %}
本节任务：理解工具正确性、步骤数、回滚和恢复并建立可复核判断。核心内容：工具正确性、步骤数、回滚和恢复。

机制：工具正确性、步骤数、回滚和恢复。先核对输入、状态、输出、失败和复测证据，再用“长任务只看末态、轨迹含未脱敏输入”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“分段验收”聚焦“工具正确性、步骤数、回滚和恢复”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。具体取舍是：保留“分段验收”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“长任务只看末态、轨迹含未脱敏输入”，先停止并保留原始事件。本节的失败样例是“长任务只看末态、轨迹含未脱敏输入”，恢复或放行动作必须可重放。
{% endnote %}

### 轨迹隐私

{% note info flat %}
“轨迹隐私”聚焦“长任务分段验收”。把数据内容与控制指令分离，并沿存储、Context、日志、缓存和外发路径检查；测试通过要同时说明覆盖范围和未覆盖面。复核时把动作和产物对应起来：逐步 Trace 和中间验收记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“长任务只看末态、轨迹含未脱敏输入”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：对本地 Agent 运行记录逐步评分，区分结果失败、工具失败和计划失败。

机制：对本地 Agent 运行记录逐步评分，区分结果失败、工具失败和计划失败。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“长任务只看末态、轨迹含未脱敏输入”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“长任务分段验收”。围绕“长任务分段验收”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：逐步 Trace 和中间验收记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“长任务只看末态、轨迹含未脱敏输入”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"AI 质量与安全","article":"AI 质量与安全(六)Agent轨迹与长任务","order":6,"topic":"Agent轨迹与长任务","experiment":"对本地 Agent 运行记录逐步评分，区分结果失败、工具失败和计划失败。","focus":["最终结果与过程轨迹","工具正确性、步骤数、回滚和恢复","长任务分段验收","轨迹隐私和可复现"],"artifacts":["逐步 Trace 和中间验收记录"],"failures":["长任务只看末态、轨迹含未脱敏输入"],"mode":"quality"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    trace = read_json(root / "graded-trace.json")
    incomplete = [step for step in trace["steps"] if step != "verify"]
    try:
        if "verify" not in incomplete:
            raise RuntimeError("long task reached final output without an intermediate gate")
    ...
    assert "verify" in trace["steps"] and trace["steps"][-1] == "stop"
    record_event(events_file, "recovery_verified", action="restore-intermediate-gate")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"quality","online_evidence":"not_run","scenario":"trace-grading","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "quality", "online_evidence": "not_run", "scenario": "trace-grading", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“轨迹隐私和可复现”。把数据内容与控制指令分离，并沿存储、Context、日志、缓存和外发路径检查；测试通过要同时说明覆盖范围和未覆盖面。先把“轨迹隐私和可复现”变成一个可观察字段，再以“对本地 Agent 运行记录逐步评分，区分结果失败、工具失败和计划失败。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“长任务只看末态、轨迹含未脱敏输入”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“最终结果与过程轨迹”。围绕“最终结果与过程轨迹”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“长任务只看末态、轨迹含未脱敏输入”，先停止并保留原始事件。本节的失败样例是“长任务只看末态、轨迹含未脱敏输入”，恢复或放行动作必须可重放。
{% endnote %}

## 长任务分段

{% note info flat %}
本节任务：理解轨迹隐私和可复现并建立可复核判断。核心内容：轨迹隐私和可复现。

机制：轨迹隐私和可复现。先核对输入、状态、输出、失败和复测证据，再用“长任务只看末态、轨迹含未脱敏输入”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“轨迹隐私”聚焦“轨迹隐私和可复现”。把数据内容与控制指令分离，并沿存储、Context、日志、缓存和外发路径检查；测试通过要同时说明覆盖范围和未覆盖面。先把“轨迹隐私和可复现”变成一个可观察字段，再以“对本地 Agent 运行记录逐步评分，区分结果失败、工具失败和计划失败。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“长任务只看末态、轨迹含未脱敏输入”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：逐步 Trace 和中间验收记录。预期：能区分计划、工具、恢复和结果失败。失败复验：长任务只看末态、轨迹含未脱敏输入。

机制：验收证据：逐步 Trace 和中间验收记录。预期：能区分计划、工具、恢复和结果失败。失败复验：长任务只看末态、轨迹含未脱敏输入。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“长任务只看末态、轨迹含未脱敏输入”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“最终结果与过程轨迹”。围绕“最终结果与过程轨迹”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“长任务只看末态、轨迹含未脱敏输入”，先停止并保留原始事件。本节的失败样例是“长任务只看末态、轨迹含未脱敏输入”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 逐步 Trace 和中间验收记录 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 长任务只看末态、轨迹含未脱敏输入 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“工具正确性、步骤数、回滚和恢复”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。复核时把动作和产物对应起来：逐步 Trace 和中间验收记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“长任务只看末态、轨迹含未脱敏输入”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-ai-quality-security-6-1 deck:"AI 质量与安全" priority:1 tags:"边界验证" %}
--- question
为什么 Agent 评测不能只看最终答案
--- answer
最终答案可能掩盖错误计划、工具失败、重试、越权或浪费；Agent 评测必须保留逐步 Trace。
--- explanation
Agent 轨迹评测把最终结果拆回计划、工具、恢复和停止事件，才能定位长任务的第一处偏差并保护隐私。问题“为什么 Agent 评测不能只看最终答案”的关键不在背诵名词，而在于：最终答案可能掩盖错误计划、工具失败、重试、越权或浪费；Agent 评测必须保留逐步 Trace。 先判断题目中的硬门槛和适用范围，再看“长任务只看末态、轨迹含未脱敏输入”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-ai-quality-security-6-2 deck:"AI 质量与安全" priority:1 tags:"证据链" %}
--- question
长任务如何设置中间验收点
--- answer
在计划批准、关键工具调用、恢复点和最终提交处设置中间验收；每个检查点都应有通过条件和停止动作。
--- explanation
Agent 轨迹评测把最终结果拆回计划、工具、恢复和停止事件，才能定位长任务的第一处偏差并保护隐私。问题“长任务如何设置中间验收点”的关键不在背诵名词，而在于：在计划批准、关键工具调用、恢复点和最终提交处设置中间验收；每个检查点都应有通过条件和停止动作。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
