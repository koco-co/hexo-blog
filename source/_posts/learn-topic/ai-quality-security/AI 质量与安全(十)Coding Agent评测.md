---
title: AI 质量与安全(十)Coding Agent评测
tags:
  - AI 质量与安全
  - Coding Agent评测
categories:
  - Learn Topic
  - AI 质量与安全
description: 代码行为、可维护性和安全行为分别可验收
...
{% note primary flat %}
本节围绕“Coding Agent评测”建立可复核的工作模型。学习成果：代码行为、可维护性和安全行为分别可验收。实验：用临时仓库设计 bugfix、重构和安全任务，收集 Diff、测试、日志和权限证据。视觉辅助：代码 Agent 证据链；评分表。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
代码任务通过测试，却把秘密写进日志并修改范围外文件。编译、行为、Diff、权限和人工审查是不同证据层。

本篇把“Coding Agent评测”落到一次可回放的本地任务：用临时仓库设计 bugfix、重构和安全任务，收集 Diff、测试、日志和权限证据。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：代码行为、可维护性和安全行为分别可验收。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“编译、测试、静态检查和行为验收；Diff、回滚、秘密和越权；仓库任务集与环境一致性”拆成输入、状态、输出和证据。Coding Agent 评测将编译、测试、Diff、秘密、权限、环境和人工审查分层；绿色测试不是安全证明。
{% endnote %}
...
{% note primary flat %}
实验步骤：用临时仓库设计 bugfix、重构和安全任务，收集 Diff、测试、日志和权限证据。

验收产物：bugfix/重构/安全任务的 Diff、测试和权限报告

失败注入：通过测试但越权、Diff 超范围、环境不可复现
{% endnote %}

## 代码证据

{% note info flat %}
本节任务：理解编译、测试、静态检查和行为验收并建立可复核判断。核心内容：编译、测试、静态检查和行为验收。

机制：编译、测试、静态检查和行为验收。先核对输入、状态、输出、失败和复测证据，再用“通过测试但越权、Diff 超范围、环境不可复现”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“代码验收”聚焦“编译、测试、静态检查和行为验收”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。先把“编译、测试、静态检查和行为验收”变成一个可观察字段，再以“用临时仓库设计 bugfix、重构和安全任务，收集 Diff、测试、日志和权限证据。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“通过测试但越权、Diff 超范围、环境不可复现”，恢复或放行动作必须可重放。
{% endnote %}

### Diff与权限

{% note info flat %}
“Diff与权限”聚焦“Diff、回滚、秘密和越权”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“Diff与权限”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“通过测试但越权、Diff 超范围、环境不可复现”，先停止并保留原始事件。本节的失败样例是“通过测试但越权、Diff 超范围、环境不可复现”，恢复或放行动作必须可重放。
{% endnote %}

## Diff与权限

{% note info flat %}
本节任务：理解Diff、回滚、秘密和越权并建立可复核判断。核心内容：Diff、回滚、秘密和越权。

机制：Diff、回滚、秘密和越权。先核对输入、状态、输出、失败和复测证据，再用“通过测试但越权、Diff 超范围、环境不可复现”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“环境一致性”聚焦“Diff、回滚、秘密和越权”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。具体取舍是：保留“环境一致性”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“通过测试但越权、Diff 超范围、环境不可复现”，先停止并保留原始事件。本节的失败样例是“通过测试但越权、Diff 超范围、环境不可复现”，恢复或放行动作必须可重放。
{% endnote %}

### 人工复现

{% note info flat %}
“人工复现”聚焦“仓库任务集与环境一致性”。围绕“仓库任务集与环境一致性”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：bugfix/重构/安全任务的 Diff、测试和权限报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“通过测试但越权、Diff 超范围、环境不可复现”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：用临时仓库设计 bugfix、重构和安全任务，收集 Diff、测试、日志和权限证据。

机制：用临时仓库设计 bugfix、重构和安全任务，收集 Diff、测试、日志和权限证据。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“通过测试但越权、Diff 超范围、环境不可复现”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“仓库任务集与环境一致性”。围绕“仓库任务集与环境一致性”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：bugfix/重构/安全任务的 Diff、测试和权限报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“通过测试但越权、Diff 超范围、环境不可复现”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"AI 质量与安全","article":"AI 质量与安全(十)Coding Agent评测","order":10,"topic":"Coding Agent评测","experiment":"用临时仓库设计 bugfix、重构和安全任务，收集 Diff、测试、日志和权限证据。","focus":["编译、测试、静态检查和行为验收","Diff、回滚、秘密和越权","仓库任务集与环境一致性","人工审查和可复现执行"],"artifacts":["bugfix/重构/安全任务的 Diff、测试和权限报告"],"failures":["通过测试但越权、Diff 超范围、环境不可复现"],"mode":"quality"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    report = read_json(root / "coding-evaluation.json")
    unsafe_diff = {"file": "../outside.py", "secret": "api_key=fixture"}
    try:
        if unsafe_diff["file"].startswith("../") or "api_key=" in unsafe_diff["secret"]:
            raise PermissionError("passing tests cannot override scope or secret gates")
    ...
    report["scope"] = True
    report["secret_scan"] = "pass"
    write_json(root / "coding-evaluation.json", report)
    assert read_json(root / "coding-evaluation.json")["scope"] and read_json(root / "coding-evaluation.json")["secret_scan"] == "pass"
    record_event(events_file, "recovery_verified", action="retain-security-gates")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"quality","online_evidence":"not_run","scenario":"coding-eval","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "quality", "online_evidence": "not_run", "scenario": "coding-eval", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“人工审查和可复现执行”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。先把“人工审查和可复现执行”变成一个可观察字段，再以“用临时仓库设计 bugfix、重构和安全任务，收集 Diff、测试、日志和权限证据。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“通过测试但越权、Diff 超范围、环境不可复现”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“编译、测试、静态检查和行为验收”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“通过测试但越权、Diff 超范围、环境不可复现”，先停止并保留原始事件。本节的失败样例是“通过测试但越权、Diff 超范围、环境不可复现”，恢复或放行动作必须可重放。
{% endnote %}

## 复现环境

{% note info flat %}
本节任务：理解人工审查和可复现执行并建立可复核判断。核心内容：人工审查和可复现执行。

机制：人工审查和可复现执行。先核对输入、状态、输出、失败和复测证据，再用“通过测试但越权、Diff 超范围、环境不可复现”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“人工复现”聚焦“人工审查和可复现执行”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。先把“人工审查和可复现执行”变成一个可观察字段，再以“用临时仓库设计 bugfix、重构和安全任务，收集 Diff、测试、日志和权限证据。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“通过测试但越权、Diff 超范围、环境不可复现”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：bugfix/重构/安全任务的 Diff、测试和权限报告。预期：代码行为、可维护性和安全行为分别可验收。失败复验：通过测试但越权、Diff 超范围、环境不可复现。

机制：验收证据：bugfix/重构/安全任务的 Diff、测试和权限报告。预期：代码行为、可维护性和安全行为分别可验收。失败复验：通过测试但越权、Diff 超范围、环境不可复现。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“通过测试但越权、Diff 超范围、环境不可复现”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“编译、测试、静态检查和行为验收”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“通过测试但越权、Diff 超范围、环境不可复现”，先停止并保留原始事件。本节的失败样例是“通过测试但越权、Diff 超范围、环境不可复现”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | bugfix/重构/安全任务的 Diff、测试和权限报告 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 通过测试但越权、Diff 超范围、环境不可复现 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“Diff、回滚、秘密和越权”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。复核时把动作和产物对应起来：bugfix/重构/安全任务的 Diff、测试和权限报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“通过测试但越权、Diff 超范围、环境不可复现”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-ai-quality-security-10-1 deck:"AI 质量与安全" priority:1 tags:"边界验证" %}
--- question
代码生成通过测试是否足够
--- answer
通过测试只证明覆盖到的行为在该环境中通过，不能证明权限、秘密、范围、依赖和异常路径安全。
--- explanation
Coding Agent 评测将编译、测试、Diff、秘密、权限、环境和人工审查分层；绿色测试不是安全证明。问题“代码生成通过测试是否足够”的关键不在背诵名词，而在于：通过测试只证明覆盖到的行为在该环境中通过，不能证明权限、秘密、范围、依赖和异常路径安全。 先判断题目中的硬门槛和适用范围，再看“通过测试但越权、Diff 超范围、环境不可复现”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-ai-quality-security-10-2 deck:"AI 质量与安全" priority:1 tags:"证据链" %}
--- question
Coding Agent 的安全分数如何定义
--- answer
安全分数应由可复核维度构成，例如秘密暴露、越权访问、危险动作拦截、Diff 范围和审查结果，并报告样本与权重。
--- explanation
Coding Agent 评测将编译、测试、Diff、秘密、权限、环境和人工审查分层；绿色测试不是安全证明。问题“Coding Agent 的安全分数如何定义”的关键不在背诵名词，而在于：安全分数应由可复核维度构成，例如秘密暴露、越权访问、危险动作拦截、Diff 范围和审查结果，并报告样本与权重。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
