---
title: AI 质量与安全(十一)Prompt Injection
tags:
  - AI 质量与安全
  - Prompt Injection
categories:
  - Learn Topic
  - AI 质量与安全
description: 不可信输入不能覆盖约束或触发高风险工具
...
{% note primary flat %}
本节围绕“Prompt Injection”建立可复核的工作模型。学习成果：不可信输入不能覆盖约束或触发高风险工具。实验：在本地文档和工具夹具中注入指令，验证隔离、拒绝、日志和人工确认。视觉辅助：注入路径 Mermaid；威胁矩阵。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
攻击文本藏在 Issue 引用中，模型把它当更高优先级指令。数据与指令分离、最小权限和人工确认必须在工具调用前生效。

本篇把“Prompt Injection”落到一次可回放的本地任务：在本地文档和工具夹具中注入指令，验证隔离、拒绝、日志和人工确认。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：不可信输入不能覆盖约束或触发高风险工具。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“指令层级与不可信输入；网页、文档、Issue 和工具结果中的间接注入；数据与指令分离、最小权限和确认”拆成输入、状态、输出和证据。Prompt Injection 防护从不可信数据与指令分离开始，再用最小权限、参数校验、确认和回归测试阻断影响。
{% endnote %}
...
{% note primary flat %}
实验步骤：在本地文档和工具夹具中注入指令，验证隔离、拒绝、日志和人工确认。

验收产物：直接/间接注入样例、阻断日志和回归集

失败注入：文档/网页注入、跨轮持久注入、误报导致业务中断
{% endnote %}

## 注入路径

{% note info flat %}
本节任务：理解指令层级与不可信输入并建立可复核判断。核心内容：指令层级与不可信输入。

机制：指令层级与不可信输入。先核对输入、状态、输出、失败和复测证据，再用“文档/网页注入、跨轮持久注入、误报导致业务中断”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“指令层级”聚焦“指令层级与不可信输入”。围绕“指令层级与不可信输入”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“指令层级与不可信输入”变成一个可观察字段，再以“在本地文档和工具夹具中注入指令，验证隔离、拒绝、日志和人工确认。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“文档/网页注入、跨轮持久注入、误报导致业务中断”，恢复或放行动作必须可重放。
{% endnote %}

### 间接注入

{% note info flat %}
“间接注入”聚焦“网页、文档、Issue 和工具结果中的间接注入”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。具体取舍是：保留“间接注入”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“文档/网页注入、跨轮持久注入、误报导致业务中断”，先停止并保留原始事件。本节的失败样例是“文档/网页注入、跨轮持久注入、误报导致业务中断”，恢复或放行动作必须可重放。
{% endnote %}

## 输入隔离

{% note info flat %}
本节任务：理解网页、文档、Issue 和工具结果中的间接注入并建立可复核判断。核心内容：网页、文档、Issue 和工具结果中的间接注入。

机制：网页、文档、Issue 和工具结果中的间接注入。先核对输入、状态、输出、失败和复测证据，再用“文档/网页注入、跨轮持久注入、误报导致业务中断”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“输入隔离”聚焦“网页、文档、Issue 和工具结果中的间接注入”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。具体取舍是：保留“输入隔离”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“文档/网页注入、跨轮持久注入、误报导致业务中断”，先停止并保留原始事件。本节的失败样例是“文档/网页注入、跨轮持久注入、误报导致业务中断”，恢复或放行动作必须可重放。
{% endnote %}

### 阻断回归

{% note info flat %}
“阻断回归”聚焦“数据与指令分离、最小权限和确认”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：直接/间接注入样例、阻断日志和回归集应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“文档/网页注入、跨轮持久注入、误报导致业务中断”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：在本地文档和工具夹具中注入指令，验证隔离、拒绝、日志和人工确认。

机制：在本地文档和工具夹具中注入指令，验证隔离、拒绝、日志和人工确认。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“文档/网页注入、跨轮持久注入、误报导致业务中断”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“数据与指令分离、最小权限和确认”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：直接/间接注入样例、阻断日志和回归集应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“文档/网页注入、跨轮持久注入、误报导致业务中断”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"AI 质量与安全","article":"AI 质量与安全(十一)Prompt Injection","order":11,"topic":"Prompt Injection","experiment":"在本地文档和工具夹具中注入指令，验证隔离、拒绝、日志和人工确认。","focus":["指令层级与不可信输入","网页、文档、Issue 和工具结果中的间接注入","数据与指令分离、最小权限和确认","检测、阻断、回归和误报"],"artifacts":["直接/间接注入样例、阻断日志和回归集"],"failures":["文档/网页注入、跨轮持久注入、误报导致业务中断"],"mode":"quality"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    report = read_json(root / "injection-decision.json")
    report["decision"] = "allow"
    try:
        if report["decision"] != "reject":
            raise PermissionError("untrusted instruction reached an executable path")
    ...
    write_json(root / "injection-decision.json", report)
    assert read_json(root / "injection-decision.json")["decision"] == "reject"
    record_event(events_file, "recovery_verified", action="keep-injection-deny")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"quality","online_evidence":"not_run","scenario":"injection-regression","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "quality", "online_evidence": "not_run", "scenario": "injection-regression", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“检测、阻断、回归和误报”。围绕“检测、阻断、回归和误报”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“检测、阻断、回归和误报”变成一个可观察字段，再以“在本地文档和工具夹具中注入指令，验证隔离、拒绝、日志和人工确认。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“文档/网页注入、跨轮持久注入、误报导致业务中断”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“指令层级与不可信输入”。围绕“指令层级与不可信输入”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“文档/网页注入、跨轮持久注入、误报导致业务中断”，先停止并保留原始事件。本节的失败样例是“文档/网页注入、跨轮持久注入、误报导致业务中断”，恢复或放行动作必须可重放。
{% endnote %}

## 阻断回归

{% note info flat %}
本节任务：理解检测、阻断、回归和误报并建立可复核判断。核心内容：检测、阻断、回归和误报。

机制：检测、阻断、回归和误报。先核对输入、状态、输出、失败和复测证据，再用“文档/网页注入、跨轮持久注入、误报导致业务中断”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“阻断回归”聚焦“检测、阻断、回归和误报”。围绕“检测、阻断、回归和误报”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“检测、阻断、回归和误报”变成一个可观察字段，再以“在本地文档和工具夹具中注入指令，验证隔离、拒绝、日志和人工确认。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“文档/网页注入、跨轮持久注入、误报导致业务中断”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：直接/间接注入样例、阻断日志和回归集。预期：不可信输入不能覆盖约束或触发高风险工具。失败复验：文档/网页注入、跨轮持久注入、误报导致业务中断。

机制：验收证据：直接/间接注入样例、阻断日志和回归集。预期：不可信输入不能覆盖约束或触发高风险工具。失败复验：文档/网页注入、跨轮持久注入、误报导致业务中断。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“文档/网页注入、跨轮持久注入、误报导致业务中断”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“指令层级与不可信输入”。围绕“指令层级与不可信输入”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“文档/网页注入、跨轮持久注入、误报导致业务中断”，先停止并保留原始事件。本节的失败样例是“文档/网页注入、跨轮持久注入、误报导致业务中断”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 直接/间接注入样例、阻断日志和回归集 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 文档/网页注入、跨轮持久注入、误报导致业务中断 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“网页、文档、Issue 和工具结果中的间接注入”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。复核时把动作和产物对应起来：直接/间接注入样例、阻断日志和回归集应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“文档/网页注入、跨轮持久注入、误报导致业务中断”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-ai-quality-security-11-1 deck:"AI 质量与安全" priority:1 tags:"边界验证" %}
--- question
为什么 RAG 文档也可能成为攻击载体
--- answer
RAG 文档会进入模型 Context，文档中的伪指令可能影响工具选择或答案，因此必须视为不可信数据。
--- explanation
Prompt Injection 防护从不可信数据与指令分离开始，再用最小权限、参数校验、确认和回归测试阻断影响。问题“为什么 RAG 文档也可能成为攻击载体”的关键不在背诵名词，而在于：RAG 文档会进入模型 Context，文档中的伪指令可能影响工具选择或答案，因此必须视为不可信数据。 先判断题目中的硬门槛和适用范围，再看“文档/网页注入、跨轮持久注入、误报导致业务中断”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-ai-quality-security-11-2 deck:"AI 质量与安全" priority:1 tags:"证据链" %}
--- question
Prompt 注入为什么不能只靠改系统提示词解决
--- answer
系统提示词不能单独抵御注入；还需分离数据与指令、最小权限、工具参数校验、输出过滤和高风险人工确认。
--- explanation
Prompt Injection 防护从不可信数据与指令分离开始，再用最小权限、参数校验、确认和回归测试阻断影响。问题“Prompt 注入为什么不能只靠改系统提示词解决”的关键不在背诵名词，而在于：系统提示词不能单独抵御注入；还需分离数据与指令、最小权限、工具参数校验、输出过滤和高风险人工确认。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
