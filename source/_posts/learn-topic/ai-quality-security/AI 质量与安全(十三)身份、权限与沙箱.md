...
{% note primary flat %}
本节围绕“身份、权限与沙箱”建立可复核的工作模型。学习成果：Agent 身份不等于用户全部权限。实验：为 Agent 工具集设计最小权限策略，用本地拒绝测试验证。视觉辅助：权限矩阵；隔离边界图。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
用户拥有部署权限，不代表每个 Agent 都能直接部署。身份映射、沙箱、网络和应急停止共同定义能力边界。

本篇把“身份、权限与沙箱”落到一次可回放的本地任务：为 Agent 工具集设计最小权限策略，用本地拒绝测试验证。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：Agent 身份不等于用户全部权限。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“用户、Agent、工具和服务身份；读写执行、网络、秘密和部署权限；容器、虚拟机、Worktree 和云环境”拆成输入、状态、输出和证据。身份、权限与沙箱分别回答谁在行动、能做什么和在哪些资源上做；用户授权不能自动转成 Agent 的全部权限。
{% endnote %}
...
{% note primary flat %}
实验步骤：为 Agent 工具集设计最小权限策略，用本地拒绝测试验证。

验收产物：身份映射、最小权限矩阵和拒绝测试

失败注入：权限提升、网络外发、沙箱边界不一致
{% endnote %}

## 身份模型

{% note info flat %}
本节任务：理解用户、Agent、工具和服务身份并建立可复核判断。核心内容：用户、Agent、工具和服务身份。

机制：用户、Agent、工具和服务身份。先核对输入、状态、输出、失败和复测证据，再用“权限提升、网络外发、沙箱边界不一致”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“身份层级”聚焦“用户、Agent、工具和服务身份”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。先把“用户、Agent、工具和服务身份”变成一个可观察字段，再以“为 Agent 工具集设计最小权限策略，用本地拒绝测试验证。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“权限提升、网络外发、沙箱边界不一致”，恢复或放行动作必须可重放。
{% endnote %}

### 权限范围

{% note info flat %}
“权限范围”聚焦“读写执行、网络、秘密和部署权限”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“权限范围”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“权限提升、网络外发、沙箱边界不一致”，先停止并保留原始事件。本节的失败样例是“权限提升、网络外发、沙箱边界不一致”，恢复或放行动作必须可重放。
{% endnote %}

## 权限矩阵

{% note info flat %}
本节任务：理解读写执行、网络、秘密和部署权限并建立可复核判断。核心内容：读写执行、网络、秘密和部署权限。

机制：读写执行、网络、秘密和部署权限。先核对输入、状态、输出、失败和复测证据，再用“权限提升、网络外发、沙箱边界不一致”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“隔离层级”聚焦“读写执行、网络、秘密和部署权限”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“隔离层级”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“权限提升、网络外发、沙箱边界不一致”，先停止并保留原始事件。本节的失败样例是“权限提升、网络外发、沙箱边界不一致”，恢复或放行动作必须可重放。
{% endnote %}

### 撤销与停止

{% note info flat %}
“撤销与停止”聚焦“容器、虚拟机、Worktree 和云环境”。围绕“容器、虚拟机、Worktree 和云环境”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：身份映射、最小权限矩阵和拒绝测试应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“权限提升、网络外发、沙箱边界不一致”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：为 Agent 工具集设计最小权限策略，用本地拒绝测试验证。

机制：为 Agent 工具集设计最小权限策略，用本地拒绝测试验证。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“权限提升、网络外发、沙箱边界不一致”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“容器、虚拟机、Worktree 和云环境”。围绕“容器、虚拟机、Worktree 和云环境”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：身份映射、最小权限矩阵和拒绝测试应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“权限提升、网络外发、沙箱边界不一致”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"AI 质量与安全","article":"AI 质量与安全(十三)身份、权限与沙箱","order":13,"topic":"身份、权限与沙箱","experiment":"为 Agent 工具集设计最小权限策略，用本地拒绝测试验证。","focus":["用户、Agent、工具和服务身份","读写执行、网络、秘密和部署权限","容器、虚拟机、Worktree 和云环境","权限审计、撤销和应急停止"],"artifacts":["身份映射、最小权限矩阵和拒绝测试"],"failures":["权限提升、网络外发、沙箱边界不一致"],"mode":"quality"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    policy = read_json(root / "identity-policy.json")["policy"]
    policy["deploy"] = True
    try:
        if policy["deploy"]:
            raise PermissionError("agent identity attempted to inherit deploy permission")
    ...
    policy["deploy"] = False
    write_json(root / "identity-policy.json", {"identity": read_json(root / "identity-policy.json")["identity"], "policy": policy})
    assert not read_json(root / "identity-policy.json")["policy"]["deploy"]
    record_event(events_file, "recovery_verified", action="revoke-deploy-capability")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"quality","online_evidence":"not_run","scenario":"permission-gate","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "quality", "online_evidence": "not_run", "scenario": "permission-gate", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“权限审计、撤销和应急停止”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。先把“权限审计、撤销和应急停止”变成一个可观察字段，再以“为 Agent 工具集设计最小权限策略，用本地拒绝测试验证。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“权限提升、网络外发、沙箱边界不一致”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“用户、Agent、工具和服务身份”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“权限提升、网络外发、沙箱边界不一致”，先停止并保留原始事件。本节的失败样例是“权限提升、网络外发、沙箱边界不一致”，恢复或放行动作必须可重放。
{% endnote %}

## 隔离验证

{% note info flat %}
本节任务：理解权限审计、撤销和应急停止并建立可复核判断。核心内容：权限审计、撤销和应急停止。

机制：权限审计、撤销和应急停止。先核对输入、状态、输出、失败和复测证据，再用“权限提升、网络外发、沙箱边界不一致”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“撤销与停止”聚焦“权限审计、撤销和应急停止”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。先把“权限审计、撤销和应急停止”变成一个可观察字段，再以“为 Agent 工具集设计最小权限策略，用本地拒绝测试验证。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“权限提升、网络外发、沙箱边界不一致”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：身份映射、最小权限矩阵和拒绝测试。预期：Agent 身份不等于用户全部权限。失败复验：权限提升、网络外发、沙箱边界不一致。

机制：验收证据：身份映射、最小权限矩阵和拒绝测试。预期：Agent 身份不等于用户全部权限。失败复验：权限提升、网络外发、沙箱边界不一致。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“权限提升、网络外发、沙箱边界不一致”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“用户、Agent、工具和服务身份”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“权限提升、网络外发、沙箱边界不一致”，先停止并保留原始事件。本节的失败样例是“权限提升、网络外发、沙箱边界不一致”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 身份映射、最小权限矩阵和拒绝测试 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 权限提升、网络外发、沙箱边界不一致 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“读写执行、网络、秘密和部署权限”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：身份映射、最小权限矩阵和拒绝测试应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“权限提升、网络外发、沙箱边界不一致”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-ai-quality-security-13-1 deck:"AI 质量与安全" priority:1 tags:"边界验证" %}
--- question
Agent 身份为什么不能等同于用户身份
--- answer
用户身份表达的是授权主体，Agent 身份是受限执行主体；默认不能继承用户的全部权限。
--- explanation
身份、权限与沙箱分别回答谁在行动、能做什么和在哪些资源上做；用户授权不能自动转成 Agent 的全部权限。问题“Agent 身份为什么不能等同于用户身份”的关键不在背诵名词，而在于：用户身份表达的是授权主体，Agent 身份是受限执行主体；默认不能继承用户的全部权限。 先判断题目中的硬门槛和适用范围，再看“权限提升、网络外发、沙箱边界不一致”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-ai-quality-security-13-2 deck:"AI 质量与安全" priority:1 tags:"证据链" %}
--- question
沙箱和审批分别解决什么问题
--- answer
沙箱限制运行环境与资源，审批决定某次高风险动作是否获准；两者分别是技术隔离和人/策略决策。
--- explanation
身份、权限与沙箱分别回答谁在行动、能做什么和在哪些资源上做；用户授权不能自动转成 Agent 的全部权限。问题“沙箱和审批分别解决什么问题”的关键不在背诵名词，而在于：沙箱限制运行环境与资源，审批决定某次高风险动作是否获准；两者分别是技术隔离和人/策略决策。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
