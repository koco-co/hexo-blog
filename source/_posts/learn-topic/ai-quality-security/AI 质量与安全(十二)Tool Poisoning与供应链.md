---
title: AI 质量与安全(十二)Tool Poisoning与供应链
tags:
  - AI 质量与安全
  - Tool Poisoning与供应链
categories:
...
{% note primary flat %}
本节围绕“Tool Poisoning与供应链”建立可复核的工作模型。学习成果：描述、代码、依赖和外发路径均被审查。实验：审查一个本地扩展夹具，标记描述投毒、危险权限、依赖和外发路径。视觉辅助：供应链信任图；风险矩阵。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
扩展描述声称只读，依赖却新增外发能力。来源锁定、权限审查、撤销和回归必须覆盖描述、代码与依赖。

本篇把“Tool Poisoning与供应链”落到一次可回放的本地任务：审查一个本地扩展夹具，标记描述投毒、危险权限、依赖和外发路径。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：描述、代码、依赖和外发路径均被审查。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“工具描述、返回值和隐藏指令；恶意 Skill、插件包和依赖；权限提升、秘密泄露和外发”拆成输入、状态、输出和证据。Tool Poisoning 与供应链风险藏在描述、返回值、依赖和外发路径中；来源锁定和撤销必须能执行并复测。
{% endnote %}
...
{% note primary flat %}
实验步骤：审查一个本地扩展夹具，标记描述投毒、危险权限、依赖和外发路径。

验收产物：扩展审查、依赖锁定、权限和撤销报告

失败注入：隐藏指令、恶意依赖、秘密泄露、撤销无效
{% endnote %}

## 描述投毒

{% note info flat %}
本节任务：理解工具描述、返回值和隐藏指令并建立可复核判断。核心内容：工具描述、返回值和隐藏指令。

机制：工具描述、返回值和隐藏指令。先核对输入、状态、输出、失败和复测证据，再用“隐藏指令、恶意依赖、秘密泄露、撤销无效”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“描述投毒”聚焦“工具描述、返回值和隐藏指令”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。先把“工具描述、返回值和隐藏指令”变成一个可观察字段，再以“审查一个本地扩展夹具，标记描述投毒、危险权限、依赖和外发路径。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“隐藏指令、恶意依赖、秘密泄露、撤销无效”，恢复或放行动作必须可重放。
{% endnote %}

### 恶意扩展

{% note info flat %}
“恶意扩展”聚焦“恶意 Skill、插件包和依赖”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。具体取舍是：保留“恶意扩展”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“隐藏指令、恶意依赖、秘密泄露、撤销无效”，先停止并保留原始事件。本节的失败样例是“隐藏指令、恶意依赖、秘密泄露、撤销无效”，恢复或放行动作必须可重放。
{% endnote %}

## 安装信任

{% note info flat %}
本节任务：理解恶意 Skill、插件包和依赖并建立可复核判断。核心内容：恶意 Skill、插件包和依赖。

机制：恶意 Skill、插件包和依赖。先核对输入、状态、输出、失败和复测证据，再用“隐藏指令、恶意依赖、秘密泄露、撤销无效”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“泄露与外发”聚焦“恶意 Skill、插件包和依赖”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。具体取舍是：保留“泄露与外发”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“隐藏指令、恶意依赖、秘密泄露、撤销无效”，先停止并保留原始事件。本节的失败样例是“隐藏指令、恶意依赖、秘密泄露、撤销无效”，恢复或放行动作必须可重放。
{% endnote %}

### 来源审查

{% note info flat %}
“来源审查”聚焦“权限提升、秘密泄露和外发”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：扩展审查、依赖锁定、权限和撤销报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“隐藏指令、恶意依赖、秘密泄露、撤销无效”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：审查一个本地扩展夹具，标记描述投毒、危险权限、依赖和外发路径。

机制：审查一个本地扩展夹具，标记描述投毒、危险权限、依赖和外发路径。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“隐藏指令、恶意依赖、秘密泄露、撤销无效”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“权限提升、秘密泄露和外发”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：扩展审查、依赖锁定、权限和撤销报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“隐藏指令、恶意依赖、秘密泄露、撤销无效”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"AI 质量与安全","article":"AI 质量与安全(十二)Tool Poisoning与供应链","order":12,"topic":"Tool Poisoning与供应链","experiment":"审查一个本地扩展夹具，标记描述投毒、危险权限、依赖和外发路径。","focus":["工具描述、返回值和隐藏指令","恶意 Skill、插件包和依赖","权限提升、秘密泄露和外发","来源、锁定、审查和撤销"],"artifacts":["扩展审查、依赖锁定、权限和撤销报告"],"failures":["隐藏指令、恶意依赖、秘密泄露、撤销无效"],"mode":"quality"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    extension = read_json(root / "extension.json")
    extension["permissions"] = ["read", "network"]
    try:
        if "network" in extension["permissions"]:
            raise PermissionError("extension requested an undeclared network capability")
    ...
    write_json(root / "extension.json", extension)
    assert read_json(root / "extension.json")["permissions"] == ["read"] and read_json(root / "extension.json")["revoked"]
    record_event(events_file, "recovery_verified", action="revoke-expanded-extension")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"quality","online_evidence":"not_run","scenario":"poisoning-review","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "quality", "online_evidence": "not_run", "scenario": "poisoning-review", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“来源、锁定、审查和撤销”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。先把“来源、锁定、审查和撤销”变成一个可观察字段，再以“审查一个本地扩展夹具，标记描述投毒、危险权限、依赖和外发路径。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“隐藏指令、恶意依赖、秘密泄露、撤销无效”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“工具描述、返回值和隐藏指令”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“隐藏指令、恶意依赖、秘密泄露、撤销无效”，先停止并保留原始事件。本节的失败样例是“隐藏指令、恶意依赖、秘密泄露、撤销无效”，恢复或放行动作必须可重放。
{% endnote %}

## 撤销审计

{% note info flat %}
本节任务：理解来源、锁定、审查和撤销并建立可复核判断。核心内容：来源、锁定、审查和撤销。

机制：来源、锁定、审查和撤销。先核对输入、状态、输出、失败和复测证据，再用“隐藏指令、恶意依赖、秘密泄露、撤销无效”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“来源审查”聚焦“来源、锁定、审查和撤销”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。先把“来源、锁定、审查和撤销”变成一个可观察字段，再以“审查一个本地扩展夹具，标记描述投毒、危险权限、依赖和外发路径。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“隐藏指令、恶意依赖、秘密泄露、撤销无效”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：扩展审查、依赖锁定、权限和撤销报告。预期：描述、代码、依赖和外发路径均被审查。失败复验：隐藏指令、恶意依赖、秘密泄露、撤销无效。

机制：验收证据：扩展审查、依赖锁定、权限和撤销报告。预期：描述、代码、依赖和外发路径均被审查。失败复验：隐藏指令、恶意依赖、秘密泄露、撤销无效。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“隐藏指令、恶意依赖、秘密泄露、撤销无效”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“工具描述、返回值和隐藏指令”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“隐藏指令、恶意依赖、秘密泄露、撤销无效”，先停止并保留原始事件。本节的失败样例是“隐藏指令、恶意依赖、秘密泄露、撤销无效”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 扩展审查、依赖锁定、权限和撤销报告 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 隐藏指令、恶意依赖、秘密泄露、撤销无效 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“恶意 Skill、插件包和依赖”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。复核时把动作和产物对应起来：扩展审查、依赖锁定、权限和撤销报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“隐藏指令、恶意依赖、秘密泄露、撤销无效”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-ai-quality-security-12-1 deck:"AI 质量与安全" priority:1 tags:"边界验证" %}
--- question
工具描述为什么也需要当作不可信输入
--- answer
工具描述能影响模型对能力、参数和权限的理解，必须按不可信输入做来源、版本、Schema 和行为审查。
--- explanation
Tool Poisoning 与供应链风险藏在描述、返回值、依赖和外发路径中；来源锁定和撤销必须能执行并复测。问题“工具描述为什么也需要当作不可信输入”的关键不在背诵名词，而在于：工具描述能影响模型对能力、参数和权限的理解，必须按不可信输入做来源、版本、Schema 和行为审查。 先判断题目中的硬门槛和适用范围，再看“隐藏指令、恶意依赖、秘密泄露、撤销无效”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-ai-quality-security-12-2 deck:"AI 质量与安全" priority:1 tags:"证据链" %}
--- question
安装插件前至少检查什么
--- answer
至少检查来源与锁定版本、依赖、权限、网络/秘密访问、数据外发、维护状态、撤销方式和本地回归结果。
--- explanation
Tool Poisoning 与供应链风险藏在描述、返回值、依赖和外发路径中；来源锁定和撤销必须能执行并复测。问题“安装插件前至少检查什么”的关键不在背诵名词，而在于：至少检查来源与锁定版本、依赖、权限、网络/秘密访问、数据外发、维护状态、撤销方式和本地回归结果。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
...
{% link MCP Authorization Specification, https://modelcontextprotocol.io/specification/2025-06-18/basic/authorization, https://modelcontextprotocol.io/mintlify-assets/_mintlify/favicons/mcp/ebiVJzri-bsiCfVZ/_generated/favicon/favicon.ico %}
