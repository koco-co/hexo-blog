...
{% note primary flat %}
本节围绕“红队与故障响应”建立可复核的工作模型。学习成果：事件证据、责任、通知和恢复动作完整。实验：对本地 Agent 注入工具失败、秘密泄露尝试和错误记忆，完成事件报告。视觉辅助：事件响应流程图；红队用例表。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
红队报告只建议修改 Prompt，工具仍能读取秘密。检测、遏制、证据保全、修复、通知和复测必须闭环。

本篇把“红队与故障响应”落到一次可回放的本地任务：对本地 Agent 注入工具失败、秘密泄露尝试和错误记忆，完成事件报告。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：事件证据、责任、通知和恢复动作完整。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“威胁场景、攻击前提和影响；注入、越权、泄露、错误工具和供应链故障；检测、遏制、修复和复盘”拆成输入、状态、输出和证据。红队与故障响应把检测、遏制、证据保全、修复、通知和复测串成事件闭环，不能只改 Prompt。
{% endnote %}
...
{% note primary flat %}
实验步骤：对本地 Agent 注入工具失败、秘密泄露尝试和错误记忆，完成事件报告。

验收产物：攻击/故障用例、遏制、修复和复测报告

失败注入：只修 Prompt、不隔离工具、证据丢失
{% endnote %}

## 攻击场景

{% note info flat %}
本节任务：理解威胁场景、攻击前提和影响并建立可复核判断。核心内容：威胁场景、攻击前提和影响。

机制：威胁场景、攻击前提和影响。先核对输入、状态、输出、失败和复测证据，再用“只修 Prompt、不隔离工具、证据丢失”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
Codex CLI 先通过 npm 安装并用 codex --version 记录入口；启动成功与 OpenAI 登录、模型可用是独立检查。本节围绕“攻击场景”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“只修 Prompt、不隔离工具、证据丢失”，对应产物为攻击/故障用例、遏制、修复和复测报告。
{% endnote %}

### 攻击类型

{% note info flat %}
Codex CLI 先通过 npm 安装并用 codex --version 记录入口；启动成功与 OpenAI 登录、模型可用是独立检查。本节围绕“攻击类型”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“只修 Prompt、不隔离工具、证据丢失”，对应产物为攻击/故障用例、遏制、修复和复测报告。
{% endnote %}

## 事件处置

{% note info flat %}
本节任务：理解注入、越权、泄露、错误工具和供应链故障并建立可复核判断。核心内容：注入、越权、泄露、错误工具和供应链故障。

机制：注入、越权、泄露、错误工具和供应链故障。先核对输入、状态、输出、失败和复测证据，再用“只修 Prompt、不隔离工具、证据丢失”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
codex exec 适合可记录的一次性任务，交互模式适合连续协作；两者都要把输入、工作目录和输出边界写进任务记录。本节围绕“事件处置”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“只修 Prompt、不隔离工具、证据丢失”，对应产物为攻击/故障用例、遏制、修复和复测报告。
{% endnote %}

### 通知证据

{% note info flat %}
codex exec 适合可记录的一次性任务，交互模式适合连续协作；两者都要把输入、工作目录和输出边界写进任务记录。本节围绕“通知证据”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“只修 Prompt、不隔离工具、证据丢失”，对应产物为攻击/故障用例、遏制、修复和复测报告。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：对本地 Agent 注入工具失败、秘密泄露尝试和错误记忆，完成事件报告。

机制：对本地 Agent 注入工具失败、秘密泄露尝试和错误记忆，完成事件报告。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“只修 Prompt、不隔离工具、证据丢失”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
sandbox_mode 和 approval_policy 共同决定执行面；先用只读/低权限基线，再明确哪一个高风险动作需要审批。本节围绕“准备输入”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“只修 Prompt、不隔离工具、证据丢失”，对应产物为攻击/故障用例、遏制、修复和复测报告。
{% endnote %}
...
DATA = json.loads('{"series":"AI 质量与安全","article":"AI 质量与安全(十五)红队与故障响应","order":15,"topic":"红队与故障响应","experiment":"对本地 Agent 注入工具失败、秘密泄露尝试和错误记忆，完成事件报告。","focus":["威胁场景、攻击前提和影响","注入、越权、泄露、错误工具和供应链故障","检测、遏制、修复和复盘","证据保全与用户通知"],"artifacts":["攻击/故障用例、遏制、修复和复测报告"],"failures":["只修 Prompt、不隔离工具、证据丢失"],"mode":"quality"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    incident = read_json(root / "incident.json")
    try:
        if "contain" not in incident["states"]:
            raise RuntimeError("incident has no containment state")
        incident["states"].remove("contain")
        if "contain" not in incident["states"]:
            raise RuntimeError("tool path was not isolated")
    ...
    incident["states"] = ["detect", "contain", "preserve", "repair"]
    write_json(root / "incident.json", incident)
    assert read_json(root / "incident.json")["states"] == ["detect", "contain", "preserve", "repair"]
    record_event(events_file, "recovery_verified", action="restore-containment-and-evidence")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"quality","online_evidence":"not_run","scenario":"incident-response","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "quality", "online_evidence": "not_run", "scenario": "incident-response", "status": "pass"}
```

### 执行步骤

{% note info flat %}
sandbox_mode 和 approval_policy 共同决定执行面；先用只读/低权限基线，再明确哪一个高风险动作需要审批。本节围绕“执行步骤”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“只修 Prompt、不隔离工具、证据丢失”，对应产物为攻击/故障用例、遏制、修复和复测报告。
{% endnote %}

### 观察输出

{% note info flat %}
sandbox_mode 和 approval_policy 共同决定执行面；先用只读/低权限基线，再明确哪一个高风险动作需要审批。本节围绕“观察输出”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“只修 Prompt、不隔离工具、证据丢失”，对应产物为攻击/故障用例、遏制、修复和复测报告。
{% endnote %}

## 复盘证据

{% note info flat %}
本节任务：理解证据保全与用户通知并建立可复核判断。核心内容：证据保全与用户通知。

机制：证据保全与用户通知。先核对输入、状态、输出、失败和复测证据，再用“只修 Prompt、不隔离工具、证据丢失”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
AGENTS.md 是仓库约定的输入来源，不能替代沙箱；冲突时保留文件层级、加载顺序和被拒绝的动作。本节围绕“通知证据”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“只修 Prompt、不隔离工具、证据丢失”，对应产物为攻击/故障用例、遏制、修复和复测报告。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：攻击/故障用例、遏制、修复和复测报告。预期：事件证据、责任、通知和恢复动作完整。失败复验：只修 Prompt、不隔离工具、证据丢失。

机制：验收证据：攻击/故障用例、遏制、修复和复测报告。预期：事件证据、责任、通知和恢复动作完整。失败复验：只修 Prompt、不隔离工具、证据丢失。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“只修 Prompt、不隔离工具、证据丢失”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
Skills、MCP 与 Subagents 会扩大上下文和能力发现范围；锁定来源、参数 Schema、网络权限和取消条件后再合并结果。本节围绕“验收证据”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“只修 Prompt、不隔离工具、证据丢失”，对应产物为攻击/故障用例、遏制、修复和复测报告。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 攻击/故障用例、遏制、修复和复测报告 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 只修 Prompt、不隔离工具、证据丢失 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
Skills、MCP 与 Subagents 会扩大上下文和能力发现范围；锁定来源、参数 Schema、网络权限和取消条件后再合并结果。本节围绕“复测动作”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“只修 Prompt、不隔离工具、证据丢失”，对应产物为攻击/故障用例、遏制、修复和复测报告。
{% endnote %}
...
{% flashcard basic id:ai-ai-quality-security-15-1 deck:"AI 质量与安全" priority:1 tags:"边界验证" %}
--- question
安全测试如何区分漏洞和模型随机错误
--- answer
用重复性、控制组、边界样例和 Trace 区分漏洞；随机模型错误不能仅凭一次异常就定为安全漏洞。
--- explanation
红队与故障响应把检测、遏制、证据保全、修复、通知和复测串成事件闭环，不能只改 Prompt。问题“安全测试如何区分漏洞和模型随机错误”的关键不在背诵名词，而在于：用重复性、控制组、边界样例和 Trace 区分漏洞；随机模型错误不能仅凭一次异常就定为安全漏洞。 先判断题目中的硬门槛和适用范围，再看“只修 Prompt、不隔离工具、证据丢失”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-ai-quality-security-15-2 deck:"AI 质量与安全" priority:1 tags:"证据链" %}
--- question
事件响应的第一步是什么
--- answer
第一步是检测并确认事件范围，同时停止扩散或高风险动作、保存证据，再进入遏制、修复和复测。
--- explanation
红队与故障响应把检测、遏制、证据保全、修复、通知和复测串成事件闭环，不能只改 Prompt。问题“事件响应的第一步是什么”的关键不在背诵名词，而在于：第一步是检测并确认事件范围，同时停止扩散或高风险动作、保存证据，再进入遏制、修复和复测。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
