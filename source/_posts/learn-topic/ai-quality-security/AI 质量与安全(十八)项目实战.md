...
categories:
  - Learn Topic
  - AI 质量与安全
description: 为一个 Agent 应用建立评测集、安全测试、轨迹审计和 CI 门禁。
...
{% note primary flat %}
本节围绕“AI 质量与安全(十八)项目实战”建立可复核的工作模型。学习成果：为一个 Agent 应用建立评测集、安全测试、轨迹审计和 CI 门禁。实验：使用本地 Harness 和脱敏数据完成一轮可重复评测，输出报告、失败样例、修复和复测证据。视觉辅助：质量与安全流水线 Mermaid；门禁矩阵。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
项目报告只有一张总分表，无法回答失败来自数据、权限还是工具。质量、安全、Trace、门禁和回滚证据必须汇合。

本篇把“AI 质量与安全(十八)项目实战”落到一次可回放的本地任务：使用本地 Harness 和脱敏数据完成一轮可重复评测，输出报告、失败样例、修复和复测证据。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：为一个 Agent 应用建立评测集、安全测试、轨迹审计和 CI 门禁。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“质量模型、数据集、评测器和安全场景；运行回归、权限测试、注入测试和故障注入；形成报告、门禁、回滚和责任记录”拆成输入、状态、输出和证据。项目实战将数据、评测、权限、安全、Trace、门禁和回滚汇合，同时明确未覆盖场景和不作安全证明的边界。
{% endnote %}
...
{% note primary flat %}
实验步骤：使用本地 Harness 和脱敏数据完成一轮可重复评测，输出报告、失败样例、修复和复测证据。
...
{% note info flat %}
本节任务：理解质量模型、数据集、评测器和安全场景并建立可复核判断。核心内容：质量模型、数据集、评测器和安全场景。

机制：质量模型、数据集、评测器和安全场景。先核对输入、状态、输出、失败和复测证据，再用“真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“需求和非目标”聚焦“质量模型、数据集、评测器和安全场景”。把模型/Provider 选择、认证状态和任务输入分成三列；凭据缺失时应停止在本地静态演示，不输出看似真实的服务响应。先把“质量模型、数据集、评测器和安全场景”变成一个可观察字段，再以“使用本地 Harness 和脱敏数据完成一轮可重复评测，输出报告、失败样例、修复和复测证据。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围”，恢复或放行动作必须可重放。
{% endnote %}

### 验收证据

{% note info flat %}
“验收证据”聚焦“运行回归、权限测试、注入测试和故障注入”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围”，先停止并保留原始事件。本节的失败样例是“真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围”，恢复或放行动作必须可重放。
{% endnote %}

## 架构设计

{% note info flat %}
本节任务：理解运行回归、权限测试、注入测试和故障注入并建立可复核判断。核心内容：运行回归、权限测试、注入测试和故障注入。

机制：运行回归、权限测试、注入测试和故障注入。先核对输入、状态、输出、失败和复测证据，再用“真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“能力组合”聚焦“运行回归、权限测试、注入测试和故障注入”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“能力组合”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围”，先停止并保留原始事件。本节的失败样例是“真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围”，恢复或放行动作必须可重放。
{% endnote %}

### 证据链

{% note info flat %}
“证据链”聚焦“形成报告、门禁、回滚和责任记录”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。复核时把动作和产物对应起来：项目测试、评测报告、权限矩阵、脱敏 Trace 和回滚记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围”，恢复或放行动作必须可重放。
{% endnote %}

## 实现步骤

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：使用本地 Harness 和脱敏数据完成一轮可重复评测，输出报告、失败样例、修复和复测证据。

机制：使用本地 Harness 和脱敏数据完成一轮可重复评测，输出报告、失败样例、修复和复测证据。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“最小闭环”聚焦“形成报告、门禁、回滚和责任记录”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。复核时把动作和产物对应起来：项目测试、评测报告、权限矩阵、脱敏 Trace 和回滚记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"AI 质量与安全","article":"AI 质量与安全(十八)项目实战","order":18,"topic":"AI 质量与安全(十八)项目实战","experiment":"使用本地 Harness 和脱敏数据完成一轮可重复评测，输出报告、失败样例、修复和复测证据。","focus":["质量模型、数据集、评测器和安全场景","运行回归、权限测试、注入测试和故障注入","形成报告、门禁、回滚和责任记录","不声称安全证明或覆盖所有未知攻击"],"artifacts":["项目测试、评测报告、权限矩阵、脱敏 Trace 和回滚记录"],"failures":["真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围"],"mode":"project"}')
SCENARIOS = json.loads('{"18":{"scenario":"comparison-project","checks":4}}')
...
{% note info flat %}
“组合能力”聚焦“不声称安全证明或覆盖所有未知攻击”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。先把“不声称安全证明或覆盖所有未知攻击”变成一个可观察字段，再以“使用本地 Harness 和脱敏数据完成一轮可重复评测，输出报告、失败样例、修复和复测证据。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围”，恢复或放行动作必须可重放。
{% endnote %}

### 故障注入

{% note info flat %}
“故障注入”聚焦“质量模型、数据集、评测器和安全场景”。把模型/Provider 选择、认证状态和任务输入分成三列；凭据缺失时应停止在本地静态演示，不输出看似真实的服务响应。具体取舍是：保留“故障注入”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围”，先停止并保留原始事件。本节的失败样例是“真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围”，恢复或放行动作必须可重放。
{% endnote %}
...
机制：真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围。选择依据：风险或可恢复失败需要直接可见。先核对限制、风险、恢复或替代动作，再用“真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围”作为反例；去掉颜色后仍保留完整文字。
{% endnote %}
...
{% note info flat %}
“质量证据”聚焦“不声称安全证明或覆盖所有未知攻击”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。先把“不声称安全证明或覆盖所有未知攻击”变成一个可观察字段，再以“使用本地 Harness 和脱敏数据完成一轮可重复评测，输出报告、失败样例、修复和复测证据。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围”，恢复或放行动作必须可重放。
{% endnote %}

### 残余风险

{% note info flat %}
“残余风险”聚焦“质量模型、数据集、评测器和安全场景”。把模型/Provider 选择、认证状态和任务输入分成三列；凭据缺失时应停止在本地静态演示，不输出看似真实的服务响应。具体取舍是：保留“残余风险”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围”，先停止并保留原始事件。本节的失败样例是“真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-ai-quality-security-18-1 deck:"AI 质量与安全" priority:1 tags:"边界验证" %}
--- question
AI 应用上线前的最小质量安全证据是什么
--- answer
最小证据包括版本化数据与任务、质量指标/基线、失败样例、权限与安全测试、脱敏 Trace、门禁决定和回滚方案。
--- explanation
项目实战将数据、评测、权限、安全、Trace、门禁和回滚汇合，同时明确未覆盖场景和不作安全证明的边界。问题“AI 应用上线前的最小质量安全证据是什么”的关键不在背诵名词，而在于：最小证据包括版本化数据与任务、质量指标/基线、失败样例、权限与安全测试、脱敏 Trace、门禁决定和回滚方案。 先判断题目中的硬门槛和适用范围，再看“真实生产安全、完整模型覆盖和所有未知攻击不在项目证明范围”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-ai-quality-security-18-2 deck:"AI 质量与安全" priority:1 tags:"证据链" %}
--- question
如何表达评测覆盖范围而不夸大
--- answer
明确评测集范围、样本数、版本、未覆盖场景、失败率和线上未验证项；覆盖范围的边界要与结论一起展示。
--- explanation
项目实战将数据、评测、权限、安全、Trace、门禁和回滚汇合，同时明确未覆盖场景和不作安全证明的边界。问题“如何表达评测覆盖范围而不夸大”的关键不在背诵名词，而在于：明确评测集范围、样本数、版本、未覆盖场景、失败率和线上未验证项；覆盖范围的边界要与结论一起展示。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
