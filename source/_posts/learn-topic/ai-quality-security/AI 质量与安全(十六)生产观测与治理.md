...
categories:
  - Learn Topic
  - AI 质量与安全
description: 模型/Prompt/Tool/Skill 变更可追踪和回滚
...
{% note primary flat %}
本节围绕“生产观测与治理”建立可复核的工作模型。学习成果：模型/Prompt/Tool/Skill 变更可追踪和回滚。实验：为一个 Agent 应用设计上线检查、回滚条件和月度复评记录。视觉辅助：生产治理闭环；版本矩阵。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
版本升级后成本翻倍，系统静默降级却没有责任人。SLO、版本快照、变更审批、回滚和退出机制把观测变成治理。

本篇把“生产观测与治理”落到一次可回放的本地任务：为一个 Agent 应用设计上线检查、回滚条件和月度复评记录。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：模型/Prompt/Tool/Skill 变更可追踪和回滚。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“SLO、质量门槛和版本快照；成本、延迟、容量和降级；模型/Prompt/Tool/Skill 变更治理”拆成输入、状态、输出和证据。生产治理把版本快照、SLO、成本、延迟、降级、责任、审批、回滚和退出机制放在同一变更证据中。
{% endnote %}
...
{% note primary flat %}
实验步骤：为一个 Agent 应用设计上线检查、回滚条件和月度复评记录。

验收产物：版本门禁、SLO、成本、回滚和责任记录

失败注入：静默降级、无版本、成本失控、责任不清
{% endnote %}

## 版本门禁

{% note info flat %}
本节任务：理解SLO、质量门槛和版本快照并建立可复核判断。核心内容：SLO、质量门槛和版本快照。

机制：SLO、质量门槛和版本快照。先核对输入、状态、输出、失败和复测证据，再用“静默降级、无版本、成本失控、责任不清”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
ZCode 以桌面安装包/AppImage 为主要入口；安装包可启动只证明客户端存在，还要记录 GLM Coding Plan/API 的连接状态。本节围绕“版本门禁”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“静默降级、无版本、成本失控、责任不清”，对应产物为版本门禁、SLO、成本、回滚和责任记录。
{% endnote %}

### SLO与成本

{% note info flat %}
ZCode 以桌面安装包/AppImage 为主要入口；安装包可启动只证明客户端存在，还要记录 GLM Coding Plan/API 的连接状态。本节围绕“SLO与成本”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“静默降级、无版本、成本失控、责任不清”，对应产物为版本门禁、SLO、成本、回滚和责任记录。
{% endnote %}

## SLO与成本

{% note info flat %}
本节任务：理解成本、延迟、容量和降级并建立可复核判断。核心内容：成本、延迟、容量和降级。

机制：成本、延迟、容量和降级。先核对输入、状态、输出、失败和复测证据，再用“静默降级、无版本、成本失控、责任不清”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
GLM 模型、Coding Plan 与普通 API 入口可能有不同额度和权限；把 endpoint、model、套餐和失败码分别记录。本节围绕“变更治理”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“静默降级、无版本、成本失控、责任不清”，对应产物为版本门禁、SLO、成本、回滚和责任记录。
{% endnote %}

### 责任治理

{% note info flat %}
GLM 模型、Coding Plan 与普通 API 入口可能有不同额度和权限；把 endpoint、model、套餐和失败码分别记录。本节围绕“责任治理”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“静默降级、无版本、成本失控、责任不清”，对应产物为版本门禁、SLO、成本、回滚和责任记录。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：为一个 Agent 应用设计上线检查、回滚条件和月度复评记录。

机制：为一个 Agent 应用设计上线检查、回滚条件和月度复评记录。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“静默降级、无版本、成本失控、责任不清”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
代码任务先固定项目目录和上下文文件，再观察 ZCode 生成的 Diff；不要让插件市场内容成为隐式规则。本节围绕“准备输入”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“静默降级、无版本、成本失控、责任不清”，对应产物为版本门禁、SLO、成本、回滚和责任记录。
{% endnote %}
...
DATA = json.loads('{"series":"AI 质量与安全","article":"AI 质量与安全(十六)生产观测与治理","order":16,"topic":"生产观测与治理","experiment":"为一个 Agent 应用设计上线检查、回滚条件和月度复评记录。","focus":["SLO、质量门槛和版本快照","成本、延迟、容量和降级","模型/Prompt/Tool/Skill 变更治理","责任分配、审计和退出机制"],"artifacts":["版本门禁、SLO、成本、回滚和责任记录"],"failures":["静默降级、无版本、成本失控、责任不清"],"mode":"quality"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    governance = read_json(root / "governance.json")
    governance["slo"]["cost"] = False
    try:
        if not governance["slo"]["cost"]:
            raise RuntimeError("cost SLO regression requires rollback")
    ...
    governance["slo"]["cost"] = True
    governance["rollback"] = True
    write_json(root / "governance.json", governance)
    assert all(read_json(root / "governance.json")["slo"].values())
    record_event(events_file, "recovery_verified", action="rollback-cost-regression")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"quality","online_evidence":"not_run","scenario":"governance-slo","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "quality", "online_evidence": "not_run", "scenario": "governance-slo", "status": "pass"}
```

### 执行步骤

{% note info flat %}
代码任务先固定项目目录和上下文文件，再观察 ZCode 生成的 Diff；不要让插件市场内容成为隐式规则。本节围绕“执行步骤”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“静默降级、无版本、成本失控、责任不清”，对应产物为版本门禁、SLO、成本、回滚和责任记录。
{% endnote %}

### 观察输出

{% note info flat %}
代码任务先固定项目目录和上下文文件，再观察 ZCode 生成的 Diff；不要让插件市场内容成为隐式规则。本节围绕“观察输出”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“静默降级、无版本、成本失控、责任不清”，对应产物为版本门禁、SLO、成本、回滚和责任记录。
{% endnote %}

## 责任治理

{% note info flat %}
本节任务：理解责任分配、审计和退出机制并建立可复核判断。核心内容：责任分配、审计和退出机制。

机制：责任分配、审计和退出机制。先核对输入、状态、输出、失败和复测证据，再用“静默降级、无版本、成本失控、责任不清”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
插件市场的来源、版本和 MCP 权限决定扩展面；安装前列出读写、网络和秘密访问，撤销后复查配置。本节围绕“责任治理”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“静默降级、无版本、成本失控、责任不清”，对应产物为版本门禁、SLO、成本、回滚和责任记录。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：版本门禁、SLO、成本、回滚和责任记录。预期：模型/Prompt/Tool/Skill 变更可追踪和回滚。失败复验：静默降级、无版本、成本失控、责任不清。

机制：验收证据：版本门禁、SLO、成本、回滚和责任记录。预期：模型/Prompt/Tool/Skill 变更可追踪和回滚。失败复验：静默降级、无版本、成本失控、责任不清。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“静默降级、无版本、成本失控、责任不清”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
权限确认应对应一次写入、命令或外发动作，并可在 Diff/测试中回溯；静态产品图不证明它真的执行过。本节围绕“验收证据”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“静默降级、无版本、成本失控、责任不清”，对应产物为版本门禁、SLO、成本、回滚和责任记录。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 版本门禁、SLO、成本、回滚和责任记录 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 静默降级、无版本、成本失控、责任不清 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
权限确认应对应一次写入、命令或外发动作，并可在 Diff/测试中回溯；静态产品图不证明它真的执行过。本节围绕“复测动作”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“静默降级、无版本、成本失控、责任不清”，对应产物为版本门禁、SLO、成本、回滚和责任记录。
{% endnote %}
...
{% flashcard basic id:ai-ai-quality-security-16-1 deck:"AI 质量与安全" priority:1 tags:"边界验证" %}
--- question
动态模型上线前必须冻结什么
--- answer
上线前冻结模型/提示/工具/数据/权限/评测版本及成本、延迟和安全基线，确保回归可比较。
--- explanation
生产治理把版本快照、SLO、成本、延迟、降级、责任、审批、回滚和退出机制放在同一变更证据中。问题“动态模型上线前必须冻结什么”的关键不在背诵名词，而在于：上线前冻结模型/提示/工具/数据/权限/评测版本及成本、延迟和安全基线，确保回归可比较。 先判断题目中的硬门槛和适用范围，再看“静默降级、无版本、成本失控、责任不清”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-ai-quality-security-16-2 deck:"AI 质量与安全" priority:1 tags:"证据链" %}
--- question
质量和安全门禁如何避免互相绕过
--- answer
把质量和安全设为独立且不可绕过的硬门禁，共享同一版本与审计记录；任一失败都停止或回滚。
--- explanation
生产治理把版本快照、SLO、成本、延迟、降级、责任、审批、回滚和退出机制放在同一变更证据中。问题“质量和安全门禁如何避免互相绕过”的关键不在背诵名词，而在于：把质量和安全设为独立且不可绕过的硬门禁，共享同一版本与审计记录；任一失败都停止或回滚。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
