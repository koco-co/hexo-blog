...
{% note primary flat %}
本节围绕“指标、统计与实验”建立可复核的工作模型。学习成果：结论说明样本量和不确定性。实验：用本地结果计算指标、置信区间和差异，演示小样本下不能下结论。视觉辅助：实验流程图；统计表。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
两次运行给出不同胜率，报告却只保留较好的一次。重复实验、基线、样本量和区间让“变好”带上不确定性。

本篇把“指标、统计与实验”落到一次可回放的本地任务：用本地结果计算指标、置信区间和差异，演示小样本下不能下结论。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：结论说明样本量和不确定性。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“准确率、召回率、通过率、胜率和成本；随机性、重复实验和置信区间；A/B、基线、样本量和显著性”拆成输入、状态、输出和证据。统计结论需要指标口径、重复运行、基线、样本量和不确定性；单次胜率不能承担升级决策。
{% endnote %}
...
{% note primary flat %}
实验步骤：用本地结果计算指标、置信区间和差异，演示小样本下不能下结论。

验收产物：重复实验、基线、置信区间和回归报告
...
{% note info flat %}
本节任务：理解准确率、召回率、通过率、胜率和成本并建立可复核判断。核心内容：准确率、召回率、通过率、胜率和成本。

机制：准确率、召回率、通过率、胜率和成本。先核对输入、状态、输出、失败和复测证据，再用“单次运行、基线缺失、指标作弊”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“指标与成本”聚焦“准确率、召回率、通过率、胜率和成本”。先固定分母、基线、版本和观测窗口，再解释指标；平均分或一次成功不能替代难例、长尾、成本和失败归因。先把“准确率、召回率、通过率、胜率和成本”变成一个可观察字段，再以“用本地结果计算指标、置信区间和差异，演示小样本下不能下结论。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“单次运行、基线缺失、指标作弊”，恢复或放行动作必须可重放。
{% endnote %}

### 重复与区间

{% note info flat %}
“重复与区间”聚焦“随机性、重复实验和置信区间”。围绕“随机性、重复实验和置信区间”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“重复与区间”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“单次运行、基线缺失、指标作弊”，先停止并保留原始事件。本节的失败样例是“单次运行、基线缺失、指标作弊”，恢复或放行动作必须可重放。
{% endnote %}

## 统计置信

{% note info flat %}
本节任务：理解随机性、重复实验和置信区间并建立可复核判断。核心内容：随机性、重复实验和置信区间。

机制：随机性、重复实验和置信区间。先核对输入、状态、输出、失败和复测证据，再用“单次运行、基线缺失、指标作弊”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“基线与显著性”聚焦“随机性、重复实验和置信区间”。围绕“随机性、重复实验和置信区间”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“基线与显著性”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“单次运行、基线缺失、指标作弊”，先停止并保留原始事件。本节的失败样例是“单次运行、基线缺失、指标作弊”，恢复或放行动作必须可重放。
{% endnote %}

### 归因检查

{% note info flat %}
“归因检查”聚焦“A/B、基线、样本量和显著性”。围绕“A/B、基线、样本量和显著性”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：重复实验、基线、置信区间和回归报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“单次运行、基线缺失、指标作弊”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：用本地结果计算指标、置信区间和差异，演示小样本下不能下结论。

机制：用本地结果计算指标、置信区间和差异，演示小样本下不能下结论。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“单次运行、基线缺失、指标作弊”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“A/B、基线、样本量和显著性”。围绕“A/B、基线、样本量和显著性”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：重复实验、基线、置信区间和回归报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“单次运行、基线缺失、指标作弊”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"AI 质量与安全","article":"AI 质量与安全(四)指标、统计与实验","order":4,"topic":"指标、统计与实验","experiment":"用本地结果计算指标、置信区间和差异，演示小样本下不能下结论。","focus":["准确率、召回率、通过率、胜率和成本","随机性、重复实验和置信区间","A/B、基线、样本量和显著性","指标作弊与错误归因"],"artifacts":["重复实验、基线、置信区间和回归报告"],"failures":["单次运行、基线缺失、指标作弊"],"mode":"quality"}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    scores = read_json(root / "repeated-scores.json")["scores"]
    single_run = [scores[0]]
    try:
        if len(single_run) < 2:
            raise ValueError("a single run cannot support a variance claim")
    ...
    assert len(scores) == 3 and min(scores) >= 0 and max(scores) <= 1
    record_event(events_file, "recovery_verified", action="retain-repeated-baseline")
...
assert result == {"artifact_count":1,"checks":4,"failure_count":1,"mode":"quality","online_evidence":"not_run","scenario":"metric-repeat","status":"pass"}
...
```text
{"artifact_count": 1, "checks": 4, "failure_count": 1, "mode": "quality", "online_evidence": "not_run", "scenario": "metric-repeat", "status": "pass"}
```

### 执行步骤

{% note info flat %}
“执行步骤”聚焦“指标作弊与错误归因”。先固定分母、基线、版本和观测窗口，再解释指标；平均分或一次成功不能替代难例、长尾、成本和失败归因。先把“指标作弊与错误归因”变成一个可观察字段，再以“用本地结果计算指标、置信区间和差异，演示小样本下不能下结论。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“单次运行、基线缺失、指标作弊”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“准确率、召回率、通过率、胜率和成本”。先固定分母、基线、版本和观测窗口，再解释指标；平均分或一次成功不能替代难例、长尾、成本和失败归因。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“单次运行、基线缺失、指标作弊”，先停止并保留原始事件。本节的失败样例是“单次运行、基线缺失、指标作弊”，恢复或放行动作必须可重放。
{% endnote %}

## 回归判断

{% note info flat %}
本节任务：理解指标作弊与错误归因并建立可复核判断。核心内容：指标作弊与错误归因。

机制：指标作弊与错误归因。先核对输入、状态、输出、失败和复测证据，再用“单次运行、基线缺失、指标作弊”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“归因检查”聚焦“指标作弊与错误归因”。先固定分母、基线、版本和观测窗口，再解释指标；平均分或一次成功不能替代难例、长尾、成本和失败归因。先把“指标作弊与错误归因”变成一个可观察字段，再以“用本地结果计算指标、置信区间和差异，演示小样本下不能下结论。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“单次运行、基线缺失、指标作弊”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：重复实验、基线、置信区间和回归报告。预期：结论说明样本量和不确定性。失败复验：单次运行、基线缺失、指标作弊。

机制：验收证据：重复实验、基线、置信区间和回归报告。预期：结论说明样本量和不确定性。失败复验：单次运行、基线缺失、指标作弊。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“单次运行、基线缺失、指标作弊”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“准确率、召回率、通过率、胜率和成本”。先固定分母、基线、版本和观测窗口，再解释指标；平均分或一次成功不能替代难例、长尾、成本和失败归因。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“单次运行、基线缺失、指标作弊”，先停止并保留原始事件。本节的失败样例是“单次运行、基线缺失、指标作弊”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 重复实验、基线、置信区间和回归报告 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 单次运行、基线缺失、指标作弊 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“随机性、重复实验和置信区间”。围绕“随机性、重复实验和置信区间”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：重复实验、基线、置信区间和回归报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“单次运行、基线缺失、指标作弊”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-ai-quality-security-4-1 deck:"AI 质量与安全" priority:1 tags:"边界验证" %}
--- question
为什么一次运行不能证明模型更好
--- answer
一次运行受随机性、样本和环境影响，不能证明模型更好；至少需要重复运行、区间和预先定义的比较规则。
--- explanation
统计结论需要指标口径、重复运行、基线、样本量和不确定性；单次胜率不能承担升级决策。问题“为什么一次运行不能证明模型更好”的关键不在背诵名词，而在于：一次运行受随机性、样本和环境影响，不能证明模型更好；至少需要重复运行、区间和预先定义的比较规则。 先判断题目中的硬门槛和适用范围，再看“单次运行、基线缺失、指标作弊”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-ai-quality-security-4-2 deck:"AI 质量与安全" priority:1 tags:"证据链" %}
--- question
质量回归需要什么基线
--- answer
质量回归需要固定版本和评测集、历史基线、指标、样本量、随机种子/重复次数以及失败样例差异。
--- explanation
统计结论需要指标口径、重复运行、基线、样本量和不确定性；单次胜率不能承担升级决策。问题“质量回归需要什么基线”的关键不在背诵名词，而在于：质量回归需要固定版本和评测集、历史基线、指标、样本量、随机种子/重复次数以及失败样例差异。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
