...
--- explanation
模拟器适合快速比较 API、尺寸和逻辑路径；启动、渲染、内存可以用它建立早期趋势，但功耗、温度、后台限制和真实输入要转到固定条件的真机，并记录设备、亮度、网络和充电状态。
{% endflashcard %}
...
--- explanation
卡住可能来自网络等待、渲染掉帧、业务状态错误或关键线程无响应。保存前后日志、dumpsys 与必要的 traces，才能区分应用 ANR、服务端慢和测试环境失效。
{% endflashcard %}
