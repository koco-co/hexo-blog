...
城里的信差收到一张“请打开这扇门”的委托，却不能直接跑进宅院。他先把委托交给翻译官；翻译官把它改写成管家听得懂的指令；管家再通过城门守卫确认是哪个宅院、哪把钥匙和哪位住客。门终于打开后，回信也沿原路带回。

一次开门失败时，委托人只说“门没有开”并不够。信差可能写错了地址，翻译官可能不认识这种委托，守卫可能没有放行，宅院本身也可能已经换了门锁。每一站留下的回执，才让后来的人能分辨消息在哪一层停住。

他们后来把这段往返称为一次正式委托：在结束前，地址、翻译、守卫和宅院都保持同一组约定；无论成功还是失败，都要把这次委托关掉，不能把钥匙遗给下一位信差。

<!-- concept-story:end -->

{% note primary flat %}
故事中的信差、翻译官、守卫和宅院依次对应客户端、Appium Server、UiAutomator2 驱动与 ADB/设备上的应用。一次 WebDriver 会话固定这条调用链的目标与状态；分层日志是定位失败位置的证据，而不是“会话失败”的替代说法。
{% endnote %}
