...
--- explanation
语义定位是按控件的名称、角色或 resource id 找元素；图像自动化则比较截图、模板或 OCR (文字识别) 结果，适合没有稳定元素树的画布。先确认 Appium 语义定位确实不可用，再评估分辨率、主题、动画、匹配阈值和维护成本。图像命中仍需通过业务或系统状态断言，不能把“截图相似”当成完整功能通过。
{% endflashcard %}
...
--- explanation
设备云是在远程真实手机上运行测试的服务，能快速扩大型号、系统 API 和地域覆盖，但网络链路、镜像配置和工件下载也会成为变量。云端执行要记录镜像、设备、API、网络和可下载工件；云端失败先回本地同系统复验，区分 App 缺陷、云镜像差异和服务端/网络问题，不能只看云端绿灯。
{% endflashcard %}
