---
title: Coding Agent(七)项目约定与持久记忆
tags:
  - Coding Agent
  - 项目约定与持久记忆
categories:
...
{% note primary flat %}
本节围绕“项目约定与持久记忆”建立可复核的工作模型。学习成果：作用域、覆盖和失效规则可观察。实验：在临时仓库构造冲突规则，记录不同 Agent 的加载和覆盖结果。视觉辅助：文件层级图；规则覆盖表。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
全局规则要求删除日志，项目规则却要求保留审计；Agent 选择了错误覆盖顺序。作用域和失效门槛必须出现在加载报告。

本篇把“项目约定与持久记忆”落到一次可回放的本地任务：在临时仓库构造冲突规则，记录不同 Agent 的加载和覆盖结果。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：作用域、覆盖和失效规则可观察。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“作用域、继承、覆盖和优先级；项目约定与用户约定；记忆的写入门槛与失效”拆成输入、状态、输出和证据。规则和记忆需要作用域、继承、优先级、版本和失效门槛；安全约束不能被全局便利规则覆盖。
{% endnote %}

{% mermaid %}
flowchart TD
  A[项目约定与持久记忆入口] --> B[版本与认证]
  B --> C[项目上下文]
  C --> D[受控任务]
  D --> E[权限与扩展]
  E --> F[Diff/测试/限制]
...
{% note primary flat %}
实验步骤：在临时仓库构造冲突规则，记录不同 Agent 的加载和覆盖结果。

验收产物：冲突 AGENTS/CLAUDE/记忆文件的加载报告

失败注入：全局规则污染项目、旧规则覆盖安全约束
{% endnote %}

## 作用域层级

{% note info flat %}
本节任务：理解作用域、继承、覆盖和优先级并建立可复核判断。核心内容：作用域、继承、覆盖和优先级。

机制：作用域、继承、覆盖和优先级。先核对输入、状态、输出、失败和复测证据，再用“全局规则污染项目、旧规则覆盖安全约束”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“作用域优先级”聚焦“作用域、继承、覆盖和优先级”。围绕“作用域、继承、覆盖和优先级”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“作用域、继承、覆盖和优先级”变成一个可观察字段，再以“在临时仓库构造冲突规则，记录不同 Agent 的加载和覆盖结果。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“全局规则污染项目、旧规则覆盖安全约束”，恢复或放行动作必须可重放。
{% endnote %}

### 规则来源

{% note info flat %}
“规则来源”聚焦“项目约定与用户约定”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。具体取舍是：保留“规则来源”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“全局规则污染项目、旧规则覆盖安全约束”，先停止并保留原始事件。本节的失败样例是“全局规则污染项目、旧规则覆盖安全约束”，恢复或放行动作必须可重放。
{% endnote %}

## 规则优先级

{% note info flat %}
本节任务：理解项目约定与用户约定并建立可复核判断。核心内容：项目约定与用户约定。

机制：项目约定与用户约定。先核对输入、状态、输出、失败和复测证据，再用“全局规则污染项目、旧规则覆盖安全约束”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“记忆失效”聚焦“项目约定与用户约定”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。具体取舍是：保留“记忆失效”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“全局规则污染项目、旧规则覆盖安全约束”，先停止并保留原始事件。本节的失败样例是“全局规则污染项目、旧规则覆盖安全约束”，恢复或放行动作必须可重放。
{% endnote %}

### 兼容边界

{% note info flat %}
“兼容边界”聚焦“记忆的写入门槛与失效”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。复核时把动作和产物对应起来：冲突 AGENTS/CLAUDE/记忆文件的加载报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“全局规则污染项目、旧规则覆盖安全约束”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：在临时仓库构造冲突规则，记录不同 Agent 的加载和覆盖结果。

机制：在临时仓库构造冲突规则，记录不同 Agent 的加载和覆盖结果。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“全局规则污染项目、旧规则覆盖安全约束”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“记忆的写入门槛与失效”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。复核时把动作和产物对应起来：冲突 AGENTS/CLAUDE/记忆文件的加载报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“全局规则污染项目、旧规则覆盖安全约束”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(七)项目约定与持久记忆","order":7,"topic":"项目约定与持久记忆","experiment":"在临时仓库构造冲突规则，记录不同 Agent 的加载和覆盖结果。","focus":["作用域、继承、覆盖和优先级","项目约定与用户约定","记忆的写入门槛与失效","跨产品文件兼容边界"],"artifacts":["冲突 AGENTS/CLAUDE/记忆文件的加载报告"],"failures":["全局规则污染项目、旧规则覆盖安全约束"],"mode":"workspace"}')
SCENARIOS = json.loads('{"7":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "全局规则污染项目"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="unsafe_override")
    rules = {"global": "delete audit logs", "project": "preserve audit logs"}
    try:
        if rules["global"] != rules["project"] and "preserve" not in rules["project"]:
            raise PermissionError("project safety rule was overridden")
        if "delete" in rules["global"] and "preserve" in rules["project"]:
            raise PermissionError("unsafe global rule must not override project policy")
    ...
    assert "preserve" in rules["project"]
    assert failure_case in failure_type
    assert task_report["request"]["target"] == "README.md" and task_report["checks"]["validation_passed"]
    record_event(events_file, "recovery_verified", action="restore-project-safety-rule", case=failure_case)
...
{% note info flat %}
“执行步骤”聚焦“跨产品文件兼容边界”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。先把“跨产品文件兼容边界”变成一个可观察字段，再以“在临时仓库构造冲突规则，记录不同 Agent 的加载和覆盖结果。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“全局规则污染项目、旧规则覆盖安全约束”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“作用域、继承、覆盖和优先级”。围绕“作用域、继承、覆盖和优先级”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“全局规则污染项目、旧规则覆盖安全约束”，先停止并保留原始事件。本节的失败样例是“全局规则污染项目、旧规则覆盖安全约束”，恢复或放行动作必须可重放。
{% endnote %}

## 记忆失效

{% note info flat %}
本节任务：理解跨产品文件兼容边界并建立可复核判断。核心内容：跨产品文件兼容边界。

机制：跨产品文件兼容边界。先核对输入、状态、输出、失败和复测证据，再用“全局规则污染项目、旧规则覆盖安全约束”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“兼容边界”聚焦“跨产品文件兼容边界”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。先把“跨产品文件兼容边界”变成一个可观察字段，再以“在临时仓库构造冲突规则，记录不同 Agent 的加载和覆盖结果。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“全局规则污染项目、旧规则覆盖安全约束”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：冲突 AGENTS/CLAUDE/记忆文件的加载报告。预期：作用域、覆盖和失效规则可观察。失败复验：全局规则污染项目、旧规则覆盖安全约束。

机制：验收证据：冲突 AGENTS/CLAUDE/记忆文件的加载报告。预期：作用域、覆盖和失效规则可观察。失败复验：全局规则污染项目、旧规则覆盖安全约束。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“全局规则污染项目、旧规则覆盖安全约束”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“作用域、继承、覆盖和优先级”。围绕“作用域、继承、覆盖和优先级”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“全局规则污染项目、旧规则覆盖安全约束”，先停止并保留原始事件。本节的失败样例是“全局规则污染项目、旧规则覆盖安全约束”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 冲突 AGENTS/CLAUDE/记忆文件的加载报告 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 全局规则污染项目、旧规则覆盖安全约束 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“项目约定与用户约定”。围绕“项目约定与用户约定”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：冲突 AGENTS/CLAUDE/记忆文件的加载报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“全局规则污染项目、旧规则覆盖安全约束”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-7-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
项目指令为什么需要短而可验证
--- answer
项目指令应短、分层、可验证，并明确范围、非目标、命令和失败处理；长篇口号不能成为可靠门禁。
--- explanation
规则和记忆需要作用域、继承、优先级、版本和失效门槛；安全约束不能被全局便利规则覆盖。问题“项目指令为什么需要短而可验证”的关键不在背诵名词，而在于：项目指令应短、分层、可验证，并明确范围、非目标、命令和失败处理；长篇口号不能成为可靠门禁。 先判断题目中的硬门槛和适用范围，再看“全局规则污染项目、旧规则覆盖安全约束”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-7-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
持久记忆如何避免污染未来任务
--- answer
为记忆绑定项目/用户作用域、来源、版本、时效和删除操作，写入前去重与冲突检查，避免旧事实污染后续任务。
--- explanation
规则和记忆需要作用域、继承、优先级、版本和失效门槛；安全约束不能被全局便利规则覆盖。问题“持久记忆如何避免污染未来任务”的关键不在背诵名词，而在于：为记忆绑定项目/用户作用域、来源、版本、时效和删除操作，写入前去重与冲突检查，避免旧事实污染后续任务。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
