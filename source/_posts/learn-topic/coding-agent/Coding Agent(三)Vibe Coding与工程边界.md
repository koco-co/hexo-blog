---
title: Coding Agent(三)Vibe Coding与工程边界
tags:
  - Coding Agent
  - Vibe Coding与工程边界
categories:
...
{% note primary flat %}
本节围绕“Vibe Coding与工程边界”建立可复核的工作模型。学习成果：快速探索最终转化为可审查工程变更。实验：对同一小功能比较一次性生成和增量验收，记录差异、测试和返工。视觉辅助：收益—风险矩阵；反馈循环图。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
Vibe Coding 很快产出按钮，却把错误状态和测试债务留给维护者。小步 Diff、测试和责任记录让原型有工程出口。

本篇把“Vibe Coding与工程边界”落到一次可回放的本地任务：对同一小功能比较一次性生成和增量验收，记录差异、测试和返工。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：快速探索最终转化为可审查工程变更。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“Vibe Coding 的来源与准确用法；反馈循环、原型和工程化；速度、质量、债务和责任”拆成输入、状态、输出和证据。Vibe Coding 是快速反馈的协作方式，不是跳过工程化；原型要经过契约、测试、Diff 和责任交接才能进入共享代码。
{% endnote %}
...
{% note primary flat %}
实验步骤：对同一小功能比较一次性生成和增量验收，记录差异、测试和返工。

验收产物：一次性生成与增量验收的 Diff/测试对照

失败注入：无验收生成、债务累积、责任边界模糊
{% endnote %}

## 术语边界

{% note info flat %}
本节任务：理解Vibe Coding 的来源与准确用法并建立可复核判断。核心内容：Vibe Coding 的来源与准确用法。

机制：Vibe Coding 的来源与准确用法。先核对输入、状态、输出、失败和复测证据，再用“无验收生成、债务累积、责任边界模糊”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“术语用法”聚焦“Vibe Coding 的来源与准确用法”。围绕“Vibe Coding 的来源与准确用法”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“Vibe Coding 的来源与准确用法”变成一个可观察字段，再以“对同一小功能比较一次性生成和增量验收，记录差异、测试和返工。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“无验收生成、债务累积、责任边界模糊”，恢复或放行动作必须可重放。
{% endnote %}

### 反馈循环与原型和工程化

{% note info flat %}
“反馈循环与原型和工程化”聚焦“反馈循环、原型和工程化”。围绕“反馈循环、原型和工程化”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“反馈循环与原型和工程化”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“无验收生成、债务累积、责任边界模糊”，先停止并保留原始事件。本节的失败样例是“无验收生成、债务累积、责任边界模糊”，恢复或放行动作必须可重放。
{% endnote %}

## 反馈循环

{% note info flat %}
本节任务：理解反馈循环、原型和工程化并建立可复核判断。核心内容：反馈循环、原型和工程化。

机制：反馈循环、原型和工程化。先核对输入、状态、输出、失败和复测证据，再用“无验收生成、债务累积、责任边界模糊”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“速度与质量与债务和责任”聚焦“反馈循环、原型和工程化”。围绕“反馈循环、原型和工程化”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“速度与质量与债务和责任”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“无验收生成、债务累积、责任边界模糊”，先停止并保留原始事件。本节的失败样例是“无验收生成、债务累积、责任边界模糊”，恢复或放行动作必须可重放。
{% endnote %}

### 工程护栏

{% note info flat %}
“工程护栏”聚焦“速度、质量、债务和责任”。围绕“速度、质量、债务和责任”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：一次性生成与增量验收的 Diff/测试对照应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“无验收生成、债务累积、责任边界模糊”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：对同一小功能比较一次性生成和增量验收，记录差异、测试和返工。

机制：对同一小功能比较一次性生成和增量验收，记录差异、测试和返工。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“无验收生成、债务累积、责任边界模糊”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“速度、质量、债务和责任”。围绕“速度、质量、债务和责任”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：一次性生成与增量验收的 Diff/测试对照应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“无验收生成、债务累积、责任边界模糊”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(三)Vibe Coding与工程边界","order":3,"topic":"Vibe Coding与工程边界","experiment":"对同一小功能比较一次性生成和增量验收，记录差异、测试和返工。","focus":["Vibe Coding 的来源与准确用法","反馈循环、原型和工程化","速度、质量、债务和责任","从 Vibe 到可维护交付的护栏"],"artifacts":["一次性生成与增量验收的 Diff/测试对照"],"failures":["无验收生成、债务累积、责任边界模糊"],"mode":"workspace"}')
SCENARIOS = json.loads('{"3":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "无验收生成"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="missing_acceptance")
    failed_report = dict(task_report)
    failed_report["checks"] = dict(task_report["checks"])
    failed_report["checks"]["validation_passed"] = False
    try:
        if not failed_report["checks"]["validation_passed"]:
            raise RuntimeError("generated change has no passing acceptance check")
    ...
    assert task_report["checks"]["validation_passed"] and task_report["checks"]["validation_executed"]
    assert failure_case in failure_type
    assert task_report["request"]["target"] == "README.md" and task_report["checks"]["validation_passed"]
    record_event(events_file, "recovery_verified", action="restore-validation-gate", case=failure_case)
...
{% note info flat %}
“执行步骤”聚焦“从 Vibe 到可维护交付的护栏”。围绕“从 Vibe 到可维护交付的护栏”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“从 Vibe 到可维护交付的护栏”变成一个可观察字段，再以“对同一小功能比较一次性生成和增量验收，记录差异、测试和返工。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“无验收生成、债务累积、责任边界模糊”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“Vibe Coding 的来源与准确用法”。围绕“Vibe Coding 的来源与准确用法”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“无验收生成、债务累积、责任边界模糊”，先停止并保留原始事件。本节的失败样例是“无验收生成、债务累积、责任边界模糊”，恢复或放行动作必须可重放。
{% endnote %}

## 工程护栏

{% note info flat %}
本节任务：理解从 Vibe 到可维护交付的护栏并建立可复核判断。核心内容：从 Vibe 到可维护交付的护栏。

机制：从 Vibe 到可维护交付的护栏。先核对输入、状态、输出、失败和复测证据，再用“无验收生成、债务累积、责任边界模糊”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“工程护栏”聚焦“从 Vibe 到可维护交付的护栏”。围绕“从 Vibe 到可维护交付的护栏”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“从 Vibe 到可维护交付的护栏”变成一个可观察字段，再以“对同一小功能比较一次性生成和增量验收，记录差异、测试和返工。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“无验收生成、债务累积、责任边界模糊”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：一次性生成与增量验收的 Diff/测试对照。预期：快速探索最终转化为可审查工程变更。失败复验：无验收生成、债务累积、责任边界模糊。

机制：验收证据：一次性生成与增量验收的 Diff/测试对照。预期：快速探索最终转化为可审查工程变更。失败复验：无验收生成、债务累积、责任边界模糊。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“无验收生成、债务累积、责任边界模糊”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“Vibe Coding 的来源与准确用法”。围绕“Vibe Coding 的来源与准确用法”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“无验收生成、债务累积、责任边界模糊”，先停止并保留原始事件。本节的失败样例是“无验收生成、债务累积、责任边界模糊”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 一次性生成与增量验收的 Diff/测试对照 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 无验收生成、债务累积、责任边界模糊 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“反馈循环、原型和工程化”。围绕“反馈循环、原型和工程化”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：一次性生成与增量验收的 Diff/测试对照应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“无验收生成、债务累积、责任边界模糊”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-3-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
Vibe Coding 不等于不写代码
--- answer
Vibe Coding 仍然需要阅读、修改和验证真实代码；自然语言生成的是协作入口，不是工程责任的替代品。
--- explanation
Vibe Coding 是快速反馈的协作方式，不是跳过工程化；原型要经过契约、测试、Diff 和责任交接才能进入共享代码。问题“Vibe Coding 不等于不写代码”的关键不在背诵名词，而在于：Vibe Coding 仍然需要阅读、修改和验证真实代码；自然语言生成的是协作入口，不是工程责任的替代品。 先判断题目中的硬门槛和适用范围，再看“无验收生成、债务累积、责任边界模糊”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-3-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
快速原型何时必须转入工程流程
--- answer
当原型要进入共享仓库、涉及数据/权限/外部接口或需要长期维护时，必须转入契约、测试、审查和发布流程。
--- explanation
Vibe Coding 是快速反馈的协作方式，不是跳过工程化；原型要经过契约、测试、Diff 和责任交接才能进入共享代码。问题“快速原型何时必须转入工程流程”的关键不在背诵名词，而在于：当原型要进入共享仓库、涉及数据/权限/外部接口或需要长期维护时，必须转入契约、测试、审查和发布流程。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
...
{% linkgroup %}
{% link Git Documentation, https://git-scm.com/docs, https://git-scm.com/favicon.ico %}
