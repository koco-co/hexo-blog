...
categories:
  - Learn Topic
  - Coding Agent
description: 能独立完成 Aider 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。
...
{% note primary flat %}
本节围绕“Aider”建立可复核的工作模型。学习成果：能独立完成 Aider 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。实验：按官方入口完成 Aider 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。视觉辅助：产品入口或配置截图（真实验证后）；能力表；核心流程 Mermaid。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
Aider 的 Repo Map 控制上下文大小，Architect 与 Editor 分开计划和修改；只有 Git Diff、Lint 与 Test 通过，结对结果才有交付资格。

本篇把“Aider”落到一次可回放的本地任务：按官方入口完成 Aider 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：能独立完成 Aider 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“终端结对编程、Repo Map、Provider、命令、Architect/Editor、Git、Lint/Test 和回滚”拆成输入、状态、输出和证据。Aider 的 Repo Map 决定上下文取舍，Architect/Editor 分离计划与修改，Git、Lint 和 Test 决定是否把结对编程结果交付。
{% endnote %}

{% mermaid %}
flowchart TD
  A[Aider终端] --> B[Provider/模型]
  B --> C[Repo Map上下文]
  C --> D{Architect或Editor}
  D --> E[Git变更]
  E --> F[Lint/Test]
  F --> G[提交或回滚]
{% endmermaid %}
...
{% note primary flat %}
实验步骤：按官方入口完成 Aider 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。

验收产物：Aider 的版本/Commit 或文档快照；Aider 受控小仓库的任务输出；Aider 的 Diff、测试、权限或失败日志
...
{% note info flat %}
本节任务：确认Aider的入口并完成最小安装。核心内容：Aider：安装入口、Provider配置。

机制：Aider：安装入口、Provider配置。选择依据：Aider的入口、平台和版本是离散选择，表格能并列官方入口与适用条件。先核对Aider的入口类型、支持平台、安装命令和版本输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Aider入口或截图不可用时保留官方 URL、命令和版本检查。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Aider的入口类型、支持平台、安装命令和版本输出 |
| 失败降级 | Aider入口或截图不可用时保留官方 URL、命令和版本检查 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

{% note info flat %}
Aider执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）
{% endnote %}

![Aider执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）](/img/picgo-images/coding-agent-product-30.svg)

| 能力面 | 本篇观察对象 | 本地夹具证据 | 线上边界 |
| --- | --- | --- | --- |
| 入口与版本 | 终端、Provider、Repo Map、Architect/Editor、Git、Lint/Test 与回滚 | 记录任务契约与 fixture 版本 | 需按官方入口、平台和版本复核 |
...
{% note primary flat %}
入口只展示官方操作路径，本次不执行安装或真实账号调用。先记录Aider的版本，再用同一受控任务比较上下文、权限和交付证据。
{% endnote %}

| 项目 | 可复核内容 |
| --- | --- |
| 入口 | Aider |
| 安装/启动 | `python -m pip install aider-install && aider-install` |
| 版本检查 | `aider --version` |
| 配置观察 | .aider.conf.yml；Repo Map；/architect；Git；Lint/Test |
| 最小任务 | aider --no-auto-commits：先用 /architect 规划 README.md 修改 |
...
{% note info flat %}
Aider 的 aider-install/官方入口准备 CLI，aider --version 后再配置 Provider；程序可启动不等于模型认证通过。本节围绕“Provider配置”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Aider 的版本/Commit 或文档快照。
{% endnote %}

## Repo Map

{% note primary flat %}
本节任务：让Aider使用指定模型完成一次脱敏认证。核心内容：Aider：Repo Map、上下文选择。

机制：Aider：Repo Map、上下文选择。选择依据：Aider的认证示例需要可复制命令、环境变量占位符和脱敏输出，代码最不含糊。先核对Aider的模型 ID、认证方式、权限确认和脱敏输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Aider没有凭据时运行本地 fixture，禁止伪造在线响应。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Aider的模型 ID、认证方式、权限确认和脱敏输出 |
| 失败降级 | Aider没有凭据时运行本地 fixture，禁止伪造在线响应 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

```text
Repo Map: 输入 -> 校验 -> 执行 -> 证据
```

### 上下文选择

{% note info flat %}
Provider 配置影响模型和费用，Repo Map 影响上下文；分别记录模型、输入文件和被忽略的目录。本节围绕“上下文选择”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Aider 受控小仓库的任务输出。
{% endnote %}

## Architect模式

{% note info flat %}
本节任务：在Aider的项目上下文中定位一个受控任务。核心内容：Aider：Architect模式、Editor模式。

机制：Aider：Architect模式、Editor模式。选择依据：Aider的上下文、Session 和任务状态是有向关系，图下补充每个状态的文字含义。先核对Aider的规则来源、上下文范围、Session 状态和任务输入，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Aider图表失败时按“入口→上下文→任务→结果”列出状态。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Aider的规则来源、上下文范围、Session 状态和任务输入 |
| 失败降级 | Aider图表失败时按“入口→上下文→任务→结果”列出状态 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |
...
{% note info flat %}
Architect 负责提出方案，Editor 负责修改；两步之间保存计划与 Diff，避免把规划文本当实现。本节围绕“Editor模式”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Aider 的 Diff、测试、权限或失败日志。
{% endnote %}

## 提交与回滚

{% note info flat %}
本节任务：为Aider启用一个工具或扩展并撤销它。核心内容：Aider：提交与回滚、Lint与Test。

机制：Aider：提交与回滚、Lint与Test。选择依据：Aider的工具、Skill、MCP 和插件需要逐项比较来源、权限、输入输出，表格便于审查。先核对Aider的来源/版本、权限、输入、输出和撤销动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Aider扩展不可安装时使用静态 manifest 和拒绝测试。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Aider的来源/版本、权限、输入、输出和撤销动作 |
| 失败降级 | Aider扩展不可安装时使用静态 manifest 和拒绝测试 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### Lint与Test

{% note info flat %}
提交/回滚围绕 Git，Lint/Test 围绕行为；先在不自动提交模式下审 Diff，再运行确定性检查。本节围绕“Lint与Test”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Aider 的版本/Commit 或文档快照。
{% endnote %}

## 交互命令

{% note danger flat %}
本节任务：在Aider中审批一次有副作用的动作。核心内容：Aider：交互命令、脚本模式。

机制：Aider：交互命令、脚本模式。选择依据：Aider的权限与外发风险必须脱离图表颜色仍直接可见，danger 强调不可逆后果。先核对Aider的审批点、工作区、网络、秘密和停止条件，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Aider拒绝或超时后保留未执行证据与恢复/回滚步骤。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Aider的审批点、工作区、网络、秘密和停止条件 |
| 失败降级 | Aider拒绝或超时后保留未执行证据与恢复/回滚步骤 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 脚本模式

{% note info flat %}
交互命令和脚本模式的输入/输出边界不同；固定 stdin、工作目录和退出码，才能放进 CI。本节围绕“脚本模式”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Aider 受控小仓库的任务输出。
{% endnote %}

## 模型/仓库边界

{% note warning flat %}
本节任务：复现Aider的一项限制并记录恢复。核心内容：Aider：模型/仓库边界、项目验证。

机制：Aider：模型/仓库边界、项目验证。选择依据：Aider的版本、平台、Preview 和退出条件决定能否照搬，提示块突出边界而非宣传。先核对Aider的版本/平台、功能限制、失败状态和替代路径，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Aider在线能力变化时使用日期快照和本地等价任务。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Aider的版本/平台、功能限制、失败状态和替代路径 |
| 失败降级 | Aider在线能力变化时使用日期快照和本地等价任务 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 项目验证

{% note info flat %}
模型、仓库、秘密、网络和平台构成硬边界；Repo Map 或成功补丁不能推出生产安全。本节围绕“项目验证”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Aider 的 Diff、测试、权限或失败日志。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用证据验收Aider。核心内容：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 Aider 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。

机制：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 Aider 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。选择依据：产品验收需要逐项核对版本、行为和安全证据。先核对验收标准、失败样例和复测动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；在线不可用时保留本地验证结果。
{% endnote %}
...
{% note info flat %}
本节把Aider的“版本和任务证据”收束为版本、权限、任务、Diff 和测试证据；Aider 的 Repo Map 决定上下文取舍，Architect/Editor 分离计划与修改，Git、Lint 和 Test 决定是否把结对编程结果交付。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | Aider 的版本/Commit 或文档快照 | 只有最终成功文字 | 保存中间状态与失败记录 |
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(三十)Aider","order":30,"topic":"Aider","experiment":"按官方入口完成 Aider 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。","focus":["终端结对编程、Repo Map、Provider、命令、Architect/Editor、Git、Lint/Test 和回滚。","安装与模型","Repo Map与上下文","Architect与Editor"],"artifacts":["Aider 的版本/Commit 或文档快照","Aider 受控小仓库的任务输出","Aider 的 Diff、测试、权限或失败日志"],"failures":["认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代"],"mode":"workspace"}')
SCENARIOS = json.loads('{"30":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    record_event(events_file, "recovery_verified", action="record-aider-provider-boundary", case=failure_case)
...
{% note info flat %}
本节把Aider的“失败复测”收束为版本、权限、任务、Diff 和测试证据；Aider 的 Repo Map 决定上下文取舍，Architect/Editor 分离计划与修改，Git、Lint 和 Test 决定是否把结对编程结果交付。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-30-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
Aider 的默认权限和最小安全配置是什么
--- answer
不能把 Aider 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。
--- explanation
Aider 的 Repo Map 决定上下文取舍，Architect/Editor 分离计划与修改，Git、Lint 和 Test 决定是否把结对编程结果交付。问题“Aider 的默认权限和最小安全配置是什么”的关键不在背诵名词，而在于：不能把 Aider 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。 先判断题目中的硬门槛和适用范围，再看“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-30-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
Aider 的核心上下文、工具或扩展边界是什么
--- answer
Aider 的核心边界是 终端、Provider、Repo Map、Architect/Editor、Git、Lint/Test 与回滚；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。
--- explanation
Aider 的 Repo Map 决定上下文取舍，Architect/Editor 分离计划与修改，Git、Lint 和 Test 决定是否把结对编程结果交付。问题“Aider 的核心上下文、工具或扩展边界是什么”的关键不在背诵名词，而在于：Aider 的核心边界是 终端、Provider、Repo Map、Architect/Editor、Git、Lint/Test 与回滚；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
