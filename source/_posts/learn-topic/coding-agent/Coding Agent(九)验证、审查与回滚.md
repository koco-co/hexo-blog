...
categories:
  - Learn Topic
  - Coding Agent
description: 构建成功不被误判为功能正确，回滚保留无关修改
...
{% note primary flat %}
本节围绕“验证、审查与回滚”建立可复核的工作模型。学习成果：构建成功不被误判为功能正确，回滚保留无关修改。实验：在临时仓库制造测试失败和错误 Diff，完成审查和安全回退。视觉辅助：证据层级表；回滚流程图。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
构建通过的 Diff 仍扩大权限并删除用户改动。静态、动态、浏览器和人工证据共同决定是否合并或回滚。

本篇把“验证、审查与回滚”落到一次可回放的本地任务：在临时仓库制造测试失败和错误 Diff，完成审查和安全回退。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：构建成功不被误判为功能正确，回滚保留无关修改。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“静态、动态、浏览器和人工证据；失败输出与修复闭环；回滚、恢复和保留无关修改”拆成输入、状态、输出和证据。验证是证据组合而非单个绿色命令：静态、动态、浏览器、人工、安全与回滚分别回答不同问题。
{% endnote %}
...
{% note primary flat %}
实验步骤：在临时仓库制造测试失败和错误 Diff，完成审查和安全回退。

验收产物：静态、动态、浏览器、人工和 Git 证据

失败注入：只跑一项检查、回滚误删用户修改
{% endnote %}

## 证据层级

{% note info flat %}
本节任务：理解静态、动态、浏览器和人工证据并建立可复核判断。核心内容：静态、动态、浏览器和人工证据。

机制：静态、动态、浏览器和人工证据。先核对输入、状态、输出、失败和复测证据，再用“只跑一项检查、回滚误删用户修改”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“证据层级”聚焦“静态、动态、浏览器和人工证据”。围绕“静态、动态、浏览器和人工证据”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“静态、动态、浏览器和人工证据”变成一个可观察字段，再以“在临时仓库制造测试失败和错误 Diff，完成审查和安全回退。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“只跑一项检查、回滚误删用户修改”，恢复或放行动作必须可重放。
{% endnote %}

### 修复闭环

{% note info flat %}
“修复闭环”聚焦“失败输出与修复闭环”。围绕“失败输出与修复闭环”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“修复闭环”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“只跑一项检查、回滚误删用户修改”，先停止并保留原始事件。本节的失败样例是“只跑一项检查、回滚误删用户修改”，恢复或放行动作必须可重放。
{% endnote %}

## 审查流程

{% note info flat %}
本节任务：理解失败输出与修复闭环并建立可复核判断。核心内容：失败输出与修复闭环。

机制：失败输出与修复闭环。先核对输入、状态、输出、失败和复测证据，再用“只跑一项检查、回滚误删用户修改”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“安全回退”聚焦“失败输出与修复闭环”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“安全回退”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“只跑一项检查、回滚误删用户修改”，先停止并保留原始事件。本节的失败样例是“只跑一项检查、回滚误删用户修改”，恢复或放行动作必须可重放。
{% endnote %}

### 检查边界

{% note info flat %}
“检查边界”聚焦“回滚、恢复和保留无关修改”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：静态、动态、浏览器、人工和 Git 证据应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“只跑一项检查、回滚误删用户修改”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：在临时仓库制造测试失败和错误 Diff，完成审查和安全回退。

机制：在临时仓库制造测试失败和错误 Diff，完成审查和安全回退。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“只跑一项检查、回滚误删用户修改”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“回滚、恢复和保留无关修改”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。复核时把动作和产物对应起来：静态、动态、浏览器、人工和 Git 证据应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“只跑一项检查、回滚误删用户修改”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(九)验证、审查与回滚","order":9,"topic":"验证、审查与回滚","experiment":"在临时仓库制造测试失败和错误 Diff，完成审查和安全回退。","focus":["静态、动态、浏览器和人工证据","失败输出与修复闭环","回滚、恢复和保留无关修改","产品内置检查与项目检查的边界"],"artifacts":["静态、动态、浏览器、人工和 Git 证据"],"failures":["只跑一项检查、回滚误删用户修改"],"mode":"workspace"}')
SCENARIOS = json.loads('{"9":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "只跑一项检查"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="single_evidence")
    observed_checks = {"tests": True}
    try:
        if set(observed_checks) != set(task_report["checks"]):
            raise ValueError("one green check cannot stand in for the evidence layers")
    ...
    assert set(task_report["checks"]) == {"read", "write", "validation_executed", "validation_passed"}
    assert failure_case in failure_type
    assert task_report["request"]["target"] == "README.md" and task_report["checks"]["validation_passed"]
    record_event(events_file, "recovery_verified", action="restore-multi-layer-checks", case=failure_case)
...
{% note info flat %}
“执行步骤”聚焦“产品内置检查与项目检查的边界”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。先把“产品内置检查与项目检查的边界”变成一个可观察字段，再以“在临时仓库制造测试失败和错误 Diff，完成审查和安全回退。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“只跑一项检查、回滚误删用户修改”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“静态、动态、浏览器和人工证据”。围绕“静态、动态、浏览器和人工证据”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“只跑一项检查、回滚误删用户修改”，先停止并保留原始事件。本节的失败样例是“只跑一项检查、回滚误删用户修改”，恢复或放行动作必须可重放。
{% endnote %}

## 安全回退

{% note info flat %}
本节任务：理解产品内置检查与项目检查的边界并建立可复核判断。核心内容：产品内置检查与项目检查的边界。

机制：产品内置检查与项目检查的边界。先核对输入、状态、输出、失败和复测证据，再用“只跑一项检查、回滚误删用户修改”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“检查边界”聚焦“产品内置检查与项目检查的边界”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。先把“产品内置检查与项目检查的边界”变成一个可观察字段，再以“在临时仓库制造测试失败和错误 Diff，完成审查和安全回退。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“只跑一项检查、回滚误删用户修改”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：静态、动态、浏览器、人工和 Git 证据。预期：构建成功不被误判为功能正确，回滚保留无关修改。失败复验：只跑一项检查、回滚误删用户修改。

机制：验收证据：静态、动态、浏览器、人工和 Git 证据。预期：构建成功不被误判为功能正确，回滚保留无关修改。失败复验：只跑一项检查、回滚误删用户修改。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“只跑一项检查、回滚误删用户修改”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“静态、动态、浏览器和人工证据”。围绕“静态、动态、浏览器和人工证据”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“只跑一项检查、回滚误删用户修改”，先停止并保留原始事件。本节的失败样例是“只跑一项检查、回滚误删用户修改”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 静态、动态、浏览器、人工和 Git 证据 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 只跑一项检查、回滚误删用户修改 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“失败输出与修复闭环”。围绕“失败输出与修复闭环”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：静态、动态、浏览器、人工和 Git 证据应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“只跑一项检查、回滚误删用户修改”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-9-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
构建成功为什么不是功能正确
--- answer
构建只证明代码能被构建，不证明需求、权限、Diff、用户流程和异常行为正确；还需动态、人工和安全证据。
--- explanation
验证是证据组合而非单个绿色命令：静态、动态、浏览器、人工、安全与回滚分别回答不同问题。问题“构建成功为什么不是功能正确”的关键不在背诵名词，而在于：构建只证明代码能被构建，不证明需求、权限、Diff、用户流程和异常行为正确；还需动态、人工和安全证据。 先判断题目中的硬门槛和适用范围，再看“只跑一项检查、回滚误删用户修改”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-9-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
回滚前要保留哪些证据
--- answer
保留原始输入、版本、工作树、Diff、测试/日志、权限决定、失败原因和恢复点，回滚时先确认副作用状态。
--- explanation
验证是证据组合而非单个绿色命令：静态、动态、浏览器、人工、安全与回滚分别回答不同问题。问题“回滚前要保留哪些证据”的关键不在背诵名词，而在于：保留原始输入、版本、工作树、Diff、测试/日志、权限决定、失败原因和恢复点，回滚时先确认副作用状态。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
