...
{% note primary flat %}
本节围绕“工作区与权限边界”建立可复核的工作模型。学习成果：工作目录、工作树、权限和回滚点明确。实验：用本地临时仓库验证读、写、执行和越权请求的差异。视觉辅助：Mermaid 权限边界图；产品默认值表。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
Agent 在仓库根目录外生成补丁，回滚可能伤及用户的无关修改。固定工作区、权限和回滚点后，工具活动范围才可审查。

本篇把“工作区与权限边界”落到一次可回放的本地任务：用本地临时仓库验证读、写、执行和越权请求的差异。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：工作目录、工作树、权限和回滚点明确。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“工作目录、仓库和工作树；读写执行与外发边界；权限确认、Dry Run 和回滚点”拆成输入、状态、输出和证据。工作区边界决定 Agent 能看到和改变什么；工作树隔离变更，权限矩阵限制动作，回滚点保留恢复选择。
{% endnote %}
...
{% note primary flat %}
实验步骤：用本地临时仓库验证读、写、执行和越权请求的差异。

验收产物：临时仓库的读写执行拒绝记录

失败注入：越出工作区、修改无关文件、执行未授权命令
{% endnote %}

## 工作区模型

{% note info flat %}
本节任务：理解工作目录、仓库和工作树并建立可复核判断。核心内容：工作目录、仓库和工作树。

机制：工作目录、仓库和工作树。先核对输入、状态、输出、失败和复测证据，再用“越出工作区、修改无关文件、执行未授权命令”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“目录与工作树”聚焦“工作目录、仓库和工作树”。围绕“工作目录、仓库和工作树”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“工作目录、仓库和工作树”变成一个可观察字段，再以“用本地临时仓库验证读、写、执行和越权请求的差异。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“越出工作区、修改无关文件、执行未授权命令”，恢复或放行动作必须可重放。
{% endnote %}

### 读写与外发

{% note info flat %}
“读写与外发”聚焦“读写执行与外发边界”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“读写与外发”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“越出工作区、修改无关文件、执行未授权命令”，先停止并保留原始事件。本节的失败样例是“越出工作区、修改无关文件、执行未授权命令”，恢复或放行动作必须可重放。
{% endnote %}

## 执行权限

{% note info flat %}
本节任务：理解读写执行与外发边界并建立可复核判断。核心内容：读写执行与外发边界。

机制：读写执行与外发边界。先核对输入、状态、输出、失败和复测证据，再用“越出工作区、修改无关文件、执行未授权命令”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“审批与回滚”聚焦“读写执行与外发边界”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“审批与回滚”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“越出工作区、修改无关文件、执行未授权命令”，先停止并保留原始事件。本节的失败样例是“越出工作区、修改无关文件、执行未授权命令”，恢复或放行动作必须可重放。
{% endnote %}

### 安全模型

{% note info flat %}
“安全模型”聚焦“权限确认、Dry Run 和回滚点”。把模型/Provider 选择、认证状态和任务输入分成三列；凭据缺失时应停止在本地静态演示，不输出看似真实的服务响应。复核时把动作和产物对应起来：临时仓库的读写执行拒绝记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“越出工作区、修改无关文件、执行未授权命令”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：用本地临时仓库验证读、写、执行和越权请求的差异。

机制：用本地临时仓库验证读、写、执行和越权请求的差异。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“越出工作区、修改无关文件、执行未授权命令”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“权限确认、Dry Run 和回滚点”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：临时仓库的读写执行拒绝记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“越出工作区、修改无关文件、执行未授权命令”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(二)工作区与权限边界","order":2,"topic":"工作区与权限边界","experiment":"用本地临时仓库验证读、写、执行和越权请求的差异。","focus":["工作目录、仓库和工作树","读写执行与外发边界","权限确认、Dry Run 和回滚点","不同产品的默认安全模型"],"artifacts":["临时仓库的读写执行拒绝记录"],"failures":["越出工作区、修改无关文件、执行未授权命令"],"mode":"workspace"}')
SCENARIOS = json.loads('{"2":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "越出工作区"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="workspace_escape")
    try:
        target = (root / "../outside.py").resolve()
        target.relative_to(root.resolve())
        raise RuntimeError("path unexpectedly stayed inside the workspace")
    ...
    assert not (root.parent / "outside.py").exists()
    assert failure_case in failure_type
    assert task_report["request"]["target"] == "README.md" and task_report["checks"]["validation_passed"]
    record_event(events_file, "recovery_verified", action="reject-outside-workspace", case=failure_case)
...
{% note info flat %}
“执行步骤”聚焦“不同产品的默认安全模型”。把模型/Provider 选择、认证状态和任务输入分成三列；凭据缺失时应停止在本地静态演示，不输出看似真实的服务响应。先把“不同产品的默认安全模型”变成一个可观察字段，再以“用本地临时仓库验证读、写、执行和越权请求的差异。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“越出工作区、修改无关文件、执行未授权命令”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“工作目录、仓库和工作树”。围绕“工作目录、仓库和工作树”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“越出工作区、修改无关文件、执行未授权命令”，先停止并保留原始事件。本节的失败样例是“越出工作区、修改无关文件、执行未授权命令”，恢复或放行动作必须可重放。
{% endnote %}

## 回滚证据

{% note info flat %}
本节任务：理解不同产品的默认安全模型并建立可复核判断。核心内容：不同产品的默认安全模型。

机制：不同产品的默认安全模型。先核对输入、状态、输出、失败和复测证据，再用“越出工作区、修改无关文件、执行未授权命令”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“安全模型”聚焦“不同产品的默认安全模型”。把模型/Provider 选择、认证状态和任务输入分成三列；凭据缺失时应停止在本地静态演示，不输出看似真实的服务响应。先把“不同产品的默认安全模型”变成一个可观察字段，再以“用本地临时仓库验证读、写、执行和越权请求的差异。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“越出工作区、修改无关文件、执行未授权命令”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：临时仓库的读写执行拒绝记录。预期：工作目录、工作树、权限和回滚点明确。失败复验：越出工作区、修改无关文件、执行未授权命令。

机制：验收证据：临时仓库的读写执行拒绝记录。预期：工作目录、工作树、权限和回滚点明确。失败复验：越出工作区、修改无关文件、执行未授权命令。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“越出工作区、修改无关文件、执行未授权命令”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“工作目录、仓库和工作树”。围绕“工作目录、仓库和工作树”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“越出工作区、修改无关文件、执行未授权命令”，先停止并保留原始事件。本节的失败样例是“越出工作区、修改无关文件、执行未授权命令”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 临时仓库的读写执行拒绝记录 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 越出工作区、修改无关文件、执行未授权命令 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“读写执行与外发边界”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。复核时把动作和产物对应起来：临时仓库的读写执行拒绝记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“越出工作区、修改无关文件、执行未授权命令”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-2-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
默认权限为什么不能直接当作安全策略
--- answer
默认权限只是产品初始值，不是安全策略；应从只读、受控工作树、无秘密和显式审批开始。
--- explanation
工作区边界决定 Agent 能看到和改变什么；工作树隔离变更，权限矩阵限制动作，回滚点保留恢复选择。问题“默认权限为什么不能直接当作安全策略”的关键不在背诵名词，而在于：默认权限只是产品初始值，不是安全策略；应从只读、受控工作树、无秘密和显式审批开始。 先判断题目中的硬门槛和适用范围，再看“越出工作区、修改无关文件、执行未授权命令”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-2-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
工作树如何降低修改风险
--- answer
工作树把变更限制在独立目录或分支，便于比较 Diff、拒绝越界文件和整体回滚，不会保护错误的业务逻辑。
--- explanation
工作区边界决定 Agent 能看到和改变什么；工作树隔离变更，权限矩阵限制动作，回滚点保留恢复选择。问题“工作树如何降低修改风险”的关键不在背诵名词，而在于：工作树把变更限制在独立目录或分支，便于比较 Diff、拒绝越界文件和整体回滚，不会保护错误的业务逻辑。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
