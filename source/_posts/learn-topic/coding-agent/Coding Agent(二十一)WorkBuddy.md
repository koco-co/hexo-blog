---
title: Coding Agent(二十一)WorkBuddy
tags:
  - Coding Agent
  - WorkBuddy
categories:
  - Learn Topic
  - Coding Agent
description: 能独立完成 WorkBuddy 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。
...
{% note primary flat %}
本节围绕“WorkBuddy”建立可复核的工作模型。学习成果：能独立完成 WorkBuddy 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。实验：按官方入口完成 WorkBuddy 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。视觉辅助：产品入口或配置截图（真实验证后）；能力表；核心流程 Mermaid。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
WorkBuddy 的 Coding Mode 生成了修复草稿，审查者仍要确认项目上下文、调试步骤和执行确认。桌面体验不能替代受控 Diff、测试和权限记录。

本篇把“WorkBuddy”落到一次可回放的本地任务：按官方入口完成 WorkBuddy 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：能独立完成 WorkBuddy 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“桌面工作台与 Coding Mode、模型、项目上下文、生成/审查/调试/重构、权限和可用平台”拆成输入、状态、输出和证据。WorkBuddy 的产品任务围绕桌面工作台和 Coding Mode 展开，项目上下文、生成/审查/调试/重构与执行确认需要逐项记录。
{% endnote %}

{% mermaid %}
flowchart TD
  A[WorkBuddy桌面工作台] --> B[Coding Mode/模型]
  B --> C[目录与文件上下文]
  C --> D[生成/调试/重构]
  D --> E{执行确认}
  E -->|通过| F[测试与Diff]
...
{% note primary flat %}
实验步骤：按官方入口完成 WorkBuddy 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。

验收产物：WorkBuddy 的版本/Commit 或文档快照；WorkBuddy 受控小仓库的任务输出；WorkBuddy 的 Diff、测试、权限或失败日志
...
{% note info flat %}
本节任务：确认WorkBuddy的入口并完成最小安装。核心内容：WorkBuddy：桌面工作台、平台支持。

机制：WorkBuddy：桌面工作台、平台支持。选择依据：WorkBuddy的入口、平台和版本是离散选择，表格能并列官方入口与适用条件。先核对WorkBuddy的入口类型、支持平台、安装命令和版本输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；WorkBuddy入口或截图不可用时保留官方 URL、命令和版本检查。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | WorkBuddy的入口类型、支持平台、安装命令和版本输出 |
| 失败降级 | WorkBuddy入口或截图不可用时保留官方 URL、命令和版本检查 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

{% note info flat %}
WorkBuddy执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）
{% endnote %}

![WorkBuddy执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）](/img/picgo-images/coding-agent-product-21.svg)

| 能力面 | 本篇观察对象 | 本地夹具证据 | 线上边界 |
| --- | --- | --- | --- |
| 入口与版本 | 桌面工作台、Coding Mode、项目目录、生成/调试流程与执行确认 | 记录任务契约与 fixture 版本 | 需按官方入口、平台和版本复核 |
...
{% note primary flat %}
入口只展示官方操作路径，本次不执行安装或真实账号调用。先记录WorkBuddy desktop的版本，再用同一受控任务比较上下文、权限和交付证据。
{% endnote %}

| 项目 | 可复核内容 |
| --- | --- |
| 入口 | WorkBuddy desktop |
| 安装/启动 | `从 workbuddy.ai 官方入口安装桌面客户端` |
| 版本检查 | `在客户端记录版本与可用平台` |
| 配置观察 | Coding Mode；project directory；model；generate/review/debug/refactor；execution confirmation |
| 最小任务 | 打开 Coding Mode 读取 README.md，生成一行候选修改后拒绝写入 |
...
{% note info flat %}
WorkBuddy 是桌面工作台，先从官方入口记录客户端版本和平台；不能用通用 CLI 检查替代产品入口证据。本节围绕“平台支持”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为WorkBuddy 的版本/Commit 或文档快照。
{% endnote %}

## 模型选择

{% note primary flat %}
本节任务：让WorkBuddy使用指定模型完成一次脱敏认证。核心内容：WorkBuddy：模型选择、Coding Mode。

机制：WorkBuddy：模型选择、Coding Mode。选择依据：WorkBuddy的认证示例需要可复制命令、环境变量占位符和脱敏输出，代码最不含糊。先核对WorkBuddy的模型 ID、认证方式、权限确认和脱敏输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；WorkBuddy没有凭据时运行本地 fixture，禁止伪造在线响应。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | WorkBuddy的模型 ID、认证方式、权限确认和脱敏输出 |
| 失败降级 | WorkBuddy没有凭据时运行本地 fixture，禁止伪造在线响应 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |
...
{% note info flat %}
Coding Mode 的模型选择影响生成与调试行为，但账户/订阅和执行确认是独立状态；逐项保存界面选择。本节围绕“Coding Mode”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为WorkBuddy 受控小仓库的任务输出。
{% endnote %}

## 目录与文件

{% note info flat %}
本节任务：在WorkBuddy的项目上下文中定位一个受控任务。核心内容：WorkBuddy：目录与文件、任务上下文。

机制：WorkBuddy：目录与文件、任务上下文。选择依据：WorkBuddy的上下文、Session 和任务状态是有向关系，图下补充每个状态的文字含义。先核对WorkBuddy的规则来源、上下文范围、Session 状态和任务输入，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；WorkBuddy图表失败时按“入口→上下文→任务→结果”列出状态。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | WorkBuddy的规则来源、上下文范围、Session 状态和任务输入 |
| 失败降级 | WorkBuddy图表失败时按“入口→上下文→任务→结果”列出状态 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |
...
{% note info flat %}
目录与文件上下文限定了它能看到的项目；任务记录应包含目录范围、被读取文件和未纳入的秘密。本节围绕“任务上下文”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为WorkBuddy 的 Diff、测试、权限或失败日志。
{% endnote %}

## 生成与调试

{% note info flat %}
本节任务：为WorkBuddy启用一个工具或扩展并撤销它。核心内容：WorkBuddy：生成与调试、重构与审查。

机制：WorkBuddy：生成与调试、重构与审查。选择依据：WorkBuddy的工具、Skill、MCP 和插件需要逐项比较来源、权限、输入输出，表格便于审查。先核对WorkBuddy的来源/版本、权限、输入、输出和撤销动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；WorkBuddy扩展不可安装时使用静态 manifest 和拒绝测试。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | WorkBuddy的来源/版本、权限、输入、输出和撤销动作 |
| 失败降级 | WorkBuddy扩展不可安装时使用静态 manifest 和拒绝测试 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 重构与审查

{% note info flat %}
生成、调试、重构和审查是不同动作，比较每步产生的 Diff、测试和错误，而不是只截取最终面板。本节围绕“重构与审查”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为WorkBuddy 的版本/Commit 或文档快照。
{% endnote %}

## 执行确认

{% note danger flat %}
本节任务：在WorkBuddy中审批一次有副作用的动作。核心内容：WorkBuddy：执行确认、扩展边界。

机制：WorkBuddy：执行确认、扩展边界。选择依据：WorkBuddy的权限与外发风险必须脱离图表颜色仍直接可见，danger 强调不可逆后果。先核对WorkBuddy的审批点、工作区、网络、秘密和停止条件，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；WorkBuddy拒绝或超时后保留未执行证据与恢复/回滚步骤。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | WorkBuddy的审批点、工作区、网络、秘密和停止条件 |
| 失败降级 | WorkBuddy拒绝或超时后保留未执行证据与恢复/回滚步骤 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 扩展边界

{% note info flat %}
执行确认必须绑定具体命令或写入目标；拒绝后检查没有残留文件、缓存或外发请求。本节围绕“扩展边界”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为WorkBuddy 受控小仓库的任务输出。
{% endnote %}

## 账户与网络

{% note warning flat %}
本节任务：复现WorkBuddy的一项限制并记录恢复。核心内容：WorkBuddy：账户与网络、项目验证。

机制：WorkBuddy：账户与网络、项目验证。选择依据：WorkBuddy的版本、平台、Preview 和退出条件决定能否照搬，提示块突出边界而非宣传。先核对WorkBuddy的版本/平台、功能限制、失败状态和替代路径，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；WorkBuddy在线能力变化时使用日期快照和本地等价任务。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | WorkBuddy的版本/平台、功能限制、失败状态和替代路径 |
| 失败降级 | WorkBuddy在线能力变化时使用日期快照和本地等价任务 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 项目验证

{% note info flat %}
账户、网络和平台差异无法由本地 fixture 证明；保留录屏/日志的来源和时间，未观察项不排名。本节围绕“项目验证”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为WorkBuddy 的 Diff、测试、权限或失败日志。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用证据验收WorkBuddy。核心内容：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 WorkBuddy 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。

机制：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 WorkBuddy 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。选择依据：产品验收需要逐项核对版本、行为和安全证据。先核对验收标准、失败样例和复测动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；在线不可用时保留本地验证结果。
{% endnote %}
...
{% note info flat %}
本节把WorkBuddy的“版本和任务证据”收束为版本、权限、任务、Diff 和测试证据；WorkBuddy 的产品任务围绕桌面工作台和 Coding Mode 展开，项目上下文、生成/审查/调试/重构与执行确认需要逐项记录。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | WorkBuddy 的版本/Commit 或文档快照 | 只有最终成功文字 | 保存中间状态与失败记录 |
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(二十一)WorkBuddy","order":21,"topic":"WorkBuddy","experiment":"按官方入口完成 WorkBuddy 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。","focus":["桌面工作台与 Coding Mode、模型、项目上下文、生成/审查/调试/重构、权限和可用平台。","入口与环境","模型与Coding Mode","项目上下文"],"artifacts":["WorkBuddy 的版本/Commit 或文档快照","WorkBuddy 受控小仓库的任务输出","WorkBuddy 的 Diff、测试、权限或失败日志"],"failures":["认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代"],"mode":"workspace"}')
SCENARIOS = json.loads('{"21":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "平台差异"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="platform_difference")
    preflight = {"product": DATA["topic"], "failure_kind": "platform_difference", "failure_case": failure_case, "declared_failure": failure_type, "online_evidence": "not_run", "platform_match": False}
    try:
        if not preflight["platform_match"]:
            raise PermissionError("platform-specific path is unavailable")
    ...
    record_event(events_file, "recovery_verified", action="record-desktop-platform-boundary", case=failure_case)
...
{% note info flat %}
本节把WorkBuddy的“失败复测”收束为版本、权限、任务、Diff 和测试证据；WorkBuddy 的产品任务围绕桌面工作台和 Coding Mode 展开，项目上下文、生成/审查/调试/重构与执行确认需要逐项记录。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-21-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
WorkBuddy 的默认权限和最小安全配置是什么
--- answer
不能把 WorkBuddy 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。
--- explanation
WorkBuddy 的产品任务围绕桌面工作台和 Coding Mode 展开，项目上下文、生成/审查/调试/重构与执行确认需要逐项记录。问题“WorkBuddy 的默认权限和最小安全配置是什么”的关键不在背诵名词，而在于：不能把 WorkBuddy 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。 先判断题目中的硬门槛和适用范围，再看“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-21-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
WorkBuddy 的核心上下文、工具或扩展边界是什么
--- answer
WorkBuddy 的核心边界是 桌面工作台、Coding Mode、项目目录、生成/调试流程与执行确认；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。
--- explanation
WorkBuddy 的产品任务围绕桌面工作台和 Coding Mode 展开，项目上下文、生成/审查/调试/重构与执行确认需要逐项记录。问题“WorkBuddy 的核心上下文、工具或扩展边界是什么”的关键不在背诵名词，而在于：WorkBuddy 的核心边界是 桌面工作台、Coding Mode、项目目录、生成/调试流程与执行确认；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
