...
categories:
  - Learn Topic
  - Coding Agent
description: 能独立完成 Cursor 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。
...
{% note primary flat %}
本节围绕“Cursor”建立可复核的工作模型。学习成果：能独立完成 Cursor 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。实验：按官方入口完成 Cursor 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。视觉辅助：产品入口或配置截图（真实验证后）；能力表；核心流程 Mermaid。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
Cursor 同时修改多个文件后，Checkpoint 让审查者可以回到前一状态；Rules、Terminal 和 MCP 仍需按项目权限复核。

本篇把“Cursor”落到一次可回放的本地任务：按官方入口完成 Cursor 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：能独立完成 Cursor 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“编辑器 Agent、Rules/AGENTS.md、代码库上下文、终端、Checkpoint、MCP、审查和权限”拆成输入、状态、输出和证据。Cursor 的 Rules/AGENTS.md、代码库 Context、多文件编辑、Terminal、Checkpoint 与 MCP 权限共同影响可复现性，不能只看生成速度。
{% endnote %}

{% mermaid %}
flowchart TD
  A[Cursor Editor Agent] --> B[Project Rules/AGENTS.md]
  B --> C[代码库上下文]
  C --> D[多文件编辑/Terminal]
  D --> E[Checkpoint]
  E --> F{MCP权限}
...
{% note primary flat %}
实验步骤：按官方入口完成 Cursor 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。

验收产物：Cursor 的版本/Commit 或文档快照；Cursor 受控小仓库的任务输出；Cursor 的 Diff、测试、权限或失败日志
...
{% note info flat %}
本节任务：确认Cursor的入口并完成最小安装。核心内容：Cursor：Editor Agent、Ask/模式选择。

机制：Cursor：Editor Agent、Ask/模式选择。选择依据：Cursor的入口、平台和版本是离散选择，表格能并列官方入口与适用条件。先核对Cursor的入口类型、支持平台、安装命令和版本输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Cursor入口或截图不可用时保留官方 URL、命令和版本检查。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Cursor的入口类型、支持平台、安装命令和版本输出 |
| 失败降级 | Cursor入口或截图不可用时保留官方 URL、命令和版本检查 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

{% note info flat %}
Cursor执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）
{% endnote %}

![Cursor执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）](/img/picgo-images/coding-agent-product-27.svg)

| 能力面 | 本篇观察对象 | 本地夹具证据 | 线上边界 |
| --- | --- | --- | --- |
| 入口与版本 | Editor Agent、Project Rules/AGENTS.md、代码库 Context、Checkpoint、MCP 与权限 | 记录任务契约与 fixture 版本 | 需按官方入口、平台和版本复核 |
...
{% note primary flat %}
入口只展示官方操作路径，本次不执行安装或真实账号调用。先记录Cursor的版本，再用同一受控任务比较上下文、权限和交付证据。
{% endnote %}

| 项目 | 可复核内容 |
| --- | --- |
| 入口 | Cursor |
| 安装/启动 | `从 cursor.com 官方入口安装桌面应用或 CLI` |
| 版本检查 | `打开 Cursor 后记录版本；另行核对 cursor-agent CLI` |
| 配置观察 | .cursor/rules/*.mdc；AGENTS.md；Checkpoint；Terminal；MCP |
| 最小任务 | Cursor Agent 只读取 README.md，生成 Checkpoint 后等待确认 |
...
{% note info flat %}
Cursor 用官方桌面/CLI 入口安装，先记录客户端与 Agent 版本；编辑器打开不等于模型、Terminal 和 MCP 已授权。本节围绕“Ask/模式选择”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Cursor 的版本/Commit 或文档快照。
{% endnote %}

## Project Rules

{% note primary flat %}
本节任务：让Cursor使用指定模型完成一次脱敏认证。核心内容：Cursor：Project Rules、代码库上下文。

机制：Cursor：Project Rules、代码库上下文。选择依据：Cursor的认证示例需要可复制命令、环境变量占位符和脱敏输出，代码最不含糊。先核对Cursor的模型 ID、认证方式、权限确认和脱敏输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Cursor没有凭据时运行本地 fixture，禁止伪造在线响应。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Cursor的模型 ID、认证方式、权限确认和脱敏输出 |
| 失败降级 | Cursor没有凭据时运行本地 fixture，禁止伪造在线响应 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

```text
Project Rules: 输入 -> 校验 -> 执行 -> 证据
```

### 代码库上下文

{% note info flat %}
Ask/其他模式决定是否允许生成或执行；用只读提问建立基线，再确认模式切换产生的动作。本节围绕“代码库上下文”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Cursor 受控小仓库的任务输出。
{% endnote %}

## 多文件编辑

{% note info flat %}
本节任务：在Cursor的项目上下文中定位一个受控任务。核心内容：Cursor：多文件编辑、Terminal工具。

机制：Cursor：多文件编辑、Terminal工具。选择依据：Cursor的上下文、Session 和任务状态是有向关系，图下补充每个状态的文字含义。先核对Cursor的规则来源、上下文范围、Session 状态和任务输入，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Cursor图表失败时按“入口→上下文→任务→结果”列出状态。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Cursor的规则来源、上下文范围、Session 状态和任务输入 |
| 失败降级 | Cursor图表失败时按“入口→上下文→任务→结果”列出状态 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |
...
{% note info flat %}
`.cursor/rules/*.mdc` 与 AGENTS.md 是上下文来源，规则范围和优先级需与代码库 Context 分开记录。本节围绕“Terminal工具”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Cursor 的 Diff、测试、权限或失败日志。
{% endnote %}

## Checkpoint

{% note info flat %}
本节任务：为Cursor启用一个工具或扩展并撤销它。核心内容：Cursor：Checkpoint、测试与审查。

机制：Cursor：Checkpoint、测试与审查。选择依据：Cursor的工具、Skill、MCP 和插件需要逐项比较来源、权限、输入输出，表格便于审查。先核对Cursor的来源/版本、权限、输入、输出和撤销动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Cursor扩展不可安装时使用静态 manifest 和拒绝测试。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Cursor的来源/版本、权限、输入、输出和撤销动作 |
| 失败降级 | Cursor扩展不可安装时使用静态 manifest 和拒绝测试 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 测试与审查

{% note info flat %}
多文件编辑和 Terminal 是不同副作用；每次生成后检查文件清单、命令确认和 Checkpoint，而非只审单个片段。本节围绕“测试与审查”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Cursor 的版本/Commit 或文档快照。
{% endnote %}

## MCP连接

{% note danger flat %}
本节任务：在Cursor中审批一次有副作用的动作。核心内容：Cursor：MCP连接、权限确认。

机制：Cursor：MCP连接、权限确认。选择依据：Cursor的权限与外发风险必须脱离图表颜色仍直接可见，danger 强调不可逆后果。先核对Cursor的审批点、工作区、网络、秘密和停止条件，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Cursor拒绝或超时后保留未执行证据与恢复/回滚步骤。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Cursor的审批点、工作区、网络、秘密和停止条件 |
| 失败降级 | Cursor拒绝或超时后保留未执行证据与恢复/回滚步骤 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 权限确认

{% note info flat %}
MCP 连接需确认传输方式、服务器来源、工具权限和数据外发；拒绝后验证没有后台调用。本节围绕“权限确认”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Cursor 受控小仓库的任务输出。
{% endnote %}

## 隐私与平台边界

{% note danger flat %}
本节任务：复现Cursor的一项限制并记录恢复。核心内容：Cursor：隐私与平台边界、项目验证。

机制：Cursor：隐私与平台边界、项目验证。选择依据：Cursor的版本、平台、Preview 和退出条件决定能否照搬，提示块突出边界而非宣传。先核对Cursor的版本/平台、功能限制、失败状态和替代路径，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Cursor在线能力变化时使用日期快照和本地等价任务。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Cursor的版本/平台、功能限制、失败状态和替代路径 |
| 失败降级 | Cursor在线能力变化时使用日期快照和本地等价任务 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 项目验证

{% note info flat %}
隐私/平台复盘覆盖索引范围、遥测、系统版本和网络；本地 Diff 不能证明云端数据处理符合要求。本节围绕“项目验证”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Cursor 的 Diff、测试、权限或失败日志。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用证据验收Cursor。核心内容：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 Cursor 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。

机制：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 Cursor 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。选择依据：产品验收需要逐项核对版本、行为和安全证据。先核对验收标准、失败样例和复测动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；在线不可用时保留本地验证结果。
{% endnote %}
...
{% note info flat %}
本节把Cursor的“版本和任务证据”收束为版本、权限、任务、Diff 和测试证据；Cursor 的 Rules/AGENTS.md、代码库 Context、多文件编辑、Terminal、Checkpoint 与 MCP 权限共同影响可复现性，不能只看生成速度。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | Cursor 的版本/Commit 或文档快照 | 只有最终成功文字 | 保存中间状态与失败记录 |
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(二十七)Cursor","order":27,"topic":"Cursor","experiment":"按官方入口完成 Cursor 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。","focus":["编辑器 Agent、Rules/AGENTS.md、代码库上下文、终端、Checkpoint、MCP、审查和权限。","入口与模式","Rules与上下文","编辑与终端"],"artifacts":["Cursor 的版本/Commit 或文档快照","Cursor 受控小仓库的任务输出","Cursor 的 Diff、测试、权限或失败日志"],"failures":["认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代"],"mode":"workspace"}')
SCENARIOS = json.loads('{"27":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "权限拒绝"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="mcp_permission")
    preflight = {"product": DATA["topic"], "failure_kind": "mcp_permission", "failure_case": failure_case, "declared_failure": failure_type, "online_evidence": "not_run", "approved": False}
    try:
        if not preflight["approved"]:
            raise PermissionError("MCP capability has no approval")
    ...
    record_event(events_file, "recovery_verified", action="stop-unapproved-mcp", case=failure_case)
...
{% note info flat %}
本节把Cursor的“失败复测”收束为版本、权限、任务、Diff 和测试证据；Cursor 的 Rules/AGENTS.md、代码库 Context、多文件编辑、Terminal、Checkpoint 与 MCP 权限共同影响可复现性，不能只看生成速度。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-27-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
Cursor 的默认权限和最小安全配置是什么
--- answer
不能把 Cursor 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。
--- explanation
Cursor 的 Rules/AGENTS.md、代码库 Context、多文件编辑、Terminal、Checkpoint 与 MCP 权限共同影响可复现性，不能只看生成速度。问题“Cursor 的默认权限和最小安全配置是什么”的关键不在背诵名词，而在于：不能把 Cursor 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。 先判断题目中的硬门槛和适用范围，再看“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-27-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
Cursor 的核心上下文、工具或扩展边界是什么
--- answer
Cursor 的核心边界是 Editor Agent、Project Rules/AGENTS.md、代码库 Context、Checkpoint、MCP 与权限；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。
--- explanation
Cursor 的 Rules/AGENTS.md、代码库 Context、多文件编辑、Terminal、Checkpoint 与 MCP 权限共同影响可复现性，不能只看生成速度。问题“Cursor 的核心上下文、工具或扩展边界是什么”的关键不在背诵名词，而在于：Cursor 的核心边界是 Editor Agent、Project Rules/AGENTS.md、代码库 Context、Checkpoint、MCP 与权限；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
