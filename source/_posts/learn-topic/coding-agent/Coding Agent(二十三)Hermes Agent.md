---
title: Coding Agent(二十三)Hermes Agent
tags:
  - Coding Agent
  - Hermes Agent
categories:
  - Learn Topic
  - Coding Agent
description: 能独立完成 Hermes Agent 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。
...
{% note primary flat %}
本节围绕“Hermes Agent”建立可复核的工作模型。学习成果：能独立完成 Hermes Agent 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。实验：按官方入口完成 Hermes Agent 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。视觉辅助：产品入口或配置截图（真实验证后）；能力表；核心流程 Mermaid。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
Hermes Agent 可以组合 Provider、终端或浏览器工具与持久记忆，但每个扩展都要重新检查来源、秘密和撤销。开源不等于默认隔离。

本篇把“Hermes Agent”落到一次可回放的本地任务：按官方入口完成 Hermes Agent 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：能独立完成 Hermes Agent 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“开源通用 Agent 的安装、Provider、终端/浏览器工具、持久记忆、Skills、消息集成和安全”拆成输入、状态、输出和证据。Hermes Agent 的开源边界要从 Provider、终端/浏览器工具、持久记忆、Skills 和消息集成分别验证，不能把扩展能力视作默认安全。
{% endnote %}

{% mermaid %}
flowchart TD
  A[Hermes安装] --> B[Provider/终端工具]
  B --> C[持久记忆/Skills]
  C --> D[浏览器任务]
  D --> E[消息集成]
  E --> F{权限与秘密}
...
{% note primary flat %}
实验步骤：按官方入口完成 Hermes Agent 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。

验收产物：Hermes Agent 的版本/Commit 或文档快照；Hermes Agent 受控小仓库的任务输出；Hermes Agent 的 Diff、测试、权限或失败日志
...
{% note info flat %}
本节任务：确认Hermes Agent的入口并完成最小安装。核心内容：Hermes Agent：产品范围、安装与依赖。

机制：Hermes Agent：产品范围、安装与依赖。选择依据：Hermes Agent的入口、平台和版本是离散选择，表格能并列官方入口与适用条件。先核对Hermes Agent的入口类型、支持平台、安装命令和版本输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Hermes Agent入口或截图不可用时保留官方 URL、命令和版本检查。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Hermes Agent的入口类型、支持平台、安装命令和版本输出 |
| 失败降级 | Hermes Agent入口或截图不可用时保留官方 URL、命令和版本检查 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

{% note info flat %}
Hermes Agent执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）
{% endnote %}

![Hermes Agent执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）](/img/picgo-images/coding-agent-product-23.svg)

| 能力面 | 本篇观察对象 | 本地夹具证据 | 线上边界 |
| --- | --- | --- | --- |
| 入口与版本 | Provider、终端/浏览器工具、持久记忆、Skills、消息集成与秘密边界 | 记录任务契约与 fixture 版本 | 需按官方入口、平台和版本复核 |
...
{% note primary flat %}
入口只展示官方操作路径，本次不执行安装或真实账号调用。先记录Hermes Agent的版本，再用同一受控任务比较上下文、权限和交付证据。
{% endnote %}

| 项目 | 可复核内容 |
| --- | --- |
| 入口 | Hermes Agent |
| 安装/启动 | `curl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash` |
| 版本检查 | `hermes --version` |
| 配置观察 | hermes setup；provider；browser/terminal tools；memory；Skills；integrations |
| 最小任务 | hermes setup 后仅在本地 fixture 中规划 README.md 变更，不发送消息 |
...
{% note info flat %}
Hermes Agent 由官方安装脚本准备运行环境，再用 hermes setup 选择 Provider；安装器完成不表示模型请求已授权。本节围绕“安装与依赖”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Hermes Agent 的版本/Commit 或文档快照。
{% endnote %}

## Provider配置

{% note primary flat %}
本节任务：让Hermes Agent使用指定模型完成一次脱敏认证。核心内容：Hermes Agent：Provider配置、终端工具。

机制：Hermes Agent：Provider配置、终端工具。选择依据：Hermes Agent的认证示例需要可复制命令、环境变量占位符和脱敏输出，代码最不含糊。先核对Hermes Agent的模型 ID、认证方式、权限确认和脱敏输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Hermes Agent没有凭据时运行本地 fixture，禁止伪造在线响应。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Hermes Agent的模型 ID、认证方式、权限确认和脱敏输出 |
| 失败降级 | Hermes Agent没有凭据时运行本地 fixture，禁止伪造在线响应 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

```text
Provider配置: 输入 -> 校验 -> 执行 -> 证据
```

### 终端工具

{% note info flat %}
Provider 配置和终端/浏览器工具是两种权限；先禁用外部消息与网络，再用本地任务验证命令参数。本节围绕“终端工具”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Hermes Agent 受控小仓库的任务输出。
{% endnote %}

## 持久记忆

{% note info flat %}
本节任务：在Hermes Agent的项目上下文中定位一个受控任务。核心内容：Hermes Agent：持久记忆、Skill加载。

机制：Hermes Agent：持久记忆、Skill加载。选择依据：Hermes Agent的上下文、Session 和任务状态是有向关系，图下补充每个状态的文字含义。先核对Hermes Agent的规则来源、上下文范围、Session 状态和任务输入，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Hermes Agent图表失败时按“入口→上下文→任务→结果”列出状态。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Hermes Agent的规则来源、上下文范围、Session 状态和任务输入 |
| 失败降级 | Hermes Agent图表失败时按“入口→上下文→任务→结果”列出状态 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |
...
{% note info flat %}
持久记忆和 Skill 加载分别影响事实与能力；为每条记忆写作用域、过期和删除动作，避免旧事实进入新任务。本节围绕“Skill加载”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Hermes Agent 的 Diff、测试、权限或失败日志。
{% endnote %}

## 浏览器工具

{% note info flat %}
本节任务：为Hermes Agent启用一个工具或扩展并撤销它。核心内容：Hermes Agent：浏览器工具、任务验证。

机制：Hermes Agent：浏览器工具、任务验证。选择依据：Hermes Agent的工具、Skill、MCP 和插件需要逐项比较来源、权限、输入输出，表格便于审查。先核对Hermes Agent的来源/版本、权限、输入、输出和撤销动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Hermes Agent扩展不可安装时使用静态 manifest 和拒绝测试。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Hermes Agent的来源/版本、权限、输入、输出和撤销动作 |
| 失败降级 | Hermes Agent扩展不可安装时使用静态 manifest 和拒绝测试 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 任务验证

{% note info flat %}
浏览器工具应限制域名、Cookie、下载目录和外发；只用本地页面做 selector/产物验证，不假称线上浏览器成功。本节围绕“任务验证”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Hermes Agent 的版本/Commit 或文档快照。
{% endnote %}

## 消息集成

{% note danger flat %}
本节任务：在Hermes Agent中审批一次有副作用的动作。核心内容：Hermes Agent：消息集成、权限与秘密。

机制：Hermes Agent：消息集成、权限与秘密。选择依据：Hermes Agent的权限与外发风险必须脱离图表颜色仍直接可见，danger 强调不可逆后果。先核对Hermes Agent的审批点、工作区、网络、秘密和停止条件，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Hermes Agent拒绝或超时后保留未执行证据与恢复/回滚步骤。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Hermes Agent的审批点、工作区、网络、秘密和停止条件 |
| 失败降级 | Hermes Agent拒绝或超时后保留未执行证据与恢复/回滚步骤 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 权限与秘密

{% note info flat %}
消息集成会把代码任务带到第三方渠道；秘密、用户身份、回执和撤销必须成为单独的审批记录。本节围绕“权限与秘密”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Hermes Agent 受控小仓库的任务输出。
{% endnote %}

## 扩展入口

{% note warning flat %}
本节任务：复现Hermes Agent的一项限制并记录恢复。核心内容：Hermes Agent：扩展入口、项目复盘。

机制：Hermes Agent：扩展入口、项目复盘。选择依据：Hermes Agent的版本、平台、Preview 和退出条件决定能否照搬，提示块突出边界而非宣传。先核对Hermes Agent的版本/平台、功能限制、失败状态和替代路径，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Hermes Agent在线能力变化时使用日期快照和本地等价任务。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Hermes Agent的版本/平台、功能限制、失败状态和替代路径 |
| 失败降级 | Hermes Agent在线能力变化时使用日期快照和本地等价任务 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 项目复盘

{% note info flat %}
扩展复盘比较依赖版本、Provider、工具权限和回滚结果；开源仓库可读不等于运行时自动隔离。本节围绕“项目复盘”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Hermes Agent 的 Diff、测试、权限或失败日志。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用证据验收Hermes Agent。核心内容：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 Hermes Agent 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。

机制：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 Hermes Agent 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。选择依据：产品验收需要逐项核对版本、行为和安全证据。先核对验收标准、失败样例和复测动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；在线不可用时保留本地验证结果。
{% endnote %}
...
{% note info flat %}
本节把Hermes Agent的“版本和任务证据”收束为版本、权限、任务、Diff 和测试证据；Hermes Agent 的开源边界要从 Provider、终端/浏览器工具、持久记忆、Skills 和消息集成分别验证，不能把扩展能力视作默认安全。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | Hermes Agent 的版本/Commit 或文档快照 | 只有最终成功文字 | 保存中间状态与失败记录 |
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(二十三)Hermes Agent","order":23,"topic":"Hermes Agent","experiment":"按官方入口完成 Hermes Agent 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。","focus":["开源通用 Agent 的安装、Provider、终端/浏览器工具、持久记忆、Skills、消息集成和安全。","定位与安装","模型与工具","记忆与Skills"],"artifacts":["Hermes Agent 的版本/Commit 或文档快照","Hermes Agent 受控小仓库的任务输出","Hermes Agent 的 Diff、测试、权限或失败日志"],"failures":["认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代"],"mode":"workspace"}')
SCENARIOS = json.loads('{"23":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "认证失败"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="provider_unconfigured")
    preflight = {"product": DATA["topic"], "failure_kind": "provider_unconfigured", "failure_case": failure_case, "declared_failure": failure_type, "online_evidence": "not_run", "provider_ready": False}
    try:
    ...
    record_event(events_file, "recovery_verified", action="record-provider-not-run", case=failure_case)
...
{% note info flat %}
本节把Hermes Agent的“失败复测”收束为版本、权限、任务、Diff 和测试证据；Hermes Agent 的开源边界要从 Provider、终端/浏览器工具、持久记忆、Skills 和消息集成分别验证，不能把扩展能力视作默认安全。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-23-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
Hermes Agent 的默认权限和最小安全配置是什么
--- answer
不能把 Hermes Agent 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。
--- explanation
Hermes Agent 的开源边界要从 Provider、终端/浏览器工具、持久记忆、Skills 和消息集成分别验证，不能把扩展能力视作默认安全。问题“Hermes Agent 的默认权限和最小安全配置是什么”的关键不在背诵名词，而在于：不能把 Hermes Agent 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。 先判断题目中的硬门槛和适用范围，再看“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-23-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
Hermes Agent 的核心上下文、工具或扩展边界是什么
--- answer
Hermes Agent 的核心边界是 Provider、终端/浏览器工具、持久记忆、Skills、消息集成与秘密边界；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。
--- explanation
Hermes Agent 的开源边界要从 Provider、终端/浏览器工具、持久记忆、Skills 和消息集成分别验证，不能把扩展能力视作默认安全。问题“Hermes Agent 的核心上下文、工具或扩展边界是什么”的关键不在背诵名词，而在于：Hermes Agent 的核心边界是 Provider、终端/浏览器工具、持久记忆、Skills、消息集成与秘密边界；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
