...
categories:
  - Learn Topic
  - Coding Agent
description: 能独立完成 Kiro 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。
...
{% note primary flat %}
本节围绕“Kiro”建立可复核的工作模型。学习成果：能独立完成 Kiro 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。实验：按官方入口完成 Kiro 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。视觉辅助：产品入口或配置截图（真实验证后）；能力表；核心流程 Mermaid。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
Kiro 的 Spec 看起来已经完成，实际实现仍可能在 Hooks、MCP 或 Headless 运行中失败。把规格状态和代码验证分成两个门禁。

本篇把“Kiro”落到一次可回放的本地任务：按官方入口完成 Kiro 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：能独立完成 Kiro 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“IDE、CLI、Web、Specs、Steering、Hooks、MCP、Custom Agents、审批和 CI/headless 工作流”拆成输入、状态、输出和证据。Kiro 把 Specs、Steering、Agent、Hooks/MCP 和 Headless/CI 串成工作流；规格完成不等于实现已通过测试和权限门禁。
{% endnote %}

{% mermaid %}
flowchart TD
  A[Kiro IDE/CLI/Web] --> B[Spec驱动]
  B --> C[Steering规则]
  C --> D[Agent与上下文]
  D --> E[Hooks/MCP]
  E --> F{审批/权限}
  F -->|通过| G[Headless/CI验证]
  F -->|拒绝| H[平台边界]
{% endmermaid %}
...
{% note primary flat %}
实验步骤：按官方入口完成 Kiro 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。

验收产物：Kiro 的版本/Commit 或文档快照；Kiro 受控小仓库的任务输出；Kiro 的 Diff、测试、权限或失败日志
...
{% note info flat %}
本节任务：确认Kiro的入口并完成最小安装。核心内容：Kiro：IDE、CLI与Web、工作区。

机制：Kiro：IDE、CLI与Web、工作区。选择依据：Kiro的入口、平台和版本是离散选择，表格能并列官方入口与适用条件。先核对Kiro的入口类型、支持平台、安装命令和版本输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Kiro入口或截图不可用时保留官方 URL、命令和版本检查。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Kiro的入口类型、支持平台、安装命令和版本输出 |
| 失败降级 | Kiro入口或截图不可用时保留官方 URL、命令和版本检查 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

{% note info flat %}
Kiro执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）
{% endnote %}

![Kiro执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）](/img/picgo-images/coding-agent-product-22.svg)

| 能力面 | 本篇观察对象 | 本地夹具证据 | 线上边界 |
| --- | --- | --- | --- |
| 入口与版本 | IDE/CLI/Web、Spec、Steering、Agent 上下文、Hooks/MCP 与 Headless/CI | 记录任务契约与 fixture 版本 | 需按官方入口、平台和版本复核 |
...
{% note primary flat %}
入口只展示官方操作路径，本次不执行安装或真实账号调用。先记录Kiro CLI的版本，再用同一受控任务比较上下文、权限和交付证据。
{% endnote %}

| 项目 | 可复核内容 |
| --- | --- |
| 入口 | Kiro CLI |
| 安装/启动 | `curl -fsSL https://cli.kiro.dev/install | bash` |
| 版本检查 | `kiro-cli --version` |
| 配置观察 | .kiro/specs/；steering；hooks；MCP；custom agents；headless/CI |
| 最小任务 | kiro-cli chat：先把 README.md 任务写进 Spec，再运行只读验证 |
...
{% note info flat %}
Kiro CLI 通过官方安装脚本进入项目，kiro-cli --version 固定版本；CLI 能启动不等于 Spec/Headless 能力一致。本节围绕“工作区”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Kiro 的版本/Commit 或文档快照。
{% endnote %}

## Spec驱动

{% note primary flat %}
本节任务：让Kiro使用指定模型完成一次脱敏认证。核心内容：Kiro：Spec驱动、Steering规则。

机制：Kiro：Spec驱动、Steering规则。选择依据：Kiro的认证示例需要可复制命令、环境变量占位符和脱敏输出，代码最不含糊。先核对Kiro的模型 ID、认证方式、权限确认和脱敏输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Kiro没有凭据时运行本地 fixture，禁止伪造在线响应。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Kiro的模型 ID、认证方式、权限确认和脱敏输出 |
| 失败降级 | Kiro没有凭据时运行本地 fixture，禁止伪造在线响应 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |
...
{% note info flat %}
Spec 先描述需求、任务和验收，Steering 负责持续规则；两者冲突时保留优先级，不让生成结果覆盖验收。本节围绕“Steering规则”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Kiro 受控小仓库的任务输出。
{% endnote %}

## 任务入口

{% note info flat %}
本节任务：在Kiro的项目上下文中定位一个受控任务。核心内容：Kiro：任务入口、项目上下文。

机制：Kiro：任务入口、项目上下文。选择依据：Kiro的上下文、Session 和任务状态是有向关系，图下补充每个状态的文字含义。先核对Kiro的规则来源、上下文范围、Session 状态和任务输入，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Kiro图表失败时按“入口→上下文→任务→结果”列出状态。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Kiro的规则来源、上下文范围、Session 状态和任务输入 |
| 失败降级 | Kiro图表失败时按“入口→上下文→任务→结果”列出状态 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |
...
{% note info flat %}
任务入口绑定项目上下文和 `.kiro` 范围；从 Spec 到代码要有一个可检查的状态转移和停止点。本节围绕“项目上下文”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Kiro 的 Diff、测试、权限或失败日志。
{% endnote %}

## Hooks触发

{% note info flat %}
本节任务：为Kiro启用一个工具或扩展并撤销它。核心内容：Kiro：Hooks触发、MCP连接。

机制：Kiro：Hooks触发、MCP连接。选择依据：Kiro的工具、Skill、MCP 和插件需要逐项比较来源、权限、输入输出，表格便于审查。先核对Kiro的来源/版本、权限、输入、输出和撤销动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Kiro扩展不可安装时使用静态 manifest 和拒绝测试。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Kiro的来源/版本、权限、输入、输出和撤销动作 |
| 失败降级 | Kiro扩展不可安装时使用静态 manifest 和拒绝测试 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### MCP连接

{% note info flat %}
Hooks 在事件边界触发，MCP 在外部能力边界触发；分别记录触发条件、参数、权限和撤销，不能合并成“插件可用”。本节围绕“MCP连接”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Kiro 的版本/Commit 或文档快照。
{% endnote %}

## 审批与权限

{% note danger flat %}
本节任务：在Kiro中审批一次有副作用的动作。核心内容：Kiro：审批与权限、Headless/CI。

机制：Kiro：审批与权限、Headless/CI。选择依据：Kiro的权限与外发风险必须脱离图表颜色仍直接可见，danger 强调不可逆后果。先核对Kiro的审批点、工作区、网络、秘密和停止条件，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Kiro拒绝或超时后保留未执行证据与恢复/回滚步骤。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Kiro的审批点、工作区、网络、秘密和停止条件 |
| 失败降级 | Kiro拒绝或超时后保留未执行证据与恢复/回滚步骤 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### Headless/CI

{% note info flat %}
Headless/CI 需要非交互认证、工作区和测试权限；本地 IDE 的一次通过不能推出流水线可运行。本节围绕“Headless/CI”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Kiro 受控小仓库的任务输出。
{% endnote %}

## 平台边界

{% note warning flat %}
本节任务：复现Kiro的一项限制并记录恢复。核心内容：Kiro：平台边界、项目验证。

机制：Kiro：平台边界、项目验证。选择依据：Kiro的版本、平台、Preview 和退出条件决定能否照搬，提示块突出边界而非宣传。先核对Kiro的版本/平台、功能限制、失败状态和替代路径，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Kiro在线能力变化时使用日期快照和本地等价任务。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Kiro的版本/平台、功能限制、失败状态和替代路径 |
| 失败降级 | Kiro在线能力变化时使用日期快照和本地等价任务 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 项目验证

{% note info flat %}
平台验证至少记录操作系统、CLI 版本、预览标记和失败日志；差异未复现时维持未验证。本节围绕“项目验证”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Kiro 的 Diff、测试、权限或失败日志。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用证据验收Kiro。核心内容：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 Kiro 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。

机制：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 Kiro 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。选择依据：产品验收需要逐项核对版本、行为和安全证据。先核对验收标准、失败样例和复测动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；在线不可用时保留本地验证结果。
{% endnote %}
...
{% note info flat %}
本节把Kiro的“版本和任务证据”收束为版本、权限、任务、Diff 和测试证据；Kiro 把 Specs、Steering、Agent、Hooks/MCP 和 Headless/CI 串成工作流；规格完成不等于实现已通过测试和权限门禁。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | Kiro 的版本/Commit 或文档快照 | 只有最终成功文字 | 保存中间状态与失败记录 |
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(二十二)Kiro","order":22,"topic":"Kiro","experiment":"按官方入口完成 Kiro 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。","focus":["IDE、CLI、Web、Specs、Steering、Hooks、MCP、Custom Agents、审批和 CI/headless 工作流。","入口与工作区","Specs与Steering","Agent与上下文"],"artifacts":["Kiro 的版本/Commit 或文档快照","Kiro 受控小仓库的任务输出","Kiro 的 Diff、测试、权限或失败日志"],"failures":["认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代"],"mode":"workspace"}')
SCENARIOS = json.loads('{"22":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    record_event(events_file, "recovery_verified", action="record-headless-boundary", case=failure_case)
...
{% note info flat %}
本节把Kiro的“失败复测”收束为版本、权限、任务、Diff 和测试证据；Kiro 把 Specs、Steering、Agent、Hooks/MCP 和 Headless/CI 串成工作流；规格完成不等于实现已通过测试和权限门禁。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-22-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
Kiro 的默认权限和最小安全配置是什么
--- answer
不能把 Kiro 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。
--- explanation
Kiro 把 Specs、Steering、Agent、Hooks/MCP 和 Headless/CI 串成工作流；规格完成不等于实现已通过测试和权限门禁。问题“Kiro 的默认权限和最小安全配置是什么”的关键不在背诵名词，而在于：不能把 Kiro 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。 先判断题目中的硬门槛和适用范围，再看“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-22-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
Kiro 的核心上下文、工具或扩展边界是什么
--- answer
Kiro 的核心边界是 IDE/CLI/Web、Spec、Steering、Agent 上下文、Hooks/MCP 与 Headless/CI；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。
--- explanation
Kiro 把 Specs、Steering、Agent、Hooks/MCP 和 Headless/CI 串成工作流；规格完成不等于实现已通过测试和权限门禁。问题“Kiro 的核心上下文、工具或扩展边界是什么”的关键不在背诵名词，而在于：Kiro 的核心边界是 IDE/CLI/Web、Spec、Steering、Agent 上下文、Hooks/MCP 与 Headless/CI；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
