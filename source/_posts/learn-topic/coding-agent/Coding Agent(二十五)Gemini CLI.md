---
title: Coding Agent(二十五)Gemini CLI
tags:
  - Coding Agent
  - Gemini CLI
categories:
  - Learn Topic
  - Coding Agent
description: 能独立完成 Gemini CLI 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。
...
{% note primary flat %}
本节围绕“Gemini CLI”建立可复核的工作模型。学习成果：能独立完成 Gemini CLI 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。实验：按官方入口完成 Gemini CLI 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。视觉辅助：产品入口或配置截图（真实验证后）；能力表；核心流程 Mermaid。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
Gemini CLI 读取 GEMINI.md 后可能获得更完整的项目上下文，但 Extensions、命令和沙箱仍要单独审查。安装成功只证明入口，不证明任务安全。

本篇把“Gemini CLI”落到一次可回放的本地任务：按官方入口完成 Gemini CLI 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：能独立完成 Gemini CLI 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“安装、认证、GEMINI.md、Context、Tools、Extensions、MCP、沙箱、命令和项目工作流”拆成输入、状态、输出和证据。Gemini CLI 的 GEMINI.md、内置 Tools/命令、Extensions/MCP 与沙箱共同决定项目上下文和执行边界，安装成功不等于任务安全。
{% endnote %}

{% mermaid %}
flowchart TD
  A[Gemini CLI npm/npx] --> B[GEMINI.md上下文]
  B --> C[内置Tools/命令]
  C --> D[Extensions/MCP]
  D --> E{沙箱策略}
  E -->|允许| F[测试与Diff]
...
{% note primary flat %}
实验步骤：按官方入口完成 Gemini CLI 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。

验收产物：Gemini CLI 的版本/Commit 或文档快照；Gemini CLI 受控小仓库的任务输出；Gemini CLI 的 Diff、测试、权限或失败日志
...
{% note info flat %}
本节任务：确认Gemini CLI的入口并完成最小安装。核心内容：Gemini CLI：npm/npx入口、认证方式。

机制：Gemini CLI：npm/npx入口、认证方式。选择依据：Gemini CLI的入口、平台和版本是离散选择，表格能并列官方入口与适用条件。先核对Gemini CLI的入口类型、支持平台、安装命令和版本输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Gemini CLI入口或截图不可用时保留官方 URL、命令和版本检查。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Gemini CLI的入口类型、支持平台、安装命令和版本输出 |
| 失败降级 | Gemini CLI入口或截图不可用时保留官方 URL、命令和版本检查 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

{% note info flat %}
Gemini CLI执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）
{% endnote %}

![Gemini CLI执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）](/img/picgo-images/coding-agent-product-25.svg)

| 能力面 | 本篇观察对象 | 本地夹具证据 | 线上边界 |
| --- | --- | --- | --- |
| 入口与版本 | npm/npx、GEMINI.md、内置 Tools、Extensions/MCP 与沙箱策略 | 记录任务契约与 fixture 版本 | 需按官方入口、平台和版本复核 |
...
{% note primary flat %}
入口只展示官方操作路径，本次不执行安装或真实账号调用。先记录Gemini CLI的版本，再用同一受控任务比较上下文、权限和交付证据。
{% endnote %}

| 项目 | 可复核内容 |
| --- | --- |
| 入口 | Gemini CLI |
| 安装/启动 | `npm install -g @google/gemini-cli` |
| 版本检查 | `gemini --version` |
| 配置观察 | GEMINI.md；Extensions/MCP；Tools/commands；--sandbox；workspace |
| 最小任务 | gemini --sandbox -p "只读取 README.md，返回验证步骤" |
...
{% note info flat %}
Gemini CLI 通过 npm 安装，gemini --version 只证明入口；认证方式和可用模型要另行记录。本节围绕“认证方式”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Gemini CLI 的版本/Commit 或文档快照。
{% endnote %}

## GEMINI.md

{% note primary flat %}
本节任务：让Gemini CLI使用指定模型完成一次脱敏认证。核心内容：Gemini CLI：GEMINI.md、Context管理。

机制：Gemini CLI：GEMINI.md、Context管理。选择依据：Gemini CLI的认证示例需要可复制命令、环境变量占位符和脱敏输出，代码最不含糊。先核对Gemini CLI的模型 ID、认证方式、权限确认和脱敏输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Gemini CLI没有凭据时运行本地 fixture，禁止伪造在线响应。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Gemini CLI的模型 ID、认证方式、权限确认和脱敏输出 |
| 失败降级 | Gemini CLI没有凭据时运行本地 fixture，禁止伪造在线响应 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

```text
GEMINI.md: 输入 -> 校验 -> 执行 -> 证据
```

### Context管理

{% note info flat %}
GEMINI.md 是项目 Context 的来源，层级与内容范围要可读；它不能越过工具权限和沙箱。本节围绕“Context管理”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Gemini CLI 受控小仓库的任务输出。
{% endnote %}

## 内置工具

{% note info flat %}
本节任务：在Gemini CLI的项目上下文中定位一个受控任务。核心内容：Gemini CLI：内置工具、CLI命令。

机制：Gemini CLI：内置工具、CLI命令。选择依据：Gemini CLI的上下文、Session 和任务状态是有向关系，图下补充每个状态的文字含义。先核对Gemini CLI的规则来源、上下文范围、Session 状态和任务输入，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Gemini CLI图表失败时按“入口→上下文→任务→结果”列出状态。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Gemini CLI的规则来源、上下文范围、Session 状态和任务输入 |
| 失败降级 | Gemini CLI图表失败时按“入口→上下文→任务→结果”列出状态 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |
...
{% note info flat %}
内置 Tools/commands 要区分读取、写入和 shell 副作用；用只读 `--sandbox` 基线再扩大范围。本节围绕“CLI命令”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Gemini CLI 的 Diff、测试、权限或失败日志。
{% endnote %}

## Extensions

{% note info flat %}
本节任务：为Gemini CLI启用一个工具或扩展并撤销它。核心内容：Gemini CLI：Extensions、MCP连接。

机制：Gemini CLI：Extensions、MCP连接。选择依据：Gemini CLI的工具、Skill、MCP 和插件需要逐项比较来源、权限、输入输出，表格便于审查。先核对Gemini CLI的来源/版本、权限、输入、输出和撤销动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Gemini CLI扩展不可安装时使用静态 manifest 和拒绝测试。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Gemini CLI的来源/版本、权限、输入、输出和撤销动作 |
| 失败降级 | Gemini CLI扩展不可安装时使用静态 manifest 和拒绝测试 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### MCP连接

{% note info flat %}
Extensions 与 MCP 的描述、依赖和返回文本都不可信；锁定来源后测试能力发现、参数校验和撤销。本节围绕“MCP连接”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Gemini CLI 的版本/Commit 或文档快照。
{% endnote %}

## 沙箱策略

{% note danger flat %}
本节任务：在Gemini CLI中审批一次有副作用的动作。核心内容：Gemini CLI：沙箱策略、测试与Diff。

机制：Gemini CLI：沙箱策略、测试与Diff。选择依据：Gemini CLI的权限与外发风险必须脱离图表颜色仍直接可见，danger 强调不可逆后果。先核对Gemini CLI的审批点、工作区、网络、秘密和停止条件，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Gemini CLI拒绝或超时后保留未执行证据与恢复/回滚步骤。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Gemini CLI的审批点、工作区、网络、秘密和停止条件 |
| 失败降级 | Gemini CLI拒绝或超时后保留未执行证据与恢复/回滚步骤 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 测试与Diff

{% note info flat %}
沙箱策略应保存模式、挂载目录、网络和审批；测试通过只覆盖该环境的行为，不覆盖宿主秘密。本节围绕“测试与Diff”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Gemini CLI 受控小仓库的任务输出。
{% endnote %}

## 模型/平台边界

{% note warning flat %}
本节任务：复现Gemini CLI的一项限制并记录恢复。核心内容：Gemini CLI：模型/平台边界、项目验证。

机制：Gemini CLI：模型/平台边界、项目验证。选择依据：Gemini CLI的版本、平台、Preview 和退出条件决定能否照搬，提示块突出边界而非宣传。先核对Gemini CLI的版本/平台、功能限制、失败状态和替代路径，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Gemini CLI在线能力变化时使用日期快照和本地等价任务。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Gemini CLI的版本/平台、功能限制、失败状态和替代路径 |
| 失败降级 | Gemini CLI在线能力变化时使用日期快照和本地等价任务 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 项目验证

{% note info flat %}
模型/平台边界包括 CLI 版本、系统、认证和上下文文件；无法重放的项列为条件而不是结论。本节围绕“项目验证”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Gemini CLI 的 Diff、测试、权限或失败日志。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用证据验收Gemini CLI。核心内容：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 Gemini CLI 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。

机制：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 Gemini CLI 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。选择依据：产品验收需要逐项核对版本、行为和安全证据。先核对验收标准、失败样例和复测动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；在线不可用时保留本地验证结果。
{% endnote %}
...
{% note info flat %}
本节把Gemini CLI的“版本和任务证据”收束为版本、权限、任务、Diff 和测试证据；Gemini CLI 的 GEMINI.md、内置 Tools/命令、Extensions/MCP 与沙箱共同决定项目上下文和执行边界，安装成功不等于任务安全。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | Gemini CLI 的版本/Commit 或文档快照 | 只有最终成功文字 | 保存中间状态与失败记录 |
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(二十五)Gemini CLI","order":25,"topic":"Gemini CLI","experiment":"按官方入口完成 Gemini CLI 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。","focus":["安装、认证、GEMINI.md、Context、Tools、Extensions、MCP、沙箱、命令和项目工作流。","安装与认证","GEMINI.md与上下文","Tools与命令"],"artifacts":["Gemini CLI 的版本/Commit 或文档快照","Gemini CLI 受控小仓库的任务输出","Gemini CLI 的 Diff、测试、权限或失败日志"],"failures":["认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代"],"mode":"workspace"}')
SCENARIOS = json.loads('{"25":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    record_event(events_file, "recovery_verified", action="record-model-auth-not-run", case=failure_case)
...
{% note info flat %}
本节把Gemini CLI的“失败复测”收束为版本、权限、任务、Diff 和测试证据；Gemini CLI 的 GEMINI.md、内置 Tools/命令、Extensions/MCP 与沙箱共同决定项目上下文和执行边界，安装成功不等于任务安全。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-25-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
Gemini CLI 的默认权限和最小安全配置是什么
--- answer
不能把 Gemini CLI 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。
--- explanation
Gemini CLI 的 GEMINI.md、内置 Tools/命令、Extensions/MCP 与沙箱共同决定项目上下文和执行边界，安装成功不等于任务安全。问题“Gemini CLI 的默认权限和最小安全配置是什么”的关键不在背诵名词，而在于：不能把 Gemini CLI 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。 先判断题目中的硬门槛和适用范围，再看“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-25-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
Gemini CLI 的核心上下文、工具或扩展边界是什么
--- answer
Gemini CLI 的核心边界是 npm/npx、GEMINI.md、内置 Tools、Extensions/MCP 与沙箱策略；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。
--- explanation
Gemini CLI 的 GEMINI.md、内置 Tools/命令、Extensions/MCP 与沙箱共同决定项目上下文和执行边界，安装成功不等于任务安全。问题“Gemini CLI 的核心上下文、工具或扩展边界是什么”的关键不在背诵名词，而在于：Gemini CLI 的核心边界是 npm/npx、GEMINI.md、内置 Tools、Extensions/MCP 与沙箱策略；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
