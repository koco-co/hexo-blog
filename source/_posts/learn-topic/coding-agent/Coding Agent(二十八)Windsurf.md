---
title: Coding Agent(二十八)Windsurf
tags:
  - Coding Agent
  - Windsurf
categories:
  - Learn Topic
  - Coding Agent
description: 能独立完成 Windsurf 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。
...
{% note primary flat %}
本节围绕“Windsurf”建立可复核的工作模型。学习成果：能独立完成 Windsurf 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。实验：按官方入口完成 Windsurf 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。视觉辅助：产品入口或配置截图（真实验证后）；能力表；核心流程 Mermaid。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
Windsurf 的 Cascade 在 Plan 与 Code 模式中的行为不同；Workflows、MCP 和 Checkpoint 让计划、执行和回退需要分开验收。

本篇把“Windsurf”落到一次可回放的本地任务：按官方入口完成 Windsurf 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：能独立完成 Windsurf 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“Cascade 的 Code/Plan/Ask、项目分析、Context、Terminal、Workflows、MCP、Checkpoint 和审查”拆成输入、状态、输出和证据。Windsurf 的 Cascade Code/Plan/Ask、上下文分析、Workflows、MCP 和 Checkpoint 要按模式记录；计划视图不等于已执行代码。
{% endnote %}

{% mermaid %}
flowchart TD
  A[Windsurf Cascade] --> B[代码库分析/Context]
  B --> C{Plan或Code}
  C -->|Plan| D[任务计划]
  C -->|Code| E[执行]
  D --> E
...
{% note primary flat %}
实验步骤：按官方入口完成 Windsurf 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。

验收产物：Windsurf 的版本/Commit 或文档快照；Windsurf 受控小仓库的任务输出；Windsurf 的 Diff、测试、权限或失败日志
...
{% note info flat %}
本节任务：确认Windsurf的入口并完成最小安装。核心内容：Windsurf：Cascade模式、工作区。

机制：Windsurf：Cascade模式、工作区。选择依据：Windsurf的入口、平台和版本是离散选择，表格能并列官方入口与适用条件。先核对Windsurf的入口类型、支持平台、安装命令和版本输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Windsurf入口或截图不可用时保留官方 URL、命令和版本检查。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Windsurf的入口类型、支持平台、安装命令和版本输出 |
| 失败降级 | Windsurf入口或截图不可用时保留官方 URL、命令和版本检查 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

{% note info flat %}
Windsurf执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）
{% endnote %}

![Windsurf执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）](/img/picgo-images/coding-agent-product-28.svg)

| 能力面 | 本篇观察对象 | 本地夹具证据 | 线上边界 |
| --- | --- | --- | --- |
| 入口与版本 | Cascade、代码库 Context、Plan/Code、Workflows/MCP、Checkpoint 与权限 | 记录任务契约与 fixture 版本 | 需按官方入口、平台和版本复核 |
...
{% note primary flat %}
入口只展示官方操作路径，本次不执行安装或真实账号调用。先记录Windsurf Cascade的版本，再用同一受控任务比较上下文、权限和交付证据。
{% endnote %}

| 项目 | 可复核内容 |
| --- | --- |
| 入口 | Windsurf Cascade |
| 安装/启动 | `从 windsurf.com 官方入口安装桌面应用` |
| 版本检查 | `在 Cascade 中记录版本与 Code/Plan/Ask 模式` |
| 配置观察 | Cascade mode；.windsurf/workflows；MCP；Checkpoint；permissions |
| 最小任务 | 用 Plan 模式拆分 README.md 任务，不切换到 Code 写入 |
...
{% note info flat %}
Windsurf Cascade 先从官方桌面入口安装并记录版本；Cascade 可见不代表 Code/Plan/Ask 三种模式都可用。本节围绕“工作区”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Windsurf 的版本/Commit 或文档快照。
{% endnote %}

## 代码库分析

{% note primary flat %}
本节任务：让Windsurf使用指定模型完成一次脱敏认证。核心内容：Windsurf：代码库分析、上下文选择。

机制：Windsurf：代码库分析、上下文选择。选择依据：Windsurf的认证示例需要可复制命令、环境变量占位符和脱敏输出，代码最不含糊。先核对Windsurf的模型 ID、认证方式、权限确认和脱敏输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Windsurf没有凭据时运行本地 fixture，禁止伪造在线响应。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Windsurf的模型 ID、认证方式、权限确认和脱敏输出 |
| 失败降级 | Windsurf没有凭据时运行本地 fixture，禁止伪造在线响应 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |
...
{% note info flat %}
工作区上下文来自代码库分析和项目文件；缩小范围并检查索引/忽略规则，避免把秘密带入 Context。本节围绕“上下文选择”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Windsurf 受控小仓库的任务输出。
{% endnote %}

## Plan任务

{% note info flat %}
本节任务：在Windsurf的项目上下文中定位一个受控任务。核心内容：Windsurf：Plan任务、Code执行。

机制：Windsurf：Plan任务、Code执行。选择依据：Windsurf的上下文、Session 和任务状态是有向关系，图下补充每个状态的文字含义。先核对Windsurf的规则来源、上下文范围、Session 状态和任务输入，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Windsurf图表失败时按“入口→上下文→任务→结果”列出状态。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Windsurf的规则来源、上下文范围、Session 状态和任务输入 |
| 失败降级 | Windsurf图表失败时按“入口→上下文→任务→结果”列出状态 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |
...
{% note info flat %}
Plan 只产生计划，Code 才执行写入；在同一任务中保存计划、批准、Diff 和测试的因果顺序。本节围绕“Code执行”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Windsurf 的 Diff、测试、权限或失败日志。
{% endnote %}

## Workflows

{% note info flat %}
本节任务：为Windsurf启用一个工具或扩展并撤销它。核心内容：Windsurf：Workflows、MCP与工具。

机制：Windsurf：Workflows、MCP与工具。选择依据：Windsurf的工具、Skill、MCP 和插件需要逐项比较来源、权限、输入输出，表格便于审查。先核对Windsurf的来源/版本、权限、输入、输出和撤销动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Windsurf扩展不可安装时使用静态 manifest 和拒绝测试。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Windsurf的来源/版本、权限、输入、输出和撤销动作 |
| 失败降级 | Windsurf扩展不可安装时使用静态 manifest 和拒绝测试 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### MCP与工具

{% note info flat %}
Workflows、MCP 和工具分别带来可复用指令、外部能力和副作用；每项都要有来源、参数和撤销。本节围绕“MCP与工具”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Windsurf 的版本/Commit 或文档快照。
{% endnote %}

## Checkpoint

{% note danger flat %}
本节任务：在Windsurf中审批一次有副作用的动作。核心内容：Windsurf：Checkpoint、执行确认。

机制：Windsurf：Checkpoint、执行确认。选择依据：Windsurf的权限与外发风险必须脱离图表颜色仍直接可见，danger 强调不可逆后果。先核对Windsurf的审批点、工作区、网络、秘密和停止条件，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Windsurf拒绝或超时后保留未执行证据与恢复/回滚步骤。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Windsurf的审批点、工作区、网络、秘密和停止条件 |
| 失败降级 | Windsurf拒绝或超时后保留未执行证据与恢复/回滚步骤 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 执行确认

{% note info flat %}
Checkpoint 是恢复线索而非 Git 替代；执行确认后仍用 Git/Diff 与测试判断是否可交付。本节围绕“执行确认”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Windsurf 受控小仓库的任务输出。
{% endnote %}

## 平台边界

{% note warning flat %}
本节任务：复现Windsurf的一项限制并记录恢复。核心内容：Windsurf：平台边界、项目验证。

机制：Windsurf：平台边界、项目验证。选择依据：Windsurf的版本、平台、Preview 和退出条件决定能否照搬，提示块突出边界而非宣传。先核对Windsurf的版本/平台、功能限制、失败状态和替代路径，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Windsurf在线能力变化时使用日期快照和本地等价任务。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Windsurf的版本/平台、功能限制、失败状态和替代路径 |
| 失败降级 | Windsurf在线能力变化时使用日期快照和本地等价任务 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 项目验证

{% note info flat %}
平台边界记录 Cascade 版本、模式、模型、权限和网络条件；缺少线上证据时保留静态/fixture 结论。本节围绕“项目验证”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Windsurf 的 Diff、测试、权限或失败日志。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用证据验收Windsurf。核心内容：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 Windsurf 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。

机制：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 Windsurf 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。选择依据：产品验收需要逐项核对版本、行为和安全证据。先核对验收标准、失败样例和复测动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；在线不可用时保留本地验证结果。
{% endnote %}
...
{% note info flat %}
本节把Windsurf的“版本和任务证据”收束为版本、权限、任务、Diff 和测试证据；Windsurf 的 Cascade Code/Plan/Ask、上下文分析、Workflows、MCP 和 Checkpoint 要按模式记录；计划视图不等于已执行代码。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | Windsurf 的版本/Commit 或文档快照 | 只有最终成功文字 | 保存中间状态与失败记录 |
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(二十八)Windsurf","order":28,"topic":"Windsurf","experiment":"按官方入口完成 Windsurf 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。","focus":["Cascade 的 Code/Plan/Ask、项目分析、Context、Terminal、Workflows、MCP、Checkpoint 和审查。","入口与模式","Cascade上下文","Plan与Code"],"artifacts":["Windsurf 的版本/Commit 或文档快照","Windsurf 受控小仓库的任务输出","Windsurf 的 Diff、测试、权限或失败日志"],"failures":["认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代"],"mode":"workspace"}')
SCENARIOS = json.loads('{"28":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "预览能力"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="mode_mismatch")
    preflight = {"product": DATA["topic"], "failure_kind": "mode_mismatch", "failure_case": failure_case, "declared_failure": failure_type, "online_evidence": "not_run", "mode_matches": False}
    try:
        if not preflight["mode_matches"]:
            raise PermissionError("requested mode is not available in this fixture")
    ...
    record_event(events_file, "recovery_verified", action="record-cascade-mode-boundary", case=failure_case)
...
{% note info flat %}
本节把Windsurf的“失败复测”收束为版本、权限、任务、Diff 和测试证据；Windsurf 的 Cascade Code/Plan/Ask、上下文分析、Workflows、MCP 和 Checkpoint 要按模式记录；计划视图不等于已执行代码。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-28-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
Windsurf 的默认权限和最小安全配置是什么
--- answer
不能把 Windsurf 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。
--- explanation
Windsurf 的 Cascade Code/Plan/Ask、上下文分析、Workflows、MCP 和 Checkpoint 要按模式记录；计划视图不等于已执行代码。问题“Windsurf 的默认权限和最小安全配置是什么”的关键不在背诵名词，而在于：不能把 Windsurf 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。 先判断题目中的硬门槛和适用范围，再看“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-28-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
Windsurf 的核心上下文、工具或扩展边界是什么
--- answer
Windsurf 的核心边界是 Cascade、代码库 Context、Plan/Code、Workflows/MCP、Checkpoint 与权限；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。
--- explanation
Windsurf 的 Cascade Code/Plan/Ask、上下文分析、Workflows、MCP 和 Checkpoint 要按模式记录；计划视图不等于已执行代码。问题“Windsurf 的核心上下文、工具或扩展边界是什么”的关键不在背诵名词，而在于：Windsurf 的核心边界是 Cascade、代码库 Context、Plan/Code、Workflows/MCP、Checkpoint 与权限；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
