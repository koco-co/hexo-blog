---
title: Coding Agent(二十六)OpenCode
tags:
  - Coding Agent
  - OpenCode
categories:
  - Learn Topic
  - Coding Agent
description: 能独立完成 OpenCode 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。
...
{% note primary flat %}
本节围绕“OpenCode”建立可复核的工作模型。学习成果：能独立完成 OpenCode 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。实验：按官方入口完成 OpenCode 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。视觉辅助：产品入口或配置截图（真实验证后）；能力表；核心流程 Mermaid。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
OpenCode 的 v1/v2 文档和 Agent 模式发生迁移时，旧配置可能仍能启动却改变权限或工具选择。版本快照与回退记录比界面印象更可靠。

本篇把“OpenCode”落到一次可回放的本地任务：按官方入口完成 OpenCode 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：能独立完成 OpenCode 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“当前 v1/v2 文档边界、TUI/IDE/桌面入口、Build/Plan/General/Explore Agent、Provider、插件、MCP 和权限”拆成输入、状态、输出和证据。OpenCode 的 v1/v2 文档边界、Build/Plan/General/Explore Agent、Provider、Session 和插件迁移需要以版本快照分别复核。
{% endnote %}

{% mermaid %}
flowchart TD
  A[OpenCode TUI/IDE] --> B[Build/Plan/General/Explore]
  B --> C[Provider与模型]
  C --> D[Session与工具]
  D --> E[Plugins/Skills/MCP]
  E --> F{版本迁移}
  F -->|可复现| G[安全验证]
  F -->|冲突| H[回退记录]
{% endmermaid %}
...
{% note primary flat %}
实验步骤：按官方入口完成 OpenCode 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。

验收产物：OpenCode 的版本/Commit 或文档快照；OpenCode 受控小仓库的任务输出；OpenCode 的 Diff、测试、权限或失败日志
...
{% note info flat %}
本节任务：确认OpenCode的入口并完成最小安装。核心内容：OpenCode：v1/v2边界、TUI/IDE/桌面。

机制：OpenCode：v1/v2边界、TUI/IDE/桌面。选择依据：OpenCode的入口、平台和版本是离散选择，表格能并列官方入口与适用条件。先核对OpenCode的入口类型、支持平台、安装命令和版本输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；OpenCode入口或截图不可用时保留官方 URL、命令和版本检查。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | OpenCode的入口类型、支持平台、安装命令和版本输出 |
| 失败降级 | OpenCode入口或截图不可用时保留官方 URL、命令和版本检查 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

{% note info flat %}
OpenCode执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）
{% endnote %}

![OpenCode执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）](/img/picgo-images/coding-agent-product-26.svg)

| 能力面 | 本篇观察对象 | 本地夹具证据 | 线上边界 |
| --- | --- | --- | --- |
| 入口与版本 | TUI/IDE/桌面、Build/Plan/General/Explore、Provider、Session 与 Plugins/MCP | 记录任务契约与 fixture 版本 | 需按官方入口、平台和版本复核 |
...
{% note primary flat %}
入口只展示官方操作路径，本次不执行安装或真实账号调用。先记录OpenCode的版本，再用同一受控任务比较上下文、权限和交付证据。
{% endnote %}

| 项目 | 可复核内容 |
| --- | --- |
| 入口 | OpenCode |
| 安装/启动 | `curl -fsSL https://opencode.ai/install | bash` |
| 版本检查 | `opencode --version` |
| 配置观察 | opencode.json；Build/Plan/General/Explore；Provider；Session；Plugins/Skills/MCP |
| 最小任务 | opencode：先用 Plan 产出 README.md 的一行 Diff，再切换验证 |
...
{% note info flat %}
OpenCode 的官方安装器与 npm/v1/v2 入口可能并存；先用 opencode --version 和 `opencode --help` 确认实际二进制。本节围绕“TUI/IDE/桌面”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为OpenCode 的版本/Commit 或文档快照。
{% endnote %}

## Build/Plan

{% note primary flat %}
本节任务：让OpenCode使用指定模型完成一次脱敏认证。核心内容：OpenCode：Build/Plan、General/Explore。

机制：OpenCode：Build/Plan、General/Explore。选择依据：OpenCode的认证示例需要可复制命令、环境变量占位符和脱敏输出，代码最不含糊。先核对OpenCode的模型 ID、认证方式、权限确认和脱敏输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；OpenCode没有凭据时运行本地 fixture，禁止伪造在线响应。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | OpenCode的模型 ID、认证方式、权限确认和脱敏输出 |
| 失败降级 | OpenCode没有凭据时运行本地 fixture，禁止伪造在线响应 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

```text
Build/Plan: 输入 -> 校验 -> 执行 -> 证据
```

### General/Explore

{% note info flat %}
Build、Plan、General 和 Explore 是不同 Agent 角色；先在 Plan 产生计划，再验证切换是否改变工具和写入权限。本节围绕“General/Explore”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为OpenCode 受控小仓库的任务输出。
{% endnote %}

## Provider配置

{% note info flat %}
本节任务：在OpenCode的项目上下文中定位一个受控任务。核心内容：OpenCode：Provider配置、模型选择。

机制：OpenCode：Provider配置、模型选择。选择依据：OpenCode的上下文、Session 和任务状态是有向关系，图下补充每个状态的文字含义。先核对OpenCode的规则来源、上下文范围、Session 状态和任务输入，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；OpenCode图表失败时按“入口→上下文→任务→结果”列出状态。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | OpenCode的规则来源、上下文范围、Session 状态和任务输入 |
| 失败降级 | OpenCode图表失败时按“入口→上下文→任务→结果”列出状态 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |
...
{% note info flat %}
Provider 配置保存模型、endpoint 和认证引用；模型可列出不等于该请求、上下文和工具能力全部可用。本节围绕“模型选择”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为OpenCode 的 Diff、测试、权限或失败日志。
{% endnote %}

## Session

{% note info flat %}
本节任务：为OpenCode启用一个工具或扩展并撤销它。核心内容：OpenCode：Session、工具权限。

机制：OpenCode：Session、工具权限。选择依据：OpenCode的工具、Skill、MCP 和插件需要逐项比较来源、权限、输入输出，表格便于审查。先核对OpenCode的来源/版本、权限、输入、输出和撤销动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；OpenCode扩展不可安装时使用静态 manifest 和拒绝测试。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | OpenCode的来源/版本、权限、输入、输出和撤销动作 |
| 失败降级 | OpenCode扩展不可安装时使用静态 manifest 和拒绝测试 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 工具权限

{% note info flat %}
Session 恢复时核对当前工具 allowlist、工作目录和分支；旧 Session 不应悄悄继承新权限。本节围绕“工具权限”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为OpenCode 的版本/Commit 或文档快照。
{% endnote %}

## Plugins

{% note danger flat %}
本节任务：在OpenCode中审批一次有副作用的动作。核心内容：OpenCode：Plugins、Skills与MCP。

机制：OpenCode：Plugins、Skills与MCP。选择依据：OpenCode的权限与外发风险必须脱离图表颜色仍直接可见，danger 强调不可逆后果。先核对OpenCode的审批点、工作区、网络、秘密和停止条件，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；OpenCode拒绝或超时后保留未执行证据与恢复/回滚步骤。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | OpenCode的审批点、工作区、网络、秘密和停止条件 |
| 失败降级 | OpenCode拒绝或超时后保留未执行证据与恢复/回滚步骤 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### Skills与MCP

{% note info flat %}
Plugins、Skills 和 MCP 的来源与版本需要锁定；返回的工具描述只作为待审查输入，不是授权本身。本节围绕“Skills与MCP”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为OpenCode 受控小仓库的任务输出。
{% endnote %}

## 版本迁移

{% note warning flat %}
本节任务：复现OpenCode的一项限制并记录恢复。核心内容：OpenCode：版本迁移、配置迁移与客户端/插件迁移。

机制：OpenCode：版本迁移、配置迁移与客户端/插件迁移。选择依据：OpenCode的版本、平台、Preview 和退出条件决定能否照搬，提示块突出边界而非宣传。先核对OpenCode的版本/平台、功能限制、失败状态和替代路径，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；OpenCode在线能力变化时使用日期快照和本地等价任务。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | OpenCode的版本/平台、功能限制、失败状态和替代路径 |
| 失败降级 | OpenCode在线能力变化时使用日期快照和本地等价任务 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 配置迁移与客户端/插件迁移

{% note info flat %}
版本迁移同时测试配置、客户端、插件和 Session；冲突时停在兼容层并保留旧/新配置差异。本节围绕“配置迁移与客户端/插件迁移”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为OpenCode 的 Diff、测试、权限或失败日志。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用证据验收OpenCode。核心内容：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 OpenCode 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。

机制：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 OpenCode 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。选择依据：产品验收需要逐项核对版本、行为和安全证据。先核对验收标准、失败样例和复测动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；在线不可用时保留本地验证结果。
{% endnote %}
...
{% note info flat %}
本节把OpenCode的“版本和任务证据”收束为版本、权限、任务、Diff 和测试证据；OpenCode 的 v1/v2 文档边界、Build/Plan/General/Explore Agent、Provider、Session 和插件迁移需要以版本快照分别复核。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | OpenCode 的版本/Commit 或文档快照 | 只有最终成功文字 | 保存中间状态与失败记录 |
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(二十六)OpenCode","order":26,"topic":"OpenCode","experiment":"按官方入口完成 OpenCode 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。","focus":["当前 v1/v2 文档边界、TUI/IDE/桌面入口、Build/Plan/General/Explore Agent、Provider、插件、MCP 和权限。","版本与入口","Agent模式","模型与Provider"],"artifacts":["OpenCode 的版本/Commit 或文档快照","OpenCode 受控小仓库的任务输出","OpenCode 的 Diff、测试、权限或失败日志"],"failures":["认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代"],"mode":"workspace"}')
SCENARIOS = json.loads('{"26":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "版本漂移"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="migration_conflict")
    preflight = {"product": DATA["topic"], "failure_kind": "migration_conflict", "failure_case": failure_case, "declared_failure": failure_type, "online_evidence": "not_run", "migration_safe": False}
    try:
        if not preflight["migration_safe"]:
            raise PermissionError("configuration migration would change behavior")
    ...
    record_event(events_file, "recovery_verified", action="stop-version-migration", case=failure_case)
...
{% note info flat %}
本节把OpenCode的“失败复测”收束为版本、权限、任务、Diff 和测试证据；OpenCode 的 v1/v2 文档边界、Build/Plan/General/Explore Agent、Provider、Session 和插件迁移需要以版本快照分别复核。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-26-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
OpenCode 的默认权限和最小安全配置是什么
--- answer
不能把 OpenCode 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。
--- explanation
OpenCode 的 v1/v2 文档边界、Build/Plan/General/Explore Agent、Provider、Session 和插件迁移需要以版本快照分别复核。问题“OpenCode 的默认权限和最小安全配置是什么”的关键不在背诵名词，而在于：不能把 OpenCode 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。 先判断题目中的硬门槛和适用范围，再看“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-26-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
OpenCode 的核心上下文、工具或扩展边界是什么
--- answer
OpenCode 的核心边界是 TUI/IDE/桌面、Build/Plan/General/Explore、Provider、Session 与 Plugins/MCP；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。
--- explanation
OpenCode 的 v1/v2 文档边界、Build/Plan/General/Explore Agent、Provider、Session 和插件迁移需要以版本快照分别复核。问题“OpenCode 的核心上下文、工具或扩展边界是什么”的关键不在背诵名词，而在于：OpenCode 的核心边界是 TUI/IDE/桌面、Build/Plan/General/Explore、Provider、Session 与 Plugins/MCP；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
