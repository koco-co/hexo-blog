...
categories:
  - Learn Topic
  - Coding Agent
description: 每个步骤有输入、输出、失败和停止条件
...
{% note primary flat %}
本节围绕“任务拆解与验收”建立可复核的工作模型。学习成果：每个步骤有输入、输出、失败和停止条件。实验：把一个真实维护任务拆成可独立验证的阶段，不执行写入。视觉辅助：任务契约模板；验收矩阵。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
一句“修一下登录”同时包含需求、非目标、权限和上线风险。分阶段写出输入、输出和停止条件，计划才可验收。

本篇把“任务拆解与验收”落到一次可回放的本地任务：把一个真实维护任务拆成可独立验证的阶段，不执行写入。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：每个步骤有输入、输出、失败和停止条件。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“任务目标、约束、非目标和风险；计划粒度与依赖；验收标准、测试和人工审查”拆成输入、状态、输出和证据。任务拆解把目标、非目标、依赖和验收转成可执行步骤；每个步骤都应有完成证据和停止条件。
{% endnote %}
...
{% note primary flat %}
实验步骤：把一个真实维护任务拆成可独立验证的阶段，不执行写入。

验收产物：目标/非目标/步骤/验收矩阵

失败注入：计划无证据、验收不可执行、范围漂移
{% endnote %}

## 任务契约

{% note info flat %}
本节任务：理解任务目标、约束、非目标和风险并建立可复核判断。核心内容：任务目标、约束、非目标和风险。

机制：任务目标、约束、非目标和风险。先核对输入、状态、输出、失败和复测证据，再用“计划无证据、验收不可执行、范围漂移”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“任务契约”聚焦“任务目标、约束、非目标和风险”。围绕“任务目标、约束、非目标和风险”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“任务目标、约束、非目标和风险”变成一个可观察字段，再以“把一个真实维护任务拆成可独立验证的阶段，不执行写入。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“计划无证据、验收不可执行、范围漂移”，恢复或放行动作必须可重放。
{% endnote %}

### 计划依赖

{% note info flat %}
“计划依赖”聚焦“计划粒度与依赖”。围绕“计划粒度与依赖”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“计划依赖”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“计划无证据、验收不可执行、范围漂移”，先停止并保留原始事件。本节的失败样例是“计划无证据、验收不可执行、范围漂移”，恢复或放行动作必须可重放。
{% endnote %}

## 执行计划

{% note info flat %}
本节任务：理解计划粒度与依赖并建立可复核判断。核心内容：计划粒度与依赖。

机制：计划粒度与依赖。先核对输入、状态、输出、失败和复测证据，再用“计划无证据、验收不可执行、范围漂移”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“验收门槛”聚焦“计划粒度与依赖”。围绕“计划粒度与依赖”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“验收门槛”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“计划无证据、验收不可执行、范围漂移”，先停止并保留原始事件。本节的失败样例是“计划无证据、验收不可执行、范围漂移”，恢复或放行动作必须可重放。
{% endnote %}

### 重新规划

{% note info flat %}
“重新规划”聚焦“验收标准、测试和人工审查”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。复核时把动作和产物对应起来：目标/非目标/步骤/验收矩阵应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“计划无证据、验收不可执行、范围漂移”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：把一个真实维护任务拆成可独立验证的阶段，不执行写入。

机制：把一个真实维护任务拆成可独立验证的阶段，不执行写入。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“计划无证据、验收不可执行、范围漂移”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“验收标准、测试和人工审查”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。复核时把动作和产物对应起来：目标/非目标/步骤/验收矩阵应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“计划无证据、验收不可执行、范围漂移”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(五)任务拆解与验收","order":5,"topic":"任务拆解与验收","experiment":"把一个真实维护任务拆成可独立验证的阶段，不执行写入。","focus":["任务目标、约束、非目标和风险","计划粒度与依赖","验收标准、测试和人工审查","失败后的重新规划"],"artifacts":["目标/非目标/步骤/验收矩阵"],"failures":["计划无证据、验收不可执行、范围漂移"],"mode":"workspace"}')
SCENARIOS = json.loads('{"5":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "计划无证据"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="plan_without_evidence")
    bad_report = dict(task_report)
    bad_report.pop("diff", None)
    try:
        if "diff" not in bad_report:
            raise ValueError("plan has no observable diff evidence")
    ...
    assert task_report["diff"]["files"] == ["README.md"] and task_report["checks"]["validation_passed"]
    assert failure_case in failure_type
    assert task_report["request"]["target"] == "README.md" and task_report["checks"]["validation_passed"]
    record_event(events_file, "recovery_verified", action="restore-diff-evidence", case=failure_case)
...
{% note info flat %}
“执行步骤”聚焦“失败后的重新规划”。围绕“失败后的重新规划”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“失败后的重新规划”变成一个可观察字段，再以“把一个真实维护任务拆成可独立验证的阶段，不执行写入。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“计划无证据、验收不可执行、范围漂移”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“任务目标、约束、非目标和风险”。围绕“任务目标、约束、非目标和风险”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“计划无证据、验收不可执行、范围漂移”，先停止并保留原始事件。本节的失败样例是“计划无证据、验收不可执行、范围漂移”，恢复或放行动作必须可重放。
{% endnote %}

## 验收标准

{% note info flat %}
本节任务：理解失败后的重新规划并建立可复核判断。核心内容：失败后的重新规划。

机制：失败后的重新规划。先核对输入、状态、输出、失败和复测证据，再用“计划无证据、验收不可执行、范围漂移”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“重新规划”聚焦“失败后的重新规划”。围绕“失败后的重新规划”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“失败后的重新规划”变成一个可观察字段，再以“把一个真实维护任务拆成可独立验证的阶段，不执行写入。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“计划无证据、验收不可执行、范围漂移”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：目标/非目标/步骤/验收矩阵。预期：每个步骤有输入、输出、失败和停止条件。失败复验：计划无证据、验收不可执行、范围漂移。

机制：验收证据：目标/非目标/步骤/验收矩阵。预期：每个步骤有输入、输出、失败和停止条件。失败复验：计划无证据、验收不可执行、范围漂移。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“计划无证据、验收不可执行、范围漂移”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“任务目标、约束、非目标和风险”。围绕“任务目标、约束、非目标和风险”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“计划无证据、验收不可执行、范围漂移”，先停止并保留原始事件。本节的失败样例是“计划无证据、验收不可执行、范围漂移”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 目标/非目标/步骤/验收矩阵 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 计划无证据、验收不可执行、范围漂移 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“计划粒度与依赖”。围绕“计划粒度与依赖”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：目标/非目标/步骤/验收矩阵应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“计划无证据、验收不可执行、范围漂移”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-5-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
计划不是结果
--- answer
计划只是对步骤和风险的假设，不能证明结果已经完成；结果必须由 Diff、测试和非目标检查验收。
--- explanation
任务拆解把目标、非目标、依赖和验收转成可执行步骤；每个步骤都应有完成证据和停止条件。问题“计划不是结果”的关键不在背诵名词，而在于：计划只是对步骤和风险的假设，不能证明结果已经完成；结果必须由 Diff、测试和非目标检查验收。 先判断题目中的硬门槛和适用范围，再看“计划无证据、验收不可执行、范围漂移”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-5-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
验收标准如何避免“看起来完成”
--- answer
把验收写成可观察条件：文件范围、行为、测试命令、权限、失败路径和回滚证据，而不是“看起来完成”。
--- explanation
任务拆解把目标、非目标、依赖和验收转成可执行步骤；每个步骤都应有完成证据和停止条件。问题“验收标准如何避免“看起来完成””的关键不在背诵名词，而在于：把验收写成可观察条件：文件范围、行为、测试命令、权限、失败路径和回滚证据，而不是“看起来完成”。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
