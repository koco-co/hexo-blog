...
categories:
  - Learn Topic
  - Coding Agent
description: 行为变更与重构可区分，任一阶段可回退
...
{% note primary flat %}
本节围绕“增量修改与重构”建立可复核的工作模型。学习成果：行为变更与重构可区分，任一阶段可回退。实验：在一个小模块上分两次修改，比较一次性重构和增量重构的证据。视觉辅助：增量循环图；Diff 分类表。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
一次大重构同时改变命名、行为和依赖，失败后没人知道回退哪一步。增量修改把行为变化和纯重构分开。

本篇把“增量修改与重构”落到一次可回放的本地任务：在一个小模块上分两次修改，比较一次性重构和增量重构的证据。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：行为变更与重构可区分，任一阶段可回退。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“计划—小改—测试—复查循环；重构与行为变化分离；依赖、迁移和回滚”拆成输入、状态、输出和证据。增量修改把行为变化和纯重构分离，每个小 Diff 都配测试与复查；当范围漂移时，停止比继续生成更安全。
{% endnote %}
...
{% note primary flat %}
实验步骤：在一个小模块上分两次修改，比较一次性重构和增量重构的证据。

验收产物：分阶段 Diff、测试和回滚点

失败注入：大 Diff、隐式行为变化、越改越偏
{% endnote %}

## 增量循环

{% note info flat %}
本节任务：理解计划—小改—测试—复查循环并建立可复核判断。核心内容：计划—小改—测试—复查循环。

机制：计划—小改—测试—复查循环。先核对输入、状态、输出、失败和复测证据，再用“大 Diff、隐式行为变化、越改越偏”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“增量循环”聚焦“计划—小改—测试—复查循环”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。先把“计划—小改—测试—复查循环”变成一个可观察字段，再以“在一个小模块上分两次修改，比较一次性重构和增量重构的证据。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“大 Diff、隐式行为变化、越改越偏”，恢复或放行动作必须可重放。
{% endnote %}

### 重构边界

{% note info flat %}
“重构边界”聚焦“重构与行为变化分离”。把主体、资源、动作、审批和拒绝结果放在同一条证据链；模型建议只是意图，不能越过最小权限、沙箱或人工门禁。具体取舍是：保留“重构边界”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“大 Diff、隐式行为变化、越改越偏”，先停止并保留原始事件。本节的失败样例是“大 Diff、隐式行为变化、越改越偏”，恢复或放行动作必须可重放。
{% endnote %}

## 重构边界

{% note info flat %}
本节任务：理解重构与行为变化分离并建立可复核判断。核心内容：重构与行为变化分离。

机制：重构与行为变化分离。先核对输入、状态、输出、失败和复测证据，再用“大 Diff、隐式行为变化、越改越偏”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“迁移与回滚”聚焦“重构与行为变化分离”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。具体取舍是：保留“迁移与回滚”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“大 Diff、隐式行为变化、越改越偏”，先停止并保留原始事件。本节的失败样例是“大 Diff、隐式行为变化、越改越偏”，恢复或放行动作必须可重放。
{% endnote %}

### 停止条件

{% note info flat %}
“停止条件”聚焦“依赖、迁移和回滚”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。复核时把动作和产物对应起来：分阶段 Diff、测试和回滚点应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“大 Diff、隐式行为变化、越改越偏”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：在一个小模块上分两次修改，比较一次性重构和增量重构的证据。

机制：在一个小模块上分两次修改，比较一次性重构和增量重构的证据。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“大 Diff、隐式行为变化、越改越偏”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“依赖、迁移和回滚”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。复核时把动作和产物对应起来：分阶段 Diff、测试和回滚点应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“大 Diff、隐式行为变化、越改越偏”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(八)增量修改与重构","order":8,"topic":"增量修改与重构","experiment":"在一个小模块上分两次修改，比较一次性重构和增量重构的证据。","focus":["计划—小改—测试—复查循环","重构与行为变化分离","依赖、迁移和回滚","何时停止让 Agent 继续修改"],"artifacts":["分阶段 Diff、测试和回滚点"],"failures":["大 Diff、隐式行为变化、越改越偏"],"mode":"workspace"}')
SCENARIOS = json.loads('{"8":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "大 Diff"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="oversized_diff")
    planned_files = ["README.md", "src/unrelated.py"]
    try:
        if len(planned_files) != 1 or planned_files != ["README.md"]:
            raise RuntimeError("diff exceeds the declared task scope")
    ...
    assert planned_files == task_report["diff"]["files"]
    assert failure_case in failure_type
    assert task_report["request"]["target"] == "README.md" and task_report["checks"]["validation_passed"]
    record_event(events_file, "recovery_verified", action="restore-one-file-scope", case=failure_case)
...
{% note info flat %}
“执行步骤”聚焦“何时停止让 Agent 继续修改”。围绕“何时停止让 Agent 继续修改”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“何时停止让 Agent 继续修改”变成一个可观察字段，再以“在一个小模块上分两次修改，比较一次性重构和增量重构的证据。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“大 Diff、隐式行为变化、越改越偏”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“计划—小改—测试—复查循环”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“大 Diff、隐式行为变化、越改越偏”，先停止并保留原始事件。本节的失败样例是“大 Diff、隐式行为变化、越改越偏”，恢复或放行动作必须可重放。
{% endnote %}

## 停止条件

{% note info flat %}
本节任务：理解何时停止让 Agent 继续修改并建立可复核判断。核心内容：何时停止让 Agent 继续修改。

机制：何时停止让 Agent 继续修改。先核对输入、状态、输出、失败和复测证据，再用“大 Diff、隐式行为变化、越改越偏”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“停止条件”聚焦“何时停止让 Agent 继续修改”。围绕“何时停止让 Agent 继续修改”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“何时停止让 Agent 继续修改”变成一个可观察字段，再以“在一个小模块上分两次修改，比较一次性重构和增量重构的证据。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“大 Diff、隐式行为变化、越改越偏”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：分阶段 Diff、测试和回滚点。预期：行为变更与重构可区分，任一阶段可回退。失败复验：大 Diff、隐式行为变化、越改越偏。

机制：验收证据：分阶段 Diff、测试和回滚点。预期：行为变更与重构可区分，任一阶段可回退。失败复验：大 Diff、隐式行为变化、越改越偏。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“大 Diff、隐式行为变化、越改越偏”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“计划—小改—测试—复查循环”。用受控文件的 Diff、可执行检查、失败输出和回滚结果交叉验收；单一绿色命令不能覆盖范围、行为和副作用。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“大 Diff、隐式行为变化、越改越偏”，先停止并保留原始事件。本节的失败样例是“大 Diff、隐式行为变化、越改越偏”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 分阶段 Diff、测试和回滚点 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 大 Diff、隐式行为变化、越改越偏 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“重构与行为变化分离”。围绕“重构与行为变化分离”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。复核时把动作和产物对应起来：分阶段 Diff、测试和回滚点应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“大 Diff、隐式行为变化、越改越偏”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-8-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
为什么大 Diff 难以审查
--- answer
大 Diff 同时改变很多假设，审查者难以定位风险、回归和责任；增量提交能缩小验证与回滚范围。
--- explanation
增量修改把行为变化和纯重构分离，每个小 Diff 都配测试与复查；当范围漂移时，停止比继续生成更安全。问题“为什么大 Diff 难以审查”的关键不在背诵名词，而在于：大 Diff 同时改变很多假设，审查者难以定位风险、回归和责任；增量提交能缩小验证与回滚范围。 先判断题目中的硬门槛和适用范围，再看“大 Diff、隐式行为变化、越改越偏”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-8-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
如何判断 Agent 越改越偏
--- answer
对比原始契约、变更文件、测试和失败记录；若任务范围扩大、反复改同一点或无法解释 Diff，就应暂停并回退。
--- explanation
增量修改把行为变化和纯重构分离，每个小 Diff 都配测试与复查；当范围漂移时，停止比继续生成更安全。问题“如何判断 Agent 越改越偏”的关键不在背诵名词，而在于：对比原始契约、变更文件、测试和失败记录；若任务范围扩大、反复改同一点或无法解释 Diff，就应暂停并回退。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
