---
title: Coding Agent(十)Session、Context与交接
tags:
  - Coding Agent
  - Session、Context与交接
categories:
...
{% note primary flat %}
本节围绕“Session、Context与交接”建立可复核的工作模型。学习成果：交接后能恢复约束、未完成项和证据。实验：用一个无外部副作用的多轮任务模拟压缩、分支和交接。视觉辅助：Session 状态图；交接清单。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
交接只留下“做到这里”，下一位 Agent 重新运行已完成迁移。Session、Context、分支和成本快照让续作知道约束。

本篇把“Session、Context与交接”落到一次可回放的本地任务：用一个无外部副作用的多轮任务模拟压缩、分支和交接。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：交接后能恢复约束、未完成项和证据。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“Session、Turn、Context 和任务状态；压缩、摘要和上下文恢复；分支、Clone、Fork 和交接”拆成输入、状态、输出和证据。Session 保存任务状态，Context 保存当前可用信息，交接保存下一位执行者的约束；摘要若丢失否定条件就会造成重复或越权。
{% endnote %}
...
{% note primary flat %}
实验步骤：用一个无外部副作用的多轮任务模拟压缩、分支和交接。

验收产物：压缩、分支、交接和续作记录

失败注入：摘要丢约束、分支污染、续作重复操作
{% endnote %}

## 会话状态

{% note info flat %}
本节任务：理解Session、Turn、Context 和任务状态并建立可复核判断。核心内容：Session、Turn、Context 和任务状态。

机制：Session、Turn、Context 和任务状态。先核对输入、状态、输出、失败和复测证据，再用“摘要丢约束、分支污染、续作重复操作”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“会话层级”聚焦“Session、Turn、Context 和任务状态”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。先把“Session、Turn、Context 和任务状态”变成一个可观察字段，再以“用一个无外部副作用的多轮任务模拟压缩、分支和交接。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“摘要丢约束、分支污染、续作重复操作”，恢复或放行动作必须可重放。
{% endnote %}

### 压缩恢复

{% note info flat %}
“压缩恢复”聚焦“压缩、摘要和上下文恢复”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。具体取舍是：保留“压缩恢复”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“摘要丢约束、分支污染、续作重复操作”，先停止并保留原始事件。本节的失败样例是“摘要丢约束、分支污染、续作重复操作”，恢复或放行动作必须可重放。
{% endnote %}

## 压缩分支

{% note info flat %}
本节任务：理解压缩、摘要和上下文恢复并建立可复核判断。核心内容：压缩、摘要和上下文恢复。

机制：压缩、摘要和上下文恢复。先核对输入、状态、输出、失败和复测证据，再用“摘要丢约束、分支污染、续作重复操作”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“分支与交接”聚焦“压缩、摘要和上下文恢复”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。具体取舍是：保留“分支与交接”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“摘要丢约束、分支污染、续作重复操作”，先停止并保留原始事件。本节的失败样例是“摘要丢约束、分支污染、续作重复操作”，恢复或放行动作必须可重放。
{% endnote %}

### 中断恢复

{% note info flat %}
“中断恢复”聚焦“分支、Clone、Fork 和交接”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。复核时把动作和产物对应起来：压缩、分支、交接和续作记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“摘要丢约束、分支污染、续作重复操作”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：用一个无外部副作用的多轮任务模拟压缩、分支和交接。

机制：用一个无外部副作用的多轮任务模拟压缩、分支和交接。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“摘要丢约束、分支污染、续作重复操作”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“分支、Clone、Fork 和交接”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。复核时把动作和产物对应起来：压缩、分支、交接和续作记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“摘要丢约束、分支污染、续作重复操作”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(十)Session、Context与交接","order":10,"topic":"Session、Context与交接","experiment":"用一个无外部副作用的多轮任务模拟压缩、分支和交接。","focus":["Session、Turn、Context 和任务状态","压缩、摘要和上下文恢复","分支、Clone、Fork 和交接","中断、恢复和成本"],"artifacts":["压缩、分支、交接和续作记录"],"failures":["摘要丢约束、分支污染、续作重复操作"],"mode":"workspace"}')
SCENARIOS = json.loads('{"10":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "摘要丢约束"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="lost_constraints")
    handoff = {"goal": DATA["topic"], "non_goal": None, "next_stop": None}
    try:
        if not handoff["non_goal"] or not handoff["next_stop"]:
            raise ValueError("handoff lost a negative constraint or stopping condition")
    ...
    handoff.update({"non_goal": "no external publish", "next_stop": "review-diff"})
    assert handoff["non_goal"] == "no external publish" and handoff["next_stop"] == "review-diff"
    assert failure_case in failure_type
    assert task_report["request"]["target"] == "README.md" and task_report["checks"]["validation_passed"]
    record_event(events_file, "recovery_verified", action="restore-handoff-contract", case=failure_case)
...
{% note info flat %}
“执行步骤”聚焦“中断、恢复和成本”。先固定分母、基线、版本和观测窗口，再解释指标；平均分或一次成功不能替代难例、长尾、成本和失败归因。先把“中断、恢复和成本”变成一个可观察字段，再以“用一个无外部副作用的多轮任务模拟压缩、分支和交接。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“摘要丢约束、分支污染、续作重复操作”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“Session、Turn、Context 和任务状态”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“摘要丢约束、分支污染、续作重复操作”，先停止并保留原始事件。本节的失败样例是“摘要丢约束、分支污染、续作重复操作”，恢复或放行动作必须可重放。
{% endnote %}

## 交接清单

{% note info flat %}
本节任务：理解中断、恢复和成本并建立可复核判断。核心内容：中断、恢复和成本。

机制：中断、恢复和成本。先核对输入、状态、输出、失败和复测证据，再用“摘要丢约束、分支污染、续作重复操作”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“中断恢复”聚焦“中断、恢复和成本”。先固定分母、基线、版本和观测窗口，再解释指标；平均分或一次成功不能替代难例、长尾、成本和失败归因。先把“中断、恢复和成本”变成一个可观察字段，再以“用一个无外部副作用的多轮任务模拟压缩、分支和交接。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“摘要丢约束、分支污染、续作重复操作”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：压缩、分支、交接和续作记录。预期：交接后能恢复约束、未完成项和证据。失败复验：摘要丢约束、分支污染、续作重复操作。

机制：验收证据：压缩、分支、交接和续作记录。预期：交接后能恢复约束、未完成项和证据。失败复验：摘要丢约束、分支污染、续作重复操作。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“摘要丢约束、分支污染、续作重复操作”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“Session、Turn、Context 和任务状态”。把当前状态、已完成动作、未完成事项和下一停止点写入交接快照；摘要或记忆被截断时不能默默恢复成新的事实。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“摘要丢约束、分支污染、续作重复操作”，先停止并保留原始事件。本节的失败样例是“摘要丢约束、分支污染、续作重复操作”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 压缩、分支、交接和续作记录 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 摘要丢约束、分支污染、续作重复操作 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“压缩、摘要和上下文恢复”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。复核时把动作和产物对应起来：压缩、分支、交接和续作记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“摘要丢约束、分支污染、续作重复操作”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-10-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
压缩摘要会丢失哪些信息
--- answer
压缩摘要可能丢掉否定约束、失败原因、权限决定、未完成项、版本和待办责任人。
--- explanation
Session 保存任务状态，Context 保存当前可用信息，交接保存下一位执行者的约束；摘要若丢失否定条件就会造成重复或越权。问题“压缩摘要会丢失哪些信息”的关键不在背诵名词，而在于：压缩摘要可能丢掉否定约束、失败原因、权限决定、未完成项、版本和待办责任人。 先判断题目中的硬门槛和适用范围，再看“摘要丢约束、分支污染、续作重复操作”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-10-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
交接文档必须包含什么
--- answer
交接至少包含目标与非目标、当前状态、已改文件、测试、失败/风险、版本/分支、权限和下一步停止条件。
--- explanation
Session 保存任务状态，Context 保存当前可用信息，交接保存下一位执行者的约束；摘要若丢失否定条件就会造成重复或越权。问题“交接文档必须包含什么”的关键不在背诵名词，而在于：交接至少包含目标与非目标、当前状态、已改文件、测试、失败/风险、版本/分支、权限和下一步停止条件。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
