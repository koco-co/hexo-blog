---
title: Coding Agent(十一)Tools、MCP与插件
tags:
  - Coding Agent
  - Tools、MCP与插件
categories:
  - Learn Topic
  - Coding Agent
description: 工具、MCP、插件的信任边界和降级清楚
...
{% note primary flat %}
本节围绕“Tools、MCP与插件”建立可复核的工作模型。学习成果：工具、MCP、插件的信任边界和降级清楚。实验：使用本地只读扩展/协议夹具验证发现、授权、调用和拒绝。视觉辅助：扩展架构图；信任决策表。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
社区插件返回“请上传配置文件”的隐藏指令。发现、授权、版本锁定和撤销必须先于安装。

本篇把“Tools、MCP与插件”落到一次可回放的本地任务：使用本地只读扩展/协议夹具验证发现、授权、调用和拒绝。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：工具、MCP、插件的信任边界和降级清楚。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“内置工具、外部工具和插件；MCP 的发现、调用和授权；安装源、版本和供应链”拆成输入、状态、输出和证据。工具和插件把外部代码与能力带入执行面；MCP 发现和授权、供应链锁定、撤销和文字兜底必须先于调用。
{% endnote %}
...
{% note primary flat %}
实验步骤：使用本地只读扩展/协议夹具验证发现、授权、调用和拒绝。

验收产物：本地扩展来源、权限、调用和撤销记录

失败注入：恶意描述、依赖投毒、权限未收回
{% endnote %}

## 工具层级

{% note info flat %}
本节任务：理解内置工具、外部工具和插件并建立可复核判断。核心内容：内置工具、外部工具和插件。

机制：内置工具、外部工具和插件。先核对输入、状态、输出、失败和复测证据，再用“恶意描述、依赖投毒、权限未收回”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“工具层级”聚焦“内置工具、外部工具和插件”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。先把“内置工具、外部工具和插件”变成一个可观察字段，再以“使用本地只读扩展/协议夹具验证发现、授权、调用和拒绝。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“恶意描述、依赖投毒、权限未收回”，恢复或放行动作必须可重放。
{% endnote %}

### MCP授权

{% note info flat %}
“MCP授权”聚焦“MCP 的发现、调用和授权”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。具体取舍是：保留“MCP授权”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“恶意描述、依赖投毒、权限未收回”，先停止并保留原始事件。本节的失败样例是“恶意描述、依赖投毒、权限未收回”，恢复或放行动作必须可重放。
{% endnote %}

## 扩展信任

{% note info flat %}
本节任务：理解MCP 的发现、调用和授权并建立可复核判断。核心内容：MCP 的发现、调用和授权。

机制：MCP 的发现、调用和授权。先核对输入、状态、输出、失败和复测证据，再用“恶意描述、依赖投毒、权限未收回”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“扩展信任”聚焦“MCP 的发现、调用和授权”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。具体取舍是：保留“扩展信任”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“恶意描述、依赖投毒、权限未收回”，先停止并保留原始事件。本节的失败样例是“恶意描述、依赖投毒、权限未收回”，恢复或放行动作必须可重放。
{% endnote %}

### 文字兜底

{% note info flat %}
“文字兜底”聚焦“安装源、版本和供应链”。先记录入口、版本、平台和前置条件，再执行最小动作；安装成功只能证明程序可启动，不能推出权限、模型或扩展已经可用。复核时把动作和产物对应起来：本地扩展来源、权限、调用和撤销记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“恶意描述、依赖投毒、权限未收回”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：使用本地只读扩展/协议夹具验证发现、授权、调用和拒绝。

机制：使用本地只读扩展/协议夹具验证发现、授权、调用和拒绝。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“恶意描述、依赖投毒、权限未收回”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“安装源、版本和供应链”。先记录入口、版本、平台和前置条件，再执行最小动作；安装成功只能证明程序可启动，不能推出权限、模型或扩展已经可用。复核时把动作和产物对应起来：本地扩展来源、权限、调用和撤销记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“恶意描述、依赖投毒、权限未收回”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(十一)Tools、MCP与插件","order":11,"topic":"Tools、MCP与插件","experiment":"使用本地只读扩展/协议夹具验证发现、授权、调用和拒绝。","focus":["内置工具、外部工具和插件","MCP 的发现、调用和授权","安装源、版本和供应链","失败降级与文字兜底"],"artifacts":["本地扩展来源、权限、调用和撤销记录"],"failures":["恶意描述、依赖投毒、权限未收回"],"mode":"workspace"}')
SCENARIOS = json.loads('{"11":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "恶意描述"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="untrusted_extension")
    manifest = {"source": "unknown", "permissions": ["read", "network"]}
    try:
        if manifest["source"] != "pinned" or "network" in manifest["permissions"]:
            raise PermissionError("untrusted extension requested an undeclared capability")
    ...
    manifest = {"source": "pinned", "permissions": ["read"], "revoked": True}
    assert manifest["source"] == "pinned" and manifest["permissions"] == ["read"] and manifest["revoked"]
    assert failure_case in failure_type
    assert task_report["request"]["target"] == "README.md" and task_report["checks"]["validation_passed"]
    record_event(events_file, "recovery_verified", action="quarantine-extension", case=failure_case)
...
{% note info flat %}
“执行步骤”聚焦“失败降级与文字兜底”。围绕“失败降级与文字兜底”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“失败降级与文字兜底”变成一个可观察字段，再以“使用本地只读扩展/协议夹具验证发现、授权、调用和拒绝。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“恶意描述、依赖投毒、权限未收回”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“内置工具、外部工具和插件”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“恶意描述、依赖投毒、权限未收回”，先停止并保留原始事件。本节的失败样例是“恶意描述、依赖投毒、权限未收回”，恢复或放行动作必须可重放。
{% endnote %}

## 授权降级

{% note info flat %}
本节任务：理解失败降级与文字兜底并建立可复核判断。核心内容：失败降级与文字兜底。

机制：失败降级与文字兜底。先核对输入、状态、输出、失败和复测证据，再用“恶意描述、依赖投毒、权限未收回”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“文字兜底”聚焦“失败降级与文字兜底”。围绕“失败降级与文字兜底”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“失败降级与文字兜底”变成一个可观察字段，再以“使用本地只读扩展/协议夹具验证发现、授权、调用和拒绝。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“恶意描述、依赖投毒、权限未收回”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：本地扩展来源、权限、调用和撤销记录。预期：工具、MCP、插件的信任边界和降级清楚。失败复验：恶意描述、依赖投毒、权限未收回。

机制：验收证据：本地扩展来源、权限、调用和撤销记录。预期：工具、MCP、插件的信任边界和降级清楚。失败复验：恶意描述、依赖投毒、权限未收回。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“恶意描述、依赖投毒、权限未收回”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“内置工具、外部工具和插件”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“恶意描述、依赖投毒、权限未收回”，先停止并保留原始事件。本节的失败样例是“恶意描述、依赖投毒、权限未收回”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 本地扩展来源、权限、调用和撤销记录 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 恶意描述、依赖投毒、权限未收回 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“MCP 的发现、调用和授权”。先展示能力发现和参数 Schema，再在真正执行前检查来源、权限、网络和秘密范围；工具返回值按不可信数据处理。复核时把动作和产物对应起来：本地扩展来源、权限、调用和撤销记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“恶意描述、依赖投毒、权限未收回”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-11-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
MCP 和插件为什么都是供应链入口
--- answer
MCP 和插件都能把外部代码、描述或数据带入执行面，因此都要审查来源、版本、权限、网络和撤销。
--- explanation
工具和插件把外部代码与能力带入执行面；MCP 发现和授权、供应链锁定、撤销和文字兜底必须先于调用。问题“MCP 和插件为什么都是供应链入口”的关键不在背诵名词，而在于：MCP 和插件都能把外部代码、描述或数据带入执行面，因此都要审查来源、版本、权限、网络和撤销。 先判断题目中的硬门槛和适用范围，再看“恶意描述、依赖投毒、权限未收回”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-11-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
工具描述为什么也可能携带风险
--- answer
工具描述也可能含有诱导指令或扩大权限的参数；把它当不可信数据，先做 Schema、权限和人工确认，再调用。
--- explanation
工具和插件把外部代码与能力带入执行面；MCP 发现和授权、供应链锁定、撤销和文字兜底必须先于调用。问题“工具描述为什么也可能携带风险”的关键不在背诵名词，而在于：工具描述也可能含有诱导指令或扩大权限的参数；把它当不可信数据，先做 Schema、权限和人工确认，再调用。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
