---
title: Coding Agent(十七)DeepSeek Harness
tags:
  - Coding Agent
  - DeepSeek Harness
categories:
  - Learn Topic
  - Coding Agent
description: 能独立完成 DeepSeek Harness 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。
...
{% note primary flat %}
本节围绕“DeepSeek Harness”建立可复核的工作模型。学习成果：能独立完成 DeepSeek Harness 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。实验：按官方入口完成 DeepSeek Harness 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。视觉辅助：产品入口或配置截图（真实验证后）；能力表；核心流程 Mermaid。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
DeepSeek Harness 的预览入口能展示调用链，却不能自动证明插件和模型适配可用于生产。实验把 Harness、资源、权限和版本漂移分开记录。

本篇把“DeepSeek Harness”落到一次可回放的本地任务：按官方入口完成 DeepSeek Harness 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：能独立完成 DeepSeek Harness 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“Developer Preview 的 CLI/Web、模型适配、插件架构、Skills、MCP、会话、权限和二次开发边界”拆成输入、状态、输出和证据。DeepSeek Harness 要先区分 Developer Preview 的入口与模型适配，再检查 Harness、插件、Skills、MCP、会话和权限是否有可回放边界。
{% endnote %}

{% mermaid %}
flowchart TD
  A[DeepSeek CLI/Web] --> B[模型适配]
  B --> C[Harness运行链]
  C --> D[插件/Skills/MCP]
  D --> E{权限与审批}
  E -->|通过| F[受控任务]
...
{% note primary flat %}
实验步骤：按官方入口完成 DeepSeek Harness 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。

验收产物：DeepSeek Harness 的版本/Commit 或文档快照；DeepSeek Harness 受控小仓库的任务输出；DeepSeek Harness 的 Diff、测试、权限或失败日志
...
{% note info flat %}
本节任务：确认DeepSeek Harness的入口并完成最小安装。核心内容：DeepSeek Harness：预览版、CLI与Web。

机制：DeepSeek Harness：预览版、CLI与Web。选择依据：DeepSeek Harness的入口、平台和版本是离散选择，表格能并列官方入口与适用条件。先核对DeepSeek Harness的入口类型、支持平台、安装命令和版本输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；DeepSeek Harness入口或截图不可用时保留官方 URL、命令和版本检查。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | DeepSeek Harness的入口类型、支持平台、安装命令和版本输出 |
| 失败降级 | DeepSeek Harness入口或截图不可用时保留官方 URL、命令和版本检查 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

{% note info flat %}
DeepSeek Harness执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）
{% endnote %}

![DeepSeek Harness执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）](/img/picgo-images/coding-agent-product-17.svg)

| 能力面 | 本篇观察对象 | 本地夹具证据 | 线上边界 |
| --- | --- | --- | --- |
| 入口与版本 | CLI/Web、模型适配、Harness 状态、插件资源与审批 | 记录任务契约与 fixture 版本 | 需按官方入口、平台和版本复核 |
...
{% note primary flat %}
入口只展示官方操作路径，本次不执行安装或真实账号调用。先记录DeepSeek Harness Developer Preview的版本，再用同一受控任务比较上下文、权限和交付证据。
{% endnote %}

| 项目 | 可复核内容 |
| --- | --- |
| 入口 | DeepSeek Harness Developer Preview |
| 安装/启动 | `npx @deepseek-ai/dsh web` |
| 版本检查 | `记录本地预览入口与版本；不把预览当生产` |
| 配置观察 | model；workspace；session；plugin/Skill/MCP；approval |
| 最小任务 | 在本地预览中提交只读仓库任务，并保存调用链与拒绝记录 |
...
{% note info flat %}
DeepSeek Harness 的 npx web 入口属于 Developer Preview 路径；记录 Node、预览版本和本地端口，不把页面出现当作生产可用。本节围绕“CLI与Web”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为DeepSeek Harness 的版本/Commit 或文档快照。
{% endnote %}

## 环境准备

{% note primary flat %}
本节任务：让DeepSeek Harness使用指定模型完成一次脱敏认证。核心内容：DeepSeek Harness：环境准备、模型与认证。

机制：DeepSeek Harness：环境准备、模型与认证。选择依据：DeepSeek Harness的认证示例需要可复制命令、环境变量占位符和脱敏输出，代码最不含糊。先核对DeepSeek Harness的模型 ID、认证方式、权限确认和脱敏输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；DeepSeek Harness没有凭据时运行本地 fixture，禁止伪造在线响应。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | DeepSeek Harness的模型 ID、认证方式、权限确认和脱敏输出 |
| 失败降级 | DeepSeek Harness没有凭据时运行本地 fixture，禁止伪造在线响应 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |
...
{% note info flat %}
模型/认证配置要与 Harness 状态分开：前者决定请求能否发送，后者决定任务、资源和错误是否可追踪。本节围绕“模型与认证”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为DeepSeek Harness 受控小仓库的任务输出。
{% endnote %}

## 任务入口

{% note info flat %}
本节任务：在DeepSeek Harness的项目上下文中定位一个受控任务。核心内容：DeepSeek Harness：任务入口、Harness状态。

机制：DeepSeek Harness：任务入口、Harness状态。选择依据：DeepSeek Harness的上下文、Session 和任务状态是有向关系，图下补充每个状态的文字含义。先核对DeepSeek Harness的规则来源、上下文范围、Session 状态和任务输入，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；DeepSeek Harness图表失败时按“入口→上下文→任务→结果”列出状态。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | DeepSeek Harness的规则来源、上下文范围、Session 状态和任务输入 |
| 失败降级 | DeepSeek Harness图表失败时按“入口→上下文→任务→结果”列出状态 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |
...
{% note info flat %}
任务入口先写入受控工作区和 Session，再观察调用链；预览页面的默认 Context 不能自动获得仓库写权限。本节围绕“Harness状态”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为DeepSeek Harness 的 Diff、测试、权限或失败日志。
{% endnote %}

## 插件架构

{% note info flat %}
本节任务：为DeepSeek Harness启用一个工具或扩展并撤销它。核心内容：DeepSeek Harness：插件架构、Skills与MCP。

机制：DeepSeek Harness：插件架构、Skills与MCP。选择依据：DeepSeek Harness的工具、Skill、MCP 和插件需要逐项比较来源、权限、输入输出，表格便于审查。先核对DeepSeek Harness的来源/版本、权限、输入、输出和撤销动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；DeepSeek Harness扩展不可安装时使用静态 manifest 和拒绝测试。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | DeepSeek Harness的来源/版本、权限、输入、输出和撤销动作 |
| 失败降级 | DeepSeek Harness扩展不可安装时使用静态 manifest 和拒绝测试 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### Skills与MCP

{% note info flat %}
插件、Skills 和 MCP 的资源发现应带来源、版本和权限快照；返回文本可能含指令，必须作为不可信数据。本节围绕“Skills与MCP”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为DeepSeek Harness 的版本/Commit 或文档快照。
{% endnote %}

## 权限与审批

{% note danger flat %}
本节任务：在DeepSeek Harness中审批一次有副作用的动作。核心内容：DeepSeek Harness：权限与审批、故障验证。

机制：DeepSeek Harness：权限与审批、故障验证。选择依据：DeepSeek Harness的权限与外发风险必须脱离图表颜色仍直接可见，danger 强调不可逆后果。先核对DeepSeek Harness的审批点、工作区、网络、秘密和停止条件，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；DeepSeek Harness拒绝或超时后保留未执行证据与恢复/回滚步骤。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | DeepSeek Harness的审批点、工作区、网络、秘密和停止条件 |
| 失败降级 | DeepSeek Harness拒绝或超时后保留未执行证据与恢复/回滚步骤 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 故障验证

{% note info flat %}
审批失败要停止在实际工具调用之前，并保存 request、decision、error 和恢复动作；不使用通用成功句代替。本节围绕“故障验证”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为DeepSeek Harness 受控小仓库的任务输出。
{% endnote %}

## 核心调用链

{% note warning flat %}
本节任务：复现DeepSeek Harness的一项限制并记录恢复。核心内容：DeepSeek Harness：核心调用链、版本边界。

机制：DeepSeek Harness：核心调用链、版本边界。选择依据：DeepSeek Harness的版本、平台、Preview 和退出条件决定能否照搬，提示块突出边界而非宣传。先核对DeepSeek Harness的版本/平台、功能限制、失败状态和替代路径，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；DeepSeek Harness在线能力变化时使用日期快照和本地等价任务。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | DeepSeek Harness的版本/平台、功能限制、失败状态和替代路径 |
| 失败降级 | DeepSeek Harness在线能力变化时使用日期快照和本地等价任务 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 版本边界

{% note info flat %}
核心调用链的版本漂移尤其重要：同一任务换预览版本后若字段或事件变化，应进入兼容矩阵而非静默适配。本节围绕“版本边界”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为DeepSeek Harness 的 Diff、测试、权限或失败日志。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用证据验收DeepSeek Harness。核心内容：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 DeepSeek Harness 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。

机制：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 DeepSeek Harness 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。选择依据：产品验收需要逐项核对版本、行为和安全证据。先核对验收标准、失败样例和复测动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；在线不可用时保留本地验证结果。
{% endnote %}
...
{% note info flat %}
本节把DeepSeek Harness的“版本和任务证据”收束为版本、权限、任务、Diff 和测试证据；DeepSeek Harness 要先区分 Developer Preview 的入口与模型适配，再检查 Harness、插件、Skills、MCP、会话和权限是否有可回放边界。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | DeepSeek Harness 的版本/Commit 或文档快照 | 只有最终成功文字 | 保存中间状态与失败记录 |
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(十七)DeepSeek Harness","order":17,"topic":"DeepSeek Harness","experiment":"按官方入口完成 DeepSeek Harness 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。","focus":["Developer Preview 的 CLI/Web、模型适配、插件架构、Skills、MCP、会话、权限和二次开发边界。","定位与版本","安装与模型","Harness运行链"],"artifacts":["DeepSeek Harness 的版本/Commit 或文档快照","DeepSeek Harness 受控小仓库的任务输出","DeepSeek Harness 的 Diff、测试、权限或失败日志"],"failures":["认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代"],"mode":"workspace"}')
SCENARIOS = json.loads('{"17":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "预览能力"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="preview_unpinned")
    preflight = {"product": DATA["topic"], "failure_kind": "preview_unpinned", "failure_case": failure_case, "declared_failure": failure_type, "online_evidence": "not_run", "version_pinned": False}
    try:
        if not preflight["version_pinned"]:
            raise PermissionError("preview or headless capability is unpinned")
    ...
    record_event(events_file, "recovery_verified", action="record-preview-boundary", case=failure_case)
...
{% note info flat %}
本节把DeepSeek Harness的“失败复测”收束为版本、权限、任务、Diff 和测试证据；DeepSeek Harness 要先区分 Developer Preview 的入口与模型适配，再检查 Harness、插件、Skills、MCP、会话和权限是否有可回放边界。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-17-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
DeepSeek Harness 的默认权限和最小安全配置是什么
--- answer
不能把 DeepSeek Harness 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。
--- explanation
DeepSeek Harness 要先区分 Developer Preview 的入口与模型适配，再检查 Harness、插件、Skills、MCP、会话和权限是否有可回放边界。问题“DeepSeek Harness 的默认权限和最小安全配置是什么”的关键不在背诵名词，而在于：不能把 DeepSeek Harness 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。 先判断题目中的硬门槛和适用范围，再看“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-17-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
DeepSeek Harness 的核心上下文、工具或扩展边界是什么
--- answer
DeepSeek Harness 的核心边界是 CLI/Web、模型适配、Harness 状态、插件资源与审批；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。
--- explanation
DeepSeek Harness 要先区分 Developer Preview 的入口与模型适配，再检查 Harness、插件、Skills、MCP、会话和权限是否有可回放边界。问题“DeepSeek Harness 的核心上下文、工具或扩展边界是什么”的关键不在背诵名词，而在于：DeepSeek Harness 的核心边界是 CLI/Web、模型适配、Harness 状态、插件资源与审批；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
