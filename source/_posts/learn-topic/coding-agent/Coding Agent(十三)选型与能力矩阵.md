...
categories:
  - Learn Topic
  - Coding Agent
description: 入口、模型、权限、扩展、成本和退出策略均有证据
...
{% note primary flat %}
本节围绕“选型与能力矩阵”建立可复核的工作模型。学习成果：入口、模型、权限、扩展、成本和退出策略均有证据。实验：使用同一任务和证据标准填写三种产品的选型记录，不做无来源排名。视觉辅助：18 产品能力矩阵；决策树。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
同一任务在终端、IDE 和云端入口得到不同权限与数据边界。固定任务、证据和版本日期，才能进行可复核选型。

本篇把“选型与能力矩阵”落到一次可回放的本地任务：使用同一任务和证据标准填写三种产品的选型记录，不做无来源排名。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：入口、模型、权限、扩展、成本和退出策略均有证据。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“终端、IDE、桌面、Web、CI 和 SDK；模型来源、认证和数据边界；Session、Tools、Skills、MCP 和团队能力”拆成输入、状态、输出和证据。选型要比较硬约束和可选体验：入口、模型、认证、数据边界、权限、扩展、维护成本和退出策略都需有来源和日期。
{% endnote %}
...
{% note primary flat %}
实验步骤：使用同一任务和证据标准填写三种产品的选型记录，不做无来源排名。

验收产物：三个产品的带日期选型记录

失败注入：无来源排名、版本过期、一次体验外推永久结论
{% endnote %}

## 运行入口

{% note info flat %}
本节任务：理解终端、IDE、桌面、Web、CI 和 SDK并建立可复核判断。核心内容：终端、IDE、桌面、Web、CI 和 SDK。

机制：终端、IDE、桌面、Web、CI 和 SDK。先核对输入、状态、输出、失败和复测证据，再用“无来源排名、版本过期、一次体验外推永久结论”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“运行入口”聚焦“终端、IDE、桌面、Web、CI 和 SDK”。先记录入口、版本、平台和前置条件，再执行最小动作；安装成功只能证明程序可启动，不能推出权限、模型或扩展已经可用。先把“终端、IDE、桌面、Web、CI 和 SDK”变成一个可观察字段，再以“使用同一任务和证据标准填写三种产品的选型记录，不做无来源排名。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“无来源排名、版本过期、一次体验外推永久结论”，恢复或放行动作必须可重放。
{% endnote %}

### 模型边界

{% note info flat %}
“模型边界”聚焦“模型来源、认证和数据边界”。把模型/Provider 选择、认证状态和任务输入分成三列；凭据缺失时应停止在本地静态演示，不输出看似真实的服务响应。具体取舍是：保留“模型边界”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“无来源排名、版本过期、一次体验外推永久结论”，先停止并保留原始事件。本节的失败样例是“无来源排名、版本过期、一次体验外推永久结论”，恢复或放行动作必须可重放。
{% endnote %}

## 能力比较

{% note info flat %}
本节任务：理解模型来源、认证和数据边界并建立可复核判断。核心内容：模型来源、认证和数据边界。

机制：模型来源、认证和数据边界。先核对输入、状态、输出、失败和复测证据，再用“无来源排名、版本过期、一次体验外推永久结论”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“能力组合”聚焦“模型来源、认证和数据边界”。把模型/Provider 选择、认证状态和任务输入分成三列；凭据缺失时应停止在本地静态演示，不输出看似真实的服务响应。具体取舍是：保留“能力组合”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“无来源排名、版本过期、一次体验外推永久结论”，先停止并保留原始事件。本节的失败样例是“无来源排名、版本过期、一次体验外推永久结论”，恢复或放行动作必须可重放。
{% endnote %}

### 退出策略

{% note info flat %}
“退出策略”聚焦“Session、Tools、Skills、MCP 和团队能力”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。复核时把动作和产物对应起来：三个产品的带日期选型记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“无来源排名、版本过期、一次体验外推永久结论”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：使用同一任务和证据标准填写三种产品的选型记录，不做无来源排名。

机制：使用同一任务和证据标准填写三种产品的选型记录，不做无来源排名。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“无来源排名、版本过期、一次体验外推永久结论”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“Session、Tools、Skills、MCP 和团队能力”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。复核时把动作和产物对应起来：三个产品的带日期选型记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“无来源排名、版本过期、一次体验外推永久结论”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(十三)选型与能力矩阵","order":13,"topic":"选型与能力矩阵","experiment":"使用同一任务和证据标准填写三种产品的选型记录，不做无来源排名。","focus":["终端、IDE、桌面、Web、CI 和 SDK","模型来源、认证和数据边界","Session、Tools、Skills、MCP 和团队能力","维护成本、锁定和退出策略"],"artifacts":["三个产品的带日期选型记录"],"failures":["无来源排名、版本过期、一次体验外推永久结论"],"mode":"workspace"}')
SCENARIOS = json.loads('{"13":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "版本过期"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="stale_matrix")
    matrix = {"source": None, "version": None, "checked_at": None}
    try:
        if not matrix["source"] or not matrix["version"]:
            raise ValueError("capability matrix has no source or version snapshot")
    ...
    matrix.update({"source": "fixture-contract", "version": "fixture-v1", "checked_at": "fixture"})
    assert matrix["source"] and matrix["version"]
    assert failure_case in failure_type
    assert task_report["request"]["target"] == "README.md" and task_report["checks"]["validation_passed"]
    record_event(events_file, "recovery_verified", action="restore-source-and-version", case=failure_case)
...
{% note info flat %}
“执行步骤”聚焦“维护成本、锁定和退出策略”。先固定分母、基线、版本和观测窗口，再解释指标；平均分或一次成功不能替代难例、长尾、成本和失败归因。先把“维护成本、锁定和退出策略”变成一个可观察字段，再以“使用同一任务和证据标准填写三种产品的选型记录，不做无来源排名。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“无来源排名、版本过期、一次体验外推永久结论”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“终端、IDE、桌面、Web、CI 和 SDK”。围绕“终端、IDE、桌面、Web、CI 和 SDK”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“无来源排名、版本过期、一次体验外推永久结论”，先停止并保留原始事件。本节的失败样例是“无来源排名、版本过期、一次体验外推永久结论”，恢复或放行动作必须可重放。
{% endnote %}

## 决策记录

{% note info flat %}
本节任务：理解维护成本、锁定和退出策略并建立可复核判断。核心内容：维护成本、锁定和退出策略。

机制：维护成本、锁定和退出策略。先核对输入、状态、输出、失败和复测证据，再用“无来源排名、版本过期、一次体验外推永久结论”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“退出策略”聚焦“维护成本、锁定和退出策略”。先固定分母、基线、版本和观测窗口，再解释指标；平均分或一次成功不能替代难例、长尾、成本和失败归因。先把“维护成本、锁定和退出策略”变成一个可观察字段，再以“使用同一任务和证据标准填写三种产品的选型记录，不做无来源排名。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“无来源排名、版本过期、一次体验外推永久结论”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：三个产品的带日期选型记录。预期：入口、模型、权限、扩展、成本和退出策略均有证据。失败复验：无来源排名、版本过期、一次体验外推永久结论。

机制：验收证据：三个产品的带日期选型记录。预期：入口、模型、权限、扩展、成本和退出策略均有证据。失败复验：无来源排名、版本过期、一次体验外推永久结论。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“无来源排名、版本过期、一次体验外推永久结论”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“终端、IDE、桌面、Web、CI 和 SDK”。围绕“终端、IDE、桌面、Web、CI 和 SDK”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“无来源排名、版本过期、一次体验外推永久结论”，先停止并保留原始事件。本节的失败样例是“无来源排名、版本过期、一次体验外推永久结论”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 三个产品的带日期选型记录 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 无来源排名、版本过期、一次体验外推永久结论 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“模型来源、认证和数据边界”。把模型/Provider 选择、认证状态和任务输入分成三列；凭据缺失时应停止在本地静态演示，不输出看似真实的服务响应。复核时把动作和产物对应起来：三个产品的带日期选型记录应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“无来源排名、版本过期、一次体验外推永久结论”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-13-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
如何处理快速变化的产品矩阵
--- answer
快速变化的矩阵应记录版本、日期、来源、能力、权限、成本和未验证项，重新运行同一任务，而不是维护静态排名。
--- explanation
选型要比较硬约束和可选体验：入口、模型、认证、数据边界、权限、扩展、维护成本和退出策略都需有来源和日期。问题“如何处理快速变化的产品矩阵”的关键不在背诵名词，而在于：快速变化的矩阵应记录版本、日期、来源、能力、权限、成本和未验证项，重新运行同一任务，而不是维护静态排名。 先判断题目中的硬门槛和适用范围，再看“无来源排名、版本过期、一次体验外推永久结论”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-13-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
选型时哪些能力是硬约束
--- answer
硬约束包括平台/网络、认证、数据边界、沙箱/审批、可回滚性和团队合规；可选体验不能覆盖硬约束。
--- explanation
选型要比较硬约束和可选体验：入口、模型、认证、数据边界、权限、扩展、维护成本和退出策略都需有来源和日期。问题“选型时哪些能力是硬约束”的关键不在背诵名词，而在于：硬约束包括平台/网络、认证、数据边界、沙箱/审批、可回滚性和团队合规；可选体验不能覆盖硬约束。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
