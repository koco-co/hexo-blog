---
title: Coding Agent(十九)OpenClaw
tags:
  - Coding Agent
  - OpenClaw
categories:
  - Learn Topic
  - Coding Agent
description: 能独立完成 OpenClaw 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。
...
{% note primary flat %}
本节围绕“OpenClaw”建立可复核的工作模型。学习成果：能独立完成 OpenClaw 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。实验：按官方入口完成 OpenClaw 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。视觉辅助：产品入口或配置截图（真实验证后）；能力表；核心流程 Mermaid。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
OpenClaw 的消息入口把请求交给 Gateway，Gateway 再连接 Provider、Skill 和远程工具。只看聊天结果会漏掉会话、记忆和权限的实际边界。

本篇把“OpenClaw”落到一次可回放的本地任务：按官方入口完成 OpenClaw 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：能独立完成 OpenClaw 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“Gateway、消息渠道、coding-agent Skill、Provider、会话/记忆、工具/技能、远程控制和安全边界”拆成输入、状态、输出和证据。OpenClaw 的核心不是单一聊天入口，而是 Gateway、渠道、Provider、Coding Agent Skill、Session/Memory 和远程工具权限的组合边界。
{% endnote %}

{% mermaid %}
flowchart TD
  A[消息渠道] --> B[OpenClaw Gateway]
  B --> C[Provider与模型]
  C --> D[Coding Agent Skill]
  D --> E[Session/Memory]
  E --> F{工具权限}
  F -->|允许| G[受控代码任务]
  F -->|拒绝| H[远程控制边界]
...
{% note primary flat %}
实验步骤：按官方入口完成 OpenClaw 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。

验收产物：OpenClaw 的版本/Commit 或文档快照；OpenClaw 受控小仓库的任务输出；OpenClaw 的 Diff、测试、权限或失败日志
...
{% note info flat %}
本节任务：确认OpenClaw的入口并完成最小安装。核心内容：OpenClaw：消息渠道、Gateway。

机制：OpenClaw：消息渠道、Gateway。选择依据：OpenClaw的入口、平台和版本是离散选择，表格能并列官方入口与适用条件。先核对OpenClaw的入口类型、支持平台、安装命令和版本输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；OpenClaw入口或截图不可用时保留官方 URL、命令和版本检查。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | OpenClaw的入口类型、支持平台、安装命令和版本输出 |
| 失败降级 | OpenClaw入口或截图不可用时保留官方 URL、命令和版本检查 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

{% note info flat %}
OpenClaw执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）
{% endnote %}

![OpenClaw执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）](/img/picgo-images/coding-agent-product-19.svg)

| 能力面 | 本篇观察对象 | 本地夹具证据 | 线上边界 |
| --- | --- | --- | --- |
| 入口与版本 | Gateway、Provider、Coding Agent Skill、Session/Memory 与远程工具权限 | 记录任务契约与 fixture 版本 | 需按官方入口、平台和版本复核 |
...
{% note primary flat %}
入口只展示官方操作路径，本次不执行安装或真实账号调用。先记录OpenClaw的版本，再用同一受控任务比较上下文、权限和交付证据。
{% endnote %}

| 项目 | 可复核内容 |
| --- | --- |
| 入口 | OpenClaw |
| 安装/启动 | `curl -fsSL https://openclaw.ai/install.sh | bash` |
| 版本检查 | `openclaw --version` |
| 配置观察 | Gateway；channels；providers；coding-agent Skill；session/memory；tool permissions |
| 最小任务 | 在本地受控工作区以只读 Skill 规划 README.md 修改，不连接远程渠道 |
...
{% note info flat %}
OpenClaw 的安装脚本建立本地命令，Gateway、渠道和 Provider 还需分别配置；先记录 openclaw --version 和启用的入口。本节围绕“Gateway”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为OpenClaw 的版本/Commit 或文档快照。
{% endnote %}

## Provider认证

{% note primary flat %}
本节任务：让OpenClaw使用指定模型完成一次脱敏认证。核心内容：OpenClaw：Provider认证、模型选择。

机制：OpenClaw：Provider认证、模型选择。选择依据：OpenClaw的认证示例需要可复制命令、环境变量占位符和脱敏输出，代码最不含糊。先核对OpenClaw的模型 ID、认证方式、权限确认和脱敏输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；OpenClaw没有凭据时运行本地 fixture，禁止伪造在线响应。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | OpenClaw的模型 ID、认证方式、权限确认和脱敏输出 |
| 失败降级 | OpenClaw没有凭据时运行本地 fixture，禁止伪造在线响应 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

```text
Provider认证: 输入 -> 校验 -> 执行 -> 证据
```

### 模型选择

{% note info flat %}
Provider 认证只回答模型能否请求，Gateway/channel 还决定消息从哪里进出；禁止把远程渠道放进本地 fixture。本节围绕“模型选择”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为OpenClaw 受控小仓库的任务输出。
{% endnote %}

## Skill发现

{% note info flat %}
本节任务：在OpenClaw的项目上下文中定位一个受控任务。核心内容：OpenClaw：Skill发现、任务委派。

机制：OpenClaw：Skill发现、任务委派。选择依据：OpenClaw的上下文、Session 和任务状态是有向关系，图下补充每个状态的文字含义。先核对OpenClaw的规则来源、上下文范围、Session 状态和任务输入，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；OpenClaw图表失败时按“入口→上下文→任务→结果”列出状态。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | OpenClaw的规则来源、上下文范围、Session 状态和任务输入 |
| 失败降级 | OpenClaw图表失败时按“入口→上下文→任务→结果”列出状态 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |
...
{% note info flat %}
Skill 发现要列出 coding-agent Skill 的来源、版本和获得的工具；委派前固定输入与输出，避免隐式扩大任务。本节围绕“任务委派”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为OpenClaw 的 Diff、测试、权限或失败日志。
{% endnote %}

## Session

{% note info flat %}
本节任务：为OpenClaw启用一个工具或扩展并撤销它。核心内容：OpenClaw：Session、Memory作用域。

机制：OpenClaw：Session、Memory作用域。选择依据：OpenClaw的工具、Skill、MCP 和插件需要逐项比较来源、权限、输入输出，表格便于审查。先核对OpenClaw的来源/版本、权限、输入、输出和撤销动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；OpenClaw扩展不可安装时使用静态 manifest 和拒绝测试。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | OpenClaw的来源/版本、权限、输入、输出和撤销动作 |
| 失败降级 | OpenClaw扩展不可安装时使用静态 manifest 和拒绝测试 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### Memory作用域

{% note info flat %}
Session 与 Memory 需要按用户、项目和渠道隔离；远程消息中的旧上下文不得自动注入当前代码任务。本节围绕“Memory作用域”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为OpenClaw 的版本/Commit 或文档快照。
{% endnote %}

## 工具权限

{% note danger flat %}
本节任务：在OpenClaw中审批一次有副作用的动作。核心内容：OpenClaw：工具权限、扩展来源。

机制：OpenClaw：工具权限、扩展来源。选择依据：OpenClaw的权限与外发风险必须脱离图表颜色仍直接可见，danger 强调不可逆后果。先核对OpenClaw的审批点、工作区、网络、秘密和停止条件，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；OpenClaw拒绝或超时后保留未执行证据与恢复/回滚步骤。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | OpenClaw的审批点、工作区、网络、秘密和停止条件 |
| 失败降级 | OpenClaw拒绝或超时后保留未执行证据与恢复/回滚步骤 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 扩展来源

{% note info flat %}
扩展工具逐项声明网络、文件和秘密权限，撤销后重放拒绝；Gateway 可连接不等于操作被授权。本节围绕“扩展来源”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为OpenClaw 受控小仓库的任务输出。
{% endnote %}

## 远程控制边界

{% note warning flat %}
本节任务：复现OpenClaw的一项限制并记录恢复。核心内容：OpenClaw：远程控制边界、项目验证。

机制：OpenClaw：远程控制边界、项目验证。选择依据：OpenClaw的版本、平台、Preview 和退出条件决定能否照搬，提示块突出边界而非宣传。先核对OpenClaw的版本/平台、功能限制、失败状态和替代路径，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；OpenClaw在线能力变化时使用日期快照和本地等价任务。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | OpenClaw的版本/平台、功能限制、失败状态和替代路径 |
| 失败降级 | OpenClaw在线能力变化时使用日期快照和本地等价任务 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 项目验证

{% note info flat %}
项目验证只证明受控目录中的任务链；真实渠道、云端 Provider 和远程控制必须用独立账号与审计证据验证。本节围绕“项目验证”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为OpenClaw 的 Diff、测试、权限或失败日志。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用证据验收OpenClaw。核心内容：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 OpenClaw 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。

机制：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 OpenClaw 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。选择依据：产品验收需要逐项核对版本、行为和安全证据。先核对验收标准、失败样例和复测动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；在线不可用时保留本地验证结果。
{% endnote %}
...
{% note info flat %}
本节把OpenClaw的“版本和任务证据”收束为版本、权限、任务、Diff 和测试证据；OpenClaw 的核心不是单一聊天入口，而是 Gateway、渠道、Provider、Coding Agent Skill、Session/Memory 和远程工具权限的组合边界。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | OpenClaw 的版本/Commit 或文档快照 | 只有最终成功文字 | 保存中间状态与失败记录 |
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(十九)OpenClaw","order":19,"topic":"OpenClaw","experiment":"按官方入口完成 OpenClaw 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。","focus":["Gateway、消息渠道、coding-agent Skill、Provider、会话/记忆、工具/技能、远程控制和安全边界。","定位与网关","认证与模型","Coding Agent Skill"],"artifacts":["OpenClaw 的版本/Commit 或文档快照","OpenClaw 受控小仓库的任务输出","OpenClaw 的 Diff、测试、权限或失败日志"],"failures":["认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代"],"mode":"workspace"}')
SCENARIOS = json.loads('{"19":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "权限拒绝"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="remote_permission")
    preflight = {"product": DATA["topic"], "failure_kind": "remote_permission", "failure_case": failure_case, "declared_failure": failure_type, "online_evidence": "not_run", "permission_granted": False}
    try:
        if not preflight["permission_granted"]:
            raise PermissionError("remote operation lacks permission")
    ...
    record_event(events_file, "recovery_verified", action="stop-remote-operation", case=failure_case)
...
{% note info flat %}
本节把OpenClaw的“失败复测”收束为版本、权限、任务、Diff 和测试证据；OpenClaw 的核心不是单一聊天入口，而是 Gateway、渠道、Provider、Coding Agent Skill、Session/Memory 和远程工具权限的组合边界。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-19-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
OpenClaw 的默认权限和最小安全配置是什么
--- answer
不能把 OpenClaw 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。
--- explanation
OpenClaw 的核心不是单一聊天入口，而是 Gateway、渠道、Provider、Coding Agent Skill、Session/Memory 和远程工具权限的组合边界。问题“OpenClaw 的默认权限和最小安全配置是什么”的关键不在背诵名词，而在于：不能把 OpenClaw 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。 先判断题目中的硬门槛和适用范围，再看“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-19-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
OpenClaw 的核心上下文、工具或扩展边界是什么
--- answer
OpenClaw 的核心边界是 Gateway、Provider、Coding Agent Skill、Session/Memory 与远程工具权限；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。
--- explanation
OpenClaw 的核心不是单一聊天入口，而是 Gateway、渠道、Provider、Coding Agent Skill、Session/Memory 和远程工具权限的组合边界。问题“OpenClaw 的核心上下文、工具或扩展边界是什么”的关键不在背诵名词，而在于：OpenClaw 的核心边界是 Gateway、Provider、Coding Agent Skill、Session/Memory 与远程工具权限；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
