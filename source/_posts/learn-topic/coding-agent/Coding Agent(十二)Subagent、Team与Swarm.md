---
title: Coding Agent(十二)Subagent、Team与Swarm
tags:
  - Coding Agent
  - Subagent、Team与Swarm
categories:
...
{% note primary flat %}
本节围绕“Subagent、Team与Swarm”建立可复核的工作模型。学习成果：并行收益和协调开销可解释。实验：用本地确定性任务模拟单 Agent、Subagent 和并行 Team。视觉辅助：编排时序图；模式对照表。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
并行 Agent 各自修改同一文件，合并时丢失一个修复。只有把冲突、等待、失败和取消都记录，协作收益才可验证。

本篇把“Subagent、Team与Swarm”落到一次可回放的本地任务：用本地确定性任务模拟单 Agent、Subagent 和并行 Team。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：并行收益和协调开销可解释。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“Subagent 输入输出契约；串行、并行和 Worktree；Team、Swarm、共享状态和冲突”拆成输入、状态、输出和证据。并行 Agent 只减少等待，不保证减少工作；任务 ID、输出 Schema、Worktree 和单一合并责任者决定结果是否可复查。
{% endnote %}
...
{% note primary flat %}
实验步骤：用本地确定性任务模拟单 Agent、Subagent 和并行 Team。

验收产物：单 Agent、Subagent、Team 的任务/成本/结果对照
...
{% note info flat %}
本节任务：理解Subagent 输入输出契约并建立可复核判断。核心内容：Subagent 输入输出契约。

机制：Subagent 输入输出契约。先核对输入、状态、输出、失败和复测证据，再用“重复工作、结果丢失、并行竞态”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“Subagent契约”聚焦“Subagent 输入输出契约”。给每个参与者分配任务 ID、输入输出 Schema 和取消条件，合并前检查重复、冲突和责任归属，而不是只数完成消息。先把“Subagent 输入输出契约”变成一个可观察字段，再以“用本地确定性任务模拟单 Agent、Subagent 和并行 Team。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“重复工作、结果丢失、并行竞态”，恢复或放行动作必须可重放。
{% endnote %}

### 并行隔离

{% note info flat %}
“并行隔离”聚焦“串行、并行和 Worktree”。给每个参与者分配任务 ID、输入输出 Schema 和取消条件，合并前检查重复、冲突和责任归属，而不是只数完成消息。具体取舍是：保留“并行隔离”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“重复工作、结果丢失、并行竞态”，先停止并保留原始事件。本节的失败样例是“重复工作、结果丢失、并行竞态”，恢复或放行动作必须可重放。
{% endnote %}

## 并行隔离

{% note info flat %}
本节任务：理解串行、并行和 Worktree并建立可复核判断。核心内容：串行、并行和 Worktree。

机制：串行、并行和 Worktree。先核对输入、状态、输出、失败和复测证据，再用“重复工作、结果丢失、并行竞态”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“协作冲突”聚焦“串行、并行和 Worktree”。给每个参与者分配任务 ID、输入输出 Schema 和取消条件，合并前检查重复、冲突和责任归属，而不是只数完成消息。具体取舍是：保留“协作冲突”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“重复工作、结果丢失、并行竞态”，先停止并保留原始事件。本节的失败样例是“重复工作、结果丢失、并行竞态”，恢复或放行动作必须可重放。
{% endnote %}

### 等待与取消

{% note info flat %}
“等待与取消”聚焦“Team、Swarm、共享状态和冲突”。给每个参与者分配任务 ID、输入输出 Schema 和取消条件，合并前检查重复、冲突和责任归属，而不是只数完成消息。复核时把动作和产物对应起来：单 Agent、Subagent、Team 的任务/成本/结果对照应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“重复工作、结果丢失、并行竞态”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：用本地确定性任务模拟单 Agent、Subagent 和并行 Team。

机制：用本地确定性任务模拟单 Agent、Subagent 和并行 Team。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“重复工作、结果丢失、并行竞态”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“Team、Swarm、共享状态和冲突”。给每个参与者分配任务 ID、输入输出 Schema 和取消条件，合并前检查重复、冲突和责任归属，而不是只数完成消息。复核时把动作和产物对应起来：单 Agent、Subagent、Team 的任务/成本/结果对照应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“重复工作、结果丢失、并行竞态”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(十二)Subagent、Team与Swarm","order":12,"topic":"Subagent、Team与Swarm","experiment":"用本地确定性任务模拟单 Agent、Subagent 和并行 Team。","focus":["Subagent 输入输出契约","串行、并行和 Worktree","Team、Swarm、共享状态和冲突","等待、失败和取消"],"artifacts":["单 Agent、Subagent、Team 的任务/成本/结果对照"],"failures":["重复工作、结果丢失、并行竞态"],"mode":"workspace"}')
SCENARIOS = json.loads('{"12":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "重复工作"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="parallel_conflict")
    writes = [("README.md", "worker-a"), ("README.md", "worker-b")]
    try:
        if len({target for target, _ in writes}) != len(writes):
            raise RuntimeError("parallel workers targeted the same file")
    ...
    writes = [("README.md", "single-owner")]
    assert len(writes) == 1 and writes[0][1] == "single-owner"
    assert failure_case in failure_type
    assert task_report["request"]["target"] == "README.md" and task_report["checks"]["validation_passed"]
    record_event(events_file, "recovery_verified", action="arbitrate-shared-write", case=failure_case)
...
{% note info flat %}
“执行步骤”聚焦“等待、失败和取消”。围绕“等待、失败和取消”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“等待、失败和取消”变成一个可观察字段，再以“用本地确定性任务模拟单 Agent、Subagent 和并行 Team。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“重复工作、结果丢失、并行竞态”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“Subagent 输入输出契约”。给每个参与者分配任务 ID、输入输出 Schema 和取消条件，合并前检查重复、冲突和责任归属，而不是只数完成消息。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“重复工作、结果丢失、并行竞态”，先停止并保留原始事件。本节的失败样例是“重复工作、结果丢失、并行竞态”，恢复或放行动作必须可重放。
{% endnote %}

## 结果汇总

{% note info flat %}
本节任务：理解等待、失败和取消并建立可复核判断。核心内容：等待、失败和取消。

机制：等待、失败和取消。先核对输入、状态、输出、失败和复测证据，再用“重复工作、结果丢失、并行竞态”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“等待与取消”聚焦“等待、失败和取消”。围绕“等待、失败和取消”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“等待、失败和取消”变成一个可观察字段，再以“用本地确定性任务模拟单 Agent、Subagent 和并行 Team。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“重复工作、结果丢失、并行竞态”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：单 Agent、Subagent、Team 的任务/成本/结果对照。预期：并行收益和协调开销可解释。失败复验：重复工作、结果丢失、并行竞态。

机制：验收证据：单 Agent、Subagent、Team 的任务/成本/结果对照。预期：并行收益和协调开销可解释。失败复验：重复工作、结果丢失、并行竞态。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“重复工作、结果丢失、并行竞态”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“Subagent 输入输出契约”。给每个参与者分配任务 ID、输入输出 Schema 和取消条件，合并前检查重复、冲突和责任归属，而不是只数完成消息。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“重复工作、结果丢失、并行竞态”，先停止并保留原始事件。本节的失败样例是“重复工作、结果丢失、并行竞态”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 单 Agent、Subagent、Team 的任务/成本/结果对照 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 重复工作、结果丢失、并行竞态 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“串行、并行和 Worktree”。给每个参与者分配任务 ID、输入输出 Schema 和取消条件，合并前检查重复、冲突和责任归属，而不是只数完成消息。复核时把动作和产物对应起来：单 Agent、Subagent、Team 的任务/成本/结果对照应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“重复工作、结果丢失、并行竞态”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-12-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
并行不等于更快
--- answer
并行只减少等待，不会自动减少总工作量；共享文件、依赖和合并冲突可能让它更慢。
--- explanation
并行 Agent 只减少等待，不保证减少工作；任务 ID、输出 Schema、Worktree 和单一合并责任者决定结果是否可复查。问题“并行不等于更快”的关键不在背诵名词，而在于：并行只减少等待，不会自动减少总工作量；共享文件、依赖和合并冲突可能让它更慢。 先判断题目中的硬门槛和适用范围，再看“重复工作、结果丢失、并行竞态”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-12-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
结果汇总如何避免重复和丢失
--- answer
按任务 ID 和输出 Schema 汇总，记录贡献、状态、冲突、重复与取消；合并前由单一责任者复核最终 Diff。
--- explanation
并行 Agent 只减少等待，不保证减少工作；任务 ID、输出 Schema、Worktree 和单一合并责任者决定结果是否可复查。问题“结果汇总如何避免重复和丢失”的关键不在背诵名词，而在于：按任务 ID 和输出 Schema 汇总，记录贡献、状态、冲突、重复与取消；合并前由单一责任者复核最终 Diff。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
