...
categories:
  - Learn Topic
  - Coding Agent
description: 能独立完成 ZCode 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。
...
{% note primary flat %}
本节围绕“ZCode”建立可复核的工作模型。学习成果：能独立完成 ZCode 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。实验：按官方入口完成 ZCode 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。视觉辅助：产品入口或配置截图（真实验证后）；能力表；核心流程 Mermaid。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
ZCode 的模型计划可以让请求成功，但插件和项目上下文仍可能扩大执行面。把入口、GLM Coding Plan/API、扩展来源和拒绝记录放在同一张表中。

本篇把“ZCode”落到一次可回放的本地任务：按官方入口完成 ZCode 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：能独立完成 ZCode 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“ZCode 的入口、GLM Coding Plan/API、模型配置、插件/市场、Skills、MCP、权限和项目工作流”拆成输入、状态、输出和证据。ZCode 的学习边界是入口、GLM Coding Plan/API、项目上下文与插件扩展之间的关系；平台可用性和预览能力不能由本地演示代替。
{% endnote %}

{% mermaid %}
flowchart TD
  A[ZCode入口] --> B[GLM/Coding Plan/API]
  B --> C[代码任务上下文]
  C --> D[插件市场/Skills/MCP]
  D --> E{权限确认}
  E -->|通过| F[测试与Diff]
...
{% note primary flat %}
实验步骤：按官方入口完成 ZCode 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。

验收产物：ZCode 的版本/Commit 或文档快照；ZCode 受控小仓库的任务输出；ZCode 的 Diff、测试、权限或失败日志
...
{% note info flat %}
本节任务：确认ZCode的入口并完成最小安装。核心内容：ZCode：产品入口、版本与环境。

机制：ZCode：产品入口、版本与环境。选择依据：ZCode的入口、平台和版本是离散选择，表格能并列官方入口与适用条件。先核对ZCode的入口类型、支持平台、安装命令和版本输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；ZCode入口或截图不可用时保留官方 URL、命令和版本检查。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | ZCode的入口类型、支持平台、安装命令和版本输出 |
| 失败降级 | ZCode入口或截图不可用时保留官方 URL、命令和版本检查 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

{% note info flat %}
ZCode执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）
{% endnote %}

![ZCode执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）](/img/picgo-images/coding-agent-product-16.svg)

| 能力面 | 本篇观察对象 | 本地夹具证据 | 线上边界 |
| --- | --- | --- | --- |
| 入口与版本 | GLM/Coding Plan/API、项目上下文、插件市场、Skills 与 MCP | 记录任务契约与 fixture 版本 | 需按官方入口、平台和版本复核 |
...
{% note primary flat %}
入口只展示官方操作路径，本次不执行安装或真实账号调用。先记录ZCode desktop的版本，再用同一受控任务比较上下文、权限和交付证据。
{% endnote %}

| 项目 | 可复核内容 |
| --- | --- |
| 入口 | ZCode desktop |
| 安装/启动 | `chmod +x ZCode-*.AppImage  # Linux AppImage；其他平台使用官方安装包` |
| 版本检查 | `打开 ZCode 后记录客户端版本` |
| 配置观察 | model；GLM Coding Plan/API；workspace；approval；插件来源 |
| 最小任务 | 在受控工作区选择 GLM 模型，先生成 Diff，再确认写入 |
...
{% note info flat %}
ZCode 以桌面安装包/AppImage 为主要入口；安装包可启动只证明客户端存在，还要记录 GLM Coding Plan/API 的连接状态。本节围绕“版本与环境”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为ZCode 的版本/Commit 或文档快照。
{% endnote %}

## GLM模型

{% note primary flat %}
本节任务：让ZCode使用指定模型完成一次脱敏认证。核心内容：ZCode：GLM模型、Coding Plan/API。

机制：ZCode：GLM模型、Coding Plan/API。选择依据：ZCode的认证示例需要可复制命令、环境变量占位符和脱敏输出，代码最不含糊。先核对ZCode的模型 ID、认证方式、权限确认和脱敏输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；ZCode没有凭据时运行本地 fixture，禁止伪造在线响应。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | ZCode的模型 ID、认证方式、权限确认和脱敏输出 |
| 失败降级 | ZCode没有凭据时运行本地 fixture，禁止伪造在线响应 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |
...
{% note info flat %}
GLM 模型、Coding Plan 与普通 API 入口可能有不同额度和权限；把 endpoint、model、套餐和失败码分别记录。本节围绕“Coding Plan/API”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为ZCode 受控小仓库的任务输出。
{% endnote %}

## 代码任务

{% note info flat %}
本节任务：在ZCode的项目上下文中定位一个受控任务。核心内容：ZCode：代码任务、项目上下文。

机制：ZCode：代码任务、项目上下文。选择依据：ZCode的上下文、Session 和任务状态是有向关系，图下补充每个状态的文字含义。先核对ZCode的规则来源、上下文范围、Session 状态和任务输入，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；ZCode图表失败时按“入口→上下文→任务→结果”列出状态。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | ZCode的规则来源、上下文范围、Session 状态和任务输入 |
| 失败降级 | ZCode图表失败时按“入口→上下文→任务→结果”列出状态 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |
...
{% note info flat %}
代码任务先固定项目目录和上下文文件，再观察 ZCode 生成的 Diff；不要让插件市场内容成为隐式规则。本节围绕“项目上下文”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为ZCode 的 Diff、测试、权限或失败日志。
{% endnote %}

## 插件市场

{% note info flat %}
本节任务：为ZCode启用一个工具或扩展并撤销它。核心内容：ZCode：插件市场、Skills与MCP。

机制：ZCode：插件市场、Skills与MCP。选择依据：ZCode的工具、Skill、MCP 和插件需要逐项比较来源、权限、输入输出，表格便于审查。先核对ZCode的来源/版本、权限、输入、输出和撤销动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；ZCode扩展不可安装时使用静态 manifest 和拒绝测试。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | ZCode的来源/版本、权限、输入、输出和撤销动作 |
| 失败降级 | ZCode扩展不可安装时使用静态 manifest 和拒绝测试 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### Skills与MCP

{% note info flat %}
插件市场的来源、版本和 MCP 权限决定扩展面；安装前列出读写、网络和秘密访问，撤销后复查配置。本节围绕“Skills与MCP”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为ZCode 的版本/Commit 或文档快照。
{% endnote %}

## 权限确认

{% note danger flat %}
本节任务：在ZCode中审批一次有副作用的动作。核心内容：ZCode：权限确认、测试与Diff。

机制：ZCode：权限确认、测试与Diff。选择依据：ZCode的权限与外发风险必须脱离图表颜色仍直接可见，danger 强调不可逆后果。先核对ZCode的审批点、工作区、网络、秘密和停止条件，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；ZCode拒绝或超时后保留未执行证据与恢复/回滚步骤。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | ZCode的审批点、工作区、网络、秘密和停止条件 |
| 失败降级 | ZCode拒绝或超时后保留未执行证据与恢复/回滚步骤 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 测试与Diff

{% note info flat %}
权限确认应对应一次写入、命令或外发动作，并可在 Diff/测试中回溯；静态产品图不证明它真的执行过。本节围绕“测试与Diff”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为ZCode 受控小仓库的任务输出。
{% endnote %}

## 平台边界

{% note warning flat %}
本节任务：复现ZCode的一项限制并记录恢复。核心内容：ZCode：平台边界、实战复盘。

机制：ZCode：平台边界、实战复盘。选择依据：ZCode的版本、平台、Preview 和退出条件决定能否照搬，提示块突出边界而非宣传。先核对ZCode的版本/平台、功能限制、失败状态和替代路径，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；ZCode在线能力变化时使用日期快照和本地等价任务。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | ZCode的版本/平台、功能限制、失败状态和替代路径 |
| 失败降级 | ZCode在线能力变化时使用日期快照和本地等价任务 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 实战复盘

{% note info flat %}
平台边界包括操作系统、预览能力、模型套餐和网络；无法在同一环境重放时保留差异，不做永久排名。本节围绕“实战复盘”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为ZCode 的 Diff、测试、权限或失败日志。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用证据验收ZCode。核心内容：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 ZCode 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。

机制：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 ZCode 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。选择依据：产品验收需要逐项核对版本、行为和安全证据。先核对验收标准、失败样例和复测动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；在线不可用时保留本地验证结果。
{% endnote %}
...
{% note info flat %}
本节把ZCode的“版本和任务证据”收束为版本、权限、任务、Diff 和测试证据；ZCode 的学习边界是入口、GLM Coding Plan/API、项目上下文与插件扩展之间的关系；平台可用性和预览能力不能由本地演示代替。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | ZCode 的版本/Commit 或文档快照 | 只有最终成功文字 | 保存中间状态与失败记录 |
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(十六)ZCode","order":16,"topic":"ZCode","experiment":"按官方入口完成 ZCode 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。","focus":["ZCode 的入口、GLM Coding Plan/API、模型配置、插件/市场、Skills、MCP、权限和项目工作流。","入口与安装","模型与计划","代码任务与上下文"],"artifacts":["ZCode 的版本/Commit 或文档快照","ZCode 受控小仓库的任务输出","ZCode 的 Diff、测试、权限或失败日志"],"failures":["认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代"],"mode":"workspace"}')
SCENARIOS = json.loads('{"16":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    record_event(events_file, "recovery_verified", action="record-platform-not-run", case=failure_case)
...
{% note info flat %}
本节把ZCode的“失败复测”收束为版本、权限、任务、Diff 和测试证据；ZCode 的学习边界是入口、GLM Coding Plan/API、项目上下文与插件扩展之间的关系；平台可用性和预览能力不能由本地演示代替。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-16-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
ZCode 的默认权限和最小安全配置是什么
--- answer
不能把 ZCode 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。
--- explanation
ZCode 的学习边界是入口、GLM Coding Plan/API、项目上下文与插件扩展之间的关系；平台可用性和预览能力不能由本地演示代替。问题“ZCode 的默认权限和最小安全配置是什么”的关键不在背诵名词，而在于：不能把 ZCode 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。 先判断题目中的硬门槛和适用范围，再看“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-16-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
ZCode 的核心上下文、工具或扩展边界是什么
--- answer
ZCode 的核心边界是 GLM/Coding Plan/API、项目上下文、插件市场、Skills 与 MCP；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。
--- explanation
ZCode 的学习边界是入口、GLM Coding Plan/API、项目上下文与插件扩展之间的关系；平台可用性和预览能力不能由本地演示代替。问题“ZCode 的核心上下文、工具或扩展边界是什么”的关键不在背诵名词，而在于：ZCode 的核心边界是 GLM/Coding Plan/API、项目上下文、插件市场、Skills 与 MCP；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
