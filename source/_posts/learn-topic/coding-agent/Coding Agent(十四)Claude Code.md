---
title: Coding Agent(十四)Claude Code
tags:
  - Coding Agent
  - Claude Code
categories:
  - Learn Topic
  - Coding Agent
description: 能独立完成 Claude Code 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。
...
{% note primary flat %}
本节围绕“Claude Code”建立可复核的工作模型。学习成果：能独立完成 Claude Code 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。实验：按官方入口完成 Claude Code 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。视觉辅助：产品入口或配置截图（真实验证后）；能力表；核心流程 Mermaid。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
评审者在 Claude Code 中只改一行 README，却发现 CLAUDE.md、权限确认和 Hooks 都会改变结果。入口能力必须和项目约束、Diff 与测试一起观察。

本篇把“Claude Code”落到一次可回放的本地任务：按官方入口完成 Claude Code 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：能独立完成 Claude Code 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“终端、IDE、桌面、Web 与 CI 入口；CLAUDE.md、权限、Skills、Hooks、MCP、Subagents 和 Agent SDK”拆成输入、状态、输出和证据。Claude Code 的比较重点是入口与上下文规则如何连接权限、Tools、Skills、Hooks、MCP 和 Subagents；官方能力与本地项目策略要分开验证。
{% endnote %}

{% mermaid %}
flowchart TD
  A[Claude Code入口] --> B[CLAUDE.md上下文]
  B --> C{权限确认}
  C -->|允许| D[Tools/Skills/MCP]
  C -->|拒绝| E[停止并记录]
  D --> F[Hooks/Subagents]
  F --> G[Diff与验证]
{% endmermaid %}
...
{% note primary flat %}
实验步骤：按官方入口完成 Claude Code 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。

验收产物：Claude Code 的版本/Commit 或文档快照；Claude Code 受控小仓库的任务输出；Claude Code 的 Diff、测试、权限或失败日志
...
{% note info flat %}
本节任务：确认Claude Code的入口并完成最小安装。核心内容：Claude Code：终端、IDE、Web与CI、安装与版本。

机制：Claude Code：终端、IDE、Web与CI、安装与版本。选择依据：Claude Code的入口、平台和版本是离散选择，表格能并列官方入口与适用条件。先核对Claude Code的入口类型、支持平台、安装命令和版本输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Claude Code入口或截图不可用时保留官方 URL、命令和版本检查。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Claude Code的入口类型、支持平台、安装命令和版本输出 |
| 失败降级 | Claude Code入口或截图不可用时保留官方 URL、命令和版本检查 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

{% note info flat %}
Claude Code执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）
{% endnote %}

![Claude Code执行边界示意图（非产品截图；真实入口和版本能力仍需单独核验）](/img/picgo-images/coding-agent-product-14.svg)

| 能力面 | 本篇观察对象 | 本地夹具证据 | 线上边界 |
| --- | --- | --- | --- |
| 入口与版本 | CLAUDE.md、权限确认、Tools/Skills/MCP、Hooks 与 Subagents | 记录任务契约与 fixture 版本 | 需按官方入口、平台和版本复核 |
...
{% note primary flat %}
入口只展示官方操作路径，本次不执行安装或真实账号调用。先记录Claude Code CLI的版本，再用同一受控任务比较上下文、权限和交付证据。
{% endnote %}

| 项目 | 可复核内容 |
| --- | --- |
| 入口 | Claude Code CLI |
| 安装/启动 | `npm install -g @anthropic-ai/claude-code` |
| 版本检查 | `claude --version` |
| 配置观察 | CLAUDE.md；权限 allow/deny；Hooks、Skills、MCP、Subagents |
| 最小任务 | claude -p "只读取 README.md，提出一行修改，不执行外发" |
...
{% note info flat %}
Claude Code 的入口可用原生安装器或 npm 包；先用 claude --version 固定版本，再区分 CLI 能启动和 Anthropic 认证已通过。本节围绕“安装与版本”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Claude Code 的版本/Commit 或文档快照。
{% endnote %}

## 模型选择

{% note primary flat %}
本节任务：让Claude Code使用指定模型完成一次脱敏认证。核心内容：Claude Code：模型选择、权限确认。

机制：Claude Code：模型选择、权限确认。选择依据：Claude Code的认证示例需要可复制命令、环境变量占位符和脱敏输出，代码最不含糊。先核对Claude Code的模型 ID、认证方式、权限确认和脱敏输出，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Claude Code没有凭据时运行本地 fixture，禁止伪造在线响应。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Claude Code的模型 ID、认证方式、权限确认和脱敏输出 |
| 失败降级 | Claude Code没有凭据时运行本地 fixture，禁止伪造在线响应 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |
...
{% note info flat %}
模型与权限是两条链：/login 或 API Key 负责身份，权限设置决定文件、命令和网络动作是否需要确认；不要用一次回答证明两者都可用。本节围绕“权限确认”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Claude Code 受控小仓库的任务输出。
{% endnote %}

## CLAUDE.md

{% note info flat %}
本节任务：在Claude Code的项目上下文中定位一个受控任务。核心内容：Claude Code：CLAUDE.md、任务上下文。

机制：Claude Code：CLAUDE.md、任务上下文。选择依据：Claude Code的上下文、Session 和任务状态是有向关系，图下补充每个状态的文字含义。先核对Claude Code的规则来源、上下文范围、Session 状态和任务输入，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Claude Code图表失败时按“入口→上下文→任务→结果”列出状态。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Claude Code的规则来源、上下文范围、Session 状态和任务输入 |
| 失败降级 | Claude Code图表失败时按“入口→上下文→任务→结果”列出状态 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |
...
{% note info flat %}
CLAUDE.md 会为项目任务提供上下文，但规则文本不等于工具授权；保留加载位置、作用域和与当前任务相关的最小片段。本节围绕“任务上下文”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Claude Code 的 Diff、测试、权限或失败日志。
{% endnote %}

## 工具与扩展

{% note info flat %}
本节任务：为Claude Code启用一个工具或扩展并撤销它。核心内容：Claude Code：工具与扩展、Hooks与Subagents。

机制：Claude Code：工具与扩展、Hooks与Subagents。选择依据：Claude Code的工具、Skill、MCP 和插件需要逐项比较来源、权限、输入输出，表格便于审查。先核对Claude Code的来源/版本、权限、输入、输出和撤销动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Claude Code扩展不可安装时使用静态 manifest 和拒绝测试。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Claude Code的来源/版本、权限、输入、输出和撤销动作 |
| 失败降级 | Claude Code扩展不可安装时使用静态 manifest 和拒绝测试 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### Hooks与Subagents

{% note info flat %}
Hooks 与 Subagents 会改变调用前后行为；审查 hook 命令、子代理获得的 Context 和退出状态，再比较主任务 Diff。本节围绕“Hooks与Subagents”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Claude Code 的版本/Commit 或文档快照。
{% endnote %}

## 工作树协作

{% note danger flat %}
本节任务：在Claude Code中审批一次有副作用的动作。核心内容：Claude Code：工作树协作、CI与Agent SDK。

机制：Claude Code：工作树协作、CI与Agent SDK。选择依据：Claude Code的权限与外发风险必须脱离图表颜色仍直接可见，danger 强调不可逆后果。先核对Claude Code的审批点、工作区、网络、秘密和停止条件，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Claude Code拒绝或超时后保留未执行证据与恢复/回滚步骤。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Claude Code的审批点、工作区、网络、秘密和停止条件 |
| 失败降级 | Claude Code拒绝或超时后保留未执行证据与恢复/回滚步骤 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### CI与Agent SDK

{% note info flat %}
工作树与 CI/Agent SDK 是不同执行环境；本地一行 README 修改通过不代表 headless 或 CI 中的凭据、网络和测试条件一致。本节围绕“CI与Agent SDK”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Claude Code 受控小仓库的任务输出。
{% endnote %}

## 危险操作

{% note danger flat %}
本节任务：复现Claude Code的一项限制并记录恢复。核心内容：Claude Code：危险操作、版本边界。

机制：Claude Code：危险操作、版本边界。选择依据：Claude Code的版本、平台、Preview 和退出条件决定能否照搬，提示块突出边界而非宣传。先核对Claude Code的版本/平台、功能限制、失败状态和替代路径，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；Claude Code在线能力变化时使用日期快照和本地等价任务。
{% endnote %}

| 检查面 | 本篇要求 |
| --- | --- |
| 直接证据 | Claude Code的版本/平台、功能限制、失败状态和替代路径 |
| 失败降级 | Claude Code在线能力变化时使用日期快照和本地等价任务 |
| 验收范围 | 本地 fixture 可复核；外部能力另行确认 |

### 版本边界

{% note info flat %}
危险操作必须绑定具体命令、路径和确认结果；权限放行后仍检查 Diff、测试和未授权外发，而不是只看 Claude Code 的成功文字。本节围绕“版本边界”补齐操作证据：记录实际输入、状态、输出和恢复动作。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，对应产物为Claude Code 的 Diff、测试、权限或失败日志。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用证据验收Claude Code。核心内容：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 Claude Code 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。

机制：版本/Commit、受控 Diff、测试或检查输出、权限结果和失败日志；预期：能独立完成 Claude Code 的安装或入口配置、模型/认证设置、一次真实或本地可复现的代码任务，并说明权限、扩展和限制。选择依据：产品验收需要逐项核对版本、行为和安全证据。先核对验收标准、失败样例和复测动作，再用“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”作为反例；在线不可用时保留本地验证结果。
{% endnote %}
...
{% note info flat %}
本节把Claude Code的“版本和任务证据”收束为版本、权限、任务、Diff 和测试证据；Claude Code 的比较重点是入口与上下文规则如何连接权限、Tools、Skills、Hooks、MCP 和 Subagents；官方能力与本地项目策略要分开验证。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | Claude Code 的版本/Commit 或文档快照 | 只有最终成功文字 | 保存中间状态与失败记录 |
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(十四)Claude Code","order":14,"topic":"Claude Code","experiment":"按官方入口完成 Claude Code 的最小任务：读取一个小仓库、修改一个受控文件、运行验证并保存 Diff/日志；没有凭据或平台条件时使用本地静态演示，禁止伪造在线结果。","focus":["终端、IDE、桌面、Web 与 CI 入口；CLAUDE.md、权限、Skills、Hooks、MCP、Subagents 和 Agent SDK。","安装与入口","模型与权限","CLAUDE.md与上下文"],"artifacts":["Claude Code 的版本/Commit 或文档快照","Claude Code 受控小仓库的任务输出","Claude Code 的 Diff、测试、权限或失败日志"],"failures":["认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代"],"mode":"workspace"}')
SCENARIOS = json.loads('{"14":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "认证失败"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="authentication")
    preflight = {"product": DATA["topic"], "failure_kind": "authentication", "failure_case": failure_case, "declared_failure": failure_type, "online_evidence": "not_run", "credentials_present": False}
    try:
        if not preflight["credentials_present"]:
            raise PermissionError("not configured or not run")
    ...
    record_event(events_file, "recovery_verified", action="record-auth-not-run", case=failure_case)
...
{% note info flat %}
本节把Claude Code的“失败复测”收束为版本、权限、任务、Diff 和测试证据；Claude Code 的比较重点是入口与上下文规则如何连接权限、Tools、Skills、Hooks、MCP 和 Subagents；官方能力与本地项目策略要分开验证。失败边界是“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”，恢复动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-14-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
Claude Code 的默认权限和最小安全配置是什么
--- answer
不能把 Claude Code 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。
--- explanation
Claude Code 的比较重点是入口与上下文规则如何连接权限、Tools、Skills、Hooks、MCP 和 Subagents；官方能力与本地项目策略要分开验证。问题“Claude Code 的默认权限和最小安全配置是什么”的关键不在背诵名词，而在于：不能把 Claude Code 的默认权限当作安全策略；从只读、最小工作区、无秘密和高风险动作显式审批开始，并以本地 Diff/测试复核。 先判断题目中的硬门槛和适用范围，再看“认证失败、权限拒绝、平台差异、预览能力或版本漂移必须记录具体表现，不得用通用成功语句替代”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-14-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
Claude Code 的核心上下文、工具或扩展边界是什么
--- answer
Claude Code 的核心边界是 CLAUDE.md、权限确认、Tools/Skills/MCP、Hooks 与 Subagents；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。
--- explanation
Claude Code 的比较重点是入口与上下文规则如何连接权限、Tools、Skills、Hooks、MCP 和 Subagents；官方能力与本地项目策略要分开验证。问题“Claude Code 的核心上下文、工具或扩展边界是什么”的关键不在背诵名词，而在于：Claude Code 的核心边界是 CLAUDE.md、权限确认、Tools/Skills/MCP、Hooks 与 Subagents；每项能力都要分别记录来源、版本、权限、失败和可回滚动作。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
