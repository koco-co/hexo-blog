...
categories:
  - Learn Topic
  - Coding Agent
description: 定位证据支持后续修改，未读文件不被假设
...
{% note primary flat %}
本节围绕“代码定位与上下文”建立可复核的工作模型。学习成果：定位证据支持后续修改，未读文件不被假设。实验：在本博客仓库中用搜索、依赖和测试入口完成一个只读定位报告。视觉辅助：代码定位流程图；上下文选择表。真实产品、账号、云端执行和外部副作用必须由读者显式触发；本地 fixture 不冒充线上证据。
{% endnote %}

<!-- concept-story:start -->
Agent 根据文件名猜测调用关系，改错了入口。搜索、符号、调用链和测试点把“我以为”换成定位证据。

本篇把“代码定位与上下文”落到一次可回放的本地任务：在本博客仓库中用搜索、依赖和测试入口完成一个只读定位报告。
<!-- concept-story:end -->

## 问题入口

{% note info flat %}
读者要解决的唯一问题是：定位证据支持后续修改，未读文件不被假设。非目标、权限、版本、失败和可回退动作都在交付前可见。
{% endnote %}

## 核心模型

{% note info flat %}
先把“搜索、符号、调用链和目录地图；最小上下文与证据；上下文污染和错误假设”拆成输入、状态、输出和证据。代码定位把自然语言任务映射到入口、符号、调用链和测试；最小上下文减少错误假设，也减少敏感数据暴露。
{% endnote %}

{% mermaid %}
flowchart TD
  A[代码定位与上下文入口] --> B[版本与认证]
  B --> C[项目上下文]
  C --> D[受控任务]
  D --> E[权限与扩展]
  E --> F[Diff/测试/限制]
...
{% note primary flat %}
实验步骤：在本博客仓库中用搜索、依赖和测试入口完成一个只读定位报告。

验收产物：入口、调用链、测试点和最小上下文报告

失败注入：错误入口、上下文污染、依赖关系遗漏
{% endnote %}

## 代码地图

{% note info flat %}
本节任务：理解搜索、符号、调用链和目录地图并建立可复核判断。核心内容：搜索、符号、调用链和目录地图。

机制：搜索、符号、调用链和目录地图。先核对输入、状态、输出、失败和复测证据，再用“错误入口、上下文污染、依赖关系遗漏”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“代码地图”聚焦“搜索、符号、调用链和目录地图”。围绕“搜索、符号、调用链和目录地图”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“搜索、符号、调用链和目录地图”变成一个可观察字段，再以“在本博客仓库中用搜索、依赖和测试入口完成一个只读定位报告。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“错误入口、上下文污染、依赖关系遗漏”，恢复或放行动作必须可重放。
{% endnote %}

### 最小上下文

{% note info flat %}
“最小上下文”聚焦“最小上下文与证据”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。具体取舍是：保留“最小上下文”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“错误入口、上下文污染、依赖关系遗漏”，先停止并保留原始事件。本节的失败样例是“错误入口、上下文污染、依赖关系遗漏”，恢复或放行动作必须可重放。
{% endnote %}

## 上下文选择

{% note info flat %}
本节任务：理解最小上下文与证据并建立可复核判断。核心内容：最小上下文与证据。

机制：最小上下文与证据。先核对输入、状态、输出、失败和复测证据，再用“错误入口、上下文污染、依赖关系遗漏”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“上下文污染”聚焦“最小上下文与证据”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。具体取舍是：保留“上下文污染”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“错误入口、上下文污染、依赖关系遗漏”，先停止并保留原始事件。本节的失败样例是“错误入口、上下文污染、依赖关系遗漏”，恢复或放行动作必须可重放。
{% endnote %}

### 定位任务

{% note info flat %}
“定位任务”聚焦“上下文污染和错误假设”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。复核时把动作和产物对应起来：入口、调用链、测试点和最小上下文报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“错误入口、上下文污染、依赖关系遗漏”，恢复或放行动作必须可重放。
{% endnote %}

## 最小实践

{% note primary flat %}
本节任务：运行一个最小可复现示例。核心内容：在本博客仓库中用搜索、依赖和测试入口完成一个只读定位报告。

机制：在本博客仓库中用搜索、依赖和测试入口完成一个只读定位报告。选择依据：代码必须包含输入、关键步骤、输出和错误表现；在线请求由读者显式触发。先核对命令、环境变量、输出、证据位置，再用“错误入口、上下文污染、依赖关系遗漏”作为反例；依赖或网络不可用时使用本地 fixture，不伪造在线结果。
{% endnote %}
...
{% note info flat %}
“准备输入”聚焦“上下文污染和错误假设”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。复核时把动作和产物对应起来：入口、调用链、测试点和最小上下文报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“错误入口、上下文污染、依赖关系遗漏”，恢复或放行动作必须可重放。
{% endnote %}
...
DATA = json.loads('{"series":"Coding Agent","article":"Coding Agent(四)代码定位与上下文","order":4,"topic":"代码定位与上下文","experiment":"在本博客仓库中用搜索、依赖和测试入口完成一个只读定位报告。","focus":["搜索、符号、调用链和目录地图","最小上下文与证据","上下文污染和错误假设","从定位到可执行任务"],"artifacts":["入口、调用链、测试点和最小上下文报告"],"failures":["错误入口、上下文污染、依赖关系遗漏"],"mode":"workspace"}')
SCENARIOS = json.loads('{"4":{"scenario":"product-task","checks":4}}')
...
with tempfile.TemporaryDirectory() as directory:
    ...
    failure_case = "错误入口"
    record_event(events_file, "failure_injected", boundary=failure_type, case=failure_case, kind="wrong_entry")
    bad_request = {"target": "src/guessed-entry.py", "expected": "README.md"}
    try:
        if bad_request["target"] != bad_request["expected"]:
            raise LookupError("task was mapped to the wrong entry point")
    ...
    assert task_report["request"]["target"] == "README.md"
    assert failure_case in failure_type
    assert task_report["request"]["target"] == "README.md" and task_report["checks"]["validation_passed"]
    record_event(events_file, "recovery_verified", action="restore-task-entry", case=failure_case)
...
{% note info flat %}
“执行步骤”聚焦“从定位到可执行任务”。围绕“从定位到可执行任务”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“从定位到可执行任务”变成一个可观察字段，再以“在本博客仓库中用搜索、依赖和测试入口完成一个只读定位报告。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“错误入口、上下文污染、依赖关系遗漏”，恢复或放行动作必须可重放。
{% endnote %}

### 观察输出

{% note info flat %}
“观察输出”聚焦“搜索、符号、调用链和目录地图”。围绕“搜索、符号、调用链和目录地图”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“观察输出”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“错误入口、上下文污染、依赖关系遗漏”，先停止并保留原始事件。本节的失败样例是“错误入口、上下文污染、依赖关系遗漏”，恢复或放行动作必须可重放。
{% endnote %}

## 定位证据

{% note info flat %}
本节任务：理解从定位到可执行任务并建立可复核判断。核心内容：从定位到可执行任务。

机制：从定位到可执行任务。先核对输入、状态、输出、失败和复测证据，再用“错误入口、上下文污染、依赖关系遗漏”作为反例；外部能力不可用时保留本地 fixture 和文字结论。
{% endnote %}
...
{% note info flat %}
“定位任务”聚焦“从定位到可执行任务”。围绕“从定位到可执行任务”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。先把“从定位到可执行任务”变成一个可观察字段，再以“在本博客仓库中用搜索、依赖和测试入口完成一个只读定位报告。”建立成功基线；不要只记录模型最后说了什么。本节的失败样例是“错误入口、上下文污染、依赖关系遗漏”，恢复或放行动作必须可重放。
{% endnote %}

## 结果验证

{% note info flat %}
本节任务：用具体证据判断学习成果是否达成。核心内容：验收证据：入口、调用链、测试点和最小上下文报告。预期：定位证据支持后续修改，未读文件不被假设。失败复验：错误入口、上下文污染、依赖关系遗漏。

机制：验收证据：入口、调用链、测试点和最小上下文报告。预期：定位证据支持后续修改，未读文件不被假设。失败复验：错误入口、上下文污染、依赖关系遗漏。选择依据：不同文章的验收字段不同，使用表格明确输入、结果和判定。先核对验收标准、失败样例、复测动作，再用“错误入口、上下文污染、依赖关系遗漏”作为反例；无脚本时可以手工对照。
{% endnote %}
...
{% note info flat %}
“验收证据”聚焦“搜索、符号、调用链和目录地图”。围绕“搜索、符号、调用链和目录地图”明确一个状态转移、一个失败信号和一个可复测产物；本节不把抽象术语或产品宣传当作实现证据。具体取舍是：保留“验收证据”需要的最小输入，拒绝把无关 Context 或权限带进来；若出现“错误入口、上下文污染、依赖关系遗漏”，先停止并保留原始事件。本节的失败样例是“错误入口、上下文污染、依赖关系遗漏”，恢复或放行动作必须可重放。
{% endnote %}

| 验收项 | 通过证据 | 失败信号 | 复测动作 |
| --- | --- | --- | --- |
| 契约与范围 | 输入、非目标和停止条件可读 | 范围漂移或未授权动作 | 重新核对任务契约 |
| 运行证据 | 入口、调用链、测试点和最小上下文报告 | 只有最终成功文字 | 保存中间状态与失败记录 |
| 失败边界 | 错误入口、上下文污染、依赖关系遗漏 | 异常被静默吞掉 | 注入失败并检查降级 |
| 复现与责任 | 版本、权限、Diff/Trace 可定位 | 无法回到原始输入 | 固定版本后重跑 |

### 复测动作

{% note info flat %}
“复测动作”聚焦“最小上下文与证据”。为每段上下文标记来源、作用域、优先级和失效条件；规则进入模型上下文不等于获得执行权限，冲突时要留下拒绝理由。复核时把动作和产物对应起来：入口、调用链、测试点和最小上下文报告应能指出输入、版本、状态转移和责任人；本地 fixture 只证明确定性路径，线上能力仍为未验证。本节的失败样例是“错误入口、上下文污染、依赖关系遗漏”，恢复或放行动作必须可重放。
{% endnote %}
...
{% flashcard basic id:ai-coding-agent-4-1 deck:"Coding Agent" priority:1 tags:"边界验证" %}
--- question
为什么给 Agent 更多文件可能更差
--- answer
更多文件会增加噪声、秘密暴露和错误关联；只给完成任务所需的最小上下文通常更可控。
--- explanation
代码定位把自然语言任务映射到入口、符号、调用链和测试；最小上下文减少错误假设，也减少敏感数据暴露。问题“为什么给 Agent 更多文件可能更差”的关键不在背诵名词，而在于：更多文件会增加噪声、秘密暴露和错误关联；只给完成任务所需的最小上下文通常更可控。 先判断题目中的硬门槛和适用范围，再看“错误入口、上下文污染、依赖关系遗漏”是否会使结果直接失效；这能避免把平均成功或默认配置当成安全结论。
{% endflashcard %}

{% flashcard basic id:ai-coding-agent-4-2 deck:"Coding Agent" priority:1 tags:"证据链" %}
--- question
如何证明定位结果正确
--- answer
用符号/调用链、测试和受控变更交叉验证入口；至少能指出命中路径、依赖关系和一个可运行的行为证据。
--- explanation
代码定位把自然语言任务映射到入口、符号、调用链和测试；最小上下文减少错误假设，也减少敏感数据暴露。问题“如何证明定位结果正确”的关键不在背诵名词，而在于：用符号/调用链、测试和受控变更交叉验证入口；至少能指出命中路径、依赖关系和一个可运行的行为证据。 再把机制连接到执行证据：输入、状态、权限、产物和复测必须互相对应；若只剩最终答案，就无法解释“失败边界”发生在哪里。
{% endflashcard %}
