...
--- explanation
Class A/B/C 是早期按地址首段划分固定网络大小的分类地址模型，难以按实际规模灵活分配；CIDR 用前缀长度表达网络边界，路由也按前缀匹配。私有地址范围是“能否在公网路由”的用途划分，不是 Class A/B/C 的替代分类。回答旧资料时应明确历史背景，再说明当前按 CIDR、重叠检查和路由公告规划。

例如 `10.0.0.0/8` 是私有地址块，但仍可继续切成 `/16`、`/24` 等不同前缀；这正是分类地址无法表达的灵活性。
{% endflashcard %}
...
--- explanation
VLSM (可变长子网掩码) 允许同一地址规划使用不同的前缀长度：办公 LAN 可以拿较大的 `/24`，点到点链路可以拿 `/31`，从而减少不能使用的空闲地址。它不是“只按主机数量切块”，还要检查块不重叠、网络边界对齐、链路语义、路由公告范围和下一跳位置。

例如先给一个部门 `/26`、给点到点链路 `/31`，再核对两块是否落在父块内且互不重叠；最终还要用路由表验证这些前缀会被正确发布和选择。
{% endflashcard %}
...
--- explanation
普通广播 LAN 要把网络地址和广播地址从主机地址中排除，所以 `/31` 不能直接当作所有二进制子网的通用答案。RFC 3021 为点到点链路定义了特殊语义：两个地址分别代表链路两端，不需要传统的网络/广播地址。使用前仍要确认设备、操作系统和路由协议支持，并在规划和配置中明确这是点到点链路，而不是普通 LAN。
{% endflashcard %}
...
--- explanation
TTL (IPv4) 或 Hop Limit (IPv6) 是包头中的剩余跳数。每经过一个三层转发设备通常减一，减到零时设备丢弃该包并返回超时反馈，避免错误路由环路让包无限存在；`traceroute` 正是逐步降低这个值来观察路径。它限制的是“能经过多少跳”，不是时间秒数，也不能单独证明某个应用端口可用。

排障时把超时反馈和实际路由、接口及目标服务分开记录；路径可达不代表后续 TCP、TLS 或 HTTP 一定成功。
{% endflashcard %}
...
--- explanation
默认网关通常对应默认路由 `0.0.0.0/0`，只在没有更具体前缀时兜底。目标匹配 `/24` 时会优先于默认 `/0`，多网卡、静态/动态路由和策略路由还可能让不同目标走不同出口。可以用下面的查询把“会从哪里出去”变成证据：
...
不要只看网关配置就断定所有流量都走同一出口；还要核对最长前缀、管理距离、度量和策略规则。
{% endflashcard %}
