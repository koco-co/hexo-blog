...
本文只回答一个问题：数据知道目标 IP 之后，主机如何判断目标是否在本地、选择下一跳、构造路由路径，并处理 IPv4、IPv6、控制报文、自动配置和名称解析？

完成本文后，你应该能够：

- 从前缀长度计算网络地址、广播地址和通常的地址范围；
- 使用最长前缀匹配解释路由选择，而不是只看默认网关；
- 区分 NAT 的地址转换能力与端到端连接语义；
- 对照 IPv4 与 IPv6 的地址结构、链路本地地址和邻居边界；
- 解释 ICMP、DHCP、DNS 在访问链路中的位置；
- 识别分类地址、/31 等旧笔记表述的适用范围和迁移方式。

前置文章是[计算机网络(一)入门路线](/posts/347ce4f2/)和[计算机网络(二)网络分层与局域网](/posts/474f4d81/)。本文分配能力为 IP-001～IP-020。

## 正文大纲

- H2：地址与前缀：IPv4、CIDR、VLSM、分类地址和 /31；
- H2：路由、下一跳与 NAT：最长前缀匹配、默认路由和映射边界；
- H2：IPv6：128 位地址、全局/链路本地地址和邻居机制；
- H2：控制、配置与名称：ICMP、TTL、PMTUD、DHCP、DNS 和端点；
- H2：失败边界与验证；
- H2：常见问题；
- H2：参考资料。
...
不能。NAT 处理地址和端口映射及状态，防火墙处理允许、拒绝和审计策略。二者经常部署在同一设备上，但职责不同。
