...
两座岛之间有一条常起雾的渡线。东岛的船夫把一张写着自己出航号的木牌交给信鸽，问西岛能否靠岸。西岛守渡人收到木牌后，没有立刻放下栈桥；他把东岛的号码原样写回，又附上自己今天的出航号。

东岛船夫看见回信，才知道西岛既收到了来信，也能把消息送回来。他再送一张只确认西岛号码的回条。守渡人拿到回条，才肯把栈桥放下：现在他知道东岛不只是发出过请求，还见过自己的号码。

雾里偶尔会飘来一张旧木牌。守渡人不能因为看见第一张牌就让旧行程重新靠岸；他需要这次往返中彼此确认过的两个新号码，才把人和行李交给渡线。

<!-- concept-story:end -->

{% note info flat %}
木牌和回条对应 SYN、SYN+ACK 与最后的 ACK；两枚出航号就是双方的初始序列号（ISN）。三次握手不是为了“礼貌地说三次你好”，而是让双方完成两个方向的初始序列号确认，并让服务端知道客户端能收到它的回复。两次只能让一方确认自己的请求到达并看到对方的 ISN，不能完整确认双向可达和双方的序列空间。
{% endnote %}
