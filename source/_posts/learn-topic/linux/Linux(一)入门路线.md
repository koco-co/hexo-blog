...
| 2 | [Linux(二)系统与命令行基础](/posts/6153b09f/) | 掌握 Linux 组成、目录层级、终端与 Shell、路径、环境变量、命令解析和帮助系统。 | 第 1 篇。 | 已发布 |
| 3 | [Linux(三)文件与存储管理](/posts/5297e207/) | 掌握文件、目录项、inode、文件描述符、链接、归档、文件系统和容量。 | 第 2 篇。 | 已发布 |
| 4 | [Linux(四)文本处理与数据提取](/posts/33859692/) | 从文本和日志中筛选、转换、分组、排序并生成可复现结论。 | 第 3 篇。 | 已发布 |
| 5 | [Linux(五)Shell 语法与命令组合](/posts/72c1404c/) | 解释 Shell 如何解析、展开、重定向、连接和判断命令，使组合命令保持正确。 | 第 2～4 篇。 | 已发布 |
| 6 | [Linux(六)用户权限与软件管理](/posts/5a63b164/) | 掌握身份、权限计算、所有权、提权和软件包生命周期。 | 第 2、5 篇。 | 已发布 |
| 7 | [Linux(七)进程任务与服务管理](/posts/23ca1872/) | 掌握进程状态、信号、作业控制、优先级、systemd 服务和定时任务。 | 第 2、5、6 篇。 | 已发布 |
| 8 | [Linux(八)网络连接与远程排障](/posts/3a757ef9/) | 按网卡、路由、DNS、Socket、TLS、应用请求和抓包逐层定位网络问题。 | 第 2、5 篇；建议具备第 7 篇进程背景。 | 已发布 |
| 9 | [Linux(九)日志查看与故障定位](/posts/cade4656/) | 关联应用日志、Journal、内核消息、崩溃信息、轮转文件和压缩历史，恢复故障时间线。 | 第 4、5、7 篇。 | 已发布 |
| 10 | [Linux(十)性能分析与资源诊断](/posts/997240e7/) | 用统一方法区分 CPU、内存、I/O、容量、网络和进程瓶颈，正确解释采样指标。 | 第 3、7、8、9 篇。 | 已发布 |
| 11 | [Linux(十一)Bash 脚本与任务自动化](/posts/496664b1/) | 把交互命令转化为输入明确、错误可见、可清理、可验证的脚本。 | 第 4～7 篇。 | 已发布 |
| 12 | [Linux(十二)进阶路线](/posts/9efa11cb/) | 承接低频、平台相关、需要权限或可能产生较高风险的能力，不阻塞主课程。 | 第 7～11 篇；第 13 篇不要求本篇。 | 已发布 |
| 13 | [Linux(十三)综合实战](/posts/c0dffe07/) | 把文件、进程、服务、日志、性能和网络能力整合为完整故障诊断，并训练面试复述。 | 第 3～11 篇；第 12 篇可选。 | 已发布 |

## 开始学习

{% note info flat %}
每个实验都先确认输入和安全边界，再执行命令、解释输出并完成清理。高频命令使用双向闪卡，日志与性能内容补充字段和指标解释，危险操作单独标出安全边界；跨文章重复的问题统一引用既有闪卡。
{% endnote %}
