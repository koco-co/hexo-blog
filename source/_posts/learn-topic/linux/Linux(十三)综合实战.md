...
前面的检查应尽量只读、范围宽；越接近服务重启、配置修改或数据清理，越需要授权、回滚和影响评估。顺序的价值在于每一步都缩小下一步的搜索空间。
{% endflashcard %}
...
--- explanation
CPU 高只是现象，至少要先回答三个问题：哪个进程消耗、是计算还是等待、是否持续：

```bash
uptime
ps -eo pid,ppid,stat,pcpu,pmem,cmd --sort=-pcpu | head
mpstat -P ALL 1 3
journalctl --since '10 min ago' --priority warning..emerg --no-pager
```

重启会改变进程、缓存和日志时序，可能把真正原因抹掉。先保存采样和时间窗，评估用户影响，再选择可回滚的最小动作；如果只是短暂尖峰，重启反而可能制造第二个故障。
{% endflashcard %}
...
| 类型 | 示例 | 要求 |
| --- | --- | --- |
| 观察 | `curl` 返回 502，10:02～10:05 持续 | 带命令、时间和关键输出 |
| 推断 | 上游连接池耗尽，待日志确认 | 写出支持与反证 |
| 行动 | 限流配置已回滚，10:10 复测恢复 | 记录批准、变更和结果 |

把“看到的事实”和“对原因的解释”分开，接手者才能重新运行命令、验证假设，而不是只能相信上一位工程师的一句结论。
{% endflashcard %}
...
--- explanation
`APPLY=0` 是把观察阶段和变更阶段隔开的闸门：

```bash
APPLY=${APPLY:-0}
if [[ "$APPLY" != 1 ]]; then
  printf '%s\n' 'report-only: no service mutation'
  exit 0
fi

# 只有目标、批准、回滚命令和复测条件已记录，才进入这里。
systemctl restart "$SERVICE"
```

把默认值设为 0，可以让同一份脚本先生成报告；只有目标明确、影响获批、回滚可执行且复测指标已定义时，才显式传入 `APPLY=1`，并且一次只执行一个动作。
{% endflashcard %}
