...
print({"tasks":len(tasks),"scores":scores,"hard_gate":hard_gate,"p95_ms":latency})
...
--- explanation
模型选型是分层证据决策：业务正确、风险、延迟、成本和稳定性必须在同一任务集下比较。把这条区别落到夹具：分别构造正常、越界和撤销样本，保存两个 Fake 模型跑同一十项任务，先应用硬门禁，再比较质量、p95 和成功成本，形成可追溯选择记录。样本少、任务分布变或版本变化都会改变结论；记录时间和 fixture 版本。
{% endflashcard %}
...
--- answer
十个样本只提供覆盖关键风险；稳定性还需要在quality上由p95 与失败率确认，不能只看文本或单个事件。
--- explanation
在eval夹具中分别运行“十个样本”和“稳定性”，比较十项分层工单与独立 Oracle；样本少、任务分布变或版本变化都会改变结论；记录时间和 fixture 版本。
{% endflashcard %}
