...
造船坊接到一张“核对订单总价”的工单。学徒本想自己去找船主、取账册、开材料库，再把所有钥匙塞进工单里。老师傅却先看工单上需要的东西：船主资料由登记员准备，账册要到材料库领取，材料库还要先从库房领一把临时钥匙。

老师傅按这些关系安排人手，等材料库和登记员都到位，才把完整订单交给学徒检查。若材料库开门失败，学徒根本不该开始核对；若核对时发现总价错误，已经借出的钥匙和账册仍要按相反顺序归还。

学徒最终只在工单上写明“我需要订单”，却能看见它由谁准备、何时借来、何时收走。工单不是在偷偷调用全坊的人，而是在声明一张可追踪的准备关系。

<!-- concept-story:end -->

{% note info flat %}
工单参数、登记员/材料库和临时钥匙分别对应测试函数的 Fixture 声明、Fixture 节点和子依赖。Pytest 会先解析依赖图，再按需要建立并缓存实例；测试函数不需要手动调用 Fixture。把“准备资源”和“验证行为”分开，测试本身会更容易复用和定位。
{% endnote %}
